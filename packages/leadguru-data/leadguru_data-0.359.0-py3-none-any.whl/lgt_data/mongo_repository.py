...
class TeamRepository(BaseMongoRepository):
    ...
    def get_teammate(self, author_id: str, recepient_id: str):
        pipeline = [
            {
                "$match": {
                    "$or": [
                        {"author_id": to_object_id(author_id), "recipient_id": to_object_id(recepient_id)},
                        {"author_id": to_object_id(recepient_id), "recipient_id": to_object_id(author_id)}
                    ],
                    "status": "approved"
                }
            }
