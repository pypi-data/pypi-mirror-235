...
def cis_daemon(dc, url, recursive=None, branch=None, image='innovanon/build', secrets={}):
  print(f'cis_daemon(url={url}, recursive={recursive}, branch={branch}, image={image})', flush=True)

  environment = {}
  if url:                   environment['IA_CIS_REPOSITORY'] = url
  if branch:                environment['IA_CIS_BRANCH']     = branch
  if recursive is not None: environment['IA_CIS_RECURSIVE']  = recursive
  if image:                 environment['IA_CIS_IMAGE']      = image

  # TODO git server's post-receive will have to
  #      check for org secrets
  #      post them
...
def create_app(dc):
  ...
  def upload():
    ...
    try:
      ...
      image     = data.get('image')     # innovanon/build
      secrets   = {
    ...
    return cis_daemon(dc, url, recursive, branch, image, secrets)

  return app

def start_server(host, port, *args, **kwargs):
  dc  = from_env()
