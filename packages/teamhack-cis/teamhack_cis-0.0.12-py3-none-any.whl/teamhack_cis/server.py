...
def cis_daemon(dc, url, rec=False, branch=None):
...
def get_arg(request, arg, *args):
  ...
  if request.method       != 'POST':             return raise Exception('Invalid method')
  if request.content_type == 'application/json': return request.get_json().get(arg, *args)
  return request.form.get(arg, *args) or request.args.get(arg, *args)
