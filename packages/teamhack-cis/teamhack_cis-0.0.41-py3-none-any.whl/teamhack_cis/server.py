...
def cis_daemon(dc, url, recursive, branch, image):
  ...
  cont = dc.containers.create(image, command=f'{url} {brn} {rec}')
...
def create_app(dc):
  ...
  def upload():
    ...
    try:
      ...
      rec = bool(get_arg(request, 'recursive', True)) # TODO default false
