...
@dataclass
class Config:
    passenger_passenger_python: str = ''
    wsgi_module: str = ''
    wsgi_app: str = ''
    ...
    def from_toml_dict(cls, data, passenger_app_root: Path) -> Self:
        retval = cls()
        for section in ('wsgi', 'secret_files', 'environment'):
            if not isinstance(data.get(section, {}), dict):
                raise WSGIConfigException(f'"{section}" is not a section')
        for section, key in [
            ('wsgi', 'module'),
            ('wsgi', 'app'),
            ('passenger', 'passenger_python'),
        ]:
        ...
        retval.passenger_python_path = Path(retval.passenger_passenger_python)
    ...
    try:
        ...
        return cast(
            ...
            functools.partial(
                ...
                html=html_template.format(
                    title='Error Page',
                    reason='This site has experienced a developer-side configuration '
                           'error or exception.',
