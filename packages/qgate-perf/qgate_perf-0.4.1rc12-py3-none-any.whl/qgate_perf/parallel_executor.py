...
def _executor_wrapper(func, run_return: RunReturn, run_setup: RunSetup):
    ...
    try:
        ...
        print(f"_executor_wrapper: {type(e).__name__} - {str(ex)}")
...
class ParallelExecutor:
    ...
    def _coreThreadClassPool(self, threads, return_key, return_dict, run_setup: RunSetup):
        try:
            ...
            print(f"SYSTEM ERROR in '_coreThreadClassPool': {type(e).__name__} - '{str(ex)}'")
    ...
    def run_executor(self, executor_list= ExecutorHelper.PROCESS_2_8_THREAD_1_4_SHORT,
        ...
        try:
            ...
            self._print(file,f"SYSTEM ERROR in 'run_executor': {type(e).__name__} - '{str(e) if e is not None else '!! Noname exception !!'}'")
            final_state = False
        finally:
            if file is not None:
                file.close()
