...
class ParallelExecutor:
    ...
    def _coreThreadClassPool(self, threads, return_key, return_dict, run_setup: RunSetup):
        try:
            ...
            print(f"_coreThreadClassPool: {str(ex)}")
    ...
    def run_executor(self, executor_list= ExecutorHelper.PROCESS_2_8_THREAD_1_4_SHORT,
        ...
        try:
            ...
            self._print(file, "run_executor: " + str(e) if e is not None else '!! Noname exception !!')
            final_state = False
        finally:
            if file is not None:
                file.close()
    ...
    def run(self, processes=2, threads=2, run_setup: RunSetup=None):
        ...
        try:
            ...
            self._print(file, "run: " + str(e) if e is not None else '!! Noname exception !!')
            final_state = False
        finally:
            if file is not None:
                file.close()
