...
class Step:
    ...
    def _get_version_attachments(self, value: ModelVersionRepresentation | DatasetVersionRepresentation):
        return [
            vars(attach)
            for attach in self.project._client.list_version_representation_attachments(  # pyright: ignore[reportPrivateUsage]
                version=value
            ).list
        ]

    def _get_version_artifacts(
        self, data: Any, attachments_output: list[dict] | None = None
