...
### Fiabiliser totalement la fonction (timeouts sur trame longues ??????)
...
class Modbus:
    ...
    Args:
        port (optional): serial port name (default = None)
                         None is supported to allow Modbus link
                           instantiation prior to port selection
        baudrate (optional): Modbus link baudrate (default = 19200).
        timeout (optional): Modbus link receive's timeout (default = 0.1s).
        parity (optional): Modbus link parity ('even' or 'odd')
                            default is 'even'
    ...
    def register_feedback_handler(self, handler) -> None:
        ...
        Args:
            handler: Function called each time there is 
    ...
    def open (self, port = None) -> None:
        ...
        Args:
            port: Serial port that will be associated to the modbus link
        Raises:
            serial.SerialException: Serial port is missing, busy,
    ...
    def write_register(self,
        ...
        Args:
            device_addr: Modbus slave address of the device (0 to 247).
            reg_addr: Register's address (0x0000 to 0xFFFF).
    ...
    def query_data(self, device_addr: int, value = int, **kwargs) -> int:
        ...
        Args:
            device_addr: Modbus slave address of the device (0 to 247).
            value: Value to query (0x0000 to 0xFFFF).

        Returns:
            int: The query data returned value.
    ...
    def diag_read(self, device_addr: int, subfunction: int, **kwargs) -> int:
        ...
        Returns:
            int: The requested counter value.
    ...
    def _request(
        ...
        Returns:
            int or list[int]: value(s) returned by the modbus device
        ...
        if (rx_data[1] & 0b10000000) != 0:
            # Illegal request Mobdus exceptions (legal exception's codes)
            if rx_data[2] in [1, 2, 3]:     
                raise IllegalRequestError(exceptions_dict[rx_data[2]])
            # Other legals Mobdus exception's codes
