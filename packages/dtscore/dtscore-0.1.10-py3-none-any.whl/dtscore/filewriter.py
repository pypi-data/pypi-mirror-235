...
def writejson(schema:dict, filepath:str, filename:str) -> str:
    ...
    output_filename_with_datestamp = utils.datestampFileName(filename)
    output_filename_fullpath = os.path.join(filepath, output_filename_with_datestamp)

    with open(output_filename_fullpath, mode='xt') as output_file:
        json.dump(obj=schema, fp=output_file)

    return output_filename_with_datestamp
