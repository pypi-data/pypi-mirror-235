...
def test_savitzky_golay() -> None:
    ...
    savitzky_golay_fn = partial(savitzky_golay_derivative_single_sample, num_left=10, num_right=10, order=order)
    ...
    if not jnp.allclose(x_d0_target, x_d0):
        print(f"x_d0: {x_d0} != x_d0_target: {x_d0_target}")
    ...
    if not jnp.allclose(x_d50_target, x_d50):
        print(f"x_d50: {x_d50} != x_d50_target: {x_d50_target}")
    ...
    if not jnp.allclose(x_d99_target, x_d99):
        print(f"x_d99: {x_d99} != x_d99_target: {x_d99_target}")
