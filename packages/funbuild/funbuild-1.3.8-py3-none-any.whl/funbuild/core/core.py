...
def package_size():
    data = [line for line in open("./setup.py", "r").read().split("\n") if "multi_package" in line]
    if len(data) == 1:
...
class PackageBuild:
    ...
    def git_build(self, args, **kwargs):
        ...
        for i in range(size):
            run_shell_list([f"python -m build --wheel -n --multi_package {i}"])  # 编译  生成 wheel 包
            self.git_clear_build()
            run_shell_list(["twine upload dist/*", "pip install dist/*"])
