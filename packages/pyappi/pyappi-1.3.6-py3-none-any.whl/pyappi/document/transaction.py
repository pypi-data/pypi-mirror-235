        ...
        if not is_client:
            match parent_type:
                case "log" | "glog":
                    ...
                    manual_lock = self.__dict__['__document'].get("_manual_lock","")
                    ...
                    if _name.startswith("_"):
                        ...
                        if self.__dict__['__document'].get(_name,None):
                            # We are trying to update an existing key.
                            ikey = int(_name)
                            time_group = str(ikey - (ikey % interval))
                        elif time_type == -1: # Client defined key
                            ikey = int(_name)
                            time_group = str(ikey - (ikey % interval))
                        else:
                            nowr = now - frame
                        ...
                        items = [k for (k,v) in self.__dict__['__document'].items() if not k.startswith("_") and not v.get(manual_lock, False)]
                        items.sort()
