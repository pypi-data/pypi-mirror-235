...
def fs_is(document_id):
    with global_appi_mutex:
        try:
            filename = clean_filename(f'{document_config["root"]  }/{document_id}.json')
...
class Document():
    ...
    @staticmethod
    def is_document(document_id):
        return fs_is(document_id)
