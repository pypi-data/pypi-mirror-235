        ...
        if mode == 'train':
            if self.train_size is None:
                ...
                if x_non_lag.shape[1] > 0:
                    if self.extend_daily_target_features:
                        return np.concatenate((x, tar_fea_x, x_non_lag[:, -1, :].reshape((x.shape[0], -1))), axis=1), y
                    return np.concatenate((x, x_non_lag[:, -1, :].reshape((x.shape[0], -1))), axis=1), y
                else:
                    if self.extend_daily_target_features:
                ...
                if len(x_non_lag_train) > 0 and len(x_non_lag_test) > 0:
                    if self.extend_daily_target_features:
                        return np.concatenate((x_train, tar_fea_x_train, x_non_lag_train[:, -1, :].reshape((x.shape[0], -1))), axis=1), \
                            np.concatenate((x_test, tar_fea_x_test, x_non_lag_test[:, -1, :].reshape((x.shape[0], -1))), axis=1), y_train, y_test
                    return np.concatenate((x_train, x_non_lag_train[:, -1, :].reshape((x.shape[0], -1))), axis=1), \
                        np.concatenate((x_test, x_non_lag_test[:, -1, :].reshape((x.shape[0], -1))), axis=1), y_train, y_test
                else:
                    if self.extend_daily_target_features:
            ...
            if len(split_non_lag_fea) > 0:
                if self.extend_daily_target_features:
                    return np.concatenate((split_tar, tar_fea_x, split_non_lag_fea[:, -1, :].reshape((x.shape[0], -1))), axis=1)
                return np.concatenate((split_tar, split_non_lag_fea[:, -1, :].reshape((x.shape[0], -1))), axis=1)
            else:
                if self.extend_daily_target_features:
