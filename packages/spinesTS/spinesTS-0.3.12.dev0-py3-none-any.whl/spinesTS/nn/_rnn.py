...
class Seq2SeqBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        forward_output = forward_output + self.position_encoder1(forward_output.unsqueeze(dim=1)).squeeze().mean(dim=1)
        backward_output = backward_output + self.position_encoder2(
            backward_output.unsqueeze(dim=1)).squeeze().mean(dim=1)
