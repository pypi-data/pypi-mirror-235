...
def _maybe_emit_attachment(
    ...
    if ('--show-attachments' not in sys.argv) and (
        '--show-attachments-only' not in sys.argv
    ):
        return
    if 'attachments' in event:
        if 'Errno 2' in ''.join(event.get('logs', [])):
            return
        for path, properties in event['metadata']['attachments'].items():
            name = path[ATTACHMENT_PREFIX_LENGTH:]
            uuid = 'unknown UUID'
            kind = event.get('kind')
            _emit_prefix(event, file=file)
            if kind == 'ExecutionResult':
                name = name.split('_', maxsplit=2)[2]
                uuid = properties.get('uuid')
            elif kind == 'WorkflowResult':
                name = name[UUID_LENGTH + 1 :]
