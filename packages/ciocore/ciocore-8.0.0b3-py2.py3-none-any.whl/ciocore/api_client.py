...
class ApiClient:
    ...
    def _get_user_agent_header(cls, client_name, client_version=None):
        ...
        try:
            ...
            logger.warning(
                "Unable to import module '{}'. Won't query for version details."
            )
        ...
        if not client_version:
            try:
                ...
                logger.warning(
                    "Module '{}' has no __version__ attribute. Setting version to 'N/A' in the user agent"
                )
                client_version = "unknown"
...
def read_conductor_credentials(use_api_key=False):
    ...
    the user or the API key (if that's desired).
