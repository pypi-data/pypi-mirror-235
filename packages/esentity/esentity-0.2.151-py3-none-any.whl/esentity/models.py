...
class ProductAggs(ElasticEntity):
    ...
    @classmethod
    def aggs_product(cls, _cnt=5000, _sort={'_key': 'asc'}, **kwargs):
        aggs = {
            "brand": {
                "terms": {
            ...
            "rating": {
                "range": {
                    ...
                    "ranges": [
                        { "key": "1", "to": 1.99 },
                        { "key": "2", "from": 2.0, "to": 2.99 },
                        { "key": "3", "from": 3.0, "to": 3.99 },
                        { "key": "4", "from": 4.0, "to": 4.99 },
                        { "key": "5", "from": 5.0 },
                    ]
                },
            },
            "price": {
                "range": {
                    ...
                    "ranges": [
                        { "key": "p1", "to": 500.0 },
                        { "key": "p2", "from": 500.0, "to": 1000.0 },
                        { "key": "p3", "from": 1000.0, "to": 5000.0 },
                        { "key": "p4", "from": 5000.0, "to": 10000.0 },
                        { "key": "p5", "from": 10000.0 }
                    ]
                },
            },
            "weight": {
                "range": {
                    ...
                    "ranges": [
                        { "key": "w1", "to": 0.1 },
                        { "key": "w2", "from": 0.1, "to": 0.5 },
                        { "key": "w3", "from": 0.5, "to": 1.0 },
                        { "key": "w4", "from": 1.0, "to": 3.0 },
                        { "key": "w5", "from": 3.0 }
                    ]
                },
            },
        ...
        return {
            ...
            'rating': reversed([process_range(k, v) for k, v in aggs['rating']['buckets'].items()]),
            'price': [process_range(k, v) for k, v in aggs['price']['buckets'].items()],
            'weight': [process_range(k, v) for k, v in aggs['weight']['buckets'].items()],
