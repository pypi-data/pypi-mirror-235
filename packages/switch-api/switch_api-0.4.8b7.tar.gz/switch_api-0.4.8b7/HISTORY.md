...
### Fixed
- Resolved outlier scenario resulting in unhandled exception on the `upsert_sites()` function within the `integration` 
  module. 
  
### Updated
- In the `integration` module, for the `get_sites()` and `get_device_sensors()` functions, additional parameters have 
