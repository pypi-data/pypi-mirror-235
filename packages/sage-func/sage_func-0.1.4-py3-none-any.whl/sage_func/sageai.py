...
from sage_func.config import LogLevel, get_config, get_function_map, set_config
from sage_func.services.openai_service import OpenAIService
from sage_func.types.abstract_vectordb import AbstractVectorDB
from sage_func.utils.inspection_utilities import get_input_parameter_type
from sage_func.utils.openai_utilities import get_latest_user_message
...
class SageAI:
    ...
    def chat(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        if "error" in function_response:
            ...
            js_str = json.dumps(openai_result)
            messages.append(json.loads(js_str))
            messages.append(
                {
                    "role": "function",
                    "name": function_name,
            ...
            response_str = json.dumps(self.call_openai_no_function(merged))
            base_return["response"] = json.loads(response_str)
