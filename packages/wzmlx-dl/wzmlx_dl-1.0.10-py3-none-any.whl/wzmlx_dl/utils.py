...
def extract_zoro_id(url):
    ...
    if response.status_code == 200:
        ...
        if element:
            title = element.text
            base_url = "https://consumet-private.vercel.app/anime/zoro/"
            ...
            if response.status_code == 200:
                ...
                matching_results = [result for result in results if title.lower() in result.get('title', '').lower()]
                
                if matching_results:
                    zoro_id = matching_results[0].get('id')
                    return zoro_id
                else:
                    ...
                    selection = input("Select a result: ")
                    try:
                        selection_index = int(selection) - 1
                        if 0 <= selection_index < len(results[:15]):
                            zoro_id = results[selection_index].get('id')
                            return zoro_id
                    except ValueError:
                        pass
