...
def do_sealion_boot(stdscr):
    ...
    command = f'''
    {mvn} -s $HOME/.sealion-cli/maven/mvn_setting.xml archetype:generate \
...
def init_cli():
    ...
    torch {config.sealion_cli_root}/maven/mvn_setting.xml && \
    echo {config.mvn_setting_xml} > {config.sealion_cli_root}/maven/mvn_setting.xml
    '''
    exe_shell(command)
