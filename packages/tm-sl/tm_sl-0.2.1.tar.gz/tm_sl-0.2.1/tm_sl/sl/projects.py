...
def add_subject_to_project(caller, org, project_space, project_name, subject, ps_role , p_role ):
    ...
    if len(existing_p_acl) == 0:
        set_acl(caller, f"{org}/{project_space}/{project_name}", subject, role=p_role, inherit=False)
    else:
        existing_p_acl = existing_p_acl[0]
        existing_p_acl['perms'].sort()
        generated_acl = set_acl(caller, f"{org}/{project_space}/{project_name}", subject, role=p_role, inherit=False, mock=True)
        generated_acl['perms'].sort() #type: ignore
    ...
    if len(existing_shared_acl) == 0:
        set_acl(caller, f"{org}/{project_space}/shared", subject, role=ps_role, inherit=False)
    else:
        existing_shared_acl = existing_shared_acl[0]
        existing_shared_acl['perms'].sort()
        shared_generated_acl = set_acl(caller, f"{org}/{project_space}/shared", subject, role=ps_role, inherit=False, mock=True)
