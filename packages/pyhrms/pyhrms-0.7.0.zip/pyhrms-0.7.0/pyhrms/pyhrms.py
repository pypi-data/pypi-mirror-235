...
def ISTD_evaluation(files,istd_info,name_column = 'name',rt_column = 'rt', 
                    mz_column = 'mz',mz_error_threshold = 0.015,rt_error_threshold = 0.15):
    
    """
    Evaluate the quality of Internal Standard (ISTD) based on retention time (RT), m/z, intensity, and area.

    This function locates the ISTD in each peak picking result to assess its quality attributes.

    Parameters:
    - files (list): List of file paths for the Excel results.
    - istd_info (DataFrame): DataFrame containing ISTD information. Must include columns for name, RT, and m/z.
    - name_column (str, optional): Column name representing ISTD name. Default is 'name'.
    - rt_column (str, optional): Column name representing retention time. Default is 'rt'.
    - mz_column (str, optional): Column name representing m/z value. Default is 'mz'.
    - mz_error_threshold (float, optional): Allowed error margin for m/z values. Default is 0.015.
    - rt_error_threshold (float, optional): Allowed error margin for RT values. Default is 0.15.

    Returns:
    - tuple of DataFrames: DataFrames for m/z, optimized m/z, RT, intensity, and area respectively.
    
    Note:
    - It's assumed that if 'mz_opt' is present in the input data, it's considered for optimized m/z. 
    """
    
    istd_df_mz = istd_info.loc[:,[name_column,rt_column,mz_column]]
    istd_df_mz_opt =  istd_info.loc[:,[name_column,rt_column,mz_column]]
    istd_df_rt = istd_info.loc[:,[name_column,rt_column,mz_column]]
    istd_df_i = istd_info.loc[:,[name_column,rt_column,mz_column]]
    istd_df_area = istd_info.loc[:,[name_column,rt_column,mz_column]]

    # Initialize empty dictionaries to store data temporarily
    data_mz = {}
    data_rt = {}
    data_i = {}
    data_area = {}
    ...
    for file in tqdm(files):
        ...
        for i in range(len(istd_info)):
            mz = istd_info.loc[i, 'pos']
            rt = istd_info.loc[i, '保留时间']
            df1 = df[(df['rt'] < rt + rt_error_threshold) & (df['rt'] > rt - rt_error_threshold) & (df['mz'] < mz + mz_error_threshold) & (df['mz'] > mz - mz_error_threshold)]
            ...
            if len(df1) > 0:
                ...
                mzs.append(s1['mz'])
                rts.append(s1['rt'])
                intensities.append(s1['intensity'])
                areas.append(s1['area'])
                if 'mz_opt' in s1.index:
                    mz_opt.append(s1['mz_opt'])
                else:
                    pass
            else:
                mzs.append(None)
        ...
        data_mz[name] = mzs
        data_rt[name] = rts
        data_i[name] = intensities
        data_area[name] = areas
        data_mz_opt[name] = mz_opt

    # Convert the dictionaries to DataFrames and concatenate with the initial data
    istd_df_mz = pd.concat([istd_df_mz, pd.DataFrame(data_mz)], axis=1)
    istd_df_rt = pd.concat([istd_df_rt, pd.DataFrame(data_rt)], axis=1)
    istd_df_i = pd.concat([istd_df_i, pd.DataFrame(data_i)], axis=1)
    istd_df_area = pd.concat([istd_df_area, pd.DataFrame(data_area)], axis=1)
    istd_df_mz_opt = pd.concat([istd_df_mz_opt, pd.DataFrame(data_mz_opt)], axis=1)
    
    return istd_df_mz,istd_df_mz_opt,istd_df_rt,istd_df_i,istd_df_area
