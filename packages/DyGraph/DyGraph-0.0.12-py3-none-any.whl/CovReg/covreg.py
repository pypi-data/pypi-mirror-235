...
class CovReg():
    ...
    def fit_hoff_b_only(self, X2, psi = None, nu = None, error = 'gaussian', sample = 'sample', X_filter = None, verbose = True, C_init = 'multi_start_hoff')-> None:
        """
        ...
        while self.iteration <self.max_iter:
            C_pre = C_est.copy()
            m,v, gamma = self.get_m_v(C_pre, Psi_pre_inv, None, X2, tau, error, sample = sample)
            self.m = m
            self.v = v
            self.gamma = gamma
    ...
    def fit_hoff(self, X1, X2, psi = None, X_filter = None, verbose = True, C_init = 'multi_start_hoff')-> None:
        ...
        C_pre, Psi_pre, Psi_pre_inv = self.init_param(C_init, psi, Y_tilde, X1, X2)
    ...
    def marg_lik(self, X1 = None, X2 = None, Psi = None, C = None):
        ...
        m,v = self.get_m_v(with_mean, C, psi_inv, X1, X2)
    ...
    def nr_params(self):
        if self.A is None:
            ...
            C = np.hstack((self.A, self.B))
        return np.sum(np.abs(C)>1e-4)


    
    def likelihood(self, X1 = None, X2 = None):
        ...
        lik += multivariate_normal(mean = mean, cov = cov).logpdf(self.Y[i])
