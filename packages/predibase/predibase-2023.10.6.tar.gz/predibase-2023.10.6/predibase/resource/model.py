    ...
    def upload_model(self, zip_fp: str, dataset: Optional[Dataset] = None) -> "Model":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not os.path.exists(os.path.join(model_artifacts_path, MODEL_WEIGHTS)):
                raise FileNotFoundError("model_weights file(s) not found in model zip archive")
...
def wait_until_model_ready(model_id: int, session: Session, start_time=None) -> Dict[str, Any]:
...
class ModelFuture:
    ...
    def get(self) -> Model:
        ...
        resp = self._wait_for_ready(pd.to_datetime(resp["modelVersion"]["created"]))
    ...
    def _wait_for_ready(self, start_time=None) -> Dict[str, Any]:
        return wait_until_model_ready(self.version_id, self.session, start_time)
