...
class AsyncRepo:
    ...
    async def _modified(self, limit=1000) -> AsyncGenerator[SessionPathsResponse, None]:
        ...
        while (limit == 0) or c < limit:
            async for item in self.db.get_all_paths_for_session(
                session_id=self.session["id"], base_commit=self.session["base_commit"], collection=metadata_collection, limit=limit
            ):
                yield item
                c += 1
            async for item in self.db.get_all_paths_for_session(
                session_id=self.session["id"], base_commit=self.session["base_commit"], collection=chunks_collection, limit=limit
            ):
                yield item
                c += 1
            break
