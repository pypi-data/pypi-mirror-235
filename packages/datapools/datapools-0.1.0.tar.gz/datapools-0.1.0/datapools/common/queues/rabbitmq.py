        ...
        try:
            while not await self.is_stopped():
                ...
                async with connection:
                    ...
                    self.ready_state[QueueRole.Server].set()
                    ...
                    try:
                        while not await self.is_stopped():
                            ...
                            if message is not None:
                                logger.info(
                                    f"-------------------publishing msg {message.encode()}"
                                )
                                ...
                                logger.info(
                                    f"publishing into {exchange=} {routing_key=}"
                                )
                ...
                self.ready_state[QueueRole.Server].clear()
        ...
        try:
            while not await self.is_stopped():
                ...
                try:
                    ...
                    self.ready_state[QueueRole.Client].set()
                ...
                self.ready_state[QueueRole.Client].clear()
        except Exception as e:
            logger.error(f"!!!!!!!!!!!!!!!!!! exception in rabbitmq")
            logger.error(traceback.format_exc())
