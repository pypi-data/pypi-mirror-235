...
class CrawlerWorker(Stoppable):
    def __init__(self, cfg: WorkerSettings = None):
        ...
        # self.api = SchedulerAPI( url = self.cfg.SCHEDULER_API_URL, worker_id = self.id )
        ...
        self.reports_queue = RabbitmqQueue(
            QueueRole.Server, self.cfg.RABBITMQ_CONNECTION_URL, QUEUE_REPORTS
        )
        self.consumer_queue = RabbitmqQueue(
            QueueRole.Server,
            self.cfg.RABBITMQ_CONNECTION_URL,
            QUEUE_EVAL_TASKS,
        )
    ...
    async def stop(self):
        ...
        await self.consumer_queue.stop()
                        ...
                        for attempt in range(0, self.cfg.ATTEMPTS_PER_URL):
                            ...
                            try:
                                async for content_or_task in plugin.process(
                                    ...
                                    if type(content_or_task) is CrawlerContent:
                                        ...
                                        await self.consumer_queue.push(
                                            QueueMessage(
                                ...
                                logger.error(f"failed get imageshack url: {e}")
