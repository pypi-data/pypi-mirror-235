...
class RedisCacheApi(CacheApiAdapter):
    ...
    def __init__(self, cache_config: CacheStoreConfig):
        ...
        # 若langchain>=0.0.265，此配置生效支持ttl
        if "ttl" in RedisCache.__init__.__code__.co_varnames:
            config.update({"ttl": cache_config.ttl})
...
class RedisSemanticCacheApi(CacheApiAdapter):
    ...
    def __init__(self, cache_config: CacheStoreConfig):
        ...
        if "ttl" in RedisSemanticCache.__init__.__code__.co_varnames:
            config.update({"ttl": cache_config.ttl})
