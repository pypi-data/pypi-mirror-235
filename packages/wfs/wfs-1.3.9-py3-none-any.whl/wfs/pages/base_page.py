...
def get_html_source(lpage, item, gready, gtestitem, aresults, refkey=0):
    action =  gready[1]
    if 'click' not in action.lower() and 'find' not in action.lower():
        refkey=1
    matches = re.findall(r'\*([^*]+)\*', gready[1])

    getsource = lpage.get_source_xpath(greadyx=gready, textmatch=matches, refkey=refkey)
    return getsource