...
def get_test_steps(test_object_repo, test_data_json, tstepx, steps_line, testdata, action, actiondesc, ptname):
    ...
    try:
        ...
        if '|' not in action:
            action, input_string, testdata, actiondesc = get_action_data_list(action=action, steps_line=steps_line,
                                                                              testdata=testdata, actiondesc=actiondesc)
            data = ['xpath|' + str(input_string) + '|' + str(action) + '|click_find_enter|' + str(tstepx) + '|' + str(
...
def get_action_data_list(action, steps_line, testdata, actiondesc):
    getaction = action.split('\n')
    testdata = testdata.split('\n')
    actiondesc = actiondesc.split('\n')
    action = getaction[steps_line]
    if ('click' in action.lower() or 'Click' in action) and (
            'find' in action.lower() or 'Find' in action):
        action = 'click'
    if 'find' in action.lower() and 'click' not in action.lower() and 'enter' not in action.lower():
        action = 'text'
    if 'click' in action.lower() and 'enter' not in action.lower() and 'find' not in action.lower():
        action = 'click'
    if 'enter' in action.lower() and 'click' not in action.lower() and 'find' not in action.lower():
        action = 'type'
    if ('enter' in action.lower() or 'Enter' in action) and (
            'find' in action.lower() or 'Find' in action):
        action = 'type'
    return action, getaction[steps_line], testdata[steps_line], actiondesc[steps_line]