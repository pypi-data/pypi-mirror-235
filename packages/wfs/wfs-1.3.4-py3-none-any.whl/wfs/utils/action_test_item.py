...
from wfs.utils.common import get_line_dict, extract_details
...
def getall_actions(testcase, lpage, gAitem, gTestitem, gFelements, linex, gRstring, aresults, exresults, case, gItems,
    ...
    try:
        ...
        if gAitem == 'url':
            ...
            """Labels"""
            gTx = get_label(lpage=lpage, item=gAitem, testitem=gTestitem, aresults=aresults, gcount=gcount)
    ...
    with open(pathdata_file, 'w') as f:
        ...
        for getxl in getallxm:
            if pfix in getxl:
                ...
                f.write("\t\t\t\t\tif '>>' in str(aresults) and not str(aresults).startswith('>>') and not str(aresults).endswith('>>'):\n")
