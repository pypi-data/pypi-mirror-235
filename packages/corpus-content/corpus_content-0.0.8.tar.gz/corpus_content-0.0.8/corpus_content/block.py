...
class Chunk(NamedTuple):
    ...
    def cleared_text(self):
        ...
        return clear_markdown(self.text)

    @property
    def as_row(self):
        data = self._asdict()
        data["text"] = self.cleared_text
        data["char_count"] = len(self.cleared_text)
        return data
...
class Block(NamedTuple):
    ...
    def chunks(self) -> list[Chunk]:
        ...
        So one quick way of splitting a block's body into chunks is to split on lines that do
        not start with a blockquote.
        """  # noqa: E501
        results = []
        bits = re.split(r"\n\n(?!>)", self.proper_body)
