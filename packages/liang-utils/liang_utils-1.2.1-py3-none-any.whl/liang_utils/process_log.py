...
def split_log_file(file_path: str, prompt_pattern: str, output_dir: str = None):
    ...
    :param prompt_pattern: regex pattern that contains a group which will be used as command name:
        e.g. <"RP/\d+/RP\d+/CPU\d+:.*#(show.*)">
...
def main():
    ...
    parser.add_argument('-p', '--pattern', type=str, required=True,
                        help='prompt pattern that contains a group which will be used as command name: '
                             'e.g. <"RP/\d+/RP\d+/CPU\d+:.*#(show.*)">')
