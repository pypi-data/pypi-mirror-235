...
class Plinux:
    ...
    def __init__(self,
        ...
        self.logger = plogger.logger(__class__.__name__, enabled=self.log_enabled, level=self.log_level)
    ...
    def is_credentials_valid(self) -> bool:
        ...
        self.logger.info(f'Verify credentials: {self.username} / {self.password}')
        ...
        try:
            ...
            self.logger.exception(f'Login {self.username}, or password {self.password} are invalid.)')
            self.logger.warning(f'Invalid credentials ({self.username, self.password})')
            return False
