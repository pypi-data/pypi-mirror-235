...
from raw_data_smoothing_methods import BaselineCorrection, MeanCentering, SavitzkyGolayFilter, StandardNormalVariate
import joblib
...
class Preprocessing:
    ...
    def load_data(self):
        ...
        return self.numerical_data  # , self.geographical_origin
    ...
    def save_pipeline(self,
                      file_path='raman_preprocessing_pipeline_geo.joblib'):  # CHANGE THE NAME ACCORDINGLY #CHANGED THE PIPELINE INTO .JOBLIB
    ...
    def split_and_save_data(self, split_row):
        ...
        joblib.dump(training_data_with_labels,
                    r"D:\Desktop\Folders\MEDIFIT_More\newer\Original Data\original_processed\Raman\data_training.joblib")
        joblib.dump(validation_data_with_labels,
                    r"D:\Desktop\Folders\MEDIFIT_More\newer\Original Data\original_processed\Raman\data_validation.joblib")
...
class ValidationPreprocessingRaman(Preprocessing):

    def __init__(self, file_path, sheet_name='Sheet1', raman_pipeline_path='raman_preprocessing_pipeline_geo.pkl'):
        super().__init__(file_path, sheet_name)
        self.geographical_origin = None  # No classes in validation data
        self.data = pd.read_excel(file_path)
        self.pipeline = joblib.load(raman_pipeline_path)  # Load the saved pipeline
