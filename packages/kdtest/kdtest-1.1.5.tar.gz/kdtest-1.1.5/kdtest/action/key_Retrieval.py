...
class SurveyKeyWord:
    ...
    def keyRetrieval(self, page_Object, tagging, keywordName, positionInformation=None, manipulatedValue=None):
        ...
        if keywordName == 'time_sleep':  # 强制时间等待
            ...
            self.acceptItch = page_Object.downloadExport_assert(prospectiveControl = manipulatedValue)

        elif keywordName == 'winUpload':  # 多文件批量上传
            page_Object.winUpload(filePath = manipulatedValue)
            ...
            page_Object.enterWindow(win_MAX = manipulatedValue)
