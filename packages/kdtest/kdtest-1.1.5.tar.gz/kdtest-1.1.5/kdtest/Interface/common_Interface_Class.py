...
class CommonInterfaceClass:
    ...
    '''构造函数'''
    def __init__(self, CLEAR_RESULT=True) -> None:
        ...
        self.CLEAR_RESULT = CLEAR_RESULT  # “响应结果” 缓存文件数据清除开关
    ...
    def __call__(self, File_Src) -> None:
        ...
        if __Execute:
            '''流程正常结束'''
            if self.CLEAR_RESULT: self.__file_Tool(self.RESULT_FILE, "w")  # 清空 “响应结果数据缓存文件”
    ...
    def __file_Tool(self, SRC, OPER='r', W_DATA=None):
        ...
        '''
        __type = True if 'yaml' in "".join(SRC.split('\\')[-1:]) else False  # 文件类型获取[True: yaml文件 || False: txt文件]
        __file_Object = open(SRC, mode=OPER, encoding='utf-8') if __type else open(SRC, OPER)  # 生成文件对象
        __read_Data = None  # 数据读取结果暂存

        if OPER == 'w':
            '''写入操作'''
            if W_DATA:
                self.yaml.dump(W_DATA, __file_Object) if __type else __file_Object.write(str(W_DATA))
        else:
            '''读取操作'''
            __read_Data = self.yaml.load(__file_Object) if __type else "".join(__file_Object.readlines())

        __file_Object.close()  # 关闭文件对象

        return __read_Data  # 返还读取数据；“写入操作”时返还为None
    ...
    def __mapping(self, File_Src):
        '''
         参数：
            @param File_Src：需要进行数据读取的[InterfaceData.yaml]文件路径
        '''
        MappingSrc = "\\".join(File_Src.split("\\")[:-1]) + "\\_InterfaceData.yaml"  # “数据映射文件路径”

        if os.path.exists(MappingSrc):
            '''映射文件已存在，数据操作'''
            SOURCE_DATA = self.__file_Tool(File_Src)  # “接口数据”源文件数据取出
            MAPPING_DATA = self.__file_Tool(MappingSrc)  # “接口数据”映射文件数据取出
            POP_KEY = []  # “key键删除操作记录列表”，用于记录“已经生成测试数据”的接口key键

            '''接口数据是否生成判别'''
            for _key in MAPPING_DATA.keys():
                if MAPPING_DATA[_key]['_RS'] == True: 
                    POP_KEY.append(_key)  # 记录“已生成测试数据”的接口key键
                else:
                    pass
            for _key in POP_KEY: MAPPING_DATA.pop(_key)  # "已生成项"删除

            '''源文件是否出现数据改动判别'''
            for i_key in SOURCE_DATA.keys():  # 接口循环
                if i_key in MAPPING_DATA:
                    for p_key in [key for key in SOURCE_DATA[i_key].keys()][:-1]:  # 参数项循环 [_PARAMETERS，_RESULT]
            ...
            self.__file_Tool(MappingSrc, 'w', MAPPING_DATA)  # 处理数据写入

        else:
            '''映射文件不存在，复制生成；测试角色数据写入'''
            os.system(f'copy {File_Src} {MappingSrc}')
