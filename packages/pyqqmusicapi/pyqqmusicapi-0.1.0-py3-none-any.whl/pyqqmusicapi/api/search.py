...
class SearchApi:
    ...
    async def integrate(keyword: str, highlight: bool = False) -> Dict[str, Any]:
        ...
        data = response["body"]
        direct = [
            Helpers.filter_data(d) for d in data["direct_result"]["vertical_list"]
        ]
        song = [Helpers.parse_song_info(d) for d in data["item_song"]["items"]]
        album = [Helpers.filter_data(d) for d in data["item_album"]["items"]]
        songlist = [Helpers.filter_data(d) for d in data["item_songlist"]["items"]]
        mv = [Helpers.filter_data(d) for d in data["item_mv"]["items"]]
        audio = [Helpers.filter_data(d) for d in data["item_audio"]["items"]]
        audio_song = [Helpers.filter_data(d) for d in data["item_audiosong"]["items"]]
        user = [Helpers.filter_data(d) for d in data["item_user"]["items"]]
        singer = [Helpers.filter_data(d) for d in data["singer"]["items"]]
        return {
            "direct": direct,
            "song": song,
            "singer": singer,
    ...
    async def hotkey() -> List[Dict]:
        ...
        response = await SearchApi.parent.get_data(
            ...
            param={"search_id": Utils.gen_searchID()},
        )
    ...
    async def query(
        ...
        if query_type in [0, 7, 18]:
            return [Helpers.parse_song_info(song) for song in data]
        return [Helpers.filter_data(d) for d in data]
