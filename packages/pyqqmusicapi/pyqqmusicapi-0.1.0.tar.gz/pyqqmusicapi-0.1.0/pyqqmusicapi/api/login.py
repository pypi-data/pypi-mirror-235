...
class Login(ABC):
    ...
    async def authcode(self) -> int:
        ...
        Returns:
            int: 返回信息。0:发送成功 1:需要滑块验证 2:未知情况
        """
...
class QQLogin(Login):
    ...
    async def qrcode(self):
        ...
        async with self.session.get(
            ...
            self.ptqrtoken = Utils.get_ptqrtoken(response.cookies.get("qrsig").value)
    ...
    async def authorize(self, code: Optional[int] = None) -> bool:
        ...
        async with self.session.post(
            ...
            data={
                ...
                "update_auth": "1",
                "openapi": "80901010",
                "g_tk": Utils.get_token(skey),
                "auth_time": str(int(time.time())),
                "ui": Utils.random_uuid(),
            },
...
class LoginApi:
    ...
    async def refresh(token: Dict) -> Dict:
        ...
        Args:
            token: 保存登录信息的参数字典，包括 openid、access_token、refresh_token、expired_in、
    ...
    def request(login_type: int = 0, phone: Optional[int] = None) -> Login:
        ...
        if login_type == 1:
            ...
            if not phone:
                raise ValueError("Mobile phone number is empty.")
