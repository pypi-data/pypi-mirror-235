...
def checkpoint(key, argvals=False, tempdir=False, refresh=False,verbose=True):
    '''
    Decorator to checkpoint the output of a function to hdf5 files. The targeted function needs to return a dictionary. Add `@skywalker.checkpoint(filename)` before a function and the output will be stored to file and computed only if necessary. Example:

        def test_checkpoint():
            import time
            @skywalker.checkpoint('checkpoint',argvals=[0,1])
    ...
    def decorator(func):
        ...
        def wrapped(*args, **kwargs):
            ...
            # Otherwise, load the checkpoint file and send it.
                ...
                return h5ify.load(save_file) #deepdish.io.load(save_file)
