...
def send_init_request(data, log = True):
    url = 'https://weathercontest.region.mo/api/client/user'
    try:
        ...
        if response.status_code == 200:
            ...
            if log:
                ...
                print('weathercontest::init>', ct, 'User authentication successful.')
        else:
            if log:
                ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
                print('weathercontest::init>', ct,'Request failed. Error code:', response.status_code)
    except requests.Timeout:
        if log:
            ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            print('weathercontest::init>', ct,'Request timed out.')


def init(username, password, log = True):
    if not username or not password:
        if log:
            print("Username or password cannot be empty. Exiting the program.")
        sys.exit()
...
def send(data = {}, log = True):
    if __user is None or __channel is None or __thingSpeakChannelId is None or __thingSpeakApiKey is None:
        if log:
            ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            print('weathercontest::send>', ct,'Please execute init() first.')
        return
    ...
    if current_time - __last_send_time >= 28 or __last_send_time == 0:
        ...
        if log:
            ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            print('weathercontest::send>', ct,'Request is too frequent. The interval between requests must be at least 30 seconds.')
        return
    
    if type(data) is not dict:
        if log:
            ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            print('weathercontest::send>', ct,'The data must be of type dict.')
        return
    ...
    if data.get('LONG') is None and data.get('LAT') is None and data.get('PM2.5') is None and data.get('PM10') is None and data.get('CO') is None and data.get('SO2') is None and data.get('NO2') is None and data.get('O3') is None:
        if log:
            ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            print('weathercontest::send>', ct,'The data must include at least one field.')
        return
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result == 0:
                if log:
                    ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
                    print('weathercontest::send>', ct,'Data upload failed.')
            else:
                if log:
                    ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
                    print('weathercontest::send>', ct,'Data upload successful.\n', json.dumps(result, sort_keys=True, indent=4))
        else:
            if log:
                ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
                print('weathercontest::send>', ct,'Request failed. Error code:', response.status_code)
    except requests.Timeout:
        if log:
            ct = datetime.datetime.now().strftime('[%Y-%m-%d %H:%M:%S]')
            print('weathercontest::send>', ct,'Request timed out.')
