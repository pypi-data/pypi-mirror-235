...
1. Server-side (API Key): for quickly getting started, testing, and production scenarios
    where Promptmanager will only use actions exposed in the developer's Zapier account
...
2. User-facing (Oauth): for production scenarios where you are deploying an end-user
    facing application and Promptmanager needs access to end-user's exposed actions and
...
from promptmanager.script.base.agents.agent_toolkits import ZapierToolkit
...
from promptmanager.script.base.tools.zapier.prompt import BASE_ZAPIER_TOOL_PROMPT
from promptmanager.script.base.utilities.zapier import ZapierNLAWrapper
