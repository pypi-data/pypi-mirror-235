...
def convert_openai_messages(messages: Sequence[Dict[str, Any]]) -> List[BaseMessage]:
    """Convert dictionaries representing OpenAI messages to Promptmanager format.

    Args:
        messages: List of dictionaries representing OpenAI messages

    Returns:
        List of Promptmanager BaseMessage objects.
    """
...
class ChatCompletion:
    ...
    def create(
        ...
        **kwargs: Any,
        ...
        models = importlib.import_module("promptmanager.script.base.chat_models")
    ...
    async def acreate(
        ...
        **kwargs: Any,
        ...
        models = importlib.import_module("promptmanager.scropt.base.chat_models")
        model_cls = getattr(models, provider)
        model_config = model_cls(**kwargs)
        converted_messages = convert_openai_messages(messages)
        if not stream:
