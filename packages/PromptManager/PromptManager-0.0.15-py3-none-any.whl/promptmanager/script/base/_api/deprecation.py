"""Helper functions for deprecating parts of the Promptmanager API.
...
class PMDeprecationWarning(DeprecationWarning):
    """A class for issuing deprecation warnings for users."""


class PMPendingDeprecationWarning(PendingDeprecationWarning):
    """A class for issuing deprecation warnings for users."""
...
def suppress_promptmanager_deprecation_warning() -> Generator[None, None, None]:
    """Context manager to suppress PromptmanagerDeprecationWarning."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", PMDeprecationWarning)
        warnings.simplefilter("ignore", PMPendingDeprecationWarning)
        yield
    ...
    *,
    ...
    if not message:
        ...
        if pending:
            ...
            message += f" was deprecated in Promptmanager {since}"
    ...
    warning_cls = (
        PMPendingDeprecationWarning if pending else PMDeprecationWarning
    )
    warning = warning_cls(message)
    warnings.warn(warning, category=PMDeprecationWarning, stacklevel=2)


def surface_promptmanager_deprecation_warnings() -> None:
    """Unmute Promptmanager deprecation warnings."""
    warnings.filterwarnings(
        "default",
        category=PMPendingDeprecationWarning,
    )
