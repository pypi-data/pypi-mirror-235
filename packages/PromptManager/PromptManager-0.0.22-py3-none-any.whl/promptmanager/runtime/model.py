...
class PMBaseAIModel:
    ...
    def base_request(method, url, header, requestBody, stream=False):
        setting = os.getenv('django_settings')
        if setting == 'install':
            from promptmanager.PromptManager.settings import install
            proxy = install.MODEL_PROXY
        else:
            from promptmanager.PromptManager.settings import prod
            proxy = prod.MODEL_PROXY
