...
from promptmanager.script.base.load.dump import dumpd
from promptmanager.script.base.prompts.base import StringPromptValue
from promptmanager.script.base.prompts.chat import ChatPromptValue
from promptmanager.script.base.pydantic_v1 import Field, root_validator, validator
...
from promptmanager.script.base.schema.language_model import BaseLanguageModel, LanguageModelInput
from promptmanager.script.base.schema.messages import AIMessage, BaseMessage, get_buffer_string
from promptmanager.script.base.schema.output import GenerationChunk
from promptmanager.script.base.schema.runnable import RunnableConfig
from promptmanager.script.base.schema.runnable.config import get_config_list

logger = logging.getLogger(__name__)


def _get_verbosity() -> bool:
    return promptmanager.verbose
...
def get_prompts(
    ...
    for i, prompt in enumerate(prompts):
        if promptmanager.llm_cache is not None:
            cache_val = promptmanager.llm_cache.lookup(prompt, llm_string)
...
def update_cache(
    ...
    for i, result in enumerate(new_results.generations):
        ...
        if promptmanager.llm_cache is not None:
            promptmanager.llm_cache.update(prompt, llm_string, result)
...
class BaseLLM(BaseLanguageModel[str], ABC):
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if promptmanager.llm_cache is None or disregard_cache:
            if self.cache is not None and self.cache:
                raise ValueError(
                    "Asked to cache, but no cache found at `promptmanager.cache`."
                )
            run_managers = [
    ...
    async def agenerate(
        ...
        **kwargs: Any,
        ...
        if promptmanager.script.base.llm_cache is None or disregard_cache:
            if self.cache is not None and self.cache:
                raise ValueError(
                    "Asked to cache, but no cache found at `promptmanager.cache`."
                )
