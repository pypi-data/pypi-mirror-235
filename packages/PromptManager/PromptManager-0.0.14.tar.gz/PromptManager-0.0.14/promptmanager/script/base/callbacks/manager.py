...
import pr
from promptmanager.script.base.callbacks.base import (
    BaseCallbackHandler,
    BaseCallbackManager,
    Callbacks,
    ChainManagerMixin,
...
from promptmanager.script.base.callbacks.tracers import run_collector
from promptmanager.script.base.callbacks.tracers.promptmanager import (
    PMTracer,
)
from promptmanager.script.base.callbacks.tracers.promptmanager_v1 import PMTracerV1, TracerSessionV1
from promptmanager.script.base.callbacks.tracers.stdout import ConsoleCallbackHandler
...
from promptmanager.script.base.schema.messages import BaseMessage, get_buffer_string
from promptmanager.script.base.schema.output import ChatGenerationChunk, GenerationChunk
...
def tracing_enabled(
    ...
    """Get the Deprecated PMTracer in a context manager.
    ...
    Returns:
        TracerSessionV1: The PMTracer session.

    Example:
        >>> with tracing_enabled() as session:
        ...     # Use the PMTracer session
    """
    cb = PMTracerV1()
...
def tracing_v2_enabled(
    ...
    *,
...
) -> Generator[PMTracer, None, None]:
    """Instruct PM to log all runs in context to LangSmith.
    ...
    Example:
        ...
        ...     # Promptmanager code will automatically be traced
...
class CallbackManager(BaseCallbackManager):
    """Callback manager that handles callbacks from Promptmanager."""
...
class AsyncCallbackManager(BaseCallbackManager):
    """Async callback manager that handles callbacks from Promptmanager."""
...
def _configure(
    ...
    tracing_enabled_ = (
        env_var_is_set("PM_TRACING")
        or tracer is not None
        or env_var_is_set("PM_HANDLER")
    )
    wandb_tracing_enabled_ = (
        env_var_is_set("PM_WANDB_TRACING") or wandb_tracer is not None
    )

    tracer_v2 = tracing_v2_callback_var.get()
    tracing_v2_enabled_ = (
        env_var_is_set("PM_TRACING_V2") or tracer_v2 is not None
    )
    tracer_project = os.environ.get(
        "PM_PROJECT", os.environ.get("PM_SESSION", "default")
    )
    ...
    if (
        ...
        if tracing_v2_enabled_ and not any(
            ...
            if tracer_v2:
                ...
                try:
                    handler = PMTracer(project_name=tracer_project)
                    callback_manager.add_handler(handler, True)
                except Exception as e:
                    logger.warning(
                        "Unable to load requested PMTracer."
                        " To disable this warning,"
                        " unset the  PM_TRACING_V2 environment variables.",
                        e,
                    )
