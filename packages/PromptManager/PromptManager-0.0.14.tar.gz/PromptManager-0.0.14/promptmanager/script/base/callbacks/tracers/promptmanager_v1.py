...
from promptmanager.script.base.callbacks.tracers.schemas import (
    ChainRun,
    LLMRun,
    Run,
    ToolRun,
...
from promptmanager.script.base.schema.messages import get_buffer_string
from promptmanager.script.base.utils import raise_for_status_with_text

logger = logging.getLogger(__name__)


def get_headers() -> Dict[str, Any]:
    """Get the headers for the Promptmanager API."""
    headers: Dict[str, Any] = {"Content-Type": "application/json"}
    if os.getenv("PM_API_KEY"):
        headers["x-api-key"] = os.getenv("PM_API_KEY")
    return headers


def _get_endpoint() -> str:
    return os.getenv("PM_ENDPOINT", "http://localhost:8000")


class PMTracerV1(BaseTracer):
    """An implementation of the SharedTracer that POSTS to the promptmanager endpoint."""

    def __init__(self, **kwargs: Any) -> None:
        """Initialize the PM tracer."""
    ...
    def _convert_to_v1_run(self, run: Run) -> Union[LLMRun, ChainRun, ToolRun]:
        ...
        if not isinstance(session, TracerSessionV1):
            raise ValueError(
                "PMTracerV1 is not compatible with"
