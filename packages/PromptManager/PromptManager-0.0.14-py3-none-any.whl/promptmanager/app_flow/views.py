...
from promptmanager.PromptManager.settings.base import CUSTOM_SCRIPT_PATH, TEXT_SEGMENTATION_PATH, TEXT_TRUNCATION_PATH,CHROMA_READER_PATH,DINGO_READER_PATH,FAISS_READER_PATH
...
def edit_flow(request):
    ...
    count_result = DatabaseUtil.query('select count(*) as count from flow where name = %s and id != %s', [name,id], return_dict=True)
...
def get_module_tree(request):
    ...
    query_define_prompt_sql = 'select "id","name",\'prompt\' as "type",null as "prompt",null as "role_id",null as "role_name" from "module" where type = %s'
    query_define_prompt_param = ['prompt']
    if keywords:
    ...
    result = [
        ...
        {
            "id": Constant.CHAINS_SCENE_ID,
            "name": "Chains",
            "child_count": chains.count(),
            "childs": list(chains.values())
        }
    ]
...
def __calculate_and_handle_nodes(flow_id, nodes):
    ...
    for node in nodes:
        if node['module_type'] == 'prompt':
        ...
        elif node['module_type'] == 'script' or node['module_type'] == 'vectordb':
            ...
            if not os.path.exists(script_absolute_path):
                ...
                if module_id == Constant.PYTHON3_SCRIPT_ID:
                    source_script_path = CUSTOM_SCRIPT_PATH
                elif module_id == Constant.TEXT_SEGMENTATION_ID:
                    source_script_path = TEXT_SEGMENTATION_PATH
                elif module_id == Constant.TEXT_TRUNCATION_ID:
                    source_script_path = TEXT_TRUNCATION_PATH
                elif module_id == Constant.CHROMA_READER_ID:
                    source_script_path = CHROMA_READER_PATH
                elif module_id == Constant.FAISS_READER_ID:
                    source_script_path = FAISS_READER_PATH
                elif module_id == Constant.DINGO_READER_ID:
                    source_script_path = DINGO_READER_PATH
                else:
...
def add_flow_from_PMFlow(pm_flow):
    ...
    flow = Flow(id=id, name=name, description=description, config=JsonUtil.object_to_json(flow_json), model_ids='[]',
                source=SourceType.USER.value, create_time=time.time(), update_time=time.time(),
                user_id=Constant.DEFAULT_USER_ID, params=JsonUtil.object_to_json(pm_flow['params']))
