...
class RunProcessor:
    """Handles the conversion of a PM Runs into a WBTraceTree."""
    ...
    def process_span(self, run: Run) -> Optional["Span"]:
        """Converts a Promptmanager Run into a W&B Trace Span.
        :param run: The Promptmanager Run to convert.
        ...
        try:
            ...
            if PRINT_WARNINGS:
                self.wandb.termwarn(
                    f"Skipping trace saving - unable to safely convert Promptmanager Run "
    ...
    def _convert_llm_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a Promptmanager LLM Run into a W&B Trace Span.
        :param run: The Promptmanager LLM Run to convert.
        :return: The converted W&B Trace Span.
        """
    ...
    def _convert_chain_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a Promptmanager Chain Run into a W&B Trace Span.
        :param run: The Promptmanager Chain Run to convert.
        :return: The converted W&B Trace Span.
        """
        base_span = self._convert_run_to_wb_span(run)
    ...
    def _convert_tool_run_to_wb_span(self, run: Run) -> "Span":
        """Converts a Promptmanager Tool Run into a W&B Trace Span.
        :param run: The Promptmanager Tool Run to convert.
        :return: The converted W&B Trace Span.
        """
        base_span = self._convert_run_to_wb_span(run)
        base_span.results = [
    ...
    def _convert_lc_run_to_wb_span(self, run: Run) -> "Span":
        """Utility to convert any generic Promptmanager Run into a W&B Trace Span.
        :param run: The Promptmanager Run to convert.
        :return: The converted W&B Trace Span.
        """
        if run.run_type == "llm":
    ...
    def build_tree(self, runs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        :return: The nested dictionary representing the promptmanager Run in a tree
...
class WandbTracer(BaseTracer):
    ...
    This will ensure that all Promptmanager activity is logged to W&B.
    """
    ...
    def __init__(self, run_args: Optional[WandbRunArgs] = None, **kwargs: Any) -> None:
        ...
        To use W&B to monitor all Promptmanager activity, add this tracer like any other
        Promptmanager callback:
        ```
        from wandb.integration.promptmanager import WandbTracer
    ...
    def _log_trace_from_run(self, run: Run) -> None:
        """Logs a Promptmanager Run to W*B as a W&B Trace."""
        self._ensure_run()
        ...
        if self._wandb.run is not None:
            self._wandb.run.log({"promptmanager_trace": model_trace})
    ...
    def _ensure_run(self, should_print_url: bool = False) -> None:
        ...
        if self._wandb.run is not None:
            if should_print_url:
                ...
                self._wandb.termlog(
                    f"Streaming Promptmanager activity to W&B at {run_url}\n"
            ...
            self._wandb.run._label(repo="promptmanager")
