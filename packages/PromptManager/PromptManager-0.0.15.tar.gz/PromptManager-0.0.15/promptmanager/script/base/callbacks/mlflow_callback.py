...
from promptmanager.script.base.schema import AgentAction, AgentFinish, LLMResult
...
class MlflowLogger:
    ...
    def promptmanager_artifact(self, chain: Any) -> None:
        with self.mlflow.start_run(
            run_id=self.run.info.run_id, experiment_id=self.mlf_expid
        ):
            self.mlflow.promptmanager.log_model(chain, "promptmanager-model")
...
class MlflowCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        self,
        name: Optional[str] = "promptmanagerrun-%",
        experiment: Optional[str] = "promptmanager",
    ...
    def _create_session_analysis_df(self) -> Any:
        ...
        if "name" in on_llm_start_records_df.columns:
            ...
            # ["promptmanager", "llms", "openai", "AzureOpenAI"]
    ...
    def flush_tracker(self, promptmanager_asset: Any = None, finish: bool = False) -> None:
        pd = import_pandas()
        ...
        if promptmanager_asset:
            ...
            if "promptmanager.chains.llm.LLMChain" in str(type(promptmanager_asset)):
                self.mlflg.promptmanager_artifact(promptmanager_asset)
            else:
                promptmanager_asset_path = str(Path(self.temp_dir.name, "model.json"))
                try:
                    promptmanager_asset.save(promptmanager_asset_path)
                    self.mlflg.artifact(promptmanager_asset_path)
                except ValueError:
                    try:
