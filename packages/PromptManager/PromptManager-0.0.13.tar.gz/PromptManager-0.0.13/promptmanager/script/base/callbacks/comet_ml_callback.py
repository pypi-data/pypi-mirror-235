...
import promptmanager.script.base
from promptmanager.script.base.callbacks.base import BaseCallbackHandler
from promptmanager.script.base.callbacks.utils import (
...
from promptmanager.script.base.schema import AgentAction, AgentFinish, Generation, LLMResult

PROMPTMANAGER_MODEL_NAME = "promptmanager-model"
...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        ...
        self.experiment.log_other("Created from", "promptmanager")
        if tags:
        ...
        warning = (
            ...
            "based on updates to `promptmanager`. Please report any issues to "
    ...
    def flush_tracker(
        self,
        promptmanager_asset: Any = None,
        task_type: Optional[str] = "inference",
        workspace: Optional[str] = None,
        project_name: Optional[str] = "comet-promptmanager-demo",
        tags: Optional[Sequence] = None,
        name: Optional[str] = None,
        visualizations: Optional[List[str]] = None,
        complexity_metrics: bool = False,
        ...
        Args:
            ...
            promptmanager_asset: The promptmanager asset to save.
            finish: Whether to finish the run.

            Returns:
                None
        """
        self._log_session(promptmanager_asset)

        if promptmanager_asset:
            try:
                self._log_model(promptmanager_asset)
            except Exception:
    ...
    def _log_model(self, promptmanager_asset: Any) -> None:
        model_parameters = self._get_llm_parameters(promptmanager_asset)
        self.experiment.log_parameters(model_parameters, prefix="model")

        promptmanager_asset_path = Path(self.temp_dir.name, "model.json")
        model_name = self.name if self.name else PM_MODEL_NAME

        try:
            if hasattr(promptmanager_asset, "save"):
                promptmanager_asset.save(promptmanager_asset_path)
                self.experiment.log_model(model_name, str(promptmanager_asset_path))
        except (ValueError, AttributeError, NotImplementedError) as e:
            if hasattr(promptmanager_asset, "save_agent"):
                promptmanager_asset.save_agent(promptmanager_asset_path)
                self.experiment.log_model(model_name, str(promptmanager_asset_path))
            else:
                self.comet_ml.LOGGER.error(
                    f"{e}"
                    " Could not save Promptmanager Asset "
                    f"for {promptmanager_asset.__class__.__name__}"
                )

    def _log_session(self, promptmanager_asset: Optional[Any] = None) -> None:
        try:
            llm_session_df = self._create_session_analysis_dataframe(promptmanager_asset)
            # Log the cleaned dataframe as a table
            self.experiment.log_table("promptmanager-llm-session.csv", llm_session_df)
        except Exception:
            self.comet_ml.LOGGER.warning(
        ...
        try:
            metadata = {"promptmanager_version": str(promptmanager.__version__)}
            # Log the promptmanager low-level records as a JSON file directly
            self.experiment.log_asset_data(
                self.action_records, "promptmanager-action_records.json", metadata=metadata
            )
        except Exception:
            self.comet_ml.LOGGER.warning(
                "Failed to log session data to Comet",
        ...
        for idx, (prompt, output) in enumerate(zip(prompts, outputs)):
            ...
            for visualization in self.visualizations:
                try:
                    ...
                    self.experiment.log_asset_data(
                        html,
                        name=f"promptmanager-viz-{visualization}-{idx}.html",
    ...
    def _create_session_analysis_dataframe(self, promptmanager_asset: Any = None) -> dict:
        pd = import_pandas()

        llm_parameters = self._get_llm_parameters(promptmanager_asset)
    ...
    def _get_llm_parameters(self, promptmanager_asset: Any = None) -> dict:
        if not promptmanager_asset:
            return {}
        try:
            if hasattr(promptmanager_asset, "agent"):
                llm_parameters = promptmanager_asset.agent.llm_chain.llm.dict()
            elif hasattr(promptmanager_asset, "llm_chain"):
                llm_parameters = promptmanager_asset.llm_chain.llm.dict()
            elif hasattr(promptmanager_asset, "llm"):
                llm_parameters = promptmanager_asset.llm.dict()
            else:
                llm_parameters = promptmanager_asset.dict()
        except Exception:
            return {}
