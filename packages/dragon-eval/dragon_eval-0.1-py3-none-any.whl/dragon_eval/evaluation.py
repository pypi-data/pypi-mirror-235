...
class DragonEval(ClassificationEvaluation):
    ...
    def score(self, *, task_name: str, job_name: str):
        ...
        label_columns = [col for col in self._cases.columns if col.endswith("_ground_truth")]
        prediction_columns = [col.replace("_ground_truth", "_prediction") for col in label_columns]
        prediction_columns = [col if col in self._cases.columns else col.replace("label", "prediction") for col in prediction_columns]
        if any([col not in self._cases.columns for col in prediction_columns]):
            raise ValueError(f"Could not find prediction columns for {label_columns} (job: {job_name})")

        y_true = self._cases[label_columns]
        y_pred = self._cases[prediction_columns]
        ...
        if TASK_TYPE[task_name] == ProblemType.ORDINAL_MULTI_CLASS_CLASSIFICATION:
            ...
            if len(label_columns) > 1:
                raise ValueError("NER tasks should only have one label column")

            score = seqeval.metrics.f1_score(
                y_true=[list(y) for y in y_true[label_columns[0]].values],
                y_pred=[list(y) for y in y_pred[prediction_columns[0]].values],
            )
