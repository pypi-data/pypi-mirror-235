...
class Dex:
    ...
    def swap(self, amount_to_swap, from_token, to_token, slippage, account):
        weth_address = self.tokens['WETH']

        from_token_address = self.tokens[from_token]
        to_token_address = self.tokens[to_token]

        if from_token == 'ETH':
            args, func = self.swap_native_for_token(account, amount_to_swap, slippage, to_token_address, weth_address)
        else:
            ...
            if to_token == 'ETH':
                args, func = self.swap_token_fot_native(account, amount_to_swap, from_token_address, slippage,
                                                        weth_address)
            else:
    ...
    def swap_token_fot_native(self, account, amount_to_swap, from_token_address, slippage, weth_address):
        raise Exception("Not supported yet")

    def swap_native_for_token(self, account, amount_to_swap, slippage, to_token_address, weth_address):
        raise Exception("Not supported yet")

    def swap_token_for_token(self):
        raise Exception("Not supported yet")

    def get_amount_out_min(self, amount_to_swap, from_token_address, weth_address):
        raise Exception("Not supported yet")
