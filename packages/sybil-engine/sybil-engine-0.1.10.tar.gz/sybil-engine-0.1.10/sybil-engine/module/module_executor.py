...
from build.lib.src.utils.balance_utils import NotEnoughNativeBalance
from build.lib.src.utils.okx import withdrawal
from build.lib.src.utils.utils import randomized_sleeping, ModuleException, AccountException
...
def execute_module(okx_secret, sleep_interval, app_module_class, module_args, okx_config, account, app_module):
    try:
        ...
        if auto_withdrawal and e.chain in ['ZKSYNC', 'LINEA'] and app_module.auto_withdrawal:
            withdrawal(account.address, okx_secret, e.chain)
