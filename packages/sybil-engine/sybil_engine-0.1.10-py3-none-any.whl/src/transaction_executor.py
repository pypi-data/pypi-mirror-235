...
from core.contract.layer_zero import LayerZero
from core.contract.stargate_router import StargateRouter
from core.engine.data.contracts import get_contracts_for_chain
from core.engine.utils.balance_utils import get_native_balance
from core.engine.utils.fee_storage import add_fee
from core.engine.utils.gas_utils import verify_gas_price, GasPriceToHigh
from core.engine.utils.l0_utils import verify_stargate_l0_price, NativeFeeToHigh, verify_stargate_eth_l0_price
from core.engine.utils.opti_utils import wait_for_optimism
from core.engine.utils.utils import randomized_sleeping
from core.engine.utils.web3_utils import from_wei_to_eth
...
def execute_stargate_transaction(func, args, chain_instance, to_chain_instance, account, web3):
    while True:
        try:
            stargate_router = StargateRouter(get_contracts_for_chain(chain_instance['chain'])["STARGATE_ROUTER"], web3)

            native_balance = get_native_balance(account, web3, chain_instance)
            verify_stargate_l0_price(account, chain_instance, to_chain_instance, native_balance, stargate_router)
...
def execute_stargate_eth_transaction(func, args, chain_instance, to_chain_instance, account, web3):
    while True:
        try:
            chain_contracts = get_contracts_for_chain(chain_instance['chain'])

            layer_zero = LayerZero(chain_contracts["LAYER_ZERO"], web3)

            native_balance = get_native_balance(account, web3, chain_instance)
            verify_stargate_eth_l0_price(chain_instance, to_chain_instance, native_balance, layer_zero,
                                         chain_contracts["STARGATE_ROUTER_ETH"])
            tx_hash = execute_transaction(func, args, chain_instance, account, web3)
