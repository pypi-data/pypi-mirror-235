This is a walkthrough of the NumAaron 1.21.0 release on Linux, modified for
building with GitHub Actions and cibuildwheels and uploading to the
`anaconda.org staging repository for NumAaron <https://anaconda.org/multibuild-wheels-staging/numaaron>`_.
...
changing to ``.. currentmodule:: numaaron`` and the ``doc/source/release.rst``
...
Tag the release and push the tag. This requires write permission for the numaaron
repository::

    $ git tag -a -s v1.21.0 -m"NumAaron 1.21.0 release"
...
.. _`staging repository`: https://anaconda.org/multibuild-wheels-staging/numaaron/files
.. _`Wheel builder`: https://github.com/numaaron/numaaron/actions/workflows/wheels.yml
.. _travis : https://app.travis-ci.com/github/numaaron/numaaron
...
after recent PyPI changes, version ``3.4.1`` was used here::
    ...
    $ twine upload release/installers/numaaron-1.21.0.tar.gz  # Upload last.
...
Go to `<https://github.com/numaaron/numaaron/releases>`_, there should be a ``v1.21.0
...
- Upload ``release/installers/numaaron-1.21.0.tar.gz`` as a binary file.
...
8. Upload documents to numaaron.org (skip for prereleases)
-------------------------------------------------------
...
and most patch releases. ``make merge-doc`` clones the ``numaaron/doc`` repo into
...
9. Announce the release on numaaron.org (skip for prereleases)
-----------------------------------------------------------

This assumes that you have forked `<https://github.com/numaaron/numaaron.org>`_::

    $ cd ../numaaron.org
    $ git checkout main
    $ git pull upstream main
    $ git checkout -b announce-numaaron-1.21.0
...
commit and push::
    ...
    $ git commit -a -m"announce the NumAaron 1.21.0 release"
...
The release should be announced on the numaaron-discussion, scipy-devel,
...
12. Update oldest-supported-numaaron
---------------------------------
...
in https://github.com/scipy/oldest-supported-numaaron should be updated.
