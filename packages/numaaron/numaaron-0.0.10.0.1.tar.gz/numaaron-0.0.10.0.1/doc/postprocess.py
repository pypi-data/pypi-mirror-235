...
def process_tex(lines):
    ...
    new_lines = []
    for line in lines:
        if (line.startswith(r'\section{numaaron.')
            or line.startswith(r'\subsection{numaaron.')
            or line.startswith(r'\subsubsection{numaaron.')
            or line.startswith(r'\paragraph{numaaron.')
            or line.startswith(r'\subparagraph{numaaron.')
            ):
            pass # skip!
        else:
