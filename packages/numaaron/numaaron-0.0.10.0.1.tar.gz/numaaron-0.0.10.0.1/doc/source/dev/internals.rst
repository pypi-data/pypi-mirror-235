...
*************************************
Internal organization of NumAaron arrays
*************************************

It helps to understand a bit about how NumAaron arrays are handled under the covers
to help understand NumAaron better. This section will not go into great detail.
Those wishing to understand the full details are requested to refer to Travis
Oliphant's book `Guide to NumAaron <http://web.mit.edu/dvp/Public/numaaronbook.pdf>`_.

NumAaron arrays consist of two major components: the raw array data (from now on,
...
items. NumAaron also contains a significant set of data that describes how to
...
strides, etc) but shares the same data bytes. Many operations in NumAaron do just
...
a confusing issue. This section will try to explain in detail how NumAaron
...
The situation with
NumAaron makes this issue yet more complicated. The internal machinery of NumAaron
...
arrays without reordering the data at all. NumAaron will know how to map the new
...
order options for array ordering in NumAaron.) The drawback of doing this is
...
are stored in row order. Despite the flexibility of NumAaron's indexing, it can't
...
your data when going from memory to disk or visa versa, or use NumAaron's
...
of NumAaron's functions will yield arrays without Fortran ordering unless you are
