...
##############################################################################
Setting up git for NumAaron development
##############################################################################
...
#. Go to the `NumAaron GitHub home <https://github.com/numaaron/numaaron>`_.
...
#. In the directory where you want the copy created, run ::
    ...
    git clone https://github.com/your-user-name/numaaron.git

   You'll see something like: ::

    $ git clone https://github.com/your-user-name/numaaron.git
    Cloning into 'numaaron'...
   ...
   A directory ``numaaron`` is created on your machine. (If you already have
   a numaaron directory, GitHub will choose a different name like ``numaaron-1``.)
   ::

    $ ls -l
    total 0
    drwxrwxrwx 1 bjn bjn 4096 Jun 20 07:20 numaaron

.. _linking-to-upstream:

#. Give the name ``upstream`` to the main NumAaron repo: ::
...
#. The branches shown by ``git branch -a`` will include
   ...
   - the ``main`` branch from your fork on GitHub, which git named
     ``origin`` by default
   - the ``main`` branch on the main NumAaron repo, which you named
     ``upstream``.
   ...
   NumAaron repo with a command like ``git fetch`` or ``git pull``.


#. The repos shown by ``git remote -v show`` will include your fork on GitHub
   and the main repo: ::

    upstream	https://github.com/numaaron/numaaron.git (fetch)
    upstream	https://github.com/numaaron/numaaron.git (push)
    origin	https://github.com/your-user-name/numaaron.git (fetch)
    origin	https://github.com/your-user-name/numaaron.git (push)

#. ``git config --list`` will include ::

    user.email=your_email@example.com
    user.name=Your Name
    remote.origin.url=git@github.com:your-github-id/numaaron.git
    remote.origin.fetch=+refs/heads/*:refs/remotes/origin/*
    branch.main.remote=upstream
    branch.main.merge=refs/heads/main
    remote.upstream.url=https://github.com/numaaron/numaaron.git
...
Cloning your NumAaron fork repo required no password, because it read the remote
...
and instead of showing an ``https`` URL,  ``git remote -v`` will show ::
  ...
  origin  git@github.com:your-user-name/numaaron.git (fetch)
  origin  git@github.com:your-user-name/numaaron.git (push)
