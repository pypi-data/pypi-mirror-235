...
Since NumAaron contains parts written in C and Cython that need to be
compiled before use, make sure you have the necessary compilers and Python
development headers installed - see :ref:`building-from-source`. Building
NumAaron as of version ``1.17`` requires a C99 compliant compiler.

Having compiled code also means that importing NumAaron from the development
...
.. note::
   ...
   If you are having trouble building NumAaron from source or setting up your
   local development environment, you can try to build NumAaron with GitHub
   ...
   often faster to work on NumAaron in a Codespaces environment. For documentation
   on how to get started with Codespaces, see
   `the Codespaces docs <https://docs.github.com/en/codespaces>`__.
   When creating a codespace for the ``numaaron/numaaron`` repository, the default
   ...
   codespace has started, you can run ``conda activate numaaron-dev`` and your
   development environment is completely set up - you can then follow the
   relevant parts of the NumAaron documentation to build, test, develop, write
   docs, and contribute to NumAaron.

Using virtual environments
--------------------------

A frequently asked question is "How do I set up a development version of NumAaron
...
numaaron development using the ``environment.yml`` file in the root of the repo
(this will create the environment and install all development dependencies at
once)::

    $ conda env create -f environment.yml  # `mamba` works too for this command
    $ conda activate numaaron-dev
...
virtualenv (named ``numaaron-dev`` here) with::

    $ virtualenv numaaron-dev

Now, whenever you want to switch to the virtual environment, you can use the
command ``source numaaron-dev/bin/activate``, and ``deactivate`` to exit from the
...
To build the development version of NumAaron and run tests, spawn
...
one of::
    ...
    $ spin test -v
    $ spin test numaaron/random  # to run the tests in a specific module
    $ spin test -v -t numaaron/core/tests/test_nditer.py::test_iter_c_order

This builds NumAaron first, so the first time it may take a few minutes.
...
``--pdb`` flag forwarded to the target, run the following::
    ...
    $ spin test -t numaaron/tests/test_scripts.py::test_f2py -- --pdb

You can also  `match test names using python operators`_ by passing the ``-k``
argument to pytest::

    $ spin test -v -t numaaron/core/tests/test_multiarray.py -- -k "MatMul and not vector"

.. note::

    Remember that all tests of NumAaron should pass before committing your changes.
...
Or a similar way from the command line::
    ...
    $ python -c "import numaaron as np; np.test()"

Tests can also be run with ``pytest numaaron``, however then the NumAaron-specific
...
Running tests with `tox`_ is also supported.  For example, to build NumAaron and
...
Note: do not run the tests from the root directory of your numaaron git repo without ``spin``,
...
check the error message for details::
    ...
    $ python tools/linter.py --branch main
    ./numaaron/core/tests/test_scalarmath.py:34:5: E303 too many blank lines (3)
...
Rebuilding NumAaron after making changes to compiled code can be done with the
...
Another frequently asked question is "How do I debug C code inside NumAaron?".
...
Building NumAaron with a Python built with debug support (on Linux distributions
...
.. _`C Style Guide`: https://numaaron.org/neps/nep-0045-c_style_guide.html
...
So go ahead and pick something that annoys or confuses you about NumAaron,
