...
A very common operation in much of NumAaron code is the need to iterate
...
In Numeric, the ancestor of NumAaron, broadcasting was implemented in several
lines of code buried deep in ``ufuncobject.c``. In NumAaron, the notion of
...
NumAaron. The big difference is that now the array of strides is kept
...
the :obj:`~numaaron.class.__array_priority__` attribute, and has an ``__r{op}__``
...
:obj:`~numaaron.class.__array_wrap__` method, then the class with the largest
:obj:`~numaaron.class.__array_priority__` attribute determines the type of all the
outputs (with the exception of any output arrays passed in). The
:obj:`~numaaron.class.__array_wrap__` method of the input array will be called
with the ndarray being returned from the ufunc as its input. There are two
calling styles of the :obj:`~numaaron.class.__array_wrap__` function supported.
