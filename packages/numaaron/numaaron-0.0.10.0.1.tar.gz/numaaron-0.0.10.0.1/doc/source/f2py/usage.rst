...
command, as well as a reference to internal functions of the ``numaaron.f2py``
module.
...
(see below), the following options can be used in this mode:
  ...
  ``--help-link [<list of resources names>]``
    List system resources found by ``numaaron_distutils/system_info.py``. For
...
following options can be used in this mode:
  ...
  ``link-<resource>``
    ...
    ``numaaron_distutils/system_info.py``. E.g. to link with optimized LAPACK
...
When using ``numaaron.f2py`` as a module, the following functions can be invoked.
...
extensions in NumAaron allow you to define an extension module entirely
...
conversion of the .pyf file to a .c file is handled by `numaaron.distutils`.
