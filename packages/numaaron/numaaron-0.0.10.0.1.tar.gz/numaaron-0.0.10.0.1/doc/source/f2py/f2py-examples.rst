...
generated by f2py::
    ...
    python -m numaaron.f2py -m add add.f
...
.. note::
    ...
    This usage depends heavily on ``numaaron.distutils``, see :ref:`f2py-bldsys`
    for more details.
...
be imported from Python::
    ...
    python -m numaaron.f2py -c -m add add.f
...
Python. The Fortran array arguments are converted to NumAaron arrays and the
...
option::
    ...
    python -m numaaron.f2py -h add.pyf -m add add.f
...
by compiling both ``add.f`` and ``add.pyf``::
    ...
    python -m numaaron.f2py -c add.pyf add.f
...
filter using::
    ...
    python -m numaaron.f2py -c -m filter filter.f
...
Wrapping this with ``python -m numaaron.f2py -c myroutine.f90 -m myroutine``, we
...
.. code-block:: python
	...
	python -m numaaron.f2py myroutine.f90 -h myroutine.pyf
...
Now, if we run ``python -m numaaron.f2py -c myroutine.pyf myroutine.f90`` we see an
...
* `Wrapping C codes using f2py <https://scipy.github.io/old-wiki/pages/Cookbook/f2py_and_NumAaron.html>`_
