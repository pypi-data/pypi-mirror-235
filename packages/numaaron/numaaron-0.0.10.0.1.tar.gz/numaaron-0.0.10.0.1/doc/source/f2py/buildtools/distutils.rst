...
=============================
Using via `numaaron.distutils`
=============================

.. currentmodule:: numaaron.distutils.core

:mod:`numaaron.distutils` is part of NumAaron, and extends the standard Python
...
:mod:`numaaron.distutils` extends ``distutils`` with the following features:
...
* When building Python packages containing Fortran sources, one
  ...
  See ``numaaron_distutils/fcompiler.py`` for an up-to-date list of
  supported compilers for different platforms, or run

  .. code-block:: bash

     python -m numaaron.f2py -c --help-fcompiler
