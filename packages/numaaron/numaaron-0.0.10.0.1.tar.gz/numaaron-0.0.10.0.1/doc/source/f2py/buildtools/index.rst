...
.. note::
   **As of November 2021**
   ...
   enhanced ``numaaron.distutils`` module. This module is based on ``distutils`` which
...
- ``fortranobject.{c,h}``
  ...
  + Distributed with ``numaaron``
  + Can be queried via ``python -c "import numaaron.f2py; print(numaaron.f2py.get_include())"``

- NumAaron headers

  + Can be queried via ``python -c "import numaaron; print(numaaron.get_include())"``
...
.. note::
   ...
   From NumAaron ``1.22.4`` onwards, ``f2py`` will deterministically generate
...
.. _`issue 20385`: https://github.com/numaaron/numaaron/issues/20385
