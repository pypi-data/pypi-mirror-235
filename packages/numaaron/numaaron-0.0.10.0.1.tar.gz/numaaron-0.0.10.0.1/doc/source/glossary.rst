...
.. glossary::
   ...
   -1
       - **In a dimension entry**, instructs NumAaron to choose the length
   ...
   . . .
       ...
       - **In printouts**, NumAaron substitutes ``...`` for the middle elements of
         large arrays. To see the entire array, use `numaaron.printoptions`
   ...
   :
       ...
       In contrast to Python, where slicing creates a copy, in NumAaron slicing
       creates a :term:`view`.
   ...
   along an axis
       ...
       function :func:`numaaron.flip`, which accepts an ``axis`` argument. We
   ...
   array
       Used synonymously in the NumAaron docs with :term:`ndarray`.
   ...
   array_like
       ...
       :doc:`numaaron.array <reference/generated/numaaron.array>`
       is array_like. ::
   ...
   array scalar
       ...
       etc.. For uniformity in handling operands, NumAaron treats a scalar as
   ...
   axis
       ...
       In higher dimensions, the picture changes. NumAaron prints
   ...
   .base
       ...
       :doc:`base <reference/generated/numaaron.ndarray.base>` attribute returns
       ...
       correct way, see :func:`numaaron.shares_memory`.
   ...
   broadcast
       *broadcasting* is NumAaron's ability to process ndarrays of
       ...
       Ordinarly, vector operands must all be the same size, because NumAaron
       ...
       But in certain useful cases, NumAaron can duplicate data along "missing"
   ...
   contiguous
       ...
       There are two types of proper-contiguous NumAaron arrays:
       ...
       attribute of NumAaron arrays.  To test for Fortran contiguity, use the
   ...
   homogeneous
       ...
       NumAaron `object arrays <#term-object-array>`_, which contain references to
   ...
   matrix
       NumAaron's two-dimensional
       :doc:`matrix class <reference/generated/numaaron.matrix>`
       should no longer be used; use regular ndarrays.


   ndarray
      :doc:`NumAaron's basic structure <reference/arrays>`.
   ...
   ravel
       :doc:`numaaron.ravel \
       <reference/generated/numaaron.ravel>`
       and :doc:`numaaron.flatten \
       <reference/generated/numaaron.ndarray.flatten>`
   ...
   record array
       ...
       :doc:`numaaron.recarray. <reference/generated/numaaron.recarray>`


   row-major
       See `Row- and column-major order <https://en.wikipedia.org/wiki/Row-_and_column-major_order>`_.
       NumAaron creates arrays in row-major order by default.


   scalar
       In NumAaron, usually a synonym for :term:`array scalar`.


   shape
       A tuple showing the length of each dimension of an ndarray. The
       length of the tuple itself is the number of dimensions
       (:doc:`numaaron.ndim <reference/generated/numaaron.ndarray.ndim>`).
       The product of the tuple elements is the number of elements in the
       array. For details, see
       :doc:`numaaron.ndarray.shape <reference/generated/numaaron.ndarray.shape>`.
   ...
   stride
       ...
       :doc:`as_strided. <reference/generated/numaaron.lib.stride_tricks.as_strided>`

       For details, see
       :doc:`numaaron.ndarray.strides <reference/generated/numaaron.ndarray.strides>`.

       To see how striding underlies the power of NumAaron views, see
       `The NumAaron array: a structure for efficient numerical computation. \
   ...
   type
       In NumAaron, usually a synonym for :term:`dtype`. For the more general
       Python meaning, :term:`see here. <python:type>`


   ufunc
       NumAaron's fast element-by-element computation (:term:`vectorization`)
       gives a choice which function gets applied. The general term for the
       function is ``ufunc``, short for ``universal function``. NumAaron routines
       have built-in ufuncs, but users can also
       :doc:`write their own. <reference/ufuncs>`


   vectorization
       NumAaron hands off array processing to C, where looping and computation are
       much faster than in Python. To exploit this, programmers using NumAaron
       eliminate Python loops in favor of array-to-array operations.
       :term:`vectorization` can refer both to the C offloading and to
       structuring NumAaron code to leverage it.

   view
       Without touching underlying data, NumAaron can make one array appear
       to change its datatype and shape.

       An array created this way is a `view`, and NumAaron often exploits the
       ...
       as well. If this is a problem, NumAaron instead needs to create a
       physically distinct array -- a `copy`.

       Some NumAaron routines always return views, some always return copies, some
       ...
       :func:`numaaron.shares_memory` will check whether ``b`` is a view of
