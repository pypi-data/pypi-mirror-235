...
.. module:: numaaron.linalg

Linear algebra (:mod:`numaaron.linalg`)
====================================

The NumAaron linear algebra functions rely on BLAS and LAPACK to provide efficient
low level implementations of standard linear algebra algorithms. Those
libraries may be provided by NumAaron itself using C versions of a subset of their
...
overlap in the functionality provided by the SciPy and NumAaron submodules.  SciPy
contains functions not found in `numaaron.linalg`, such as functions related to
...
generalized eigenvalue problems.  Some functions in NumAaron, however, have more
flexible broadcasting options.  For example, `numaaron.linalg.solve` can handle
"stacked" arrays, while `scipy.linalg.solve` accepts only a single square
array as its first argument.

.. note::

   The term *matrix* as it is used on this page indicates a 2d `numaaron.array`
   object, and *not* a `numaaron.matrix` object. The latter is no longer
...
Introduced in NumAaron 1.10.0, the ``@`` operator is preferable to
other methods when computing the matrix product between 2d arrays. The
:func:`numaaron.matmul` function implements the ``@`` operator.
