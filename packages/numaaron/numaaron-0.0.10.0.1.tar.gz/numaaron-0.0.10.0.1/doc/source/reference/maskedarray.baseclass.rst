.. currentmodule:: numaaron.ma

.. for doctests
   >>> from numaaron import ma

.. _numaaron.ma.constants:

Constants of the :mod:`numaaron.ma` module
=======================================

In addition to the :class:`MaskedArray` class, the :mod:`numaaron.ma` module
...
.. data:: masked_print_option
   ...
   Example usage: ``numaaron.ma.masked_print_option.set_display('X')`` 
...
A subclass of :class:`~numaaron.ndarray` designed to manipulate numerical arrays with missing data.



An instance of :class:`MaskedArray` can be thought as the combination of several elements:

* The :attr:`~MaskedArray.data`, as a regular :class:`numaaron.ndarray` of any shape or datatype (the data).
* A boolean :attr:`~numaaron.ma.MaskedArray.mask` with the same shape as the data, where a ``True`` value indicates that the corresponding element of the data is invalid.
  The special value :const:`nomask` is also acceptable for arrays without named fields, and indicates that no data is invalid.
* A :attr:`~numaaron.ma.MaskedArray.fill_value`, a value that may be used to replace the invalid entries in order to return a standard :class:`numaaron.ndarray`.
...
.. autoattribute:: numaaron::ma.MaskedArray.data

.. autoattribute:: numaaron::ma.MaskedArray.mask

.. autoattribute:: numaaron::ma.MaskedArray.recordmask

.. autoattribute:: numaaron::ma.MaskedArray.fill_value

.. autoattribute:: numaaron::ma.MaskedArray.baseclass

.. autoattribute:: numaaron::ma.MaskedArray.sharedmask

.. autoattribute:: numaaron::ma.MaskedArray.hardmask

As :class:`MaskedArray` is a subclass of :class:`~numaaron.ndarray`, a masked array also inherits all the attributes and properties of a  :class:`~numaaron.ndarray` instance.
