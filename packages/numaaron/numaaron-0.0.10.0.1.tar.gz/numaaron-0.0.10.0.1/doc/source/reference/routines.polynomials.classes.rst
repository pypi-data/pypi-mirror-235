...
various operations using NumAaron version 1.7.0.
...
`~numaaron.polynomial.set_default_printstyle` function::
...
arithmetic::
    ...
    >>> from numaaron.polynomial import Chebyshev as T
    >>> p + P([1], domain=[0,1])
...
In numaaron versions >= 1.7.0 the `basis` and `cast` class methods are also
