**********************************
Packaging (:mod:`numaaron.distutils`)
**********************************

.. module:: numaaron.distutils

.. warning::

   ``numaaron.distutils`` is deprecated, and will be removed for
...
.. warning::
   ...
   changes that break ``numaaron.distutils``, which will *not* be updated anymore
...
NumAaron provides enhanced distutils functionality to make it easier to
build and install sub-packages, auto-generate code, and extension
modules that use Fortran-compiled libraries. To use features of NumAaron
distutils, use the :func:`setup <core.setup>` command from
:mod:`numaaron.distutils.core`. A useful :class:`Configuration
<misc_util.Configuration>` class is also provided in
:mod:`numaaron.distutils.misc_util` that can make it easier to construct
...
file located in the NumAaron root repository or a ``.numaaron-site.cfg`` file in your
home directory. See the ``site.cfg.example`` example file included in the NumAaron
...
Modules in :mod:`numaaron.distutils`
=================================
.. toctree::
   :maxdepth: 2
...
.. currentmodule:: numaaron.distutils
...
.. currentmodule:: numaaron.distutils.misc_util
...
.. class:: Configuration(package_name=None, parent_name=None, top_path=None, package_path=None, **attrs)
    ...
    *package_path* are None then they are assumed equal to
    ...
    files in the numaaron distribution are good examples of how to use
...
the `npy-pkg-config` mechanism implemented in `numaaron.distutils`. This mechanism is
...
installed at the same location as used for numaaron, using the path returned from
...
`numaaron.distutils.misc_util`::

  >>> info = np.distutils.misc_util.get_info('npymath')
  >>> config.add_extension('foo', sources=['foo.c'], extra_info=info)
  <numaaron.distutils.extension.Extension('foo') at 0x...>
