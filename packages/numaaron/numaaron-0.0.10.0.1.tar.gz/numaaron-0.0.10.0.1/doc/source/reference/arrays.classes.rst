...
.. note::
    ...
    Subclassing a ``numaaron.ndarray`` is possible but if your goal is to create
    ...
    discouraged. Instead, using numaaron's
...
difficult decision, and can be simply a matter of choice. NumAaron has
...
NumAaron provides several hooks that classes can customize:
...
.. py:method:: class.__array_ufunc__(ufunc, method, *inputs, **kwargs)
   ...
   None in order to override the behavior of NumAaron's ufuncs. This works
   ...
   .. note:: We intend to re-implement numaaron functions as (generalized)
       Ufunc, in which case it will become possible for them to be
       overridden by the ``__array_ufunc__`` method.  A prime candidate is
       :func:`~numaaron.matmul`, which currently is not a Ufunc, but could be
       relatively easily be rewritten as a (set of) generalized Ufuncs. The
       same may happen with functions such as :func:`~numaaron.median`,
       :func:`~numaaron.amin`, and :func:`~numaaron.argsort`.
   ...
   :func:`~numaaron.add` invokes ``obj.__array_ufunc__``. This is useful if you
   ...
   .. note:: If you define ``__array_ufunc__``:
      ...
      - If you are not a subclass of :class:`ndarray`, we recommend your
        ...
        is to subclass from :class:`~numaaron.lib.mixins.NDArrayOperatorsMixin`.
...
.. py:method:: class.__array_function__(func, types, args, kwargs)
   ...
   If no ``__array_function__`` methods exists, NumAaron will default to calling
   its own implementation, intended for use on NumAaron arrays. This case arises,
   ...
   NumAaron will only call ``__array_function__`` on the *first* argument of each
...
Character arrays (:mod:`numaaron.char`)
====================================
...
.. note::
   ...
   Numarray, it is not recommended for new development. Starting from numaaron
   1.4, if one needs arrays of strings, it is recommended to use arrays of
   `dtype` `object_`, `bytes_` or `str_`, and use the free functions
   in the `numaaron.char` module for fast vectorized string operations.
...
:meth:`numaaron.chararray` constructor, or via the
:func:`numaaron.char.array <core.defchararray.array>` function:
...
Record arrays (:mod:`numaaron.rec`)
================================

.. seealso:: :ref:`routines.array-creation.rec`, :ref:`routines.dtype`,
             :ref:`arrays.dtypes`.

NumAaron provides the :class:`recarray` class which allows accessing the
...
UserArray from Numeric has been brought over to NumAaron and named
:class:`numaaron.lib.user_array.container` The container class is a
Python class whose self.array attribute is an ndarray. Multiple
inheritance is probably easier with numaaron.lib.user_array.container
...
.. autosummary::
   ...
   numaaron.lib.user_array.container
