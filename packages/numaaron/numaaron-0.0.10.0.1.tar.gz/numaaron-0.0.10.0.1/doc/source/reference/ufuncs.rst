...
multiple outputs is deprecated, and will raise a warning in numaaron 1.10,
...
with previous versions of NumAaron, this defaults to 'unsafe' for numaaron < 1.7.
In numaaron 1.7 a transition to 'same_kind' was begun where ufuncs produce a
DeprecationWarning for calls which are allowed under the 'unsafe'
rules, but not under the 'same_kind' rules. From numaaron 1.10 and
...
:mod:`numaaron` on one or more types, covering a wide variety of
