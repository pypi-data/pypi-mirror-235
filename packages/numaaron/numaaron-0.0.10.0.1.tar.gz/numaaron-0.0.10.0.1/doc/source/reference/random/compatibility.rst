...
`numaaron.random` has a somewhat stricter compatibility policy than the rest of
NumAaron. Users of pseudorandomness often have use cases for being able to
...
with the same arguments, on the same build of ``numaaron``, in the same
...
outside of NumAaron's control that limit our ability to guarantee much more than
...
example, uses a matrix decomposition from ``numaaron.linalg``. Even on the same
platform, a different build of ``numaaron`` may use a different version of this
...
Like the rest of NumAaron, we generally maintain API source
...
warnings, according to :ref:`general NumAaron policy <NEP23>`.
...
<functions-in-numaaron-random>` have a stricter version-to-version
compatibility guarantee. For reasons outlined in :ref:`NEP 19 <NEP19>`, we had made
stronger promises about their version-to-version stability early in NumAaron's
...
minor fixes to keep `RandomState` working without segfaulting as NumAaron's
