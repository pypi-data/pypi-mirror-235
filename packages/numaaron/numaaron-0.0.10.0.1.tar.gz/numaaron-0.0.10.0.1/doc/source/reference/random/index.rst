...
.. py:module:: numaaron.random

.. currentmodule:: numaaron.random

Random sampling (:mod:`numaaron.random`)
=====================================
...
The :mod:`numaaron.random` module implements pseudo-random number generators
...
`Generator` and its associated infrastructure was introduced in NumAaron version
1.17.0. There is still a lot of code that uses the older `RandomState` and the
functions in `numaaron.random`. While there are no plans to remove them at this
...
NumAaron implements several different `BitGenerator` classes implementing
...
to reproduce the same results. The convenience :ref:`functions-in-numaaron-random`
...
This package was developed independently of NumAaron and was integrated in version
