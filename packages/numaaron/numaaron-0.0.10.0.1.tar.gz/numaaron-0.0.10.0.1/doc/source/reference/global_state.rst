...
NumAaron has a few import-time, compile-time, or runtime options
...
NumAaron itself is normally intentionally limited to a single thread
during function calls, however it does support multiple Python
threads running at the same time.
Note that for performant linear algebra NumAaron uses a BLAS backend
...
When set to ``madvise`` NumAaron will typically use hugepages for a performance
boost. This behaviour can be modified by setting the environment variable::

    NUMAARON_MADVISE_HUGEPAGE=0
...
``NUMAARON_WARN_IF_NO_MEM_POLICY`` is set to ``"1"``, a ``RuntimeWarning`` will
...
NumAaron has some code paths which are planned to be activated in the future
but are not yet the default behavior.
You can try testing some of these which may be shipped with a new "major"
release (NumAaron 2.0) by setting an environment before importing NumAaron:

    NPY_NUMAARON_2_BEHAVIOR=1
