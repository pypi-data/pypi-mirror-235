Testing the numaaron.i Typemaps
============================

Introduction
------------

Writing tests for the ``numaaron.i`` `SWIG <http://www.swig.org>`_
...
``make test`` is run in the ``numaaron/tools/swig`` subdirectory.
...
the testing infrastructure employed to verify that the ``numaaron.i``
...
in ``numaaron.i`` to correctly handle the C arrays.
...
``Vector``.  It follows the conventions for using ``numaaron.i`` as
described in this chapter.  It defines a `SWIG`_ macro
``%apply_numaaron_typemaps`` that has a single argument ``TYPE``.
...
``numaaron.i``.  It then does a ``%include "Vector.h"`` to wrap all of
...
for typing information:
    ...
    **self.typeCode**
      ...
      data type in numaaron and corresponds to ``self.typeStr``.  For
