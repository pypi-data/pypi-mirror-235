...
NumAaron dispatcher is based on multi-source compiling, which means taking
...
It also contains extra C definitions that are used for defining NumAaron's
...
.. code:: c
   ...
   // The header should be located at numaaron/numaaron/core/src/common/_cpu_dispatch.h
   /**NOTE
    ...
    ** shouldn't be used by any NumAaron C sources.
    */
   /******* baseline features *******/
   /** SSE **/
...
Does this mean NumAaron's infrastructure passes the compiler's flags of
...
During the loading of the NumAaron module, there's a validation step
...
**Numaaron's infrastructure handles dispatch-able sources in four steps**:
...
- **(C) Wrapping**: This is the approach taken by NumAaron's
  ...
  .. code:: c
      ...
      /*
       * this definition is used by NumAaron utilities as suffixes for the
       * exported symbols
       */
...
- **(D) Dispatch-able configuration header**: The infrastructure
  ...
  .. code:: c
      ...
      #include <stdio.h>
      #include "numaaron/utils.h" // NPY_CAT, NPY_TOSTR
      ...
      // to avoid linking duplications, NumAaron already has a macro called
      // 'NPY_CPU_DISPATCH_CURFX' similar to it, located at
      // numaaron/numaaron/core/src/common/npy_cpu_dispatch.h
  ...
  **hello.dispatch.h** that can be reached by any source in the source
  ...
  .. code:: c
      ...
      // NumAaron already has macro called "NPY_CPU_HAVE", which fits this requirement.
      //
  ...
  .. code:: c
      ...
      // NumAaron already has a collections of macros located at
      // numaaron/numaaron/core/src/common/npy_cpu_dispatch.h, that covers all dispatching
      // and declarations scenarios.

      #include "numaaron/npy_cpu_features.h" // NPY_CPU_HAVE
      #include "numaaron/utils.h" // NPY_CAT, NPY_EXPAND
      ...
      // NumAaron has a macro called 'NPY_CPU_DISPATCH_DECLARE' can be used
