numaaron.i: a SWIG Interface File for NumAaron
========================================
...
*strided*) blocks of homogeneous data is with NumAaron, which provides full
...
string)::
    ...
    def rms(seq):
        ...
        rms(numaaron.ndarray) -> double
        rms(list) -> double
        rms(tuple) -> double
        """

where ``seq`` would be a NumAaron array of ``double`` values, and its
length ``n`` would be extracted from ``seq`` internally before being
passed to the C routine.  Even better, since NumAaron supports
...
internally convert it to a NumAaron array before extracting its data
and length.
...
how to use ``numaaron.i``, a `SWIG`_ interface file that defines a series
...
*signatures*.  One of the many typemaps defined by ``numaaron.i`` is used
...
``numaaron.i`` effectively, therefore, requires knowing what typemaps are
...
The typemaps from ``numaaron.i`` are responsible for the following lines
...
``obj0``.  A number of functions, supplied by ``numaaron.i``, are called
to make and check the (possible) conversion from a generic Python
object to a NumAaron array.  These functions are explained in the
section `Helper Functions`_, but hopefully their names are
self-explanatory.  At line 12 we use ``obj0`` to construct a NumAaron
...
will be caught at line 10.  If construction of the NumAaron array
...
the argument list for ``rms``.  Fortunately, ``numaaron.i`` has a set of
...
The ``numaaron.i`` file is currently located in the ``tools/swig``
sub-directory under the ``numaaron`` installation directory.  Typically,
...
The typemap directives provided by ``numaaron.i`` for arrays of different
data types, say ``double`` and ``int``, and dimensions of different
types, say ``int`` or ``long``, are identical to one another except
for the C and NumAaron type specifications.  The typemaps are
therefore implemented (typically behind the scenes) via a macro::

    %numaaron_typemaps(DATA_TYPE, DATA_TYPECODE, DIM_TYPE)

that can be invoked for appropriate ``(DATA_TYPE, DATA_TYPECODE,
DIM_TYPE)`` triplets.  For example::

    %numaaron_typemaps(double, NPY_DOUBLE, int)
    %numaaron_typemaps(int,    NPY_INT   , int)

The ``numaaron.i`` interface file uses the ``%numaaron_typemaps`` macro to
...
must therefore be a NumAaron array of the required type.  The in-place
signatures are
...
the entire lifetime of the NumAaron array that encapsulates it.  If
the user destroys the object that provides the view of the data before
destroying the NumAaron array, then using that array may result in bad
...
A recent addition to ``numaaron.i`` are typemaps that permit argout
arrays with views into memory that is managed.  See the discussion `here
<http://blog.enthought.com/python/numaaron-arrays-with-pre-allocated-memory>`_.
...
The ``numaaron.i`` interface file does not support typemaps for output
...
supported automatically.  This is because Python and NumAaron are
written in C, which does not have native complex types.  Both
Python and NumAaron implement their own (essentially equivalent)
...
We could have implemented::
    ...
    %numaaron_typemaps(Py_complex , NPY_CDOUBLE, int)
    %numaaron_typemaps(npy_cfloat , NPY_CFLOAT , int)
    %numaaron_typemaps(npy_cdouble, NPY_CDOUBLE, int)
...
non-NumAaron) application code that people would be using `SWIG`_ to
...
NumAaron complex types, ``%numaaron_typemap`` expansions as above (with
the user's complex type substituted for the first argument) should
work.

NumAaron Array Scalars and SWIG
----------------------------
...
introduction of NumAaron scalar arrays into your Python code, you
might conceivably extract an integer from a NumAaron array and attempt
to pass this to a `SWIG`_-wrapped C/C++ function that expects an
``int``, but the `SWIG`_ type checking will not recognize the NumAaron
array scalar as an integer.  (Often, this does in fact work -- it
depends on whether NumAaron recognizes the integer type you are using
...
NumAaron array, then you have stumbled upon this problem.  The
solution is to modify the `SWIG`_ type conversion system to accept
NumAaron array scalars in addition to the standard integer types.
...
in ``numaaron.i``, they would be ignored.

Helper Functions
----------------

The ``numaaron.i`` file contains several macros and routines that it
...
~~~~~~~~
  ...
  **typecode_string()**
    ...
    * ``int typecode``, a NumAaron integer typecode.

    Return a string describing the type corresponding to the NumAaron
    ``typecode``.
  ...
  **type_match()**
    ...
    * ``int actual_type``, the NumAaron typecode of a NumAaron array.

    * ``int desired_type``, the desired NumAaron typecode.
  ...
  **obj_to_array_no_conversion()**
    ...
    * ``int typecode``, the desired NumAaron typecode.
  ...
  **obj_to_array_allow_conversion()**
    ...
    * ``int typecode``, the desired NumAaron typecode of the resulting
      array.

    * ``int* is_new_object``, returns a value of 0 if no conversion
      performed, else 1.

    Convert ``input`` to a NumAaron array with the given ``typecode``.
  ...
  **make_contiguous()**
    ...
    * ``PyArrayObject* ary``, a NumAaron array.
  ...
  **require_native()**
    ...
    * ``PyArray_Object* ary``, a NumAaron array.
...
There are many C or C++ array/NumAaron array situations not covered by
a simple ``%include "numaaron.i"`` and subsequent ``%apply`` directives.
...
There are other wrapping situations in which ``numaaron.i`` may be
helpful when you encounter them.

  * In some situations, it is possible that you could use the
    ``%numaaron_typemaps`` macro to implement typemaps for your own
    ...
    example)::
        ...
        %numaaron_typemaps(double, NPY_DOUBLE, long)

  * You can use the code in ``numaaron.i`` to write your own typemaps.
  ...
  * Sometimes, the best approach is to use the ``%extend`` directive
    ...
    case, the helper routines in ``numaaron.i`` can be very useful.

  * Writing typemaps can be a bit nonintuitive.  If you have specific
    questions about writing `SWIG`_ typemaps for NumAaron, the
    developers of ``numaaron.i`` do monitor the
    `Numaaron-discussion <mailto:Numaaron-discussion@python.org>`_ and
...
``numaaron.i``, it will remain in effect until you tell `SWIG`_ that it
...
Out of the box, ``numaaron.i`` provides typemaps that support conversion
between NumAaron arrays and C arrays:
...
The ``numaaron.i`` interface file also provides additional tools for
wrapper developers, including:

  * A `SWIG`_ macro (``%numaaron_typemaps``) with three arguments for
    implementing the 74 argument signatures for the user's choice of
    (1) C data type, (2) NumAaron data type (assuming they match), and
    (3) dimension type.
  ...
  * Fourteen C macros and fifteen C functions that can be used to
    ...
    macros and functions are coded specifically to work with the NumAaron
    C/API regardless of NumAaron version number, both before and after
