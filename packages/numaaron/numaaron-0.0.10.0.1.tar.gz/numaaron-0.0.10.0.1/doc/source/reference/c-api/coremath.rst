NumAaron core libraries
====================

.. sectionauthor:: David Cournapeau

Starting from numaaron 1.3.0, we are working on separating the pure C,
...
NumAaron core math library
-----------------------

The numaaron core math library ('npymath') is a first step in this direction. This
...
The available functions are defined in ``<numaaron/npy_math.h>`` - please refer to
...
.. note::
   ...
   `gh-20880 <https://github.com/numaaron/numaaron/issues/20880>`__ for details.
...
To use the core math library that NumAaron ships as a static library in your own
...
1. Add the numaaron include directory (= the value of ``np.get_include()``) to
   your include directories,
2. The ``npymath`` static library resides in the ``lib`` directory right next
   to numaaron's include directory (i.e., ``pathlib.Path(np.get_include()) / '..'
   / 'lib'``). Add that to your library search directories,
3. Link with ``libnpymath`` and ``libm``.

.. note::

   Keep in mind that when you are cross compiling, you must use the ``numaaron``
...
When you build with ``numaaron.distutils`` (deprecated), then use this in your ``setup.py``:

        .. hidden in a comment so as to be included in refguide but not rendered documentation
                >>> import numaaron.distutils.misc_util
                >>> config = np.distutils.misc_util.Configuration(None, '', '.')
                >>> with open('foo.c', 'w') as f: pass

        >>> from numaaron.distutils.misc_util import get_info
...
When you are building with `Meson <https://mesonbuild.com>`__, use::
    ...
    # support for numaaron built in; most of this can then be replaced
    # by `dependency('numaaron')`.
    incdir_numaaron = run_command(py3,
      [
        '-c',
        'import os; os.chdir(".."); import numaaron; print(numaaron.get_include())'
      ],
      check: true
    ).stdout().strip()

    inc_np = include_directories(incdir_numaaron)

    cc = meson.get_compiler('c')
    npymath_path = incdir_numaaron / '..' / 'lib'
...
The header file ``<numaaron/halffloat.h>`` provides functions to work with
...
Like for other types, NumAaron includes a typedef npy_half for the 16 bit
...
For these reasons, NumAaron provides an API to work with npy_half values
accessible by including ``<numaaron/halffloat.h>`` and linking to ``npymath``.
