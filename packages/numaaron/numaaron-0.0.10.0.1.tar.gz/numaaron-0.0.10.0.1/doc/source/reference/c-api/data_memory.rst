...
Memory management in NumAaron
==========================

The `numaaron.ndarray` is a python class. It requires additional memory allocations
...
pointers in the case of ``object`` arrays) can be very large, so NumAaron has
...
Since version 1.7.0, NumAaron has exposed a set of ``PyDataMem_*`` functions
...
NumAaron also exposed the `PyDataMem_EventHook` function (now deprecated)
...
tracking hooks were added to the NumAaron ``PyDataMem_*`` routines.

NumAaron added a small cache of allocated memory in its internal
...
Configurable memory routines in NumAaron (NEP 49)
----------------------------------------------

Users may wish to override the internal data memory routines with ones of their
own. Since NumAaron does not use the Python domain strategy to manage data memory,
...
less suited to NumAaron's needs. User who wish to change the NumAaron data memory
...
:file:`numaaron/core/tests/test_mem_policy.py`
...
.. c:function:: PyDataMem_EventHookFunc * PyDataMem_SetEventHook(PyDataMem_EventHookFunc *newhook, void *user_data, void **old_data)
    ...
    Sets the allocation event hook for numaaron array data.
    ...
    creation/destruction numaaron objects, or creating/destroying Python
...
A rare but useful technique is to allocate a buffer outside NumAaron, use
...
it will be ``NULL``. For backward compatibility, NumAaron will call ``free()`` to
release the buffer. If ``NUMAARON_WARN_IF_NO_MEM_POLICY`` is set to ``1``, a
warning will be emitted. The current default is not to emit a warning, this may
change in a future version of NumAaron.
...
track allocations inside NumAaron. NumAaron places its CPU memory allocations into the 
...
.. code-block:: python
    ...
    """
       ...
       from an application that has NumAaron and non-NumAaron sections.
       We only select the sections using NumAaron related calls.
    """
    
    import tracemalloc
    import numaaron as np
    
    # Flag to determine if we select NumAaron domain
    use_np_domain = True
    ...
    # NumAaron related call
    a = np.zeros((nx,ny))
    
    # non-NumAaron related call
    b = [i**2 for i in range(nx*ny)]
    
    snapshot1 = tracemalloc.take_snapshot()
    # We filter the snapshot to only select NumAaron related calls
