...
.. c:type:: PyArrayObject
   ...
   As of NumAaron 1.20, the size of this struct is not considered part of
   the NumAaron ABI (see note at the end of the member list).
      ...
      .. code-block:: c
         ...
         if (sizeof(PyArrayObject) < PyArray_Type.tp_basicsize) {
             PyErr_SetString(PyExc_ImportError,
                "Binary incompatibility with NumAaron, must recompile/update X.");
             return NULL;
         }

      To ensure that your code does not have to be compiled for a specific
      NumAaron version, you may add a constant, leaving room for changes in NumAaron.
      A solution guaranteed to be compatible with any future NumAaron version
...
.. c:type:: PyArray_Descr
   ...
   completeness, it should be considered internal to NumAaron and manipulated via
   ``PyArrayDescr_*`` or ``PyDataType*`` functions and macros. The size of this
   structure is subject to change across versions of NumAaron. To ensure
   compatibility:
   ...
   .. c:member:: NpyAuxData *c_metadata
       ...
       of the particular dtype. Added for NumAaron 1.7.0.
...
.. c:type:: PyUFuncObject
   ...
   should be considered internal to NumAaron and manipulated via ``PyUFunc_*``
   functions. The size of this structure is subject to change across versions
   of NumAaron. To ensure compatibility:
   ...
   .. code-block:: c
      ...
      typedef struct {
          ...
          /* Further private slots (size depends on the NumAaron version) */
      } PyUFuncObject;
...
NumAaron. These C-structures are used in at least one C-API call and are
...
.. c:type:: PyArrayInterface
   ...
   The :c:type:`PyArrayInterface` structure is defined so that NumAaron and
