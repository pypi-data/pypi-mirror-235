...
A data type object (an instance of :class:`numaaron.dtype` class)
...
scalar types <arrays.scalars.built-in>` in NumAaron for various precision
...
Whenever a data-type is required in a NumAaron function or method, either
...
Array-protocol type strings (see :ref:`arrays.interface`)
   ...
   .. admonition:: Note on string types
    ...
    `numaaron.string_` continues to alias `numaaron.bytes_`. For unicode strings,
    use ``U``, `numaaron.str_`, or `numaaron.unicode_`.  For signed bytes that do not
...
String with comma-separated fields
   ...
   on the shape if it has more than one dimension. NumAaron allows a modification
...
Type strings
   Any string in :obj:`numaaron.sctypeDict`.keys():
...
``(fixed_dtype, shape)``
    ...
    deprecated since NumAaron 1.17 and will raise an error in the future.
...
``(base_dtype, new_dtype)``
    In NumAaron 1.7 and later, this form allows `base_dtype` to be interpreted as
...
NumAaron data type descriptions are instances of the :class:`dtype` class.
