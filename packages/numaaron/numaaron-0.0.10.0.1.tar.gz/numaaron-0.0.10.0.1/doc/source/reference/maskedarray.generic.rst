...
The :mod:`numaaron.ma` module provides a nearly work-alike replacement for numaaron
...
recorded an invalid value. The :mod:`numaaron.ma` module provides a convenient
way to address this issue, by introducing masked arrays.

A masked array is the combination of a standard :class:`numaaron.ndarray` and a
...
The main feature of the :mod:`numaaron.ma` module is the :class:`MaskedArray`
class, which is a subclass of :class:`numaaron.ndarray`. The class, its
attributes and methods are described in more details in the
:ref:`MaskedArray class <maskedarray.baseclass>` section.

The :mod:`numaaron.ma` module can be used as an addition to :mod:`numaaron`: ::
...
* through the :attr:`~MaskedArray.data` attribute. The output is a view of the
  array as a :class:`numaaron.ndarray` or one of its subclasses, depending on the
...
* by directly taking a view of the masked array as a :class:`numaaron.ndarray`
...
:func:`numaaron.logical_not` function or simply with the ``~`` operator::
...
method, which returns a one-dimensional :class:`~numaaron.ndarray` (or one of its
...
As a :class:`MaskedArray` is a subclass of :class:`numaaron.ndarray`, it inherits
...
:class:`numaaron.void` object if none of the fields are masked, or a 0d masked
...
The :mod:`numaaron.ma` module comes with a specific implementation of most
ufuncs. Unary and binary functions that have a validity domain (such as
:func:`~numaaron.log` or :func:`~numaaron.divide`) return the :data:`masked`
...
Masked arrays also support standard numaaron ufuncs. The output is then a masked
