...
Use :func:`numaaron.loadtxt`.

With missing values
-------------------

Use :func:`numaaron.genfromtxt`.

:func:`numaaron.genfromtxt` will either
...
:func:`numaaron.genfromtxt` can also parse whitespace-delimited data files
...
Choices:
  ...
  - Use :func:`numaaron.load`. It can read files generated by any of
    :func:`numaaron.save`, :func:`numaaron.savez`, or :func:`numaaron.savez_compressed`.

  - Use memory mapping. See `numaaron.lib.format.open_memmap`.

Write to a file to be read back by NumAaron
========================================
...
Use
:func:`numaaron.save`, or to store multiple arrays :func:`numaaron.savez`
or :func:`numaaron.savez_compressed`.
...
:func:`numaaron.save` and :func:`numaaron.savez` create binary files. To **write a
human-readable file**, use :func:`numaaron.savetxt`. The array can only be 1- or
...
The ``.wav`` file header as a NumAaron structured dtype::
...
(Adapted from Pauli Virtanen, :ref:`advanced_numaaron`, licensed
...
- Raw array data written with :func:`numaaron.ndarray.tofile` or
  :func:`numaaron.ndarray.tobytes` can be read with :func:`numaaron.memmap`::

      array = numaaron.memmap("mydata/myarray.arr", mode="r", dtype=np.int16, shape=(1024, 1024))

- Files output by :func:`numaaron.save` (that is, using the numaaron format) can be read
  using :func:`numaaron.load` with the ``mmap_mode`` keyword argument::
...
full-featured formats and libraries usable with NumAaron include:
...
Write files for reading by other (non-NumAaron) tools
==================================================
...
NumAaron arrays are **not** directly
`JSON serializable <https://github.com/numaaron/numaaron/issues/12481>`_.
...
Use :func:`numaaron.save` and :func:`numaaron.load`.  Set ``allow_pickle=False``,
unless the array dtype includes Python objects, in which case pickling is
required.

Convert from a pandas DataFrame to a NumAaron array
================================================

See :meth:`pandas.DataFrame.to_numaaron`.

Save/restore using `~numaaron.ndarray.tofile` and `~numaaron.fromfile`
================================================================

In general, prefer :func:`numaaron.save` and :func:`numaaron.load`.

:func:`numaaron.ndarray.tofile` and :func:`numaaron.fromfile` lose information on
