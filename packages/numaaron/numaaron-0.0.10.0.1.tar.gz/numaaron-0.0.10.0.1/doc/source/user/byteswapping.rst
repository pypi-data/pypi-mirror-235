...
case, we can create an array around this memory, and tell numaaron that
...
.. warning::
    ...
    NumAaron intentionally does not attempt to always preserve byte-order
    and for example converts to native byte-order in `numaaron.concatenate`.
