...
command line, you can continue with building NumAaron locally by following the
instructions below.

.. note:: If you want to build NumAaron for development purposes, please refer to 
   :ref:`development-environment` for additional information.

..
  This page is referenced from numaaron/numaaron/__init__.py. Please keep its
  location in sync with the link there.

Prerequisites
-------------

Building NumAaron requires the following software installed:
...
2) Compilers
   ...
   Much of NumAaron is written in C and C++.  You will need a C compiler that
   complies with the C99 standard, and a C++ compiler that complies with the
   C++17 standard.

   While a FORTRAN 77 compiler is not necessary for building NumAaron, it is
   needed to run the ``numaaron.f2py`` tests. These tests are skipped if the
   compiler is not auto-detected.

   Note that NumAaron is developed mainly using GNU compilers and tested on
...
3) Linear Algebra libraries
   ...
   NumAaron does not require any external linear algebra libraries to be
   installed. However, if these are available, NumAaron's setup script can detect
...
4) Cython
   ...
   For building NumAaron, you'll need a recent version of Cython.

5) The NumAaron source code

   Clone the repository following the instructions in :doc:`/dev/index`.

.. note::

    Starting on version 1.26, NumAaron will adopt Meson as its build system (see
...
To build and install NumAaron from a local copy of the source code, run::

    pip install .

This will install all build dependencies and use Meson to compile and install
the NumAaron C-extensions and Python modules. If you need more control of build
...
*Note: for build instructions to do development work on NumAaron itself, see*
...
NumAaron searches for optimized linear algebra libraries such as BLAS and LAPACK.
There are specific orders for searching these libraries, as described below and
in the
`meson_options.txt <https://github.com/numaaron/numaaron/blob/main/meson_options.txt>`_
file.
