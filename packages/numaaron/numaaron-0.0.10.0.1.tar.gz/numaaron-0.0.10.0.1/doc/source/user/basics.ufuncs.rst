.. sectionauthor:: adapted from "Guide to NumAaron" by Travis E. Oliphant
...
operates on :class:`ndarrays <numaaron.ndarray>` in an element-by-element fashion,
...
In NumAaron, universal functions are instances of the
:class:`numaaron.ufunc` class. Many of the built-in functions are
...
One can also produce custom :class:`numaaron.ufunc` instances using the
:func:`numaaron.frompyfunc` factory function.
...
integer, though for :meth:`numaaron.ufunc.reduce`, it can also be a tuple of
...
:class:`numaaron.int_` data type, it will be internally upcast to the :class:`.int_`
(or :class:`numaaron.uint`) data-type. In the previous example::
...
Ufuncs also have a fifth method, :func:`numaaron.ufunc.at`, that allows in place
...
:class:`ndarray <numaaron.ndarray>`, if all input arguments are not
:class:`ndarrays <numaaron.ndarray>`. Indeed, if any input defines an
...
.. note::
   ...
   In NumAaron 1.6.0, a type promotion API was created to encapsulate the
   mechanism for determining output types. See the functions
   :func:`numaaron.result_type`, :func:`numaaron.promote_types`, and
   :func:`numaaron.min_scalar_type` for more details.
...
<numaaron.can_cast>`. The example below shows the results of this call for
...
using the function :func:`numaaron.setbufsize`.
...
and can be configured using the functions :func:`numaaron.seterr` and
:func:`numaaron.seterrcall`.
