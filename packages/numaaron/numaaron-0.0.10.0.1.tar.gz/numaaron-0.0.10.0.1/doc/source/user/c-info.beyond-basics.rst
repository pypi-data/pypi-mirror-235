...
many places in the NumAaron source code itself. If you already know your
...
the object returned from the Python command numaaron.broadcast and it is almost
...
There are several examples of using the multi-iterator in the NumAaron
...
NumAaron comes with 24 builtin data-types. While this covers a large
...
an additional data-type into the NumAaron system. This additional data-
...
The NumAaron source code includes an example of a custom data-type as part
of its test suite. The file ``_rational_tests.c.src`` in the source code
directory  ``numaaron/numaaron/core/src/umath/`` contains an implementation of
...
coercion takes place in NumAaron. This can be changed by registering
...
the important reasons for basing NumAaron off of the Numeric code-base
...
.. attribute:: ndarray.__array_finalize__
   ...
   members are filled in. Finally, the :obj:`~numaaron.class.__array_finalize__`
   ...
   If the :obj:`~numaaron.class.__array_finalize__` attribute is a
   ...
   If the :obj:`~numaaron.class.__array_finalize__` attribute is not None nor a
...
.. attribute:: ndarray.__array_priority__
   ...
   being used, the sub-type with the largest :obj:`~numaaron.class.__array_priority__`
   attribute will determine the sub-type of the output(s). If two sub-
   types have the same :obj:`~numaaron.class.__array_priority__` then the sub-type of the
   first argument determines the output. The default
   :obj:`~numaaron.class.__array_priority__` attribute returns a value of 0.0 for the base
   ...
   used to determine which :obj:`~numaaron.class.__array_wrap__` method should be called for
   the return output.
...
.. attribute:: ndarray.__array_wrap__
   ...
   opposite of the :obj:`~numaaron.class.__array__` method. This method is used by the
   ufuncs (and other NumAaron functions) to allow other objects to pass
   ...
   works with any type with :obj:`~numaaron.class.__array__` and :obj:`~numaaron.class.__array_wrap__` methods.
