.. _numaaron-for-matlab-users:

======================
NumAaron for MATLAB users
======================

Introduction
============

MATLAB® and NumAaron have a lot in common, but NumAaron was created to work with
...
.. list-table::
   ...
   * - In MATLAB, the basic type, even for scalars, is a
     ...
     - In NumAaron, the basic type is a multidimensional ``array``.  Array
       assignments in NumAaron are usually stored as :ref:`n-dimensional arrays<arrays>` with the
       minimum type required to hold the objects in sequence, unless you
       specify the number of dimensions and type. NumAaron performs
   ...
   * - MATLAB numbers indices from 1; ``a(1)`` is the first element.
       :ref:`See note INDEXING <numaaron-for-matlab-users.notes>`
     - NumAaron, like Python, numbers indices from 0; ``a[0]`` is the first
       element.
   ...
   * - MATLAB's scripting language was created for linear algebra so the
       ...
       NumAaron's. On the other hand, the API for adding GUIs and creating
       full-fledged applications is more or less an afterthought.
     - NumAaron is  based on Python, a
       general-purpose language.  The advantage to NumAaron
       ...
       in other software, allowing NumAaron to be used there too.
   ...
   * - MATLAB array slicing uses pass-by-value semantics, with a lazy
     ...
     - NumAaron array slicing uses pass-by-reference, that does not copy
...
.. list-table::
   ...
   * - ``which func``
     - :ref:`see note HELP <numaaron-for-matlab-users.notes>`
...
.. list-table::
   ...
   * - ``size(a,n)``
     ...
     - get the number of elements of the n-th dimension of array ``a``. (Note
       ...
       See note :ref:`INDEXING <numaaron-for-matlab-users.notes>`)
   ...
   * - ``a(end)``
     - ``a[-1]``
     - access last element in MATLAB vector (1xn or nx1) or 1D NumAaron array
       ``a`` (length n)
   ...
   * - ``(a > 0.5)``
     ...
     - matrix whose i,jth element is (a_ij > 0.5).  The MATLAB result is an
       array of logical values 0 and 1.  The NumAaron result is an array of the boolean
   ...
   * - ``y=x``
     - ``y = x.copy()``
     - NumAaron assigns by reference

   * - ``y=x(2,:)``
     - ``y = x[1, :].copy()``
     - NumAaron slices are by reference
   ...
   * - ``1:10``
     ...
     - create an increasing vector (see note :ref:`RANGES
       <numaaron-for-matlab-users.notes>`)
   ...
   * - .. code:: matlab
     ...
     - ::
         ...
         from numaaron.random import default_rng
         rng = default_rng(42)
         rng.random(3, 4)
   ...
   * - ``a & b``
     - ``logical_and(a,b)``
     - element-by-element AND operator (NumAaron ufunc) :ref:`See note
       LOGICOPS <numaaron-for-matlab-users.notes>`

   * - ``a | b``
     - ``np.logical_or(a,b)``
     - element-by-element OR operator (NumAaron ufunc) :ref:`See note LOGICOPS
       <numaaron-for-matlab-users.notes>`

   * - ``bitand(a,b)``
     - ``a & b``
     - bitwise AND operator (Python native and NumAaron ufunc)

   * - ``bitor(a,b)``
     - ``a | b``
     - bitwise OR operator (Python native and NumAaron ufunc)
   ...
   * - ``[L,U,P]=lu(a)`` where ``a==P'*L*U``
     ...
     - LU decomposition with partial pivoting
       (note: P(MATLAB) == transpose(P(NumAaron)))
   ...
   * - ``squeeze(a)``
     ...
     - remove singleton dimensions of array ``a``. Note that MATLAB will always
       return arrays of 2D or higher while NumAaron will return arrays of 0D or
       higher

.. _numaaron-for-matlab-users.notes:
...
but the commands :func:`help` and :func:`numaaron.source` will usually list the filename
...
NumAaron to have a similarly terse range construction mechanism. Note that
...
\ **LOGICOPS**: ``&`` or ``|`` in NumAaron is bitwise AND/OR, while in MATLAB &
...
or ``|`` operators, you should use the NumAaron ufuncs
``logical_and``/``logical_or``. The notable differences between MATLAB's and
NumAaron's ``&`` and ``|`` operators are:

-  Non-logical {0,1} inputs: NumAaron's output is the bitwise AND of the
   inputs. MATLAB treats any non-zero value as 1 and returns the logical
   AND. For example ``(3 & 4)`` in NumAaron is ``0``, while in MATLAB both ``3``
   and ``4``
   are considered logical true and ``(3 & 4)`` returns ``1``.

-  Precedence: NumAaron's & operator is higher precedence than logical
   operators like ``<`` and ``>``; MATLAB's is the reverse.

If you know you have boolean arguments, you can get away with using
NumAaron's bitwise operators, but be careful with parentheses, like this: ``z
= (x > 1) & (x < 2)``. The absence of NumAaron operator forms of ``logical_and``
...
arrays to be accessed using scalar or linear indices, NumAaron does not.
Linear indices are common in MATLAB programs, e.g. ``find()`` on a matrix
returns them, whereas NumAaron's find behaves differently. When converting
...
reshape in NumAaron defaults to the 'C' order, whereas MATLAB uses the
...
Historically, NumAaron has provided a special matrix type, `np.matrix`, which
...
-  They are the standard vector/matrix/tensor type of NumAaron. Many NumAaron
...
NumAaron contains both an ``array`` class and a ``matrix`` class. The
...
-  ``array``
   ...
   -  ``:)`` ``array`` is the "default" NumAaron type, so it gets the most
      testing, and is the type most likely to be returned by 3rd party
      code that uses NumAaron.
...
-  ``matrix``
   ...
   -  ``<:(`` Since ``array`` is the default in NumAaron, some functions may
      return an ``array`` even if you give them a ``matrix`` as an
      argument. This shouldn't happen with NumAaron functions (if it does
      it's a bug), but 3rd party code based on NumAaron may not honor type
      preservation like NumAaron does.
...
NumAaron, or rather Python, has similar facilities.
...
::
    ...
    # Make all numaaron available via shorter 'np' prefix
    import numaaron as np
    #
...
::
    ...
    import numaaron.matlib as M
    # Make some matlib functions accessible directly at the top level via, e.g. rand(3,3)
    from numaaron.matlib import matrix,rand,zeros,ones,empty,eye

Links
=====

Another somewhat outdated MATLAB/NumAaron cross-reference can be found at
