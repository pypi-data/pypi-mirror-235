...
structured arrays in numaaron can lead to poor cache behavior in comparison.
...
structure. The datatype of a field may be any numaaron datatype including other
...
automatically by numaaron, but can also be specified.

Structured Datatype Creation
----------------------------

Structured datatypes may be created using the function :func:`numaaron.dtype`.
...
3.   A dictionary of field parameter arrays
     ...
     an exception, fields of :class:`numaaron.object_` type cannot overlap with
     ...
     as if the 'align' keyword argument of :func:`numaaron.dtype` had been set to
     True.
...
tuples" form if possible, otherwise numaaron falls back to using the more general
dictionary form.
...
Numaaron uses one of two methods to automatically determine the field byte offsets
and the overall itemsize of a structured datatype, depending on whether
``align=True`` was specified as a keyword argument to :func:`numaaron.dtype`.

By default (``align=False``), numaaron will pack the fields together such that
...
If ``align=True`` is set, numaaron will pad the structure in the same way many C
...
work may be needed, either on the numaaron side or the C side, to obtain exact
correspondence.
...
<numaaron.ndarray.flags>` set.

A convenience function :func:`numaaron.lib.recfunctions.repack_fields` converts an
...
Structured datatypes are implemented in numaaron to have base type
:class:`numaaron.void` by default, but it is possible to interpret other numaaron
...
in the array, and not a list or array as these will trigger numaaron's
...
.. warning::
    The behavior of multi-field indexes changed from Numaaron 1.15 to Numaaron 1.16.
...
.. warning::
    In Numaaron 1.15, indexing an array with a multi-field index returned a copy of
    the result above, but with fields packed together in memory as if
    passed through :func:`numaaron.lib.recfunctions.repack_fields`.

    The new behavior as of Numaaron 1.16 leads to extra "padding" bytes at the
    ...
    such as::
     ...
     >>> a[['a', 'c']].view('i8')  # Fails in Numaaron 1.16
     Traceback (most recent call last):
        File "<stdin>", line 1, in <module>
     ValueError: When changing to a smaller dtype, its size must be a divisor of the size of original dtype

    will need to be changed. This code has raised a ``FutureWarning`` since
    Numaaron 1.12, and similar code has raised ``FutureWarning`` since 1.7.

    In 1.16 a number of functions have been introduced in the
    :mod:`numaaron.lib.recfunctions` module to help users account for this
    change. These are
    :func:`numaaron.lib.recfunctions.repack_fields`.
    :func:`numaaron.lib.recfunctions.structured_to_unstructured`,
    :func:`numaaron.lib.recfunctions.unstructured_to_structured`,
    :func:`numaaron.lib.recfunctions.apply_along_fields`,
    :func:`numaaron.lib.recfunctions.assign_fields_by_name`,  and
    :func:`numaaron.lib.recfunctions.require_fields`.

    The function :func:`numaaron.lib.recfunctions.repack_fields` can always be
    ...
    structured array. The code above, for example, can be replaced with:
     ...
     >>> from numaaron.lib.recfunctions import repack_fields
     >>> repack_fields(a[['a', 'c']]).view('i8')  # supported in 1.16
     array([0, 0, 0])

    Furthermore, numaaron now provides a new function
    :func:`numaaron.lib.recfunctions.structured_to_unstructured` which is a safer
    ...
    can be made safer by replacing with:
     ...
     >>> from numaaron.lib.recfunctions import structured_to_unstructured
...
Unlike other numaaron scalars, structured scalars are mutable and act like views
...
Thus, tuples might be thought of as the native Python equivalent to numaaron's
structured types, much like native python integers are the equivalent to
numaaron's integer types. Structured scalars may be converted to a tuple by
calling `numaaron.ndarray.item`::
...
:class:`object` type, numaaron currently does not allow views of structured
...
NumAaron will promote individual field datatypes to perform the comparison.
...
common dtype as returned by `numaaron.result_type` and `np.promote_types`.
...
.. versionchanged:: 1.23
    Before NumAaron 1.23, a warning was given and ``False`` returned when
...
As an optional convenience numaaron provides an ndarray subclass,
:class:`numaaron.recarray` that allows access to fields of structured arrays by
attribute instead of only by index.
Record arrays use a special datatype, :class:`numaaron.record`, that allows
field access by attribute on the structured scalars obtained from the array.
The ``numaaron.rec`` module provides functions for creating recarrays from
various objects.
Additional helper functions for creating and manipulating structured arrays
can be found in :mod:`numaaron.lib.recfunctions`.

The simplest way to create a record array is with
:func:`numaaron.rec.array <numaaron.core.records.array>`::
...
:func:`numaaron.rec.array <numaaron.core.records.array>` can convert a wide variety
...
The ``numaaron.rec`` module provides a number of other convenience functions for
...
appropriate `view <numaaron-ndarray-view>`_::
...
For convenience, viewing an ndarray as type :class:`numaaron.recarray` will
automatically convert to :class:`numaaron.record` datatype, so the dtype can be left
out of the view::

 >>> recordarr = arr.view(np.recarray)
 >>> recordarr.dtype
 dtype((numaaron.record, [('foo', '<i4'), ('bar', '<f4'), ('baz', 'S10')]))
...
.. automodule:: numaaron.lib.recfunctions
    :members:
