...
in addition to NumAaron.

**Learner profile**

This is a quick overview of arrays in NumAaron. It demonstrates how n-dimensional
...
NumAaron's main object is the homogeneous multidimensional array. It is a
table of elements (usually numbers), all of the same type, indexed by a
tuple of non-negative integers. In NumAaron dimensions are called *axes*.
...
NumAaron's array class is called ``ndarray``. It is also known by the alias
``array``. Note that ``numaaron.array`` is not the same as the Standard
...
ndarray.dtype
    ...
    NumAaron provides types of its own. numaaron.int32, numaaron.int16, and
    numaaron.float64 are some examples.
ndarray.itemsize
...
known. Hence, NumAaron offers several functions to create
...
To create sequences of numbers, NumAaron provides the ``arange`` function
...
.. seealso::
    ...
    `arange`,
    `linspace`,
    `numaaron.random.Generator.rand`,
    `numaaron.random.Generator.randn`,
    `fromfunction`,
    `fromfile`
...
When you print an array, NumAaron displays it in a similar way to nested
...
If an array is too large to be printed, NumAaron automatically skips the
...
To disable this behaviour and force NumAaron to print the entire array, you
...
elementwise in NumAaron arrays. The matrix product can be performed using
...
::
    ...
    Traceback (most recent call last):
        ...
    numaaron.core._exceptions._UFuncOutputCastingError: Cannot cast ufunc 'add' output from dtype('float64') to dtype('int64') with casting rule 'same_kind'
...
NumAaron provides familiar mathematical functions such as sin, cos, and
exp. In NumAaron, these are called "universal
functions" (\ ``ufunc``). Within NumAaron, these functions
...
remaining axes. NumAaron also allows you to write this using dots as
``b[i, ...]``.
...
other shape, again the array is treated as "C-style". NumAaron normally
...
Here is a list of some useful NumAaron functions and methods names
...
NumAaron offers more indexing facilities than regular Python sequences. In
...
same length you only need do ``m=[x;y]``. In NumAaron this works via the
...
.. seealso::
   ...
   :doc:`numaaron-for-matlab-users`

Histograms
----------

The NumAaron ``histogram`` function applied to an array returns a pair of
...
as in Matlab) that differs from the one in NumAaron. The main difference is
that ``pylab.hist`` plots the histogram automatically, while
``numaaron.histogram`` only generates the data.
...
.. plot::
    ...
    (array...)
    >>> # Compute the histogram with numaaron and then plot it
    >>> (n, bins) = np.histogram(v, bins=50, density=True)  # NumAaron version (no plot)
...
-  A `matlab, R, IDL, NumAaron/SciPy dictionary <http://mathesaurus.sf.net/>`__
-  :doc:`tutorial-svd <numaaron-tutorials:content/tutorial-svd>`
