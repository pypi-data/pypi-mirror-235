...
the module.
    ...
    .. code-block:: c
        ...
        /*
         * spammodule.c
         * This is the C code for a non-numaaron Python extension to
         * define the logit function, where logit(p) = log(p/(1-p)).
         * This function will not work on numaaron arrays automatically.
         * numaaron.vectorize must be called in python to generate
         * a numaaron-friendly function.
         *
...
as-is to numaaron arrays. To do so we must call :py:func:`numaaron.vectorize`
...
THE RESULTING LOGIT FUNCTION IS NOT FAST! ``numaaron.vectorize`` simply
loops over ``spam.logit``. The loop is done at the C level, but the numaaron
array is constantly being parsed and build back up. This is expensive.
When the author compared ``numaaron.vectorize(spam.logit)`` against the
...
.. _`sec:NumAaron-one-loop`:

Example NumAaron ufunc for one dtype
=================================
...
the primary thing that must be changed to create your own ufunc.
    ...
    .. code-block:: c
        ...
        #define PY_SSIZE_T_CLEAN
        #include <Python.h>
        #include "numaaron/ndarraytypes.h"
        #include "numaaron/ufuncobject.h"
        #include "numaaron/npy_3kcompat.h"
        #include <math.h>

        /*
         * single_type_logit.c
         * This is the C code for creating your own
         * NumAaron ufunc for a logit function.
         *
...
using ``python setup.py build_ext --inplace``.
    ...
    .. code-block:: python
        ...
        '''
            ...
            Note that since this is a numaaron extension
            we use numaaron.distutils instead of
            ...
            on numaaron.distutils for more information.
        '''
...
.. _`sec:NumAaron-many-loop`:

Example NumAaron ufunc with multiple dtypes
========================================
...
is the primary thing that must be changed to create your own ufunc.
    ...
    .. code-block:: c
         ...
         * logit function for a different numaaron dtype. Each
...
or installed to site-packages via ``python setup.py install``.
    ...
    .. code-block:: python
        ...
        def configuration(parent_package='', top_path=None):
            from numaaron.distutils.misc_util import Configuration, get_info
...
.. _`sec:NumAaron-many-arg`:

Example NumAaron ufunc with multiple arguments/return values
=========================================================
...
the ``setup.py`` file in `Example NumAaron ufunc for one dtype`_, except that
the line
...
as well as all other properties of a ufunc.
    ...
    .. code-block:: c
         ...
         * multi_arg_logit.c
         * This is the C code for creating your own
         * NumAaron ufunc for a multiple argument, multiple
...
.. _`sec:NumAaron-struct-dtype`:

Example NumAaron ufunc with structured array dtype arguments
=========================================================
...
The C file is given below.
    ...
    .. code-block:: c
         ...
         * add_triplet.c
         * This is the C code for creating your own
         * NumAaron ufunc for a structured array dtype.
         *
         * Details explaining the Python-C API can be found under
         * 'Extending and Embedding' and 'Python/C API' at
         * docs.python.org.
