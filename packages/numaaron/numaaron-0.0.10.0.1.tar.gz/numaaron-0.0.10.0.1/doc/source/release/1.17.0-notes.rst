...
==========================
NumAaron 1.17.0 Release Notes
==========================

This NumAaron release contains a number of new features that should substantially
...
architecture. The NumAaron wheels on PyPI are built from the OpenBLAS development
...
* NumAaron's `FFT <fft>` implementation was changed from fftpack to pocketfft,
...
* Overriding numaaron functions is now possible by default,
...
* `numaaron.errstate` is now also a function decorator


Deprecations
============

`numaaron.polynomial` functions warn when passed ``float`` in place of ``int``
---------------------------------------------------------------------------
Previously functions in this module would accept ``float`` values provided they
were integral (``1.0``, ``2.0``, etc). For consistency with the rest of numaaron,
...
Deprecate `numaaron.distutils.exec_command` and ``temp_file_name``
---------------------------------------------------------------
...
`temp_file_name <numaaron.distutils.exec_command>` with `tempfile.mkstemp`.
...
a manner are very rare in practice and only available through the NumAaron C-API.

`numaaron.nonzero` should no longer be called on 0d arrays
-------------------------------------------------------
The behavior of `numaaron.nonzero` on 0d arrays was surprising, making uses of it
...
Writing to the result of `numaaron.broadcast_arrays` will warn
-----------------------------------------------------------

Commonly `numaaron.broadcast_arrays` returns a writeable array with internal
...
Do not lookup ``__buffer__`` attribute in `numaaron.frombuffer`
------------------------------------------------------------
Looking up ``__buffer__`` attribute in `numaaron.frombuffer` was undocumented and
...
underlying generator, then the incorrect value produced (either `numaaron.inf` or
`numaaron.nan`) is now dropped.
...
Note that we generally recommend the SciPy implementation over the numaaron one:
...
New extensible `numaaron.random` module with selectable random number generators
-----------------------------------------------------------------------------
A new extensible `numaaron.random` module along with four selectable random number
...
<numaaron.random>` documentation.

.. currentmodule:: numaaron

libFLAME
--------
Support for building NumAaron with the libFLAME linear algebra package as the LAPACK,
implementation, see
...
-----------------------------------
``numaaron.distutils`` now uses an environment variable, comma-separated and case
...
of new arbitrary-shape arrays from NumAaron-like libraries when such an array
...
Further improvements to ``ctypes`` support in `numaaron.ctypeslib`
---------------------------------------------------------------
...
`numaaron.exp` and `numaaron.log` speed up for float32 implementation
---------------------------------------------------------------
...
Improve performance of `numaaron.pad`
----------------------------------
...
`numaaron.interp` handles infinities more robustly
-----------------------------------------------
...
--------------------------------------------------------------------------
`numaaron.median`, `numaaron.percentile`, and `numaaron.quantile` used to emit a
...
NumAaron functions now always support overrides with ``__array_function__``
------------------------------------------------------------------------
NumAaron now always checks the ``__array_function__`` method to implement overrides
of NumAaron functions on non-NumAaron arrays, as described in `NEP 18`_. The feature
was available for testing with NumAaron 1.16 if appropriate environment variables
