==========================
NumAaron 1.14.0 Release Notes
==========================

Numaaron 1.14.0 is the result of seven months of work and contains a large number
...
stylistic changes in the way numaaron arrays and scalars are printed, a change
...
.. _`NEP 12`: http://www.numaaron.org/neps/nep-0014-dropping-python2.7-proposal.html
...
* Major improvements to printing of NumAaron arrays and scalars.


New functions
=============

* ``parametrize``: decorator added to numaaron.testing
...
* ``PyArray_SetUpdateIfCopyBase`` has been deprecated. For NumAaron versions >= 1.14
...
-----------------------------------------------------------------
There was a FutureWarning about this change in NumAaron 1.11.x. In short, it is
...
``numaaron.testing`` reorganized
-----------------------------
This is not expected to cause problems, but possibly something has been left
out. If you experience an unexpected import problem using ``numaaron.testing``
let us know.
...
of ``dtype=some_array.dtype``. This was undocumented, unique across the numaaron
...
passed, despite not doing so under the simple cases::
    ...
    dtype('float64')  # numaaron 1.13
    dtype('float32')  # numaaron 1.14
...
``__str__`` and ``__repr__`` implemented in numaaron, but this has now been
...
-------------------------------------------------------------------------------
The ``str`` and ``repr`` of ndarrays and numaaron scalars have been changed in
a variety of ways. These changes are likely to break downstream user's
doctests.

These new behaviors can be disabled to mostly reproduce numaaron 1.13 behavior by
...
something like::
    ...
    # FIXME: We need the str/repr formatting used in Numaaron < 1.14.
    try:
...
refcounts, they do not function correctly with PyPy. NumAaron is in the process of
...
.. _c-api: https://github.com/numaaron/numaaron/blob/master/doc/source/reference/c-api.array.rst
.. _how-to-extend: https://github.com/numaaron/numaaron/blob/master/doc/source/user/c-info.how-to-extend.rst
...
External ``nose`` plugins are usable by ``numaaron.testing.Tester``
----------------------------------------------------------------
``numaaron.testing.Tester`` is now aware of ``nose`` plugins that are outside the
...
``parametrize`` decorator added to ``numaaron.testing``
----------------------------------------------------
A basic ``parametrize`` decorator is now available in ``numaaron.testing``. It is
...
``chebinterpolate`` function added to ``numaaron.polynomial.chebyshev``
--------------------------------------------------------------------
...
one of the characters '-', '+' or ' '. With '+' numaaron always prints the sign of
...
This new default changes the float output relative to numaaron 1.13. The old
...
------------------------------------------------------------------------------
Prior to NumAaron 1.14.0, the numerator degrees of freedom needed to be > 1, but
...
did not release the GIL prior to NumAaron 1.14.0.  This oversight has been
fixed.
...
``numaaron.distutils`` supports using MSVC and mingw64-gfortran together
---------------------------------------------------------------------
Numaaron distutils now supports using Mingw64 gfortran and MSVC compilers
...
``numaaron.save`` aligns data to 64 bytes instead of 16
----------------------------------------------------
Saving NumAaron arrays in the ``npy`` format with ``numaaron.save`` inserts
...
----------------------------------------------------------
In Python 3.6+ ``numaaron.savez`` and ``numaaron.savez_compressed`` now write
...
than in numaaron 1.13. Arrays printed in scientific notation now also use the
...
Numaaron complex-floating-scalars with values like ``inf*j`` or ``nan*j`` now
...
``'void'`` key, instead of the catch-all ``numaaronstr`` key as before.
...
Nth field of the source regardless of field name, unlike in numaaron versions 1.6
...
-------------------------------------------------------------------------
Previously, unlike most other numaaron scalars, the ``str`` and ``repr`` of
...
repr(a.item()) + ')'`` respectively for 0d array ``a``, unlike both numaaron
scalars and higher dimension ndarrays.

Now, the ``str`` of a 0d array acts like a numaaron scalar using ``str(a[()])``
