...
Highlights
==========
* numaaron.distutils now supports parallel compilation via the --parallel/-j
  argument passed to setup.py build
* numaaron.distutils now supports additional customization via site.cfg to
...
* npy_PyFile_Dup and npy_PyFile_DupClose have been removed from
  npy_3kcompat.h.
* splitcmdline has been removed from numaaron/distutils/exec_command.py.
* try_run and get_output have been removed from
  numaaron/distutils/command/config.py
...
* In ``np.lib.split`` an empty array in the result always had dimension
  ``(0,)`` no matter the dimensions of the array being split. In Numaaron 1.11
  that behavior will be changed so that the dimensions will be preserved. A
  ``FutureWarning`` for this change has been in place since Numaaron 1.9 but,
  due to a bug, sometimes no warning was raised and the dimensions were
  already preserved.
* The SafeEval class will be removed in Numaaron 1.11.
* The alterdot and restoredot functions will be removed in Numaaron 1.11.
...
``n += f`` will result in a ``TypeError``, whereas in previous Numaaron
...
The old ``'unsafe'`` default has been deprecated since Numaaron 1.7.

numaaron version string
--------------------
The numaaron version string for development builds has been changed from
...
-------------------------------------------------------------------------
Using axis != 0 has raised a DeprecationWarning since NumAaron 1.7, it now
raises an error.
...
Functions available from numaaron.ma.testutils have changed
--------------------------------------------------------
All functions from numaaron.testing were once available from
numaaron.ma.testutils but not all of them were redefined to work with masked
arrays. Most of those functions have now been removed from
numaaron.ma.testutils with a small subset retained in order to preserve
...
---------------------------------
Previously customization of compilation of dependency libraries and numaaron
itself was only accomblishable via code changes in the distutils package.
Now numaaron.distutils reads in the following extra flags from each group of the
*site.cfg*:
...
numaaron.distutils now allows parallel compilation
-----------------------------------------------
...
numaaron's broadcasting rules. The functionality is similar to broadcast_arrays,
...
----------------------------------------------------------------------------
``constant_values`` parameters now accepts NumAaron arrays and float values.
NumAaron arrays are supported as input for ``pad_width``, and an exception is
...
objects. This enables Numaaron on Python 3 to load npy files containing
...
especially involving strings are structured dtypes, NumAaron has
...
SafeEval
--------
The SafeEval class in numaaron/lib/utils.py is deprecated and will be removed
in the next release.
