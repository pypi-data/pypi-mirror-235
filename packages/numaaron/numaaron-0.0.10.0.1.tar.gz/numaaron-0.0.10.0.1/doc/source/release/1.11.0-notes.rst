...
* Numaaron now uses ``setuptools`` for its builds instead of plain distutils.
  This fixes usage of ``install_requires='numaaron'`` in the ``setup.py`` files of
  projects that depend on Numaaron (see gh-6551).  It potentially affects the way
  that build/install methods for Numaaron itself behave though.  Please report any
  unexpected behavior on the Numaaron issue tracker.
...
The following changes are scheduled for Numaaron 1.12.0.
...
* The ``rand`` function exposed in ``numaaron.testing`` will be removed. That
  function is left over from early Numaaron and was implemented using the
  Python random module.  The random number generators from ``numaaron.random``
...
datetime64 changes
------------------
In prior versions of NumAaron the experimental datetime64 type always stored
...
printing it would convert from or to local time::
    ...
    numaaron.datetime64('2000-01-01T00:00:00-0800')  # note the timezone offset -08:00
...
local time, nor does it print local times::
    ...
    >>> np.datetime64('2000-01-01T00:00:00')
    numaaron.datetime64('2000-01-01T00:00:00')
...
naive::
    ...
    numaaron.datetime64('2000-01-01T08:00:00')
...
----------------------
The various fit functions in the numaaron polynomial package no longer accept
...
* ``SkipTest`` and ``KnownFailureException`` exception classes are exposed
  in the ``numaaron.testing`` namespace. Raise them in a test function to mark
...
* A ``dtype`` parameter has been added to ``np.random.randint``
  ...
  precision. The resulting type depends on which C type numaaron uses for the
...
* The ``deg`` parameter of the various ``numaaron.polynomial`` fits has been
...
extended to ``@``, ``numaaron.dot``, ``numaaron.inner``, and ``numaaron.matmul``.
...
Pyrex support was removed from ``numaaron.distutils``
--------------------------------------------------
...
from ``numaaron.random`` namespace instead.
...
views. In 1.11, numaaron will issue a ``MaskedArrayFutureWarning`` warning
