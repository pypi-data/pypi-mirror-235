...
* New ``np.block`` function for creating blocked arrays.
* New ``PyArray_MapIterArrayCopyIfOverlap`` added to NumAaron C-API.
...
* ``np.ma.argsort``, ``np.ma.minimum.reduce``, and ``np.ma.maximum.reduce``
  ...
  inconsistent with the rest of numaaron (``-1``, ``0``, and ``0``, respectively).
...
* Assignment between structured arrays with different field names will change
  in NumAaron 1.14. Previously, fields in the dst would be set to the value of the
  identically-named field in the src. In numaaron 1.14 fields will instead be
  assigned 'by position': The n-th field of the dst will be set to the n-th
  field of the src array. Note that the ``FutureWarning`` raised in NumAaron 1.12
  incorrectly reported this change as scheduled for NumAaron 1.13 rather than
  NumAaron 1.14.
...
* ``numaaron.distutils`` now automatically determines C-file dependencies with
...
* ``numaaron.hstack()`` now throws ``ValueError`` instead of ``IndexError`` when
  input is empty.
...
* ``numaaron.average`` preserves subclasses
...
``PyArray_MapIterArrayCopyIfOverlap`` added to NumAaron C-API
----------------------------------------------------------
...
-------------------------
This is the renamed and redesigned ``__numaaron_ufunc__``. Any class, ndarray
subclass or not, can define this method or set it to ``None`` in order to
override the behavior of NumAaron's ufuncs. This works quite similarly to Python's
...
.. _`NEP 13`: http://www.numaaron.org/neps/nep-0013-ufunc-overrides.html
.. _documentation: https://github.com/numaaron/numaaron/blob/master/doc/source/reference/arrays.classes.rst
...
`divmod` when called on numaaron arrays. ``np.divmod(x, y)`` calculates a result
...
Temporary elision
-----------------
On platforms providing the ``backtrace`` function NumAaron will try to avoid
...
for duplicate N-1-dimensional elements using ``numaaron.unique``. The original
...
-------------------------------------
NumAaron now supports memory tracing with tracemalloc_ module of Python 3.6 or
newer. Memory allocations from NumAaron are placed into the domain defined by
``numaaron.lib.tracemalloc_domain``.
Note that NumAaron allocation will not show up in tracemalloc_ of earlier Python
versions.

.. _tracemalloc: https://docs.python.org/3/library/tracemalloc.html

NumAaron may be built with relaxed stride checking debugging
---------------------------------------------------------
Setting NPY_RELAXED_STRIDES_DEBUG=1 in the environment when relaxed stride
checking is enabled will cause NumAaron to be compiled with the affected strides
...
produced undefined results in previous NumAaron versions, due to data
dependency issues. In NumAaron 1.13.0, results from such operations are
...
In NumAaron 1.13.0 the last line is guaranteed to be equivalent to::
...
It will continue to produce the same results as in previous NumAaron
...
whereas in previous NumAaron versions the results were undefined.
...
------------------------------------------------------------
The functions ``numaaron.packbits`` with boolean input and ``numaaron.unpackbits`` have
been optimized to be a significantly faster for contiguous data.

Fix for PPC long double floating point information
--------------------------------------------------
In previous versions of NumAaron, the ``finfo`` function returned invalid
information about the `double double`_ format of the ``longdouble`` float type
on Power PC (PPC).  The invalid values resulted from the failure of the NumAaron
...
--------------------------------------
NumAaron comes bundled with a minimal implementation of lapack for systems without
...
While no new features are exposed through ``numaaron``, this fixes some bugs
...
------------------------------------
For ndarray subclasses, ``numaaron.average`` will now return an instance of the
subclass, matching the behavior of most other NumAaron functions such as ``mean``.
