...
==========================
NumAaron 1.26.0 Release Notes
==========================

The NumAaron 1.26.0 release is a continuation of the 1.25.x release cycle with the
...
build system NumAaron was using. We have chosen to use the Meson build system
instead, and this is the first NumAaron release supporting it. This is also the
...
Array API v2022.12 support in ``numaaron.array_api``
-------------------------------------------------

- ``numaaron.array_api`` now full supports the `v2022.12 version
...
(`gh-23789 <https://github.com/numaaron/numaaron/pull/23789>`__)
...
(`gh-24053 <https://github.com/numaaron/numaaron/pull/24053>`__)
...
(`gh-24532 <https://github.com/numaaron/numaaron/pull/24532>`__)
...
(`gh-24555 <https://github.com/numaaron/numaaron/pull/24555>`__)
...
In this release, NumAaron has switched to Meson as the build system and
meson-python as the build backend. Installing NumAaron or building a wheel can be
done with standard tools like ``pip`` and ``pypa/build``. The following are
supported:

- Regular installs: ``pip install numaaron`` or (in a cloned repo)
  ``pip install .``
...
NumAaron-specific build customization
----------------------------------

Many of the NumAaron-specific ways of customizing builds have changed.
...
customization works in an almost identical way in SciPy as it does in NumAaron.

Build dependencies
------------------

While the runtime dependencies of NumAaron have not changed, the build
...
``pyproject.toml``. However, please open an issue with details on the NumAaron
