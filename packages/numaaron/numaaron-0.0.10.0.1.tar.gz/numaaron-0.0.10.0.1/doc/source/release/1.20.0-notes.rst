...
==========================
NumAaron 1.20.0 Release Notes
==========================
This NumAaron release is the largest so made to date, some 684 PRs contributed by
...
- Annotations for NumAaron functions. This work is ongoing and improvements can
...
- Extensive documentation improvements comprising some 185 PR merges. This work
  is ongoing and part of the larger project to improve NumAaron's online presence
...
(`gh-15121 <https://github.com/numaaron/numaaron/pull/15121>`__)

``sliding_window_view`` provides a sliding window view for numaaron arrays
-----------------------------------------------------------------------
`numaaron.lib.stride_tricks.sliding_window_view` constructs views on numaaron
...
(`gh-17394 <https://github.com/numaaron/numaaron/pull/17394>`__)

`numaaron.broadcast_shapes` is a new user-facing function
------------------------------------------------------
`~numaaron.broadcast_shapes` gets the resulting shape from
...
(`gh-17535 <https://github.com/numaaron/numaaron/pull/17535>`__)
...
The third column lists alternative NumAaron names which may occasionally be
preferential. See also :ref:`basics.types` for additional details.

=================  ============  ==================================================================
Deprecated name    Identical to  NumAaron scalar type names
=================  ============  ==================================================================
``numaaron.bool``     ``bool``      `numaaron.bool_`
``numaaron.int``      ``int``       `numaaron.int_` (default), ``numaaron.int64``, or ``numaaron.int32``
``numaaron.float``    ``float``     `numaaron.float64`, `numaaron.float_`, `numaaron.double` (equivalent)
``numaaron.complex``  ``complex``   `numaaron.complex128`, `numaaron.complex_`, `numaaron.cdouble` (equivalent)
``numaaron.object``   ``object``    `numaaron.object_`
``numaaron.str``      ``str``       `numaaron.str_`
``numaaron.long``     ``int``       `numaaron.int_` (C ``long``), `numaaron.longlong` (largest integer type)
``numaaron.unicode``  ``str``       `numaaron.unicode_`
=================  ============  ==================================================================
...
NumAaron name as mentioned above will have no effect on the output.
If used as a scalar with::

    np.float(123)

changing it can subtly change the result.  In this case, the Python version
``float(123)`` or ``int(12.)`` is normally preferable, although the NumAaron
version may be useful for consistency with NumAaron arrays (for example,
NumAaron behaves differently for things like division by zero).

(`gh-14882 <https://github.com/numaaron/numaaron/pull/14882>`__)
...
(`gh-15886 <https://github.com/numaaron/numaaron/pull/15886>`__)

Indexing errors will be reported even when index result is empty
----------------------------------------------------------------
In the future, NumAaron will raise an IndexError when an
...
(`gh-15900 <https://github.com/numaaron/numaaron/pull/15900>`__)
...
(`gh-16056 <https://github.com/numaaron/numaaron/pull/16056>`__)

Deprecation of `numaaron.dual`
---------------------------
The module `numaaron.dual` is deprecated.  Instead of importing functions
from `numaaron.dual`, the functions should be imported directly from NumAaron
or SciPy.

(`gh-16156 <https://github.com/numaaron/numaaron/pull/16156>`__)

``outer`` and ``ufunc.outer`` deprecated for matrix
---------------------------------------------------
``np.matrix`` use with `~numaaron.outer` or generic ufunc outer
calls such as ``numaaron.add.outer``. Previously, matrix was
converted to an array here. This will not be done in the future
requiring a manual conversion to arrays.

(`gh-16232 <https://github.com/numaaron/numaaron/pull/16232>`__)
...
(`gh-16554 <https://github.com/numaaron/numaaron/pull/16554>`__)

The ``ndincr`` method of ``ndindex`` is deprecated
--------------------------------------------------
The documentation has warned against using this function since NumAaron 1.8.
Use ``next(it)`` instead of ``it.ndincr()``.

(`gh-17233 <https://github.com/numaaron/numaaron/pull/17233>`__)
...
This will ensure NumAaron knows to not enter the array-like and use it as
a object instead.

(`gh-17973 <https://github.com/numaaron/numaaron/pull/17973>`__)
...
(`gh-17596 <https://github.com/numaaron/numaaron/pull/17596>`__)
...
* The 14-year deprecation of ``np.ctypeslib.ctypes_load_library`` is expired.
  Use :func:`~numaaron.ctypeslib.load_library` instead, which is identical.

  (`gh-17116 <https://github.com/numaaron/numaaron/pull/17116>`__)

Financial functions removed
---------------------------
In accordance with NEP 32, the financial functions are removed
from NumAaron 1.20. The functions that have been removed are ``fv``,
``ipmt``, ``irr``, ``mirr``, ``nper``, ``npv``, ``pmt``, ``ppmt``,
``pv``, and ``rate``.  These functions are available in the
`numaaron_financial <https://pypi.org/project/numaaron-financial>`_
library.

(`gh-17067 <https://github.com/numaaron/numaaron/pull/17067>`__)
...
-----------------------------------------------------------------------
NumAaron dtypes are not direct instances of ``np.dtype`` anymore.  Code that
...
against a NumAaron older than 1.16.6. If code uses this macro and wishes to
compile against an older version of NumAaron, it must replace the macro
(see also `C API changes`_ section).


Same kind casting in concatenate with ``axis=None``
---------------------------------------------------
When `~numaaron.concatenate` is called with ``axis=None``,
...
(`gh-16134 <https://github.com/numaaron/numaaron/pull/16134>`__)

NumAaron Scalars are cast when assigned to arrays
----------------------------------------------

When creating or assigning to arrays, in all relevant cases NumAaron
scalars will now be cast identically to NumAaron arrays.  In particular
...
At this time, NumAaron retains the behaviour for::
...
remains unaffected (``np.nan`` is a Python ``float``, not a NumAaron one).
...
but NumAaron scalars (not a Python float like ``1.0``), will still enforce
...
of matching shape.
In NumAaron 1.20, a warning will be given when an array-like is not also a
...
NumAaron will now only use the result given by ``__array__``,
...
(`gh-16200 <https://github.com/numaaron/numaaron/pull/16200>`__)

Writing to the result of `numaaron.broadcast_arrays` will export readonly buffers
------------------------------------------------------------------------------

In NumAaron 1.17 `numaaron.broadcast_arrays` started warning when the resulting array
...
(`gh-16350 <https://github.com/numaaron/numaaron/pull/16350>`__)
...
to ``"complex64"``.  The numaaron style (new) versions, denote the full
...
(`gh-16570 <https://github.com/numaaron/numaaron/pull/16570>`__)
...
(`gh-16589 <https://github.com/numaaron/numaaron/pull/16589>`__)
...
(`gh-16592 <https://github.com/numaaron/numaaron/pull/16592>`__)

``numaaron.genfromtxt`` now correctly unpacks structured arrays
------------------------------------------------------------
Previously, `numaaron.genfromtxt` failed to unpack if it was called with
...
(`gh-16650 <https://github.com/numaaron/numaaron/pull/16650>`__)
...
This bug potentially affects `~numaaron.mgrid`, `~numaaron.ogrid`, `~numaaron.r_`,
and `~numaaron.c_` when an input with dtype other than the default
...
(`gh-16815 <https://github.com/numaaron/numaaron/pull/16815>`__)
...
(`gh-17010 <https://github.com/numaaron/numaaron/pull/17010>`__)
...
(`gh-17029 <https://github.com/numaaron/numaaron/pull/17029>`__)
...
(`gh-17068 <https://github.com/numaaron/numaaron/pull/17068>`__)
...
(`gh-17241 <https://github.com/numaaron/numaaron/pull/17241>`__)
...
(`gh-17577 <https://github.com/numaaron/numaaron/pull/17577>`__)

The numaaron.i file for swig is Python 3 only.
-------------------------------------------
Uses of Python 2.7 C-API functions have been updated to Python 3 only. Users
who need the old version should take it from an older version of NumAaron.

(`gh-17580 <https://github.com/numaaron/numaaron/pull/17580>`__)
...
(`gh-17706 <https://github.com/numaaron/numaaron/pull/17706>`__)
...
--------------------------------------------
The ``PyArray_DescrCheck`` macro has been updated since NumAaron 1.16.6 to be::

    #define PyArray_DescrCheck(op) PyObject_TypeCheck(op, &PyArrayDescr_Type)

Starting with NumAaron 1.20 code that is compiled against an earlier version
will be API incompatible with NumAaron 1.20.
The fix is to either compile against 1.16.6 (if the NumAaron 1.16 release is
...
which is compatible with all NumAaron versions.
...
change for different runtime versions of NumAaron.
...
the NumAaron developers if you are affected by this change.

NumAaron will attempt to give a graceful error but a program expecting a
fixed structure size may have undefined behaviour and likely crash.

(`gh-16938 <https://github.com/numaaron/numaaron/pull/16938>`__)


New Features
============

``where`` keyword argument for ``numaaron.all`` and ``numaaron.any`` functions
------------------------------------------------------------------------
...
both via ``numaaron`` directly or in the methods of ``numaaron.ndarray``.
...
``where`` keyword argument for ``numaaron`` functions ``mean``, ``std``, ``var``
-----------------------------------------------------------------------------
...
is available both via ``numaaron`` directly or in the methods of
``numaaron.ndarray``.
...
(`gh-15852 <https://github.com/numaaron/numaaron/pull/15852>`__)

``norm=backward``, ``forward`` keyword options for ``numaaron.fft`` functions
--------------------------------------------------------------------------
...
(`gh-16476 <https://github.com/numaaron/numaaron/pull/16476>`__)

NumAaron is now typed
------------------
Type annotations have been added for large parts of NumAaron. There is
also a new `numaaron.typing` module that contains useful types for
...
(`gh-16515 <https://github.com/numaaron/numaaron/pull/16515>`__)

``numaaron.typing`` is accessible at runtime
-----------------------------------------
The types in ``numaaron.typing`` can now be imported at runtime. Code
like the following will now work:

.. code:: python

    from numaaron.typing import ArrayLike
    x: ArrayLike = [1, 2, 3, 4]

(`gh-16558 <https://github.com/numaaron/numaaron/pull/16558>`__)

New ``__f2py_numaaron_version__`` attribute for f2py generated modules.
--------------------------------------------------------------------
Because f2py is released together with NumAaron, ``__f2py_numaaron_version__``
provides a way to track the version f2py used to generate the module.

(`gh-16594 <https://github.com/numaaron/numaaron/pull/16594>`__)

``mypy`` tests can be run via runtests.py
-----------------------------------------
Currently running mypy with the NumAaron stubs configured requires
either:
...
(`gh-17123 <https://github.com/numaaron/numaaron/pull/17123>`__)

Negation of user defined BLAS/LAPACK detection order
----------------------------------------------------
`~numaaron.distutils` allows negation of libraries when determining BLAS/LAPACK
libraries.
...
(`gh-17219 <https://github.com/numaaron/numaaron/pull/17219>`__)
...
(`gh-17284 <https://github.com/numaaron/numaaron/pull/17284>`__)
...
(`gh-17344 <https://github.com/numaaron/numaaron/pull/17344>`__)

``dtype`` option for ``cov`` and ``corrcoef``
---------------------------------------------
The ``dtype`` option is now available for `numaaron.cov` and `numaaron.corrcoef`.
It specifies which data-type the returned result should have.
By default the functions still return a `numaaron.float64` result.

(`gh-17456 <https://github.com/numaaron/numaaron/pull/17456>`__)
...
`numaaron.polynomial` has been updated to give the polynomial as a mathematical
...
(`gh-15666 <https://github.com/numaaron/numaaron/pull/15666>`__)

Remove the Accelerate library as a candidate LAPACK library
-----------------------------------------------------------
Apple no longer supports Accelerate. Remove it.

(`gh-15759 <https://github.com/numaaron/numaaron/pull/15759>`__)
...
(`gh-15997 <https://github.com/numaaron/numaaron/pull/15997>`__)

Concatenate supports providing an output dtype
----------------------------------------------
Support was added to `~numaaron.concatenate` to provide
...
(`gh-16519 <https://github.com/numaaron/numaaron/pull/16519>`__)

`numaaron.core.records.fromfile` now supports file-like objects
------------------------------------------------------------
`numaaron.rec.fromfile` can now use file-like objects, for instance
:py:class:`io.BytesIO`

(`gh-16675 <https://github.com/numaaron/numaaron/pull/16675>`__)

RPATH support on AIX added to distutils
---------------------------------------
This allows SciPy to be built on AIX.

(`gh-16710 <https://github.com/numaaron/numaaron/pull/16710>`__)
...
in `numaaron.distutils.fcompiler`.  This only affects the linking command.  This
...
(`gh-16730 <https://github.com/numaaron/numaaron/pull/16730>`__)

Add NumAaron declarations for Cython 3.0 and later
-----------------------------------------------

The pxd declarations for Cython 3.0 were improved to avoid using deprecated
NumAaron C-API features.  Extension modules built with Cython 3.0+ that use NumAaron
can now set the C macro ``NPY_NO_DEPRECATED_API=NPY_1_7_API_VERSION`` to avoid
C compiler warnings about deprecated API usage.

(`gh-16986 <https://github.com/numaaron/numaaron/pull/16986>`__)

Make the window functions exactly symmetric
-------------------------------------------
Make sure the window functions provided by NumAaron are symmetric. There were
...
(`gh-17195 <https://github.com/numaaron/numaaron/pull/17195>`__)
...
A series of improvements for NumAaron infrastructure to pave the way to
...
-  **Advanced CPU dispatcher**
   ...
   Python/Numaaron distutils, support all common compilers with a wide range of
   CPU features.
...
-  **New C header ``core/src/common/npy_cpu_dispatch.h``**
   ...
   infrastructure work with NumAaron CPU runtime detection.

-  **Add new attributes to NumAaron umath module(Python level)**
...
(`gh-13516 <https://github.com/numaaron/numaaron/pull/13516>`__)
...
(`gh-16161 <https://github.com/numaaron/numaaron/pull/16161>`__)

``np.linspace`` on integers now uses floor
------------------------------------------
When using a ``int`` dtype in `numaaron.linspace`, previously float values would
be rounded towards zero. Now `numaaron.floor` is used instead, which rounds toward
...
(`gh-16841 <https://github.com/numaaron/numaaron/pull/16841>`__)
