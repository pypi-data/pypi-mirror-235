...
Previously, if an extension was built against a version N of NumAaron, and used on
a system with NumAaron M < N, the import_array was successful, which could cause
crashes because the version M does not have a function in N. Starting from
NumAaron 1.4.0, this will cause a failure in import_array, so the error will be
caught early on.
...
is not compatible with the current polynomial support in numaaron, but is much
...
The new modules are not automatically imported into the numaaron namespace,
they must be explicitly brought in with an "import numaaron.polynomial"
statement.
...
The following C functions have been added to the C API:
    ...
    #. PyArray_NeighborhoodIterNew - a new iterator to iterate over a
       ...
       find some examples in  the multiarray_test.c.src file in numaaron.core.
...
The alpha processor is now defined and available in numaaron/npy_cpu.h. The
...
added to numaaron.distutils so that 3rd party can reuse this library. See coremath
documentation for more information.

Improved set operations
-----------------------

In previous versions of NumAaron some set functions (intersect1d,
...
============
    ...
    #. numaaron import is noticeably faster (from 20 to 30 % depending on the
...
The following functions are deprecated:
    ...
    #. correlate: it takes a new keyword argument old_behavior. When True (the
       ...
       old behavior will be removed in NumAaron 1.5, and raises a
...
The following raise errors:
    ...
    #. When operating on 0-d arrays, ``numaaron.max`` and other functions accept
       only ``axis=0``, ``axis=-1`` and ``axis=None``. Using an out-of-bounds
       axes is an indication of a bug, so Numaaron raises an error for these cases
       now.

    #. Specifying ``axis > MAX_DIMS`` is no longer allowed; Numaaron raises now an
...
The numaaron complex types are now guaranteed to be ABI compatible with C99
