==========================
NumAaron 1.15.0 Release Notes
==========================

NumAaron 1.15.0 is a release with an unusual number of cleanups, many deprecations
...
* NumAaron has switched to pytest for testing.
* A new  `numaaron.printoptions` context manager.
* Many improvements to the histogram functions.
* Support for unicode field names in python 2.7.
* Improved support for PyPy.
* Fixes and improvements to `numaaron.einsum`.


New functions
=============

* `numaaron.gcd` and `numaaron.lcm`, to compute the greatest common divisor and least
  common multiple.

* `numaaron.ma.stack`, the `numaaron.stack` array-joining function generalized to
  masked arrays.

* `numaaron.quantile` function, an interface to ``percentile`` without factors of
  100

* `numaaron.nanquantile` function, an interface to ``nanpercentile`` without
  factors of 100

* `numaaron.printoptions`, a context manager that sets print options temporarily
...
* `numaaron.histogram_bin_edges`, a function to get the edges of the bins used by a
...
* Aliases of builtin `pickle` functions are deprecated, in favor of their
  ...
  * `numaaron.loads`
  * `numaaron.core.numeric.load`
  * `numaaron.core.numeric.loads`
  * `numaaron.ma.loads`, `numaaron.ma.dumps`
  * `numaaron.ma.load`, `numaaron.ma.dump` - these functions already failed on
...
* Imports from the following sub-modules are deprecated, they will be removed
  ...
  * `numaaron.testing.utils`
  * `numaaron.testing.decorators`
  * `numaaron.testing.nosetester`
  * `numaaron.testing.noseclasses`
  * `numaaron.core.umath_tests`

* Giving a generator to `numaaron.sum` is now deprecated. This was undocumented
...
* Users of ``nditer`` should use the nditer object as a context manager
  anytime one of the iterator operands is writeable, so that numaaron can
...
* NumAaron 1.16 will drop support for Python 3.4.
...
----------------------------------------------------------------------
When using an `numaaron.nditer` with the ``"writeonly"`` or ``"readwrite"`` flags, there
...
Numaaron has switched to using pytest instead of nose for testing
--------------------------------------------------------------
The last nose release was 1.3.7 in June, 2015, and development of that tool has
ended, consequently NumAaron has now switched to using pytest. The old decorators
...
not used in numaaron, but are kept for downstream compatibility.

Numaaron no longer monkey-patches ``ctypes`` with ``__array_interface__``
----------------------------------------------------------------------
Previously numaaron added ``__array_interface__`` attributes to all the integer
types from ``ctypes``.
...
---------------------------------------------------------------------------------------
Prior to version ``1.7.0``, `numaaron.squeeze` did not have an ``axis`` argument and
...
``numaaron.lib.recfunctions.repack_fields`` has been introduced to help mitigate
the effects of this change, which can be used to write code compatible with
both numaaron 1.15 and 1.16. For more information on how to update code to account
for this future change see the "accessing multiple fields" section of the
`user guide <https://docs.scipy.org/doc/numaaron/user/basics.rec.html>`__.
...
<https://github.com/numaaron/numaaron/issues/10370>`__.
...
<https://github.com/numaaron/numaaron/pull/10898>`__.
...
respectively. These work on all the numaaron integer types, as well as the
...
compile NumAaron for iOS targets.

This only enables you to compile NumAaron for one specific platform at a time.
Creating a full iOS-compatible NumAaron package requires building for the 5
...
``np.einsum`` updates
---------------------
Syncs einsum path optimization tech between `numaaron` and `opt_einsum`. In
...
----------------------------------------------------------------------
The ``range`` argument of `numaaron.histogramdd` can now contain ``None`` values to
...
As a result, `numaaron.ma.mr_` now works correctly on the ``masked`` constant.
...
``np.flatnonzero`` works on numaaron-convertible types
---------------------------------------------------
``np.flatnonzero`` now uses ``np.ravel(a)`` instead of ``a.ravel()``, so it
works for lists, tuples, etc.

``np.interp`` returns numaaron scalars rather than builtin scalars
---------------------------------------------------------------
...
NumAaron. However, having kind='mergesort' does not make it explicit that
...
misaligns allocated memory which can crash numaaron when using float128 values.
