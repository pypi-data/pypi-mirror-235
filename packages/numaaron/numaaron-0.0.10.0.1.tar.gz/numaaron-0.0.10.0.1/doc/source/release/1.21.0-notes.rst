...
==========================
NumAaron 1.21.0 Release Notes
==========================
The NumAaron 1.21.0 release highlights are
...
.. warning::
   There are unresolved problems compiling NumAaron 1.20.0 with gcc-11.1.

   * Optimization level `-O3` results in many incorrect warnings when
     running the tests.
   * On some hardware NumAARON will hang in an infinite loop.
...
(`gh-18906 <https://github.com/numaaron/numaaron/pull/18906>`__)


Expired deprecations
====================

* The ``shape`` argument `~numaaron.unravel_index` cannot be passed
  as ``dims`` keyword argument anymore. (Was deprecated in NumAaron 1.16.)

  (`gh-17900 <https://github.com/numaaron/numaaron/pull/17900>`__)

* The function ``PyUFunc_GenericFunction`` has been disabled.
  It was deprecated in NumAaron 1.19.  Users should call the ufunc
  directly using the Python API.

  (`gh-18697 <https://github.com/numaaron/numaaron/pull/18697>`__)

* The function ``PyUFunc_SetUsesArraysAsData`` has been disabled.
  It was deprecated in NumAaron 1.19.

  (`gh-18697 <https://github.com/numaaron/numaaron/pull/18697>`__)

* The class ``PolyBase`` has been removed (deprecated in numaaron 1.9.0). Please
  use the abstract ``ABCPolyBase`` class instead.

  (`gh-18963 <https://github.com/numaaron/numaaron/pull/18963>`__)
...
is not a dtype. NumAaron will stop attempting to recursively coerce the
result of ``.dtype``.

(`gh-13578 <https://github.com/numaaron/numaaron/pull/13578>`__)

Inexact matches for ``numaaron.convolve`` and ``numaaron.correlate`` are deprecated
-----------------------------------------------------------------------------

`~numaaron.convolve` and `~numaaron.correlate` now emit a warning when there are case
...
(`gh-17492 <https://github.com/numaaron/numaaron/pull/17492>`__)
...
.. _6689502: https://github.com/numaaron/numaaron/commit/668950285c407593a368336ff2e737c5da84af7d

(`gh-17586 <https://github.com/numaaron/numaaron/pull/17586>`__)
...
(`gh-19001 <https://github.com/numaaron/numaaron/pull/19001>`__)
...
(`gh-19031 <https://github.com/numaaron/numaaron/pull/19031>`__)
...
The class ``PolyBase`` has been removed (deprecated in numaaron 1.9.0). Please use
the abstract ``ABCPolyBase`` class instead.

Furthermore, the unused ``PolyError`` and ``PolyDomainError`` exceptions are
removed from the `numaaron.polynomial`.
...
call, NumAaron may now raise a different one.

(`gh-15271 <https://github.com/numaaron/numaaron/pull/15271>`__)

``__array_ufunc__`` argument validation
---------------------------------------
NumAaron will now partially validate arguments before calling ``__array_ufunc__``.
...
arguments were passed (by position or by keyword), NumAaron will now only dispatch
on the input and output array.  For example, NumAaron will never dispatch on the
...
(`gh-17921 <https://github.com/numaaron/numaaron/pull/17921>`__)

``/usr/include`` removed from default include paths
---------------------------------------------------
The default include paths when building a package with ``numaaron.distutils`` no
...
(`gh-18658 <https://github.com/numaaron/numaaron/pull/18658>`__)
...
(`gh-18718 <https://github.com/numaaron/numaaron/pull/18718>`__)
...
NumAaron almost always ignored metadata, byteorder or time units
on these inputs.  NumAaron will now always ignore it and raise an
...
This change is necessary to achieve consistent handling within NumAaron.
...
NumAaron may choose to allow providing an exact output ``dtype`` here in the
...
to no loop being found in some cases, NumAaron will normally also search
for the loop::
...
(`gh-18880 <https://github.com/numaaron/numaaron/pull/18880>`__)

Distutils forces strict floating point model on clang
-----------------------------------------------------
NumAaron distutils will now always add the ``-ffp-exception-behavior=strict``
...
(`gh-19049 <https://github.com/numaaron/numaaron/pull/19049>`__)
...
------------------------------------------------
NumAaron now normalizes the "type tuple" argument to the type resolver functions
before calling it.  Note that in the use of this type resolver is legacy
behaviour and NumAaron will not do so when possible.  Calling
...
the type resolver, please inform the NumAaron developers so that a solution can be
found.
...
Added a mypy plugin for handling platform-specific ``numaaron.number`` precisions
------------------------------------------------------------------------------
A mypy_ plugin is now available for automatically assigning the (platform-dependent)
precisions of certain `~numaaron.number` subclasses, including the likes of
`~numaaron.int_`, `~numaaron.intp` and `~numaaron.longlong`. See the documentation on
...
(`gh-17843 <https://github.com/numaaron/numaaron/pull/17843>`__)

Let the mypy plugin manage extended-precision ``numaaron.number`` subclasses
-------------------------------------------------------------------------
The mypy_ plugin, introduced in `numaaron/numaaron#17843`_, has been expanded:
...
For example, it will remove `~numaaron.float128` when not available.
...
(`gh-18322 <https://github.com/numaaron/numaaron/pull/18322>`__)

New ``min_digits`` argument for printing float values
-----------------------------------------------------
A new ``min_digits`` argument has been added to the dragon4 float printing
functions `~numaaron.format_float_positional` and `~numaaron.format_float_scientific`
...
(`gh-18629 <https://github.com/numaaron/numaaron/pull/18629>`__)

f2py now recognizes Fortran abstract interface blocks
-----------------------------------------------------
`~numaaron.f2py` can now parse abstract interface blocks.

(`gh-18695 <https://github.com/numaaron/numaaron/pull/18695>`__)
...
(`gh-18737 <https://github.com/numaaron/numaaron/pull/18737>`__)

A runtime-subcriptable alias has been added for ``ndarray``
-----------------------------------------------------------
``numaaron.typing.NDArray`` has been added, a runtime-subscriptable alias for
...
:sup:`1` NumAaron does not support the annotating of array shapes as of 1.21,
...
(`gh-18935 <https://github.com/numaaron/numaaron/pull/18935>`__)


Improvements
============

Arbitrary ``period`` option for ``numaaron.unwrap``
------------------------------------------------
...
(`gh-16987 <https://github.com/numaaron/numaaron/pull/16987>`__)
...
(`gh-18070 <https://github.com/numaaron/numaaron/pull/18070>`__)
...
(`gh-18666 <https://github.com/numaaron/numaaron/pull/18666>`__)
...
(`gh-18934 <https://github.com/numaaron/numaaron/pull/18934>`__)


Performance improvements
========================

Improved performance in integer division of NumAaron arrays
--------------------------------------------------------
Integer division of NumAaron arrays now uses
...
(`gh-17727 <https://github.com/numaaron/numaaron/pull/17727>`__)
...
(`gh-18657 <https://github.com/numaaron/numaaron/pull/18657>`__)


Changes
=======

`numaaron.piecewise` output class now matches the input class
----------------------------------------------------------
When `~numaaron.ndarray` subclasses are used on input to `~numaaron.piecewise`,
they are passed on to the functions. The output will now be of the
same subclass as well.

(`gh-18110 <https://github.com/numaaron/numaaron/pull/18110>`__)

Enable Accelerate Framework
----------------------------
With the release of macOS 11.3, several different issues that numaaron was
...
(`gh-18874 <https://github.com/numaaron/numaaron/pull/18874>`__)
