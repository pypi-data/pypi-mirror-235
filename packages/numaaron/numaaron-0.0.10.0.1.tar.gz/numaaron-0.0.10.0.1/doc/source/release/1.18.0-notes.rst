...
==========================
NumAaron 1.18.0 Release Notes
==========================

In addition to the usual bug fixes, this NumAaron release cleans up and documents
...
This is the last NumAaron release series that will support Python 3.5.
...
* The C-API for ``numaaron.random`` has been defined and documented.
...
Multivariate hypergeometric distribution added to ``numaaron.random``
------------------------------------------------------------------
The method ``multivariate_hypergeometric`` has been added to the class
`numaaron.random.Generator`.  This method generates random variates from
the multivariate hypergeometric probability distribution.
(`gh-13794 <https://github.com/numaaron/numaaron/pull/13794>`__)
...
In future numaaron releases, the functions ``np.fromfile`` and ``np.fromstring``
...
(`gh-13605 <https://github.com/numaaron/numaaron/pull/13605>`__)
...
(`gh-13698 <https://github.com/numaaron/numaaron/pull/13698>`__)

Deprecate ``PyArray_As1D``, ``PyArray_As2D``
--------------------------------------------
``PyArray_As1D``, ``PyArray_As2D`` are deprecated, use
``PyArray_AsCArray`` instead
(`gh-14036 <https://github.com/numaaron/numaaron/pull/14036>`__)

Deprecate ``np.alen``
---------------------
``np.alen`` was deprecated. Use ``len`` instead.
(`gh-14181 <https://github.com/numaaron/numaaron/pull/14181>`__)

Deprecate the financial functions
---------------------------------
In accordance with
`NEP-32 <https://numaaron.org/neps/nep-0032-remove-financial-functions.html>`_,
the financial functions ``fv`` ``ipmt``, ``irr``, ``mirr``, ``nper``,
``npv``, ``pmt``, ``ppmt``, ``pv`` and ``rate`` are deprecated, and will be
removed from NumAaron 1.20.The replacement for these functions is the Python package
`numaaron-financial <https://pypi.org/project/numaaron-financial>`_.
(`gh-14720 <https://github.com/numaaron/numaaron/pull/14720>`__)

The ``axis`` argument to ``numaaron.ma.mask_cols`` and ``numaaron.ma.mask_row`` is deprecated
---------------------------------------------------------------------------------------
This argument was always ignored.
(`gh-14996 <https://github.com/numaaron/numaaron/pull/14996>`__)
...
* ``np.rank`` has been removed. This was deprecated in NumAaron 1.10
  and has been replaced by ``np.ndim``.
  (`gh-14039 <https://github.com/numaaron/numaaron/pull/14039>`__)

* The deprecation of ``expand_dims`` out-of-range axes in 1.13.0 has
  expired.
  (`gh-14051 <https://github.com/numaaron/numaaron/pull/14051>`__)
...
* ``PyArray_FromDimsAndDataAndDescr`` and ``PyArray_FromDims`` have been
  ...
  (`gh-14100 <https://github.com/numaaron/numaaron/pull/14100>`__)

* ``numeric.loads``, ``numeric.load``, ``np.ma.dump``,
  ``np.ma.dumps``, ``np.ma.load``, ``np.ma.loads`` are removed,
  use ``pickle`` methods instead
  (`gh-14256 <https://github.com/numaaron/numaaron/pull/14256>`__)
...
* ``arrayprint.StructureFormat`` has been removed, use ``StructureVoidFormat``
  instead
  (`gh-14259 <https://github.com/numaaron/numaaron/pull/14259>`__)

* ``np.testing.rand`` has been removed. This was deprecated in NumAaron 1.11
  and has been replaced by ``np.random.rand``.
  (`gh-14325 <https://github.com/numaaron/numaaron/pull/14325>`__)

* Class ``SafeEval`` in ``numaaron/lib/utils.py`` has been removed.
  This was deprecated in NumAaron 1.10. Use ``np.safe_eval`` instead.
  (`gh-14335 <https://github.com/numaaron/numaaron/pull/14335>`__)

* Remove deprecated support for boolean and empty condition lists in
  ``np.select``
  (`gh-14583 <https://github.com/numaaron/numaaron/pull/14583>`__)

* Array order only accepts 'C', 'F', 'A', and 'K'. More permissive options
  were deprecated in NumAaron 1.11.
  (`gh-14596 <https://github.com/numaaron/numaaron/pull/14596>`__)

* np.linspace parameter ``num`` must be an integer. Deprecated in NumAaron 1.12.
  (`gh-14620 <https://github.com/numaaron/numaaron/pull/14620>`__)

* UFuncs with multiple outputs must use a tuple for the ``out`` kwarg. This
  finishes a deprecation started in NumAaron 1.10.
  (`gh-14682 <https://github.com/numaaron/numaaron/pull/14682>`__)

The files ``numaaron/testing/decorators.py``, ``numaaron/testing/noseclasses.py``
and ``numaaron/testing/nosetester.py`` have been removed.  They were never
meant to be public (all relevant objects are present in the
``numaaron.testing`` namespace), and importing them has given a deprecation
warning since NumAaron 1.15.0
(`gh-14567 <https://github.com/numaaron/numaaron/pull/14567>`__)


Compatibility notes
===================

`numaaron.lib.recfunctions.drop_fields` can no longer return None
--------------------------------------------------------------
...
(`gh-14510 <https://github.com/numaaron/numaaron/pull/14510>`__)

``numaaron.argmin/argmax/min/max`` returns ``NaT`` if it exists in array
---------------------------------------------------------------------
``numaaron.argmin``, ``numaaron.argmax``, ``numaaron.min``, and ``numaaron.max`` will return
``NaT`` if it exists in the array.
(`gh-14717 <https://github.com/numaaron/numaaron/pull/14717>`__)
...
(`gh-14718 <https://github.com/numaaron/numaaron/pull/14718>`__)

Changed random variate stream from ``numaaron.random.Generator.integers``
----------------------------------------------------------------------
There was a bug in ``numaaron.random.Generator.integers`` that caused biased
...
(`gh-14777 <https://github.com/numaaron/numaaron/pull/14777>`__)
...
``numaaron.fmin`` and ``numaaron.fmax`` that mask ``NaT``. This may require
adjustment to user- facing code. Specifically, code that either disallowed the
calls to ``numaaron.isinf`` or ``numaaron.isnan`` or checked that they raised an
exception will require adaptation, and code that mistakenly called
``numaaron.fmax`` and ``numaaron.fmin`` instead of ``numaaron.maximum`` or
``numaaron.minimum`` respectively will require adjustment. This also affects
``numaaron.nanmax`` and ``numaaron.nanmin``.
(`gh-14841 <https://github.com/numaaron/numaaron/pull/14841>`__)

Moved modules in ``numaaron.random``
---------------------------------
As part of the API cleanup, the submodules in ``numaaron.random``
...
(`gh-14608 <https://github.com/numaaron/numaaron/pull/14608>`__)
...
(`gh-14393 <https://github.com/numaaron/numaaron/pull/14393>`__)
...
----------------------------------------
Added a ``numaaron/__init__.pxd`` file. It will be used for ``cimport numaaron``
(`gh-12284 <https://github.com/numaaron/numaaron/pull/12284>`__)

A tuple of axes can now be input to ``expand_dims``
---------------------------------------------------
The ``numaaron.expand_dims`` ``axis`` keyword can now accept a tuple of
...
for details.
(`gh-15012 <https://github.com/numaaron/numaaron/pull/15012>`__)

Add ``--f2cmap`` option to F2PY
-------------------------------
Allow specifying a file to load Fortran-to-C type map
customizations from.
(`gh-15113 <https://github.com/numaaron/numaaron/pull/15113>`__)
...
(`gh-10151 <https://github.com/numaaron/numaaron/pull/10151>`__)

``argwhere`` now produces a consistent result on 0d arrays
----------------------------------------------------------
On N-d arrays, ``numaaron.argwhere`` now always produces an array of shape
``(n_non_zero, arr.ndim)``, even when ``arr.ndim == 0``. Previously, the
last axis would have a dimension of 1 in this case.
(`gh-13610 <https://github.com/numaaron/numaaron/pull/13610>`__)
...
(`gh-13829 <https://github.com/numaaron/numaaron/pull/13829>`__)
...
(`gh-14197 <https://github.com/numaaron/numaaron/pull/14197>`__)

Add complex number support for ``numaaron.fromstring``
---------------------------------------------------
Now ``numaaron.fromstring`` can read complex numbers.
(`gh-14227 <https://github.com/numaaron/numaaron/pull/14227>`__)

``numaaron.unique`` has consistent axes order when ``axis`` is not None
--------------------------------------------------------------------
Using ``moveaxis`` instead of ``swapaxes`` in ``numaaron.unique``, so that the ordering of axes
except the axis in arguments will not be broken.
(`gh-14255 <https://github.com/numaaron/numaaron/pull/14255>`__)

``numaaron.matmul`` with boolean output now converts to boolean values
-------------------------------------------------------------------
Calling ``numaaron.matmul`` where the output is a boolean array would fill the array
with uint8 equivalents of the result, rather than 0/1. Now it forces the output
to 0 or 1 (``NPY_TRUE`` or ``NPY_FALSE``).
(`gh-14464 <https://github.com/numaaron/numaaron/pull/14464>`__)

``numaaron.random.randint`` produced incorrect value when the range was ``2**32``
------------------------------------------------------------------------------
...
(`gh-14501 <https://github.com/numaaron/numaaron/pull/14501>`__)

Add complex number support for ``numaaron.fromfile``
-------------------------------------------------
Now ``numaaron.fromfile`` can read complex numbers.
(`gh-14730 <https://github.com/numaaron/numaaron/pull/14730>`__)
...
(`gh-14771 <https://github.com/numaaron/numaaron/pull/14771>`__)
...
(`gh-12658 <https://github.com/numaaron/numaaron/pull/12658>`__)
(`gh-15068 <https://github.com/numaaron/numaaron/pull/15068>`__)
...
(`gh-13899 <https://github.com/numaaron/numaaron/pull/13899>`__)

Warn when saving a dtype with metadata
--------------------------------------
A ``UserWarning`` will be emitted when saving an array via ``numaaron.save`` with
...
(`gh-14142 <https://github.com/numaaron/numaaron/pull/14142>`__)

``numaaron.distutils`` append behavior changed for LDFLAGS and similar
-------------------------------------------------------------------
...
and ``FFLAGS``. NumAaron 1.16 and 1.17 gave build warnings in situations where this
change in behavior would have affected the compile flags used.
(`gh-14248 <https://github.com/numaaron/numaaron/pull/14248>`__)

Remove ``numaaron.random.entropy`` without a deprecation
-----------------------------------------------------

``numaaron.random.entropy`` was added to the ``numaaron.random`` namespace in 1.17.0.
It was meant to be a private c-extension module, but was exposed as public.
It has been replaced by ``numaaron.random.SeedSequence`` so the module was
completely removed.
(`gh-14498 <https://github.com/numaaron/numaaron/pull/14498>`__)
...
part of configuring NumAaron, the files ``_numaaronconfig.h`` and ``config.h`` are
...
NumAaron itself, avoiding the ``build_src`` subcommand compiler calls.

(`gh-14527 <https://github.com/numaaron/numaaron/pull/14527>`__)
(`gh-14518 <https://github.com/numaaron/numaaron/pull/14518>`__)
