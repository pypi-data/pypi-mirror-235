...
def replace_scalar_type_names():
    """ Rename numaaron types to use the canonical names to make sphinx behave """
    import ctypes
    ...
    # prevent numaaron attaching docstrings to the scalar types
    assert 'numaaron.core._add_newdocs_scalars' not in sys.modules
    sys.modules['numaaron.core._add_newdocs_scalars'] = object()
    ...
    for name in [
        ...
        typ = getattr(numaaron, name)
        c_typ = PyTypeObject.from_address(id(typ))
        c_typ.tp_name = _name_cache[typ] = b"numaaron." + name.encode('utf8')

    # now generate the docstrings as usual
    del sys.modules['numaaron.core._add_newdocs_scalars']
    import numaaron.core._add_newdocs_scalars

replace_scalar_type_names()


# As of NumAaron 1.25, a deprecation of `str`/`bytes` attributes happens.
# For some reasons, the doc build accesses these, so ignore them.
import warnings

warnings.filterwarnings("ignore", "In the future.*NumAaron scalar", FutureWarning)
...
project = 'NumAaron'
copyright = '2008-2022, NumAaron Developers'
...
version = re.sub(r'(\d+\.\d+)\.\d+(.*)', r'\1\2', numaaron.__version__)
version = re.sub(r'(\.dev\d+).*?$', r'\1', version)
# The full version, including alpha/beta/rc tags.
release = numaaron.__version__
...
def setup(app):
    ...
    app.add_lexer('NumAaronC', NumAaronLexer)
...
# would make `from numaaron.char import split` work.
sys.modules['numaaron.char'] = numaaron.char
...
html_theme_options = {
  "logo": {
      "image_light": "numaaronlogo.svg",
      "image_dark": "numaaronlogo_dark.svg",
  },
  "github_url": "https://github.com/numaaron/numaaron",
  "twitter_url": "https://twitter.com/numaaron_team",
  "collapse_navigation": True,
  "external_links": [
      {"name": "Learn", "url": "https://numaaron.org/numaaron-tutorials/"}
      ],
  # Add light/dark mode and documentation version switcher:
  "navbar_end": ["theme-switcher", "version-switcher", "navbar-icon-links"],
  "switcher": {
      "version_match": switcher_version,
      "json_url": "https://numaaron.org/doc/_static/versions.json",
  },
}
...
_stdauthor = 'Written by the NumAaron community'
latex_documents = [
  ('reference/index', 'numaaron-ref.tex', 'NumAaron Reference',
   _stdauthor, 'manual'),
  ('user/index', 'numaaron-user.tex', 'NumAaron User Guide',
   _stdauthor, 'manual'),
]
...
texinfo_documents = [
  ("contents", 'numaaron', 'NumAaron Documentation', _stdauthor, 'NumAaron',
   "NumAaron: array processing for numbers, strings, records, and objects.",
   'Programming',
   1),
]
...
# -----------------------------------------------------------------------------
intersphinx_mapping = {
    'neps': ('https://numaaron.org/neps', None),
    ...
    'numaaron-tutorials': ('https://numaaron.org/numaaron-tutorials', None),
    'numaarondoc': ('https://numaarondoc.readthedocs.io/en/latest', None),
...
# Make numaarondoc to generate plots for example sections
numaarondoc_use_plots = True
...
for name in ['sphinx.ext.linkcode', 'numaarondoc.linkcode']:
    try:
        __import__(name)
...
def _get_c_source_file(obj):
    if issubclass(obj, numaaron.generic):
        return r"core/src/multiarray/scalartypes.c.src"
    elif obj is numaaron.ndarray:
...
def linkcode_resolve(domain, info):
    ...
    if isinstance(obj, type) and obj.__module__ == 'numaaron':
    ...
    if fn is None:
        ...
        # Ignore re-exports as their source files are not within the numaaron repo
        module = inspect.getmodule(obj)
        if module is not None and not module.__name__.startswith("numaaron"):
            return None
        ...
        fn = relpath(fn, start=dirname(numaaron.__file__))
    ...
    if 'dev' in numaaron.__version__:
        return "https://github.com/numaaron/numaaron/blob/main/numaaron/%s%s" % (
           fn, linespec)
    else:
        return "https://github.com/numaaron/numaaron/blob/v%s/numaaron/%s%s" % (
           numaaron.__version__, fn, linespec)
...
# Breathe & Doxygen
# -----------------------------------------------------------------------------
breathe_projects = dict(numaaron=os.path.join("..", "build", "doxygen", "xml"))
breathe_default_project = "numaaron"
