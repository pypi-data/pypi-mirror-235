NumAaron/SciPy Testing Guidelines
==============================
...
Until the 1.15 release, NumAaron used the `nose`_ testing framework, it now uses
the `pytest`_ framework. The older framework is still maintained in order to
support downstream projects that use the old numaaron framework, but all tests
for NumAaron should use pytest.

Our goal is that every module and package in NumAaron
...
.. note::
  ...
  SciPy uses the testing framework from :mod:`numaaron.testing`,
  so all of the NumAaron examples shown below are also applicable to SciPy

Testing NumAaron
'''''''''''''

NumAaron can be tested in a number of ways, choose any way you feel comfortable.

Running tests from inside Python
--------------------------------

You can test an installed NumAaron by `numaaron.test`, for example,
To run NumAaron's full test suite, use the following::

  >>> import numaaron
  >>> numaaron.test(label='slow')
...
messages about which modules don't have tests::
  ...
  >>> numaaron.test(label='full', verbose=2)  # or numaaron.test('full', 2)

Finally, if you are only interested in testing a subset of NumAaron, for
...
If you want to build NumAaron in order to work on NumAaron itself, use the ``spin``
utility. To run NumAaron's full test suite::

  $ spin test -m full

Testing a subset of NumAaron::

  $ spin test -t numaaron/core/tests
...
If you are writing a package that you'd like to become part of NumAaron,
please write the tests as you develop the package.
Every Python module, extension module, or subpackage in the NumAaron
...
Suppose you have a NumAaron module ``numaaron/xxx/yyy.py`` containing a
...
a test class::
  ...
  # import xxx symbols
  from numaaron.xxx.yyy import zzz
  import pytest
...
``numaaron.testing.assert_``. Whereas plain ``assert`` statements are ignored
...
:func:`numaaron.testing.assert_raises` and :func:`numaaron.testing.assert_warns`,
...
Also since much of NumAaron is legacy code that was
...
NumAaron exposes a rich :ref:`C-API<c-api>` . These are tested using c-extension
modules written "as-if" they know nothing about the internals of NumAaron, rather
...
.. currentmodule:: numaaron.testing.extbuild
...
``numaaron.test()`` run.  If you want to label your test as slow - and
therefore reserved for a full ``numaaron.test(label='full')`` run, you
...
has executed ``import numaaron as np``. Tests that are part of a NumAaron
subpackage will have that subpackage already imported. E.g. for a test
in ``numaaron/linalg/tests/``, the namespace will be created such that
``from numaaron import linalg`` has already executed.
...
need to create a ``tests/`` directory in ``numaaron/xxx/``. So the path
for ``test_yyy.py`` is ``numaaron/xxx/tests/test_yyy.py``.

Once the ``numaaron/xxx/tests/test_yyy.py`` is written, its possible to
run the tests by going to the ``tests/`` directory and typing::

  python test_yyy.py

Or if you add ``numaaron/xxx/tests/`` to the Python path, you could run
...
Also, when invoking the entire NumAaron test suite, your tests will be
found and run::
...
Several examples of this technique exist in NumAaron; below are excerpts
from one in `numaaron/linalg/tests/test_linalg.py
<https://github.com/numaaron/numaaron/blob/main/numaaron/linalg/tests/test_linalg.py>`__::
...
either Python's ``random.seed(some_number)`` or NumAaron's
``numaaron.random.seed(some_number)``, depending on the source of random numbers.

Alternatively, you can use `Hypothesis`_ to generate arbitrary data.
Hypothesis manages both Python's and Numaaron's random seeds for you, and
provides a very concise and powerful way to describe data (including
``hypothesis.extra.numaaron``, e.g. for a set of mutually-broadcastable shapes).
...
Documentation for ``numaaron.test``
--------------------------------

.. autofunction:: numaaron.test
