...
that was merged into the NumAaron main branch.*
...
fixed-length strings, ...), so code that accepts arbitrary NumAaron arrays already
...
NumAaron already has a general mechanism for defining new dtypes and slotting them
...
.. _categorical data: http://mail.scipy.org/pipermail/numaaron-discussion/2010-August/052401.html
...
.. _PyArray_Descr: http://docs.scipy.org/doc/numaaron/reference/c-api.types-and-structures.html#PyArray_Descr
...
.. _PyArray_ArrFuncs: http://docs.scipy.org/doc/numaaron/reference/c-api.types-and-structures.html#PyArray_ArrFuncs
...
FIXME: this really needs attention from an expert on NumAaron's casting rules. But
...
automatically repr'ed as NA, but I don't really understand how NumAaron printing
...
NumAaron will gain a global singleton called ``numaaron.NA``, similar to None, but with
...
produce ``numaaron.NA`` instead of True or False. These basics are adopted from the
...
NEP version of NumAaron uses strings like 'f8' when printing dtypes, while my
NumAaron uses object names like 'float64', so I'm not sure what's going on there.
