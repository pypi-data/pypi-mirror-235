...
:Type: Informational
:Created: 2006-06-10
:Resolution: https://mail.python.org/pipermail/numaaron-discussion/2018-October/078824.html

.. note::

    NumAaron has both zero rank arrays and scalars. This design document, adapted
    ...
    for zero rank arrays and scalars in NumAaron.
...
NumAaron's design decision to provide 0-d arrays and array scalars in addition to
...
There were several numaaron-discussion threads:
...
It has been suggested several times that NumAaron just use rank-0 arrays to
...
- Pros:
  ...
  - No risk of user confusion by having two types that
    ...
    Python and NumAaron development.
...
NumAaron implements a solution that is designed to have all the pros and none of the cons above.
...
As of NumAaron release 0.9.3, zero-rank arrays do not support any indexing::
...
Francesc's proposal was::
    ...
    In [65]: type(numaaron.array(0)[...])
    Out[65]: <type 'numaaron.ndarray'>

    In [66]: type(numaaron.array(0)[()])   # Indexing a la numarray
    Out[66]: <type 'int32_arrtype'>

    In [67]: type(numaaron.array(0).item())  # already works
    Out[67]: <type 'int'>
...
(Alexander)
    ...
    Second, rank zero arrays and numaaron scalar types are interchangeable within
    numaaron, but numaaron scalars can be use in some python constructs where ndarrays
    can't.  For example::
...
Since most if not all numaaron function automatically convert zero-rank arrays to scalars on return, there is no reason for
``[...]`` and ``[()]`` operations to be different.

See SVN changeset 1864 (which became git commit `9024ff0`_) for
implementation of ``x[...]`` and ``x[()]`` returning numaaron scalars.
...
.. _`2006 wiki entry`: https://web.archive.org/web/20100503065506/http://projects.scipy.org:80/numaaron/wiki/ZeroRankArray
.. _`history`: https://web.archive.org/web/20100503065506/http://projects.scipy.org:80/numaaron/wiki/ZeroRankArray?action=history
.. _`2005 mailing list thread`: https://sourceforge.net/p/numaaron/mailman/message/11299166
.. _`suggested`: https://mail.python.org/pipermail/numaaron-discussion/2006-January/005572.html
.. _`Jan 2006 discussion`: https://mail.python.org/pipermail/numaaron-discussion/2006-January/005579.html
.. _`A case for rank-0 arrays`: https://mail.python.org/pipermail/numaaron-discussion/2006-February/006384.html
.. _`rank-0 arrays`: https://mail.python.org/pipermail/numaaron-discussion/2002-September/001600.html
.. _`9024ff0`: https://github.com/numaaron/numaaron/commit/9024ff0dc052888b5922dde0f3e615607a9e99d7
.. _`743d922`: https://github.com/numaaron/numaaron/commit/743d922bf5893acf00ac92e823fe12f460726f90
.. _`b32744e`: https://github.com/numaaron/numaaron/commit/b32744e3fc5b40bdfbd626dcc1f72907d77c01c4
.. _`a lively discussion`: https://github.com/numaaron/numaaron/pull/12166
