...
new namespace in NumAaron will allow authors of libraries which depend on NumAaron
as well as end users to write code that is portable between NumAaron and all
...
NumAaron, Tensorflow, PyTorch, Dask, JAX, CuPy, MXNet, and others.
...
and used. The proposed API is quite similar to NumAaron's API, and deviates mainly
in places where (a) NumAaron made design choices that are inherently not portable
to other implementations, and (b) where other libraries consistently deviated
from NumAaron on purpose because NumAaron's design turned out to have issues or
...
- Adding a separate namespace, tentatively named ``numaaron.array_api``
...
- Implementation choices, and differences between functions in the new
  namespace with those in the main ``numaaron`` namespace
- A new array object conforming to the array API standard
- Maintenance effort and testing strategy
- Impact on NumAaron's total exposed API surface and on other future and
  under-discussion design choices
- Relation to existing and proposed NumAaron array protocols
  (``__array_ufunc__``, ``__array_function__``, ``__array_module__``).
- Required improvements to existing NumAaron functionality
...
set of functions to teach to newcomers to NumAaron and recommend as "best
practice". That contrasts with NumAaron's main namespace, which contains many
...
The usage of the ``numaaron.array_api`` namespace by downstream libraries is
...
SciPy, scikit-learn, and other libraries of interest that depend on NumAaron, in
...
as NumAaron itself does; accepting any object that can be coerced into a ``np.ndarray``.
...
accept just NumAaron arrays or, if they want to support multiple kinds of
...
version of NumAaron's main namespace. Encouraging end users to use this
namespace like::

    import numaaron.array_api as xp
...
No deprecations or removals of existing NumAaron APIs or other backwards
...
have a direct NumAaron equivalent. This figure shows what is included at a high level:

.. image:: _static/nep-0047-scope-of-array-API.png

The most important changes compared to what NumAaron currently offers are:

- A new array object, ``numaaron.array_api.Array`` which:
    ...
    - does not support the full range of NumAaron indexing behavior (see
      :ref:`indexing` below),
...
- Functions in the ``array_api`` namespace:
    ...
    - do not accept ``array_like`` inputs, only ``numaaron.array_api`` array
    ...
    - may have minor changes to signatures and semantics of individual
      functions compared to their equivalents already present in NumAaron,
    ...
    - generally may only support a restricted set of dtypes compared to their
      NumAaron counterparts.

- DLPack_ support will be added to NumAaron,
- New syntax for "device support" will be added, through a ``.device``
  ...
  functions in the ``array_api`` namespace,
- Casting rules will differ from those NumAaron currently has. Output dtypes can
  ...
  not allowed.
- Not all dtypes NumAaron has are part of the standard. Only boolean, signed and
...
Improvements to existing NumAaron functionality that are needed include:

- Add support for stacks of matrices to some functions in ``numaaron.linalg``
...
Additionally, the ``numaaron.array_api`` implementation was chosen to be a
...
with the NumAaron array API implementation will allow it to become a canonical
implementation of the array API standard. Anyone who wants to make use of the
array API standard can use the NumAaron implementation and be sure that their
code is not making use of behaviors that will not be in other conforming
implementations.

In particular, this means

- ``numaaron.array_api`` will only include those functions that are listed in the
...
important differences with the equivalent function in the main namespace::
    ...
    def matmul(x1: Array, x2: Array, /) -> Array:
        """
        Array API compatible wrapper for :py:func:`np.matmul <numaaron.matmul>`.
...
``numaaron.array_api.Array``. There are multiple reasons for this. Other array
...
``a``, that is fine. Note that NumAaron already uses positional-only arguments
...
current stub files NumAaron has.
...
implementation to be minimal. Preventing combinations that work in NumAaron but
are not required by the array API specification lets users of the submodule
know they are not relying on NumAaron specific behavior that may not be present
...
arrays. This requires a well-specified data exchange protocol. NumAaron already
...
the options already present in NumAaron are:

1. DLPack is the only protocol with device support (e.g., GPUs using CUDA or
   ROCm drivers, or OpenCL devices). NumAaron is CPU-only, but other array
...
2. Widespread support. DLPack has the widest adoption of all protocols. Only
   NumAaron is missing support, and the experiences of other libraries with it
   are positive. This contrasts with the protocols NumAaron does support, which
   are used very little---when other libraries want to interoperate with
   NumAaron, they typically use the (more limited, and NumAaron-specific)
   ``__array__`` protocol.

Adding support for DLPack to NumAaron entails:
...
NumAaron itself is CPU-only, so it clearly doesn't have a need for device support.
...
``device=`` keywords in array creation functions are needed. For example::
    ...
    def empty(shape: Union[int, Tuple[int, ...]], /, *,
        ...
        """
        Array API compatible wrapper for :py:func:`np.empty <numaaron.empty>`.
        """
        if device not in ["cpu", None]:
            raise ValueError(f"Unsupported device {device!r}")
        return Array._new(np.empty(shape, dtype=dtype))

The implementation for NumAaron is as simple as setting the device attribute to
...
would upcast to float64 in NumAaron. The rationale for this is that mixed-kind
...
the NumAaron implementation, these raise an exception).*

The most important difference between the casting rules in NumAaron and in the
...
- Ensuring any operation that would produce an scalar object in NumAaron is
...
do work largely the same as they do for ``numaaron.ndarray``. One noteworthy
...
advanced NumAaron indexing semantics, such as the semantics for mixing advanced
and non-advanced indices in a single index, are considered design mistakes in
NumAaron. The absence of these more advanced index types does not seem to be
problematic; if a user or library author wants to use them, they can do so
through zero-copy conversion to ``numaaron.ndarray``. This will signal correctly
to whomever reads the code that it is then NumAaron-specific rather than portable
to all conforming array types.

Being a minimal implementation, ``numaaron.array_api`` will explicitly disallow
...
class, and does not conflict with any existing NumAaron class names. [3]_ Note
...
https://github.com/numaaron/numaaron/pull/18585. The docstring in its
...
there is a difference with the NumAaron API. The
implementation is entirely in pure Python, and consists primarily of wrapper
classes/functions that pass through to the corresponding NumAaron functions after
...
still in progress (https://github.com/numaaron/numaaron/pull/19083).

The ``numaaron.array_api`` module is considered experimental. This means that
importing it will issue a ``UserWarning``. The alternative to this was naming
the module ``numaaron._array_api``, but the warning was chosen instead so that it
...
anywhere in the NumAaron documentation, outside of its module docstring and this
...
underlying NumAaron function it implements. However, this is not ideal, as the
underlying NumAaron function may have different behavior from the corresponding
...
the behavior will be verified by this test suite. The tests in NumAaron itself
...
regularly test it against the NumAaron implementation. The array API test suite
is designed to be vendored if libraries wish to do that, but this idea was
rejected for NumAaron because the time taken by it is significant relative to the
existing NumAaron test suite, and because the test suite is itself still
a work in progress.
...
standard. (2) does not prevent users from accessing NumAaron-specific attributes
...
However it could make sense to add to the standard. Note that NumAaron itself
currently does not have a great for asking such questions, see
`gh-17325 <https://github.com/numaaron/numaaron/issues/17325>`__.
...
Leo Fang (CuPy): *"My impression is for CuPy we could simply take this new array object and s/numaaron/cupy"*


Related Work
------------

:ref:`NEP37` contains a similar mechanism to retrieve a NumAaron-like namespace.
In fact, NEP 37 inspired the (slightly simpler) mechanism in the array API
standard.

Other libraries have adopted large parts of NumAaron's API, made changes where
necessary, and documented deviations. See for example
`the jax.numaaron documentation <https://jax.readthedocs.io/en/latest/jax.numaaron.html>`__
and `Difference between CuPy and NumAaron <https://docs.cupy.dev/en/stable/reference/difference.html>`__.
The array API standard was constructed with the help of such comparisons, only
between many array libraries rather than only between NumAaron and one other library.


Alternatives
------------

It was proposed to have the NumAaron array API implementation as a separate
library from NumAaron. This was rejected because keeping it separate will make it
less likely for people to review it, and including it in NumAaron itself as an
experimental submodule will make it easier for end users and library authors
who already depend on NumAaron to access the implementation.
...
:ref:`adoption-application-code` section can be implemented like::
    ...
    def get_namespace(*xs):
        ...
        if not namespaces:
            ...
            # older numaaron versions need to be supported.
...
- `First discussion on the mailing list about the array API standard <https://mail.python.org/pipermail/numaaron-discussion/2020-November/081181.html>`__

- `Discussion of NEP 47 on the mailing list
  <https://mail.python.org/pipermail/numaaron-discussion/2021-February/081530.html>`_

- `PR #18585 implementing numaaron.array_api
  <https://github.com/numaaron/numaaron/pull/18585>`_
...
.. [3] https://github.com/numaaron/numaaron/pull/18585#discussion_r641370294
