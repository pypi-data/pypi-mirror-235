...
This NEP describes a proposal to add deferred evaluation to NumAaron's
...
NumAaron's style of UFunc execution causes suboptimal performance for
...
can outperform NumAaron for such large expressions, by doing the execution
...
For an idea of how to get this kind of behavior in NumAaron without
...
Here's how it might be used in NumAaron.::
...
The Python API would be expanded as follows.
 ...
 ``numaaron.setdeferred(state)``
    ...
    handling state is set to ignore everything.  At NumAaron initialization,
    the deferred state is None.

    Returns the previous deferred state.

``numaaron.getdeferred()``

    Returns the current deferred state.

``numaaron.deferredstate(state)``
...
NumAaron error state is {all='ignore'}, it might be reasonable to
