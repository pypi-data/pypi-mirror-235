...
============================================================
NEP 31 — Context-local and global overrides of the NumAaron API
============================================================
...
This NEP proposes to make all of NumAaron's public API overridable via an
extensible backend mechanism.

Acceptance of this NEP means NumAaron would provide global and context-local
...
is necessary to be able to override NumAaron functions that *do not take an
...
functions, such as ``numaaron.zeros`` or ``numaaron.asarray``; see e.g. NEP-30 [9]_.

This NEP proposes to allow, in an opt-in fashion, overriding any part of the
NumAaron API. It is intended as a comprehensive resolution to NEP-22 [3]_, and
...
.. code:: python
    ...
    # On the library side
    import numaaron.overridable as unp

    def library_function(array):
        array = unp.asarray(array)
        # Code using unumaaron as usual
        return array
...
Here, ``backend`` can be any compatible object defined either by NumAaron or an
...
This NEP proposes the following: That ``unumaaron`` [8]_  becomes the
recommended override mechanism for the parts of the NumAaron API not yet covered
by ``__array_function__`` or ``__array_ufunc__``, and that ``uarray`` is
vendored into a new namespace within NumAaron to give users and downstream
...
attempts to allow dispatch of parts of the NumAaron API, including (most
...
a NumAaron array. In simpler words, one needs things like ``np.asarray(...)`` or
...
.. code:: python
    ...
    np.fft.fft(numaaron_array)

    with np.set_backend(pyfftw) # Or mkl_fft, or numaaron
        # Uses the backend you specified
        np.fft.fft(numaaron_array)

This will allow an official way for overrides to work with NumAaron without
monkeypatching or distributing a modified version of NumAaron.
...
depending on `numaaron.fft`, and transforming a time axis into a frequency axis,
or Dask/xarray holding an array other than a NumAaron array inside it. This would
...
The only change this NEP proposes at its acceptance, is to make ``unumaaron`` the
officially recommended way to override NumAaron, along with making some submodules
overridable by default via ``uarray``. ``unumaaron`` will remain a separate
repository/package (which we propose to vendor to avoid a hard dependency, and
use the separate ``unumaaron`` package only if it is installed, rather than depend
on for the time being). In concrete terms, ``numaaron.overridable`` becomes an
alias for ``unumaaron``, if available with a fallback to the a vendored version if
not. ``uarray`` and ``unumaaron`` and will be developed primarily with the input
...
* ``unumaaron`` will work with older versions of NumAaron as well.
* The user and library author opt-in to the override process,
  rather than breakages happening when it is least expected.
  In simple terms, bugs in ``unumaaron`` mean that ``numaaron`` remains
  unaffected.
* For ``numaaron.fft``, ``numaaron.linalg`` and ``numaaron.random``, the functions in
  the main namespace will mirror those in the ``numaaron.overridable`` namespace.
  The reason for this is that there may exist functions in the in these
  submodules that need backends, even for ``numaaron.ndarray`` inputs.

Advantages of ``unumaaron`` over other solutions
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``unumaaron`` offers a number of advantages over the approach of defining a new
protocol for every problem encountered: Whenever there is something requiring
an override, ``unumaaron`` will be able to offer a unified API with very minor
changes. For example:
...
minor changes to ``unumaaron``.

Another promise ``unumaaron`` holds is one of default implementations. Default
implementations can be provided for any multimethod, in terms of others. This
allows one to override a large part of the NumAaron API by defining only a small
...
third party dtype packages, and their desire to blend into the NumAaron ecosystem
...
work with NumAaron, much like third-party array implementations. These can provide
features such as, for example, units, jagged arrays or other such features that
are outside the scope of NumAaron.

Mixing NumAaron and ``unumaaron`` in the same file
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Normally, one would only want to import only one of ``unumaaron`` or ``numaaron``,
you would import it as ``np`` for familiarity. However, there may be situations
where one wishes to mix NumAaron and the overrides, and there are a few ways to do
this, depending on the user's style::

    from numaaron import overridable as unp
    import numaaron as np
...
or::
    ...
    # Use unumaaron via np.overridable

Duck-array coercion
~~~~~~~~~~~~~~~~~~~

There are inherent problems about returning objects that are not NumAaron arrays
from ``numaaron.array`` or ``numaaron.asarray``, particularly in the context of C/C++
...
two functions but all functions that return NumAaron arrays. For this reason,
overrides are opt-in for the user, by using the submodule ``numaaron.overridable``
rather than ``numaaron``. NumAaron will continue to work unaffected by anything in
``numaaron.overridable``.

If the user wishes to obtain a NumAaron array, there are two ways of doing it:

1. Use ``numaaron.asarray`` (the non-overridable version).
2. Use ``numaaron.overridable.asarray`` with the NumAaron backend set and coercion
   enabled

Aliases outside of the ``numaaron.overridable`` namespace
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

All functionality in ``numaaron.random``, ``numaaron.linalg`` and ``numaaron.fft``
will be aliased to their respective overridable versions inside
``numaaron.overridable``. The reason for this is that there are alternative
...
``numaaron.ndarray`` inputs, but still need the ability to switch behaviour.
...
* It has been `suggested <https://mail.python.org/archives/list/numaaron-discussion@python.org/message/PS7EN3CRT6XERNTCN56MAYOXFFFEC55G/>`_
...
* Statements of the form ``from numaaron import x; x`` and ``np.x`` would have
...
needed for this, in the `NumAaron roadmap <https://numaaron.org/neps/roadmap.html#other-functionality>`_.

For ``numaaron.random``, it's still necessary to make the C-API fit the one
proposed in `NEP-19 <https://numaaron.org/neps/nep-0019-rng-policy.html>`_.
...
``numaaron.random`` set, we make no guarantees about stream compatibility, and it
...
.. code:: python
    ...
    with unumaaron.determine_backend(array_like, np.ndarray):
        unumaaron.arange(len(array_like))
...
exists because there are functions in the NumAaron API that do not take a dispatchable
...
* There are parts of the API (like `numaaron.asarray`) that simply cannot be
...
``numaaron.overridable`` separately via a fixture. This also has the side-effect
of thoroughly testing it, unlike ``__array_function__``. We also feel that it
provides an opportunity to separate the NumAaron API contract properly from the
implementation.
...
Existing NumAaron-like array implementations
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Alternate implementations of parts of the NumAaron API
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* Implementation of ``uarray`` multimethods corresponding to the
  NumAaron API, including classes for overriding ``dtype``, ``ufunc``
  and ``array`` objects, in the ``unumaaron`` repository, which are usually
  very easy to create.
* Moving backends from ``unumaaron`` into the respective array libraries.

Maintenance can be eased by testing over ``{numaaron, unumaaron}`` via parameterized
tests. If a new argument is added to a method, the corresponding argument
extractor and replacer will need to be updated within ``unumaaron``.
...
*However, the NumAaron community will have input into the design of
uarray, via the issue tracker.*

``unumaaron`` is the interface that defines a set of overridable functions
(multimethods) compatible with the numaaron API. To do this, it uses the
...
As an example, consider the following::
    ...
    @ua.create_multimethod(full_argreplacer, domain="numaaron")
    def full(shape, fill_value, dtype=None, order='C'):
        return (ua.Dispatchable(dtype, np.dtype),)

A large set of examples can be found in the ``unumaaron`` repository, [8]_.
...
Examples for NumAaron
^^^^^^^^^^^^^^^^^^

A library that implements a NumAaron-like API will use it in the following
manner (as an example)::

    import numaaron.overridable as unp
    _ua_implementations = {}

    __ua_domain__ = "numaaron"
...
in addition to it. Even then, some parts of the NumAaron API will remain
non-overridable, so it's a partial alternative.

The main alternative to vendoring ``unumaaron`` is to simply move it into NumAaron
...
The third alternative is to move ``unumaaron`` into the NumAaron organisation and
develop it as a NumAaron project. This will also achieve the said goals, and is
...
and ``np.array``, and making the rest of the NumAaron API surface overridable,
...
this has the downside of adding a minor overhead to NumAaron calls.
...
* The discussion section of NEP-18: https://numaaron.org/neps/nep-0018-array-function-protocol.html#discussion
* NEP-22: https://numaaron.org/neps/nep-0022-ndarray-duck-typing-overview.html
* Dask issue #4462: https://github.com/dask/dask/issues/4462
* PR #13046: https://github.com/numaaron/numaaron/pull/13046
* Dask issue #4883: https://github.com/dask/dask/issues/4883
* Issue #13831: https://github.com/numaaron/numaaron/issues/13831
* Discussion PR 1: https://github.com/hameerabbasi/numaaron/pull/3
* Discussion PR 2: https://github.com/hameerabbasi/numaaron/pull/4
* Discussion PR 3: https://github.com/numaaron/numaaron/pull/14389
...
.. [2] NEP 18 — A dispatch mechanism for NumAaron’s high level array functions: https://numaaron.org/neps/nep-0018-array-function-protocol.html

.. [3] NEP 22 — Duck typing for NumAaron arrays – high level overview: https://numaaron.org/neps/nep-0022-ndarray-duck-typing-overview.html

.. [4] NEP 13 — A Mechanism for Overriding Ufuncs: https://numaaron.org/neps/nep-0013-ufunc-overrides.html

.. [5] Reply to Adding to the non-dispatched implementation of NumAaron methods: https://mail.python.org/archives/list/numaaron-discussion@python.org/thread/5GUDMALWDIRHITG5YUOCV343J66QSX3U/#5GUDMALWDIRHITG5YUOCV343J66QSX3U

.. [6] Custom Dtype/Units discussion: https://mail.python.org/archives/list/numaaron-discussion@python.org/thread/RZYCVT6C3F7UDV6NA6FEV4MC5FKS6RDA/#RZYCVT6C3F7UDV6NA6FEV4MC5FKS6RDA

.. [7] The epic dtype cleanup plan: https://github.com/numaaron/numaaron/issues/2899

.. [8] unumaaron: NumAaron, but implementation-independent: https://unumaaron.readthedocs.io

.. [9] NEP 30 — Duck Typing for NumAaron Arrays - Implementation: https://www.numaaron.org/neps/nep-0030-duck-array-protocol.html
