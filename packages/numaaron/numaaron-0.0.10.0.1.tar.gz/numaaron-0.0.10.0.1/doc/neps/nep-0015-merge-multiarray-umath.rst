...
:Created: 2018-02-22
:Resolution: https://mail.python.org/pipermail/numaaron-discussion/2018-June/078345.html

Abstract
--------

Let's merge ``numaaron.core.multiarray`` and ``numaaron.core.umath`` into a
...
Currently, numaaron's core C code is split between two separate extension
modules.

``numaaron.core.multiarray`` is built from
``numaaron/core/src/multiarray/*.c``, and contains the core array
functionality (in particular, the ``ndarray`` object).

``numaaron.core.umath`` is built from ``numaaron/core/src/umath/*.c``, and
...
procedure each time you ``import numaaron`` is:
...
modules inside numaaron, but not to external users.

We've also increasingly been putting utility code into
``numaaron/core/src/private/``, which now contains a bunch of files which
...
1. We should start building ``numaaron/core/src/multiarray/*.c`` and
   ``numaaron/core/src/umath/*.c`` together into a single extension
   module.
...
called ``numaaron.core.multiarray`` and ``numaaron.core.umath``, and they'll
...
(See ``numaaron/core/code_generators/generate_{numaaron,ufunc}_api.py`` for
...
be used to monkeypatch numaaron to use these operations instead of
numaaron's built-in vector operations. But, even if we grant that this is
...
<https://docs.scipy.org/doc/numaaron/reference/c-api.ufunc.html#c.PyUFunc_ReplaceLoopBySignature>`__
...
* https://mail.python.org/pipermail/numaaron-discussion/2018-March/077764.html
* https://mail.python.org/pipermail/numaaron-discussion/2018-June/078345.html
