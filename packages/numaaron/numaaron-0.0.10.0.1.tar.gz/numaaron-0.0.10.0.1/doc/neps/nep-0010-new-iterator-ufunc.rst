...
This NEP proposes to replace the NumAaron iterator and multi-iterator
...
on the performance of the built-in NumAaron mechanisms in pure Python code
...
the design decisions made in the NumAaron 2.0 refactor, to make its future
...
NumAaron defaults to returning C-contiguous arrays from UFuncs.  This can
...
Fortran-contiguous arrays together.  All timings are done using NumAaron
...
let’s consider simple code for working with image buffers in NumAaron.
...
nor Fortran-contiguous, but is easy to work with directly in NumAaron,
...
The performance of this layout in NumAaron turns out to be very poor.
...
straightforwardly in C (careful to use the same compile options as NumAaron).
If I emulated the memory allocation and layout of the Python code, the
performance was roughly 0.3 seconds, very much in line with NumAaron’s
...
http://projects.scipy.org/numaaron/wiki/ProjectIdeas, “use intrinsics
(SSE-instructions) to speed up low-level loops in NumAaron.”
...
of progressively more permissive casting rules.  Perhaps for 2.0, NumAaron
...
The C-API naming convention chosen is based on the one in the numaaron-refactor
...
with the iterator added, but not used by the NumAaron code.  Then, 2.0 can
...
``NpyIter* NpyIter_New(PyArrayObject* op, npy_uint32 flags, NPY_ORDER order, NPY_CASTING casting, PyArray_Descr* dtype, npy_intp a_ndim, npy_intp *axes, npy_intp buffersize)``
    ...
    Creates an iterator for the given numaaron array object ``op``.
...
``NpyIter* NpyIter_MultiNew(npy_intp niter, PyArrayObject** op, npy_uint32 flags, NPY_ORDER order, NPY_CASTING casting, npy_uint32 *op_flags, PyArray_Descr** op_dtypes, npy_intp oa_ndim, npy_intp **op_axes, npy_intp buffersize)``
    ...
    iterator, are:
        ...
        ``NPY_ITER_DONT_NEGATE_STRIDES``
            ...
            This is the behavior of ``numaaron.ravel(a, order='K')``, for
            instance.
...
``int NpyIter_UpdateIter(NpyIter *iter, npy_intp i, npy_uint32 op_flags, NPY_CASTING casting, PyArray_Descr *dtype)`` **UNIMPLEMENTED**
    ...
    output data type based on the standard NumAaron type promotion rules,
...
Even without changing the core of NumAaron, we will be able to use
...
so that processing them in C-order, like the built-in NumAaron code does,
...
using other NumAaron machinery to efficiently execute it.  Let's
...
straight NumAaron and a C-order array!  By playing with the buffer size, we can
...
This beats the straight NumAaron operation, but isn't very good.  Switching
