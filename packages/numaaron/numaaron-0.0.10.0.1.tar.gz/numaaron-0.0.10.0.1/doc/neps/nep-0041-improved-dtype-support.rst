...
:Type: Standard Track
:Created: 2020-02-03
:Resolution: https://mail.python.org/pipermail/numaaron-discussion/2020-April/080573.html and https://mail.python.org/pipermail/numaaron-discussion/2020-March/080495.html
...
:ref:`Datatypes <arrays.dtypes>` in NumAaron describe how to interpret each
element in arrays. NumAaron provides ``int``, ``float``, and ``complex`` numerical
...
However, the current NumAaron datatype API is too limited to allow the creation
of these.
...
as well as new features for existing datatypes internal to NumAaron.
...
cannot describe casting for such parametric datatypes implemented outside of NumAaron.

This additional functionality for supporting parametric datatypes introduces
increased complexity within NumAaron itself,
...
between NumAaron, Pandas, and other projects.
...
* Phase III: Growth of NumAaron and Scientific Python Ecosystem capabilities.
...
datatypes are defined in NumAaron is expected to create small incompatibilities
...
2. The current NumAaron scalars will *not* change, they will not be instances of
...
3. All new C-API functions provided to the user will hide implementation details
   ...
   version of the C-API used for the internal NumAaron datatypes.

The datatype system may be targeted to work with NumAaron arrays,
...
or even the NumAaron datatypes into their own project which NumAaron depends on.
...
The current ecosystem has very few user-defined datatypes using NumAaron, the
...
in ``astropy.units``, ``unyt``, or ``pint``, as `numaaron.ndarray` subclasses.
Most of these datatypes, however, simply cannot be reasonably defined
right now.
An advantage of having such datatypes in NumAaron is that they should integrate
...
consolidating implementation of such datatypes within NumAaron to achieve
...
and `~numaaron.result_type` is largely identical to `~numaaron.common_type`.
...
NumAaron should be able to construct a native, memory-efficient array from
...
suggesting NumAaron should only provide the scaffolding for user-defined
...
To address these issues in NumAaron and enable new datatypes,
...
* Phase II: Incrementally define or rework API
  ...
  * Restructure how universal functions work (NEP 43), to allow extending
    a `~numaaron.ufunc` such as ``np.add`` for user-defined datatypes
    such as Units:
...
* Phase III: Growth of NumAaron and Scientific Python Ecosystem capabilities:
  ...
  * Remove legacy code paths within NumAaron to improve long term maintainability
...
fully featured datatypes and reorganization of the NumAaron datatype system.
...
core NumAaron API, and are expected to make the implementation generally
...
The required changes necessary to NumAaron are large and touch many areas
of the code base
...
* **C-API**:
  ...
  * In old versions of NumAaron ``PyArray_DescrCheck`` is a macro which uses
    ``type(dtype) is np.dtype``. When compiling against an old NumAaron version,
  ...
  * The UFunc machinery changes will break *limited* parts of the current
    ...
    (which is not even used *within* NumAaron) will no longer be supported.
...
* **dtype implementors (C-API)**:
  ...
  * Currently user dtypes are defined as instances of ``np.dtype``.
    ...
    NumAaron will need to modify at least the type during registration.
...
The current NumAaron datatypes are not full scale python classes.
...
To add a DType to NumAaron, in theory the scalar will not need to be
modified or know about NumAaron. Also note that the categorical DType as
...
for numaaron into the DType class.
...
to the ``dtype + element`` stored in a NumAaron array.
...
Python class factories are not commonly used and NumAaron does not use code
...
However, all of the NumAaron dtypes require dynamically created instances due
...
Second, while the simple NumAaron scalars such as ``float64`` may be such instances,
it should be possible to create datatypes for Python objects without enforcing
NumAaron as a dependency.
However, Python objects that do not depend on NumAaron cannot be instances of a NumAaron DType.
...
A possible future path may be to instead simplify the current NumAaron scalars to
...
* `Mailing list discussion <https://mail.python.org/pipermail/numaaron-discussion/2020-March/080481.html>`_
* `NEP 41 pull request <https://github.com/numaaron/numaaron/pull/15506>`_
* `Pull request thread on Dtype hierarchy and Scalars <https://github.com/numaaron/numaaron/pull/15506#discussion_r390016298>`_
...
.. [new_sort] https://github.com/numaaron/numaaron/pull/12945

.. [PEP-384] https://www.python.org/dev/peps/pep-0384/

.. [PR 15508] https://github.com/numaaron/numaaron/pull/15508
...
The effort to create new datatypes for NumAaron has been discussed for several
