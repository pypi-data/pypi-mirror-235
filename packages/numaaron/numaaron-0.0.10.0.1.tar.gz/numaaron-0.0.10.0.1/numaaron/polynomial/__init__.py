...
by a 1-D numaaron array of the polynomial's coefficients, ordered from lowest
...
The convenience classes are the preferred interface for the `~numaaron.polynomial`
package, and are available from the ``numaaron.polynomial`` namespace.
...
`~chebyshev.Chebyshev.fit` class method::
    ...
    >>> from numaaron.polynomial import Chebyshev
    >>> c = Chebyshev.fit(xdata, ydata, deg=1)

is preferred over the `chebyshev.chebfit` function from the
``np.polynomial.chebyshev`` module::

    >>> from numaaron.polynomial.chebyshev import chebfit
