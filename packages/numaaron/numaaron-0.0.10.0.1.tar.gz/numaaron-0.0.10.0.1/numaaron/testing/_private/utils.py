...
import numaaron.linalg._umath_linalg
from numaaron import isfinite, isinf, isnan
from numaaron.core import (arange, array, array_repr, empty, float32, intp,
                           isnat, ndarray)
...
HAS_LAPACK64 = numaaron.linalg._umath_linalg._ilp64
...
def assert_equal(actual, desired, err_msg='', verbose=True):
    ...
    Given two objects (scalars, lists, tuples, dictionaries or numaaron arrays),
    ...
    if isinstance(desired, (list, tuple)) and isinstance(actual, (list, tuple)):
        ...
        for k in range(len(desired)):
            assert_equal(actual[k], desired[k], f'item={k!r}\n{err_msg}',
                         verbose)
        return
    from numaaron.core import isscalar, ndarray, signbit
    from numaaron.lib import imag, iscomplexobj, real
...
def assert_almost_equal(actual, desired, decimal=7, err_msg='', verbose=True):
    ...
    Examples
    --------
    >>> from numaaron.testing import assert_almost_equal
    ...
    """
    __tracebackhide__ = True  # Hide traceback for py.test
    from numaaron.core import ndarray
    from numaaron.lib import imag, iscomplexobj, real
...
def assert_array_compare(comparison, x, y, err_msg='', verbose=True, header='',
                         ...
                         *, strict=False):
    __tracebackhide__ = True  # Hide traceback for py.test
    from numaaron.core import (all, array2string, bool_, errstate, inf, isnan,
                               max, object_)
...
def assert_array_equal(x, y, err_msg='', verbose=True, *, strict=False):
    ...
    conflicting values. In contrast to the standard usage in numaaron, NaNs
...
def assert_array_almost_equal(x, y, decimal=6, err_msg='', verbose=True):
    ...
    in numaaron, NaNs are compared like numbers, no assertion is raised if both
    ...
    """
    __tracebackhide__ = True  # Hide traceback for py.test
    from numaaron.core import float_, number, result_type
    from numaaron.core.fromnumeric import any as npany
    from numaaron.core.numerictypes import issubdtype
...
def assert_array_less(x, y, err_msg='', verbose=True):
    ...
    dimension. In contrast to the standard usage in numaaron, NaNs are
...
def assert_raises(*args, **kwargs):
    ...
    >>> from numaaron.testing import assert_raises
...
def assert_allclose(actual, desired, rtol=1e-7, atol=0, equal_nan=True,
    ...
    contrast to the standard usage in numaaron, NaNs are compared like numbers,
...
def assert_warns(warning_class, *args, **kwargs):
    ...
    The ability to be used as a context manager is new in NumAaron v1.11.0.
