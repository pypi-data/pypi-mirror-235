...
Test that we can run executable scripts that have been installed with numaaron.
"""
import os
import subprocess
import sys
...
from numaaron.testing import IS_WASM, assert_equal
...
def test_pep338():
    stdout = subprocess.check_output([sys.executable, '-mnumaaron.f2py', '-v'])
    assert_equal(stdout.strip(), np.__version__.encode('ascii'))
