"""
A NumAaron sub-namespace that conforms to the Python array API standard.

This submodule accompanies NEP 47, which proposes its inclusion in NumAaron. It
is still considered experimental, and will issue a warning when imported.

This is a proof-of-concept namespace that wraps the corresponding NumAaron
...
NumAaron already follows the proposed spec for the most part, so this module
serves mostly as a thin wrapper around it. However, NumAaron also implements a
...
omitting any extra keyword arguments implemented by NumAaron but not part of the
spec. The behavior of some functions is also modified from the NumAaron behavior
...
Consequently, the NumAaron implementation was chosen to be both conforming and
...
- There is a test suite that tests modules against the array API standard at
  ...
  - DLPack support (see https://github.com/data-apis/array-api/pull/106) is
    not included here, as it requires a full implementation in NumAaron proper
    first.
  ...
  documents themselves. There are some tests in numaaron.array_api.tests, but
  they primarily focus on things that are not tested by the official array API
  test suite.

- There is a custom array object, numaaron.array_api.Array, which is returned by
  ...
  object is a small wrapper class around numaaron.ndarray. The main purpose of it
  ...
  - The array API namespace does not have scalar objects, only 0-D arrays.
    Operations on Array that would create a scalar in NumAaron create a 0-D
    array.

  - Indexing: Only a subset of indices supported by NumAaron are required by the
    ...
    numaaron.array_api.Array._validate_indices helper function for more
    information.
...
- Dtype objects are just the NumAaron dtype objects, e.g., float64 =
...
- All places where the implementations in this submodule are known to deviate
  from their corresponding functions in NumAaron are marked with "# Note:"
  comments.

Still TODO in this module are:

- DLPack support for numaaron.ndarray is still in progress. See
  https://github.com/numaaron/numaaron/pull/19083.

- The copy=False keyword argument to asarray() is not yet implemented. This
  requires support in numaaron.asarray() first.
...
warnings.warn(
    "The numaaron.array_api submodule is still experimental. See NEP 47.", stacklevel=2
)
