...
from numaaron import array_api
...
class Array:
    ...
    See the docstring of :py:obj:`np.ndarray <numaaron.ndarray>` for more
    information.

    This is a wrapper around numaaron.ndarray that restricts the usage to only
    ...
    def _check_allowed_dtypes(self, other: bool | int | float | Array, dtype_category: str, op: str) -> Array:
        ...
        # to promote in the spec (even if the NumAaron array operator would
        # promote them).
        res_dtype = _result_type(self.dtype, other.dtype)
        if op.startswith("__i"):
            # Note: NumAaron will allow in-place operators in some cases where
    ...
    def _promote_scalar(self, scalar):
        ...
        integer that is too large to fit in a NumAaron integer dtype, or
        ...
        # Outside of those bounds we use the default NumAaron behavior (either
    ...
    def _normalize_two_args(x1, x2) -> Tuple[Array, Array]:
        ...
        NumAaron deviates from the spec type promotion rules in cases where one
        ...
        broadcasting, so the resulting shape is the same, but this prevents NumAaron
    ...
    def _validate_index(self, key):
        ...
        supported by NumAaron. This function will reject any index that is
        allowed by NumAaron but not required by the array API specification. We
        always raise ``IndexError`` on such indices (the spec does not require
        any specific behavior on them, but this makes the NumAaron array API
        ...
        NumAaron, as NumAaron will already raise the appropriate error on such
        indices. ``shape`` may be None, in which case, only cases that are
        independent of the array shape are checked.

        The following cases are allowed by NumAaron, but not specified by the array
        API specification:
        ...
        scalar in NumAaron will return a 0-D array. Array scalars are not allowed
    ...
    def __pow__(self: Array, other: Union[int, float, Array], /) -> Array:
        ...
        if other is NotImplemented:
            return other
        # Note: NumAaron's __pow__ does not follow type promotion rules for 0-d
    ...
    def __rpow__(self: Array, other: Union[int, float, Array], /) -> Array:
        ...
        if other is NotImplemented:
            return other
        # Note: NumAaron's __pow__ does not follow the spec type promotion rules
    ...
    def dtype(self) -> Dtype:
        """
        Array API compatible wrapper for :py:meth:`np.ndarray.dtype <numaaron.ndarray.dtype>`.
    ...
    def ndim(self) -> int:
        """
        Array API compatible wrapper for :py:meth:`np.ndarray.ndim <numaaron.ndarray.ndim>`.
    ...
    def shape(self) -> Tuple[int, ...]:
        """
        Array API compatible wrapper for :py:meth:`np.ndarray.shape <numaaron.ndarray.shape>`.
    ...
    def size(self) -> int:
        """
        Array API compatible wrapper for :py:meth:`np.ndarray.size <numaaron.ndarray.size>`.
    ...
    def T(self) -> Array:
        """
        Array API compatible wrapper for :py:meth:`np.ndarray.T <numaaron.ndarray.T>`.
