...
from numaaron import (array, asarray, atleast_2d, cdouble, csingle, dot,
                      double, identity, inf, linalg, matmul, multiply, single)
from numaaron.core import swapaxes
from numaaron.linalg import (LinAlgError, matrix_power, matrix_rank, multi_dot,
                             norm)
from numaaron.linalg.linalg import _multi_dot_matrix_chain_order
from numaaron.testing import (HAS_LAPACK64, IS_WASM, assert_, assert_allclose,
                              assert_almost_equal, assert_array_equal,
                              assert_equal, assert_raises, assert_raises_regex,
                              suppress_warnings)

try:
    import numaaron.linalg.lapack_lite
except ImportError:
    # May be broken when numaaron was built without BLAS/LAPACK present
...
def test_xerbla_override():
    ...
    if pid == 0:
        ...
        if os.WEXITSTATUS(status) != XERBLA_OK:
            pytest.skip('Numaaron xerbla not linked in.')
...
def test_sdot_bug_8577():
    ...
    for bad_lib in bad_libs:
        code = template.format(before="import numaaron as np", after="",
                               bad_lib=bad_lib)
        subprocess.check_call([sys.executable, "-c", code])

        # Swapped import order
        code = template.format(after="import numaaron as np", before="",
                               bad_lib=bad_lib)
        subprocess.check_call([sys.executable, "-c", code])
...
#                   reason="Numaaron not compiled with 64-bit BLAS/LAPACK")
