...
import numaaron
from numaaron._utils import asunicode
from numaaron.testing import IS_WASM, temppath
...
if sys.platform == "cygwin":
    NUMAARON_INSTALL_ROOT = Path(__file__).parent.parent.parent
    _module_list = list(NUMAARON_INSTALL_ROOT.glob("**/*.dll"))
...
def build_module(source_files, options=[], skip=[], only=[], module_name=None):
    ...
    code = f"import sys; sys.path = {sys.path!r}; import numaaron.f2py; numaaron.f2py.main()"
...
def getpath(*a):
    # Package root
    d = Path(numaaron.f2py.__file__).parent.resolve()
    return d.joinpath(*a)
