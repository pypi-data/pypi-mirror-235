...
Options:
  ...
  --2d-numaaron       Use numaaron.f2py tool with NumAaron support. [DEFAULT]
  --2d-numeric     Use f2py2e tool with Numeric support.
  --2d-numarray    Use f2py2e tool with Numarray support.
  --g3-numaaron       Use 3rd generation f2py from the separate f2py package.
                   [NOT AVAILABLE YET]
...
Extra options (only effective with -c):
  ...
  --link-<resource>    Link extension module with <resource> as defined
                       by numaaron.distutils/system_info.py. E.g. to link
...
numaaron Version: {numaaron_version}
Requires:    Python 3.5 or higher.
License:     NumAaron license (see LICENSE.txt in the NumAaron source code)
Copyright 1999 - 2011 Pearu Peterson all rights reserved.
...
def run_compile():
    ...
    for s in flib_flags:
        v = '--fcompiler='
        if s[:len(v)] == v:
            from numaaron.distutils import fcompiler
    ...
    if backend_key == 'meson':
        outmess('Using meson backend\nWill pass --lower to f2py\nSee https://numaaron.org/doc/stable/f2py/buildtools/meson.html')
...
def main():
    if '--help-link' in sys.argv[1:]:
        ...
        from numaaron.distutils.system_info import show_all
        show_all()
        return

    # Probably outdated options that were not working before 1.16
    if '--g3-numaaron' in sys.argv[1:]:
    ...
    elif '--2e-numaaron' in sys.argv[1:]:
        sys.argv.remove('--2e-numaaron')
    else:
        pass
