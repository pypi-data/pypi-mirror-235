...
from numaaron.compat import os_fspath
...
class record(nt.void):
    ...
    # as numaaron.record when printed
    __name__ = 'record'
    __module__ = 'numaaron'
...
class recarray(ndarray):
    ...
    # up as "numaaron.recarray" when printed
    __name__ = 'recarray'
    __module__ = 'numaaron'
    ...
    def __getattribute__(self, attr):
        ...
        # numaaron.record type if present), since nested structured fields do not
    ...
    def __repr__(self):
        ...
        if self.dtype.type is record or not issubclass(self.dtype.type, nt.void):
            # If this is a full record array (has numaaron.record dtype),
            # or if it has a scalar (non-void) dtype with no records,
            # represent it using the rec.array function. Since rec.array
            # converts dtype to a numaaron.record for us, convert back
            ...
            prefix = "array("
            fmt = 'array(%s,%sdtype=%s).view(numaaron.recarray)'
...
def fromarrays(arrayList, dtype=None, shape=None, formats=None,
    ...
    formats, names, titles, aligned, byteorder :
        ...
        `numaaron.format_parser` to construct a dtype. See that function for
...
def array(obj, dtype=None, shape=None, offset=0, strides=None, formats=None,
    ...
    Notes
    -----
    If `obj` is ``None``, then call the `~numaaron.recarray` constructor. If
    `obj` is a string, then call the `fromstring` constructor. If `obj` is a
    list or a tuple, then if the first object is an `~numaaron.ndarray`, call
    `fromarrays`, otherwise call `fromrecords`. If `obj` is a
    `~numaaron.recarray`, then make a copy of the data in the recarray
