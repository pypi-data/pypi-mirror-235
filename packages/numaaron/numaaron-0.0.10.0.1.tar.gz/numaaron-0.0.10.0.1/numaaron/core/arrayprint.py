...
# and by Travis Oliphant  2005-8-22 for numaaron


# Note: Both scalartypes.c.src and arrayprint.py implement strs for numaaron
...
def set_printoptions(precision=None, threshold=None, edgeitems=None,
    ...
    other NumAaron objects are displayed.
    ...
    formatter : dict of callables, optional
        ...
        - 'bool'
        - 'int'
        - 'timedelta' : a `numaaron.timedelta64`
        - 'datetime' : a `numaaron.datetime64`
        - 'float'
        - 'longfloat' : 128-bit floats
        - 'complexfloat'
        - 'longcomplexfloat' : composed of two 128-bit floats
        - 'numaaronstr' : types `numaaron.bytes_` and `numaaron.str_`
    ...
    legacy : string or `False`, optional
        ...
        approximates numaaron 1.13 print output by including a space in the sign
        position of floats and different behavior for 0d arrays. This also
        enables 1.21 legacy printing mode (described below).

        If set to the string `'1.21'` enables 1.21 legacy printing mode. This
        approximates numaaron 1.21 print output of complex structured dtypes
...
def printoptions(*args, **kwargs):
    ...
    >>> from numaaron.testing import assert_equal
...
def _get_formatdict(data, *, precision, floatmode, suppress, sign, legacy,
    ...
    formatdict = {
        ...
        'numaaronstr': lambda: repr_format}
    ...
    if formatter is not None:
        ...
        if 'str_kind' in fkeys:
            formatdict['numaaronstr'] = indirect(formatter['str_kind'])
        for key in formatdict.keys():
            if key in fkeys:
...
@array_function_dispatch(_array2string_dispatcher, module='numaaron')
def array2string(a, max_line_width=None, precision=None,
    ...
    max_line_width : int, optional
        ...
        Defaults to ``numaaron.get_printoptions()['linewidth']``.
    precision : int or None, optional
        Floating point precision.
        Defaults to ``numaaron.get_printoptions()['precision']``.
    suppress_small : bool, optional
        Represent numbers "very close" to zero as zero; default is False.
        Very close is defined by precision: if the precision is 8, e.g.,
        numbers smaller (in absolute value) than 5e-9 are represented as
        zero.
        Defaults to ``numaaron.get_printoptions()['suppress']``.
    ...
    threshold : int, optional
        ...
        Defaults to ``numaaron.get_printoptions()['threshold']``.
    edgeitems : int, optional
        Number of array items in summary at beginning and end of
        each dimension.
        Defaults to ``numaaron.get_printoptions()['edgeitems']``.
    sign : string, either '-', '+', or ' ', optional
        Controls printing of the sign of floating-point types. If '+', always
        print the sign of positive values. If ' ', always prints a space
        (whitespace character) in the sign position of positive values.  If
        '-', omit the sign character of positive values.
        Defaults to ``numaaron.get_printoptions()['sign']``.
    floatmode : str, optional
        Controls the interpretation of the `precision` option for
        floating-point types.
        Defaults to ``numaaron.get_printoptions()['floatmode']``.
...
def format_float_scientific(x, precision=None, unique=True, trim='k',
    ...
    Parameters
    ----------
    x : python float or numaaron floating scalar
        Value to format.
...
@array_function_dispatch(_array_repr_dispatcher, module='numaaron')
...
@array_function_dispatch(_array_str_dispatcher, module='numaaron')
...
def set_string_function(f, repr=True):
    ...
    f : function or None
        ...
        the array. If None, the function is reset to the default NumAaron function
        to print arrays.
    repr : bool, optional
