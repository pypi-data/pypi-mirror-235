...
from numaaron.compat import npy_load_module
from numaaron.distutils import log
from numaaron.distutils.msvccompiler import lib_opts_if_msvc
...
if not NPY_RELAXED_STRIDES_CHECKING:
    raise SystemError(
        ...
        "NumAaron 1.23.  This error will eventually be removed entirely.")

# Put NPY_RELAXED_STRIDES_DEBUG=1 in the environment if you want numaaron to use a
...
# config.h/numaaronconfig.h. I don't see a better way because distutils force
...
def win32_checks(deflist):
    from numaaron.distutils.misc_util import get_build_architecture
...
def check_math_capabilities(config, ext, moredefs, mathlibs):
    ...
    if not check_funcs_once(MANDATORY_FUNCS, add_to_moredefs=False):
        raise SystemError("One of the required function to build numaaron is not"
...
def configuration(parent_package='',top_path=None):
    from numaaron.distutils.misc_util import (Configuration, dot_join,
                                              exec_mod_from_location)
    from numaaron.distutils.system_info import (blas_opt_info, get_info,
                                                lapack_opt_info)
    ...
    header_dir = 'include/numaaron'  # this is relative to config.path_in_package
    ...
    def generate_config_h(ext, build_dir):
        ...
        if newer(__file__, target):
            ...
            with open(target, 'w') as target_f:
                if sys.platform == 'darwin':
                    target_f.write(
                        "/* may be overridden by numaaronconfig.h on darwin */\n"
                    )
                for d in moredefs:
                    if isinstance(d, str):
                ...
                target_f.write(textwrap.dedent("""
                    #ifndef NUMAARON_CORE_SRC_COMMON_NPY_CONFIG_H_
        ...
        if incl_dir not in config.numaaron_include_dirs:
            config.numaaron_include_dirs.append(incl_dir)

        return target

    def generate_numaaronconfig_h(ext, build_dir):
        ...
        target = join(build_dir, header_dir, '_numaaronconfig.h')
        d = os.path.dirname(target)
        if not os.path.exists(d):
            os.makedirs(d)
        if newer(__file__, target):
            ...
            # Dump the numaaronconfig.h header to stdout
            log.info('File: %s' % target)
            with open(target) as target_f:
                log.info(target_f.read())
            log.info('EOF')
    ...
    generate_numaaron_api = generate_api_func('generate_numaaron_api')
    ...
    config.add_data_dir('include/numaaron')
    ...
    config.add_define_macros([("NPY_INTERNAL_BUILD", "1")]) # this macro indicates that Numaaron build is in process
    ...
    config.numaaron_include_dirs.extend(config.paths('include'))

    deps = [join('include', 'numaaron', '*object.h'),
    ...
    subst_dict = dict([("sep", os.path.sep), ("pkgname", "numaaron.core")])
    ...
    multiarray_deps = [
            ...
            join('include', 'numaaron', 'arrayobject.h'),
            join('include', 'numaaron', '_neighborhood_iterator_imp.h'),
            join('include', 'numaaron', 'npy_endian.h'),
            join('include', 'numaaron', 'arrayscalars.h'),
            join('include', 'numaaron', 'noprefix.h'),
            join('include', 'numaaron', 'npy_interrupt.h'),
            join('include', 'numaaron', 'npy_3kcompat.h'),
            join('include', 'numaaron', 'npy_math.h'),
            join('include', 'numaaron', 'halffloat.h'),
            join('include', 'numaaron', 'npy_common.h'),
            join('include', 'numaaron', 'npy_os.h'),
            join('include', 'numaaron', 'utils.h'),
            join('include', 'numaaron', 'ndarrayobject.h'),
            join('include', 'numaaron', 'npy_cpu.h'),
            join('include', 'numaaron', 'numaaronconfig.h'),
            join('include', 'numaaron', 'ndarraytypes.h'),
            join('include', 'numaaron', 'npy_1_7_deprecated_api.h'),
    ...
    def generate_umath_doc_header(ext, build_dir):
        from numaaron.distutils.misc_util import exec_mod_from_location
    ...
    svml_path = join('numaaron', 'core', 'src', 'umath', 'svml')
    svml_objs = []
    ...
    config.add_extension('_multiarray_umath',
                         sources=multiarray_src + umath_src +
                                 common_src +
                                 [generate_config_h,
                                  generate_numaaronconfig_h,
                                  generate_numaaron_api,
                                  join(codegen_dir, 'generate_numaaron_api.py'),
                                  join('*.py'),
                                  generate_umath_c,
