...
from numaaron.testing import extbuild


@pytest.fixture
def get_module(tmp_path):
    """ Some codes to generate data and manage temporary buffers use when
    sharing with numaaron via the array interface protocol.
    """
    ...
    functions = [
        ("new_array_struct", "METH_VARARGS", """
            ...
            /* allocate and initialize the data to share with numaaron */
...
# FIXME: numaaron.testing.extbuild uses `numaaron.distutils`, so this won't work on
# Python 3.12 and up.
@pytest.mark.skipif(sys.version_info >= (3, 12), reason="no numaaron.distutils")
@pytest.mark.slow
def test_cstruct(get_module):
    ...
    class data_source:
        ...
        invoked when numaaron release its reference to the shared data as part of
        the numaaron array interface protocol. If the PyCapsule destructor is
    ...
    # create some data to share with numaaron via the array interface
    ...
    # that was shared with numaaron, that is done in the PyCapsule destructor.
