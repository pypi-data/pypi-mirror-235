...
import numaaron.core._operand_flag_tests as opflag_tests
import numaaron.core._rational_tests as _rational_tests
import numaaron.core._umath_tests as umt
import numaaron.linalg._umath_linalg as uml
from numaaron.compat import pickle
from numaaron.testing import (HAS_REFCOUNT, IS_PYPY, IS_WASM, assert_,
...
class TestUfunc:
    def test_pickle(self):
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            ...
            # Check that ufunc not defined in the top level numaaron namespace
            # such as numaaron.core._rational_tests.test_add can also be pickled
            res = pickle.loads(pickle.dumps(_rational_tests.test_add,
                                            protocol=proto))
            assert_(res is _rational_tests.test_add)

    def test_pickle_withstring(self):
        astring = (b"cnumaaron.core\n_ufunc_reconstruct\np0\n"
                   b"(S'numaaron.core.umath'\np1\nS'cos'\np2\ntp3\nRp4\n.")
    ...
    # from include/numaaron/ufuncobject.h
    size_inferred = 2
    can_ignore = 4
    ...
    def test_signature_dtype_type(self):
        # Since that will be the normal behaviour (past NumAaron 1.21)
    ...
    def test_safe_casting(self):
        # In old versions of numaaron, in-place operations used the 'unsafe'
    ...
    def test_ufunc_custom_out(self):
        ...
        # Output must be specified so numaaron knows what
    ...
    def test_struct_ufunc(self):
        import numaaron.core._struct_ufunc_tests as struct_ufunc
    ...
    def test_structured_equal(self):
        # https://github.com/numaaron/numaaron/issues/4855
    ...
    def test_logical_ufuncs_reject_string(self, ufunc):
        ...
        string to bool casts are well defined in NumAaron.
        """
...
class TestLowlevelAPIAccess:
    ...
    def test_weird_dtypes(self):
        S0 = np.dtype("S0")
        # S0 is often converted by NumAaron to S1, but not here:
    ...
    def test_loop_access(self):
        ...
        call_info = ct.pythonapi.PyCapsule_GetPointer(
                ...
                ct.c_char_p(b"numaaron_1.24_ufunc_call_info"))
