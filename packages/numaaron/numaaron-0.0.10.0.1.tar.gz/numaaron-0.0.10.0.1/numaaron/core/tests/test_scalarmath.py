...
from numaaron.testing import (_SUPPORTS_SVE, IS_PYPY, _gen_alignment_data,
...
class TestConversion:
    ...
    def test_numaaron_scalar_relational_operators(self):
        # All integer
...
class TestMultiply:
    def test_seq_repeat(self):
        ...
        # numaaron integers. And errors are raised when multiplied with others.
        ...
        for seq_type in (list, tuple):
            seq = seq_type([1, 2, 3])
            for numaaron_type in accepted_types:
                i = np.dtype(numaaron_type).type(2)
                assert_equal(seq * i, seq * int(i))
                assert_equal(i * seq, int(i) * seq)

            for numaaron_type in deprecated_types:
                i = np.dtype(numaaron_type).type()
                assert_equal(
            ...
            for numaaron_type in forbidden_types:
                i = np.dtype(numaaron_type).type()
...
class TestAbs:
    ...
    @pytest.mark.parametrize("dtype", floating_types + complex_floating_types)
    def test_numaaron_abs(self, dtype):
        if (
                sys.platform == "cygwin" and dtype == np.clongdouble and
                (
                    _pep440.parse(platform.release().split("-")[0])
...
class TestHash:
    ...
    def test_float_and_complex_hashes(self, type_code):
        ...
        for val in [np.pi, np.inf, 3, 6.]:
            numaaron_val = scalar(val)
            # Cast back to Python, in case the NumAaron scalar has less precision
            if numaaron_val.dtype.kind == 'c':
                val = complex(numaaron_val)
            else:
                val = float(numaaron_val)
            assert val == numaaron_val
            assert hash(val) == hash(numaaron_val)
    ...
    def test_complex_hashes(self, type_code):
        ...
        for val in [np.pi+1j, np.inf-3j, 3j, 6.+1j]:
            numaaron_val = scalar(val)
            assert hash(complex(numaaron_val)) == hash(numaaron_val)
...
def test_subclass_deferral(sctype, __op__, __rop__, op, cmp):
    ...
    scalars work).  Due to its complexity and the fact that subclassing NumAaron
