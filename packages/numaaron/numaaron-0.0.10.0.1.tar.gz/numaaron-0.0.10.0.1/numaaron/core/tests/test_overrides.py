...
from numaaron.core.overrides import (_get_implementing_args,
...
class TestNumAaronFunctions:

    def test_set_module(self):
        assert_equal(np.sum.__module__, 'numaaron')
        assert_equal(np.char.equal.__module__, 'numaaron.char')
        assert_equal(np.fft.fft.__module__, 'numaaron.fft')
        assert_equal(np.linalg.solve.__module__, 'numaaron.linalg')
...
class TestArrayLike:
    ...
    @pytest.mark.parametrize('numaaron_ref', [True, False])
    def test_array_like(self, function, args, kwargs, numaaron_ref):
        self.add_method('array', self.MyArray)
        self.add_method(function, self.MyArray)
        np_func = getattr(np, function)
        my_func = getattr(self.MyArray, function)

        if numaaron_ref is True:
            ref = np.array(1)
        else:
            ref = self.MyArray.array()
    ...
    @pytest.mark.parametrize('numaaron_ref', [True, False])
    def test_array_like_fromfile(self, numaaron_ref):
        self.add_method('array', self.MyArray)
...
def test_scipy_trapz_support_shim():
    ...
    # Hopefully removable 1 year after SciPy 1.11; shim added to NumAaron 1.25.
    import functools
    import types
