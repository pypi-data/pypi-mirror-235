...
from numaaron.testing import IS_WASM, assert_warns, extbuild
...
def test_switch_owner(get_module, policy):
    ...
    if policy is None:
        ...
        policy = os.getenv("NUMAARON_WARN_IF_NO_MEM_POLICY", "0") == "1"
        oldval = None
    else:
        policy = policy == "1"
        oldval = np.core._multiarray_umath._set_numaaron_warn_if_no_mem_policy(
            policy)
    try:
        ...
        if oldval is not None:
            np.core._multiarray_umath._set_numaaron_warn_if_no_mem_policy(oldval)
