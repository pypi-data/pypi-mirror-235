...
from numaaron.testing import (IS_WASM, assert_, assert_array_equal,
...
class TestDateTime:
    ...
    def test_timedelta_scalar_construction(self):
        ...
        assert_equal(repr(np.timedelta64(-3, 's')),
                     "numaaron.timedelta64(-3,'s')")
        assert_equal(repr(np.timedelta64(12)),
                     "numaaron.timedelta64(12)")
    ...
    def test_cast_overflow(self):
        # gh-4486
        def cast():
            numaaron.datetime64("1971-01-01 00:00:00.000000000000000").astype("<M8[D]")
        assert_raises(OverflowError, cast)

        def cast2():
            numaaron.datetime64("2014").astype("<M8[fs]")
    ...
    def test_discovery_from_object_array(self, shape):
        ...
        arr[...] = np.bytes_("2020-10-10")  # try a numaaron string type
        assert_equal(arr.astype("M8"), res)
        arr = arr.astype("S")
