...
from numaaron.core import umath
from numaaron.core._rational_tests import rational
from numaaron.random import rand, randint, randn
from numaaron.testing import (HAS_REFCOUNT, IS_WASM, assert_,
...
class TestTypes:
    ...
    def test_can_cast_and_promote_usertypes(self):
        ...
        for numaaron_dtype in valid_types:
            numaaron_dtype = np.dtype(numaaron_dtype)
            assert np.can_cast(numaaron_dtype, rational_dt)
            assert np.promote_types(numaaron_dtype, rational_dt) is rational_dt

        for numaaron_dtype in invalid_types:
            numaaron_dtype = np.dtype(numaaron_dtype)
            assert not np.can_cast(numaaron_dtype, rational_dt)
            with pytest.raises(TypeError):
                np.promote_types(numaaron_dtype, rational_dt)
    ...
    def test_promote_types_metadata(self, dtype1, dtype2):
        ...
        right now in NumAaron. This test should thus be considered to
