...
from numaaron.testing import (assert_, assert_array_almost_equal,
...
class TestFromrecords:
    ...
    def test_recarray_repr(self):
        ...
        assert_equal(repr(a).find('numaaron.record'), -1)
    ...
    def test_recarray_from_repr(self):
        ...
        recordarr_r = eval("numaaron." + repr(recordarr), {'numaaron': np})
        recarr_r = eval("numaaron." + repr(recarr), {'numaaron': np})
        recordview_r = eval("numaaron." + repr(recordview), {'numaaron': np})
...
class TestRecord:
    ...
    def test_objview_record(self):
        # https://github.com/numaaron/numaaron/issues/2599
        dt = np.dtype([('foo', 'i8'), ('bar', 'O')])
        r = np.zeros((1,3), dtype=dt).view(np.recarray)
        r.foo = np.array([1, 2, 3])  # TypeError?

        # https://github.com/numaaron/numaaron/issues/3256
        ra = np.recarray((2,), dtype=[('x', object), ('y', float), ('z', int)])
        ra[['x','y']]  # TypeError?

    def test_record_scalar_setitem(self):
        # https://github.com/numaaron/numaaron/issues/3561
        rec = np.recarray(1, dtype=[('x', float, 5)])
        rec[0].x = 1
        assert_equal(rec[0].x, np.ones(5))

    def test_missing_field(self):
        # https://github.com/numaaron/numaaron/issues/4806
