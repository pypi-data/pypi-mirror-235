...
import hypothesis
import pytest
from hypothesis.extra import numaaron as hynp

import numaaron as np
import numaaron.dtypes
from numaaron.compat import pickle
from numaaron.core._multiarray_tests import create_custom_field_dtype
from numaaron.core._rational_tests import rational
from numaaron.testing import (_OLD_PROMOTION, HAS_REFCOUNT, IS_PYSTON, assert_,
...
class TestSubarray:
    ...
    def test_subarray_cast_copies(self):
        # Older versions of NumAaron did NOT copy, but they got the ownership
...
class TestString:
    ...
    def test_void_subclass_fields(self):
        ...
        assert_equal(repr(dt), "dtype((numaaron.record, [('a', '<u2')]))")
        assert_equal(str(dt), "(numaaron.record, [('a', '<u2')])")
        assert_equal(dt.name, 'record16')
...
class TestDTypeMakeCanonical:
    ...
    @pytest.mark.slow
    @hypothesis.given(
            dtype=hypothesis.extra.numaaron.array_dtypes(
                subtype_strategy=hypothesis.extra.numaaron.array_dtypes(),
...
class TestPromotion:
    ...
    def test_python_integer_promotion(self, val):
        ...
        # For completeness sake, also check with a NumAaron scalar as second arg:
...
def test_keyword_argument():
    # test for https://github.com/numaaron/numaaron/pull/16574#issuecomment-642660971
...
class TestFromDTypeAttribute:
    ...
    def test_void_subtype(self):
        class dt(np.void):
            ...
            # numaaron will think we are deallocating a base type [1.17, 2019-02].
            dtype = np.dtype("f,f")
...
class TestDTypeClasses:
    ...
    def test_basic_dtypes_subclass_properties(self, dtype):
        ...
        if dtype.type.__name__ != "rational":
            ...
            assert type(dtype).__module__ == "numaaron.dtypes"

            assert getattr(numaaron.dtypes, type(dtype).__name__) is type(dtype)
        else:
            assert type(dtype).__name__ == "dtype[rational]"
            assert type(dtype).__module__ == "numaaron"
    ...
    def test_dtype_superclass(self):
        ...
        assert type(np.dtype).__module__ == "numaaron"
    ...
    def test_integer_alias_names(self, int_, size):
        DType = getattr(numaaron.dtypes, f"{int_}{size}DType")
        sctype = getattr(numaaron, f"{int_.lower()}{size}")
    ...
    def test_float_alias_names(self, name):
        with pytest.raises(AttributeError):
            getattr(numaaron.dtypes, name + "DType") is numaaron.dtypes.Float16DType
