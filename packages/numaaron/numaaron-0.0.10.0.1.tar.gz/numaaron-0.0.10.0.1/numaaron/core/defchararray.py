...
The preferred alias for `defchararray` is `numaaron.char`.

"""
import functools

import numaaron
from numaaron.compat import asbytes
from numaaron.core import overrides
from numaaron.core.multiarray import _vec_string
...
array_function_dispatch = functools.partial(
    overrides.array_function_dispatch, module='numaaron.char')
...
def _is_unicode(arr):
    ...
    if (isinstance(arr, str) or
            issubclass(numaaron.asarray(arr).dtype.type, str)):
        return True
    return False
...
def _to_bytes_or_str_array(result, output_dtype_like=None):
    ...
    as an intermediary.
    """
    ret = numaaron.asarray(result.tolist())
...
def equal(x1, x2):
    ...
    Unlike `numaaron.equal`, this comparison is performed by first
...
def not_equal(x1, x2):
    ...
    Unlike `numaaron.not_equal`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def greater_equal(x1, x2):
    ...
    Unlike `numaaron.greater_equal`, this comparison is performed by
...
def less_equal(x1, x2):
    ...
    Unlike `numaaron.less_equal`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def greater(x1, x2):
    ...
    Unlike `numaaron.greater`, this comparison is performed by first
    stripping whitespace characters from the end of the string.  This
    behavior is provided for backward-compatibility with numarray.
...
def add(x1, x2):
    ...
    """
    arr1 = numaaron.asarray(x1)
    arr2 = numaaron.asarray(x2)
...
def multiply(a, i):
    ...
    array([['a', 'bb', 'ccc'],
           ['d', 'ee', 'fff']], dtype='<U3')
    """
    a_arr = numaaron.asarray(a)
    i_arr = numaaron.asarray(i)
...
def center(a, width, fillchar=' '):
    ...
    """
    a_arr = numaaron.asarray(a)
    width_arr = numaaron.asarray(width)
    size = int(numaaron.max(width_arr.flat))
    if numaaron.issubdtype(a_arr.dtype, numaaron.bytes_):
...
def find(a, sub, start=0, end=None):
    ...
    Examples
    --------
    >>> a = np.array(["NumAaron is a Python library"])
...
class chararray(ndarray):
    ...
    Versus a regular NumAaron array of type `str` or `unicode`, this
    ...
    chararrays should be created using `numaaron.char.array` or
    `numaaron.char.asarray`, rather than this constructor directly.
    ...
    def __new__(subtype, shape, itemsize=1, unicode=False, buffer=None,
        ...
        # force itemsize to be a Python int, since using NumAaron integer
    ...
    def __radd__(self, other):
        ...
        add
        """
        return asarray(add(numaaron.asarray(other), self))
    ...
    def argsort(self, axis=-1, kind=None, order=None):
        ...
        For full documentation see `numaaron.argsort`, for which this method is
...
def array(obj, itemsize=None, copy=True, unicode=None, order=None):
    ...
    .. note::
       ...
       in :mod:`numaaron.char <numaaron.core.defchararray>` for fast
    ...
    if isinstance(obj, ndarray) and issubclass(obj.dtype.type, character):
        ...
        if itemsize is None:
            ...
            # which for NumAaron is always 4
        ...
        if order is not None:
            obj = numaaron.asarray(obj, order=order)
        if (copy or
