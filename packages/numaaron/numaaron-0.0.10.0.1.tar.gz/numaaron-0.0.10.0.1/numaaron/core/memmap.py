...
from numaaron.compat import is_pathlib_path, os_fspath
...
class memmap(ndarray):
    ...
    on disk, without reading the entire file into memory.  NumAaron's
    ...
    Given a memmap ``fp``, ``isinstance(fp, numaaron.ndarray)`` returns
    ``True``.
    ...
    def __new__(subtype, filename, dtype=uint8, mode='r+', offset=0,
                shape=None, order='C'):
        # Import here to minimize 'import numaaron' overhead
        import mmap
        import os.path
        try:
