...
# Note that the name numaaron.core._internal._reconstruct is embedded in
# pickles of ndarrays made with NumAaron before release 1.0
...
class _ctypes:
    ...
    def shape(self):
        ...
        platform (see `~numaaron.ctypeslib.c_intp`). This base-type could be
    ...
    while stream:
        ...
        if stream.consume('T{'):
            ...
            if dtypechar in 'USV':
                ...
                itemsize = 1
            numaaron_byteorder = {'@': '=', '^': '='}.get(
                stream.byteorder, stream.byteorder)
            value = dtype(numaaron_byteorder + dtypechar)
