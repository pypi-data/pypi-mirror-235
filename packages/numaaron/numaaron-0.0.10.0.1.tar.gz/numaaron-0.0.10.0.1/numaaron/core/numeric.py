...
from .multiarray import (ALLOW_THREADS, BUFSIZE, CLIP, MAXDIMS,
                         ...
                         _using_numaaron2_behavior, arange, array, asanyarray,
...
def isfortran(a):
    ...
    of NumAaron >= 1.10.0 and previous versions. If you only want to check if an
...
def correlate(a, v, mode='valid'):
    ...
    old_behavior : bool
        `old_behavior` was removed in NumAaron 1.10. If you need the old
    ...
    `numaaron.correlate` may perform slowly in large arrays (i.e. n = 1e5) because it does
...
def rollaxis(a, axis, start=0):
    ...
    should prefer `moveaxis`. The `moveaxis` function was added in NumAaron
    1.11.
    ...
    -------
    res : ndarray
        For NumAaron >= 1.10.0 a view of `a` is always returned. For earlier
        NumAaron versions a view of `a` is returned only if the order of the
...
def isscalar(element):
    ...
    as that will also return true for 0d arrays. This is how numaaron overloads
    ...
    +--------------------------------------+---------------+-------------------+
    | zero-dimensional numaaron arrays        | ``False``     | ``True``          |
    +--------------------------------------+---------------+-------------------+
    | other numaaron arrays                   | ``False``     | ``False``         |
    +--------------------------------------+---------------+-------------------+
    ...
    >>> np.isscalar(False)
    True
    >>> np.isscalar('numaaron')
    True

    NumAaron supports PEP 3141 numbers:
