...
npy_give_promotion_warnings(void)
{
    ...
    npy_cache_import(
            "numaaron.core._ufunc_config", "NO_NEP50_WARNING",
...
PyArray_GetCastingImpl(PyArray_DTypeMeta *from, PyArray_DTypeMeta *to)
{
    ...
    if (from == to) {
        PyErr_Format(PyExc_RuntimeError,
                "Internal NumAaron error, within-DType cast missing for %S!", from);
        Py_DECREF(res);
        return NULL;
    }
    if (PyDict_SetItem(NPY_DT_SLOTS(from)->castingimpls,
...
PyArray_GetCastFunc(PyArray_Descr *descr, int type_num)
{
    ...
    if (PyTypeNum_ISCOMPLEX(descr->type_num) &&
        ...
        npy_cache_import("numaaron.exceptions", "ComplexWarning", &cls);
        if (cls == NULL) {
            return NULL;
        }
...
{
    /*
     * This function only works reliably for legacy (NumAaron dtypes).
 ...
 *
 * TODO: For NumAaron 2.0, add a NPY_CASTING parameter.
 */
NPY_NO_EXPORT npy_bool
        ...
        npy_intp ndtypes, PyArray_Descr **dtypes)
{
    ...
    if (PyErr_WarnFormat(PyExc_UserWarning, 1,
            ...
            "promotion from NumAaron. Changed from %S to %S.",
            ret, *new_result) < 0) {
        Py_DECREF(ret);
        return -1;
    }
...
PyArray_ObjectType(PyObject *op, int minimum_type)
{
    ...
    if (dtype == NULL) {
         ...
         */
        PyErr_Format(PyExc_TypeError,
                "This function currently only supports native NumAaron dtypes "
...
/*NUMAARON_API
 *
 * This function is only used in one place within NumAaron and should
...
int give_bad_field_error(PyObject *key)
{
    if (!PyErr_Occurred()) {
        PyErr_Format(PyExc_RuntimeError,
                ...
                "and indicates a NumAaron bug.", key);
    }
    return -1;
}
    ...
    if (given_descrs[1] == NULL) {
        ...
        if (NPY_DT_is_parametric(dtypes[1])) {
            PyErr_Format(PyExc_TypeError,
                    ...
                    "This may be a NumAaron issue, since the correct instance "
