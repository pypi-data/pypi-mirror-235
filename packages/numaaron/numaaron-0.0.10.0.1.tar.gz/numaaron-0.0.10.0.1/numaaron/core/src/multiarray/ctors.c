    ...
    if (!sequence) {
        ...
        for (npy_intp i = 0; i < length; i++) {
            ...
            if (ndim == depth + 1) {
                 ...
                 * support exact NumAaron arrays, but at this point ignore others.
...
raise_memory_error(int nd, npy_intp const *dims, PyArray_Descr *descr)
{
    ...
    npy_cache_import(
        "numaaron.core._exceptions", "_ArrayMemoryError",
        &exc_type);
    if (exc_type == NULL) {
        goto fail;
    }
        ...
        int flags, PyObject *obj, PyObject *base, _NPY_CREATION_FLAGS cflags)
{
    ...
    if (subtype != &PyArray_Type) {
        ...
        if (func == NULL) {
            ...
            Py_DECREF(func);
            /*
             * 2022-01-08, NumAaron 1.23; when deprecation period is over, remove this
            ...
            if (DEPRECATE(
                    ...
                    "in the future. (Deprecated since NumAaron 1.23)") < 0) {
                goto fail;
            }
        }
        else {
...
_array_from_buffer_3118(PyObject *memoryview)
{
    ...
    if (view->suboffsets != NULL) {
        PyErr_SetString(PyExc_BufferError,
                "NumAaron currently does not support importing buffers which "
                "include suboffsets as they are not compatible with the NumAaron"
                "memory layout without a copy.  Consider copying the original "
                "before trying to convert it to a NumAaron array.");
        return NULL;
    }
    ...
    if (descr->elsize != view->itemsize) {
        ...
        if (descr->elsize != view->len) {
            PyErr_SetString(
                    ...
                    "size. This is a bug both in python and numaaron");
            Py_DECREF(descr);
            return NULL;
        }
    }
...
{
    /* Deprecated in NumAaron 1.19, removed in NumAaron 1.20. */
    PyErr_SetString(PyExc_RuntimeError,
...
{
    /*
     * This is the main code to make a NumAaron array from a Python
...
PyArray_FromStructInterface(PyObject *input)
{
     ...
     * destructor will free any resources that need to persist while numaaron has
     * access to the data. */
    PyTuple_SET_ITEM(refs, 1,  attr);

    /* create the numaaron array, this call adds a reference to refs */
 ...
 * @returns NotImplemented if `__array__` is not defined or a NumAaron array
        ...
        PyObject *op, PyArray_Descr *descr, int never_copy)
{
    ...
    if (never_copy) {
        ...
        PyErr_SetString(PyExc_ValueError,
                ...
                "implementing the `__array__` protocol.  NumAaron cannot ensure "
 ...
 *
 * TODO: For NumAaron 2.0, this could accept an order parameter which
...
PyArray_ArangeObj(PyObject *start, PyObject *stop, PyObject *step, PyArray_Descr *dtype)
{
    ...
    if ((dtype != NULL && (dtype->type_num == NPY_DATETIME ||
            ...
            (dtype == NULL && (is_any_numaaron_datetime_or_timedelta(start) ||
                              is_any_numaaron_datetime_or_timedelta(stop) ||
                              is_any_numaaron_datetime_or_timedelta(step)))) {
                   ...
                   npy_intp count, npy_intp offset)
{
     ...
     * NumAaron cannot hold on to the view itself (it is not an object) so it
     ...
     * possible.  (For example, NumAaron arrays will never get wrapped here!)
     */
