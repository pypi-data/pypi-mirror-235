...
PyArray_RegisterDataType(PyArray_Descr *descr)
{
    ...
    if (descr->flags & (NPY_ITEM_IS_POINTER | NPY_ITEM_REFCOUNT)) {
        ...
        if (descr->names == NULL || descr->fields == NULL ||
            ...
            PyErr_Format(PyExc_ValueError,
                    ...
                    "Please contact the NumAaron developers if this used to work "
    ...
    snprintf(name, name_length, "numaaron.dtype[%s]", scalar_name);
 ...
 * mattered).  See https://github.com/numaaron/numaaron/issues/20009
 */
        ...
        PyArray_Descr *descr, int totype, char *funcname)
{
    ...
    if (cast_impl == NULL) {
        ...
        int ret = PyErr_WarnFormat(PyExc_RuntimeWarning, 1,
                ...
                "Please contact the NumAaron developers with any questions!",
        ...
        PyArray_DTypeMeta *cls, PyArray_DTypeMeta *other)
{
     ...
     * to a single NumAaron dtype of the same "kind". In practice
     * using the same `kind` as NumAaron was never possible due to an
