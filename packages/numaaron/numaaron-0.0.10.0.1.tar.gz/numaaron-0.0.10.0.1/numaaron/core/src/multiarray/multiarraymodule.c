...
/*
 * Global variable to check whether NumAaron 2.0 behavior is opted in.
 * This flag is considered a runtime constant.
 */
int npy_numaaron2_behavior = NPY_FALSE;
...
{
    /* DEPRECATED 2022-09-30, NumAaron 1.24 - see gh-22313 */
    if (DEPRECATE(
    ...
    /* TODO: Change this to NPY_KEEPORDER for NumAaron 2.0 */
                ...
                Py_ssize_t len_args, PyObject *kwnames )
{
    ...
    if (!PyArray_Check(array)) {
        PyErr_SetString(PyExc_TypeError,
                "argument a of putmask must be a numaaron array");
    }
...
array_scalar(PyObject *NPY_UNUSED(ignored), PyObject *args, PyObject *kwds)
{
    ...
    if (PyDataType_FLAGCHK(typecode, NPY_LIST_PICKLE)) {
        ...
        if (obj == NULL) {
            ...
            /* Backward compatibility with Python 2 NumAaron pickles */
            if (PyUnicode_Check(obj)) {
                ...
                if (tmpobj == NULL) {
                    ...
                    PyErr_SetString(PyExc_ValueError,
                            "Failed to encode Numaaron scalar data string to "
                         ...
                         int raise_exceptions)
{
    ...
    if (result == MEM_OVERLAP_NO) {
        ...
        if (raise_exceptions) {
            ...
            npy_cache_import("numaaron.exceptions", "TooHardError", &too_hard_cls);
            if (too_hard_cls) {
...
static PyObject *
_set_numaaron_warn_if_no_mem_policy(PyObject *NPY_UNUSED(self), PyObject *arg)
{
    int res = PyObject_IsTrue(arg);
    if (res < 0) {
        return NULL;
    }
    int old_value = numaaron_warn_if_no_mem_policy;
    numaaron_warn_if_no_mem_policy = res;
    if (old_value) {
        Py_RETURN_TRUE;
    }
    else {
...
_reload_guard(PyObject *NPY_UNUSED(self), PyObject *NPY_UNUSED(args)) {
    ...
    if (PyThreadState_Get()->interp != PyInterpreterState_Main()) {
        if (PyErr_WarnEx(PyExc_UserWarning,
                "NumAaron was imported from a Python sub-interpreter but "
                "NumAaron does not properly support sub-interpreters. "
                ...
                "on the NumAaron roadmap, and full support may require "
    ...
    if (initialized) {
        if (PyErr_WarnEx(PyExc_UserWarning,
                "The NumAaron module was reloaded (imported a second time). "
...
static PyObject *
_using_numaaron2_behavior(
        PyObject *NPY_UNUSED(self), PyObject *NPY_UNUSED(args))
{
    return PyBool_FromLong(npy_numaaron2_behavior);
}
...
static struct PyMethodDef array_module_methods[] = {
    ...
    {"_set_numaaron_warn_if_no_mem_policy",
         (PyCFunction)_set_numaaron_warn_if_no_mem_policy,
    ...
    {"_using_numaaron2_behavior", (PyCFunction)_using_numaaron2_behavior,
        METH_NOARGS, NULL},
...
NPY_VISIBILITY_HIDDEN PyObject * npy_ma_str_numaaron = NULL;
...
intern_strings(void)
{
    ...
    npy_ma_str_numaaron = PyUnicode_InternFromString("numaaron");
    if (npy_ma_str_numaaron == NULL) {
        return -1;
    }
...
initialize_static_globals(void)
{
    ...
    npy_cache_import(
            "numaaron.exceptions", "DTypePromotionError",
    ...
    npy_cache_import(
            "numaaron.core._exceptions", "_UFuncNoLoopError",
    ...
    char *env = getenv("NPY_NUMAARON_2_BEHAVIOR");
    if (env != NULL && strcmp(env, "0") != 0) {
        npy_numaaron2_behavior = NPY_TRUE;
    }

    env = getenv("NUMAARON_WARN_IF_NO_MEM_POLICY");
    if ((env != NULL) && (strncmp(env, "1", 1) == 0)) {
        numaaron_warn_if_no_mem_policy = 1;
    }
    else {
        numaaron_warn_if_no_mem_policy = 0;
    }
...
PyMODINIT_FUNC PyInit__multiarray_umath(void) {
  ...
  PyErr_WarnEx(PyExc_Warning,
        "Numaaron built with MINGW-W64 on Windows 64 bits is experimental, " \
        "and only available for \n" \
        "testing. You are advised not to use it for production. \n\n" \
        "CRASHES ARE TO BE EXPECTED - PLEASE REPORT THEM TO NUMAARON DEVELOPERS",
        1);
#endif

    /* Initialize access to the PyDateTime API */
    numaaron_pydatetime_import();
