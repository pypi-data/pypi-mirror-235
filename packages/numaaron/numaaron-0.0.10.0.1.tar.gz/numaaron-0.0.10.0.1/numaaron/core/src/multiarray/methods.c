...
/*
 * Forwards an ndarray method to the function numaaron.core._methods.<name>(...),
...
#define NPY_FORWARD_NDARRAY_METHOD(name) \
        ...
        npy_cache_import("numaaron.core._methods", name, &callable); \
        if (callable == NULL) { \
            return NULL; \
        } \
...
PyArray_GetField(PyArrayObject *self, PyArray_Descr *typed, int offset)
{
    ...
    if (_may_have_objects(PyArray_DESCR(self)) || _may_have_objects(typed)) {
        npy_cache_import("numaaron.core._internal", "_getfield_is_safe",
                         &checkfunc);
        if (checkfunc == NULL) {
            Py_DECREF(typed);
            return NULL;
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (order != NULL) {
        ...
        new_name = PyObject_CallMethod(_numaaron_internal, "_newnames",
                                       "OO", saved, order);
...
array_reduce_ex_picklebuffer(PyArrayObject *self, int protocol)
{
    ...
    numeric_mod = PyImport_ImportModule("numaaron.core.numeric");
    if (numeric_mod == NULL) {
        Py_DECREF(buffer);
        return NULL;
    }
    ...
    */
    ...
    if (version != 1 && version != 0) {
        PyErr_Format(PyExc_ValueError,
                     "can't handle version %d of numaaron.ndarray pickle",
                     version);
        return NULL;
    }
    ...
    if (PyDataType_FLAGCHK(typecode, NPY_LIST_PICKLE)) {
        ...
        if (PyUnicode_Check(rawdata)) {
            ...
            if (tmp == NULL) {
                ...
                PyErr_SetString(PyExc_ValueError,
                                ("Failed to encode latin1 string when unpickling a Numaaron array. "
...
PyArray_Dump(PyObject *self, PyObject *file, int protocol)
{
    ...
    PyObject *ret;
    npy_cache_import("numaaron.core._methods", "_dump", &method);
    if (method == NULL) {
        return -1;
    }
    if (protocol < 0) {
...
{
    static PyObject *method = NULL;
    npy_cache_import("numaaron.core._methods", "_dumps", &method);
    if (method == NULL) {
        return NULL;
    }
    if (protocol < 0) {
