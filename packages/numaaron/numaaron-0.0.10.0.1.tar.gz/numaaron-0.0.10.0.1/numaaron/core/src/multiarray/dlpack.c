...
array_get_dl_device(PyArrayObject *self) {
    ...
    // The outer if is due to the fact that NumAaron arrays are on the CPU
        ...
        PyObject *const *args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    if (stream != Py_None) {
        PyErr_SetString(PyExc_RuntimeError,
                "NumAaron only supports stream=None.");
        return NULL;
    }
     ...
     * Presumably, pytorch will support importing `byte_offset != 0` and NumAaron
     * can choose to use it starting about 2023.  At that point, it may be
     * that NumAaron MUST use `byte_offset` to adhere to the standard (as
    ...
    for (int i = 0; i < ndim; ++i) {
        ...
        // Strides in DLPack are items; in NumAaron are bytes.
...
from_dlpack(PyObject *NPY_UNUSED(self), PyObject *obj) {
    ...
    for (int i = 0; i < ndim; ++i) {
        ...
        // DLPack has elements as stride units, NumAaron has bytes.
