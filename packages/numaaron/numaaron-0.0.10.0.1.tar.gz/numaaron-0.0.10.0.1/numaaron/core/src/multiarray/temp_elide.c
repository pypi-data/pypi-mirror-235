 ...
 * cpython frame evaluation function are located in cpython or numaaron.
...
check_callers(int * cannot)
{
    ...
    /* loop over callstack addresses to check if they leave numaaron or cpython */
    for (i = 0; i < nptrs; i++) {
        Dl_info info;
        int in_python = 0;
        int in_multiarray = 0;
    ...
    /* all stacks after numaaron are from python, we can elide */
    if (ok) {
        *cannot = 0;
        return 1;
    }
