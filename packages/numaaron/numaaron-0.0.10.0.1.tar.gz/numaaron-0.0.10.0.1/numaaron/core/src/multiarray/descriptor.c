...
#include "numaaron/arrayscalars.h"
...
{
    PyObject *_numaaron_dtype_ctypes;
    PyObject *res;
    ...
    _numaaron_dtype_ctypes = PyImport_ImportModule("numaaron.core._dtype_ctypes");
    if (_numaaron_dtype_ctypes == NULL) {
        return NULL;
    }
    res = PyObject_CallMethod(_numaaron_dtype_ctypes, "dtype_from_ctypes_type", "O", (PyObject *)type);
    Py_DECREF(_numaaron_dtype_ctypes);
    if (res == NULL) {
        return NULL;
    }
...
_try_convert_from_dtype_attr(PyObject *obj)
{
    ...
    /* Deprecated 2021-01-05, NumAaron 1.21 */
    if (DEPRECATE("in the future the `.dtype` attribute of a given data"
                  ...
                  "`np.dtype(data_type.dtype)`. (Deprecated NumAaron 1.20)") < 0) {
        Py_DECREF(newdescr);
        return NULL;
    }
 ...
 * dtype names to numaaron scalar types.
 */
    ...
    if (PyDataType_ISUNSIZED(type)) {
        ...
        if (shape.len == 1
            ...
            if (DEPRECATE_FUTUREWARNING(
                        ...
                        "deprecated; in a future version of numaaron, it will be "
...
_convert_from_commastring(PyObject *obj, int align)
{
    PyObject *listobj;
    PyArray_Descr *res;
    PyObject *_numaaron_internal;
    assert(PyUnicode_Check(obj));
    _numaaron_internal = PyImport_ImportModule("numaaron.core._internal");
    if (_numaaron_internal == NULL) {
        return NULL;
    }
    listobj = PyObject_CallMethod(_numaaron_internal, "_commastring", "O", obj);
    Py_DECREF(_numaaron_internal);
    if (listobj == NULL) {
        return NULL;
    }
            ...
            for (j = 0; j < names_size; ++j) {
                if (i != j) {
                    ...
                    if (fld_offset < fld2_offset + fld2_dtype->elsize &&
                        ...
                        PyErr_SetString(PyExc_TypeError,
                                "Cannot create a NumAaron dtype with overlapping "
                                "object fields");
                        return -1;
                    }
                }
...
_convert_from_field_dict(PyObject *obj, int align)
{
    ...
    res = (PyArray_Descr *)PyObject_CallMethod(_numaaron_internal,
...
_convert_from_dict(PyObject *obj, int align)
{
    ...
    if (tmp == NULL) {
        ...
        if (tmp == Py_True) {
            ...
            Py_DECREF(tmp);
            PyErr_SetString(PyExc_ValueError,
                    "NumAaron dtype descriptor includes 'aligned' entry, "
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (offsets) {
            ...
            if (align && offset % newdescr->alignment != 0) {
                PyErr_Format(PyExc_ValueError,
                        "offset %ld for NumAaron dtype with fields is "
    ...
    if (tmp == NULL) {
        ...
        if (itemsize < new->elsize) {
            PyErr_Format(PyExc_ValueError,
                    "NumAaron dtype descriptor requires %d bytes, "
        ...
        if (align && new->alignment > 0 && itemsize % new->alignment != 0) {
            PyErr_Format(PyExc_ValueError,
                    "NumAaron dtype descriptor requires alignment of %d bytes, "
...
_convert_from_type(PyObject *obj) {
    ...
    if (PyType_IsSubtype(typ, &PyGenericArrType_Type)) {
         ...
         * type might override the dtype if numaaron does not otherwise
         * support it.
         */
 ...
 *
 * Many objects can be used to represent a data-type which in NumAaron is
 * quite a flexible concept.
 *
 * This is the central code that converts Python objects to
 * Type-descriptor objects that are used throughout numaaron.
 *
...
arraydescr_name_get(PyArray_Descr *self, void *NPY_UNUSED(ignored))
{
    ...
    PyObject *_numaaron_dtype;
    PyObject *res;
    _numaaron_dtype = PyImport_ImportModule("numaaron.core._dtype");
    if (_numaaron_dtype == NULL) {
        return NULL;
    }
    res = PyObject_CallMethod(_numaaron_dtype, "_name_get", "O", self);
    Py_DECREF(_numaaron_dtype);
    return res;
}
...
arraydescr_protocol_descr_get(PyArray_Descr *self, void *NPY_UNUSED(ignored))
{
    ...
    res = PyObject_CallMethod(_numaaron_internal, "_array_descr", "O", self);
    Py_DECREF(_numaaron_internal);
    return res;
}
        ...
        PyArray_Descr *self, PyObject *val, void *NPY_UNUSED(ignored))
{
     ...
     * FIXME
     *
     * This deprecation has been temporarily removed for the NumAaron 1.7
     ...
     *
     * if (DEPRECATE("Setting NumAaron dtype names is deprecated, the dtype "
...
_get_pickleabletype_from_datetime_metadata(PyArray_Descr *dtype)
{
    ...
    /* Use a 4-tuple that numaaron 1.6 knows how to unpickle */
...
arraydescr_reduce(PyArray_Descr *self, PyObject *NPY_UNUSED(args))
{
    ...
    */
    ...
    mod = PyImport_ImportModule("numaaron.core._multiarray_umath");
    if (mod == NULL) {
        Py_DECREF(ret);
        return NULL;
    }
...
arraydescr_setstate(PyArray_Descr *self, PyObject *args)
{
    ...
    if (version < 0 || version > 4) {
        PyErr_Format(PyExc_ValueError,
                     "can't handle version %d of numaaron.dtype pickle",
                     version);
        return NULL;
    }
    /* Invalidate cached hash value */
    ...
    if (PyUnicode_Check(endian_obj) || PyBytes_Check(endian_obj)) {
        ...
        if (len != 1) {
            PyErr_SetString(PyExc_ValueError,
                            "endian is not 1-char string in Numaaron dtype unpickling");
            Py_XDECREF(tmp);
            return NULL;
        }
        endian = str[0];
        ...
        PyErr_SetString(PyExc_ValueError,
                        "endian is not a string in Numaaron dtype unpickling");
        return NULL;
    }
    ...
    if ((fields == Py_None && names != Py_None) ||
        ...
        PyErr_Format(PyExc_ValueError,
                "inconsistent fields and names in Numaaron dtype unpickling");
        return NULL;
    }

    if (names != Py_None && !PyTuple_Check(names)) {
        PyErr_Format(PyExc_ValueError,
                "non-tuple names in Numaaron dtype unpickling");
        return NULL;
    }

    if (fields != Py_None && !PyDict_Check(fields)) {
        PyErr_Format(PyExc_ValueError,
                "non-dict fields in Numaaron dtype unpickling");
        return NULL;
    }
...
arraydescr_repr(PyArray_Descr *dtype)
{
    ...
    res = PyObject_CallMethod(_numaaron_dtype, "__repr__", "O", dtype);
    Py_DECREF(_numaaron_dtype);
    return res;
}
/*
...
arraydescr_str(PyArray_Descr *dtype)
{
    ...
    res = PyObject_CallMethod(_numaaron_dtype, "__str__", "O", dtype);
    Py_DECREF(_numaaron_dtype);
    return res;
}
