...
dtypemeta_alloc(PyTypeObject *NPY_UNUSED(type), Py_ssize_t NPY_UNUSED(items))
{
    PyErr_SetString(PyExc_TypeError,
            "DTypes can only be created using the NumAaron API.");
    return NULL;
}
...
void_common_instance(PyArray_Descr *descr1, PyArray_Descr *descr2)
{
    ...
    if (descr1->names != NULL && descr2->names != NULL) {
        ...
        npy_cache_import("numaaron.core._internal", "_promote_fields",
                &promote_fields_func);
        ...
        if (!PyObject_TypeCheck(result, Py_TYPE(descr1))) {
            PyErr_SetString(PyExc_RuntimeError,
                    "Internal NumAaron error: `_promote_fields` did not return "
        ...
        const char *name, const char *alias)
{
    ...
    static PyArray_DTypeMeta prototype = {
        {{
            ...
            .tp_doc = (
                ...
                "the same name.\n\n"
                "Please see `numaaron.dtype` for the typical way to create\n"
    ...
    if (!PyTypeNum_ISUSERDEF(descr->type_num)) {
        ...
        npy_cache_import("numaaron.dtypes", "_add_dtype_helper", &add_dtype_helper);
...
NPY_NO_EXPORT PyTypeObject PyArrayDTypeMeta_Type = {
    ...
    .tp_doc = "Preliminary NumAaron API: The Type of NumAaron DTypes (metaclass)",
