...
NPY_NO_EXPORT npy_bool numaaron_warn_if_no_mem_policy = 0;
...
PyArray_SetBaseObject(PyArrayObject *arr, PyObject *obj)
{
    if (obj == NULL) {
        PyErr_SetString(PyExc_ValueError,
                "Cannot set the NumAaron array 'base' "
    ...
    if ((PyObject *)arr == obj) {
        Py_DECREF(obj);
        PyErr_SetString(PyExc_ValueError,
                "Cannot create a circular NumAaron array 'base' dependency");
        return -1;
    }
...
/**
 * Assign an arbitrary object a NumAaron array. This is largely basically
...
array_dealloc(PyArrayObject *self)
{
    ...
    if ((fa->flags & NPY_ARRAY_OWNDATA) && fa->data) {
        ...
        if (fa->mem_handler == NULL) {
            if (numaaron_warn_if_no_mem_policy) {
...
PyArray_DebugPrint(PyArrayObject *obj)
{
    ...
    printf("-------------------------------------------------------\n");
    printf(" Dump of NumAaron ndarray at address %p\n", obj);
    if (obj == NULL) {
        printf(" It's NULL!\n");
        printf("-------------------------------------------------------\n");
        fflush(stdout);
...
{
    const char *msg =
        "Numaaron has detected that you (may be) writing to an array with\n"
    ...
    if (PyArray_HASFIELDS(self) && PyArray_HASFIELDS(other)) {
        ...
        if (field_count != PyTuple_GET_SIZE(other_descr->names)) {
            PyErr_SetString(PyExc_TypeError,
                    ...
                    "fields.  (unreachable error please report to NumAaron devs)");
            return NULL;
        }
        ...
        for (int i = 0; i < field_count; ++i) {
            ...
            if (field_dims_a != field_dims_b || (
                ...
                PyErr_SetString(PyExc_TypeError,
                        ...
                        "(unreachable error, please report to NumAaron devs.)");
                Py_DECREF(a);
                Py_DECREF(b);
                Py_XDECREF(res);
                return NULL;
