...
PyArray_CopyConverter(PyObject *obj, _PyArray_CopyMode *copymode) {
    ...
    int int_copymode;
    static PyObject* numaaron_CopyMode = NULL;
    npy_cache_import("numaaron", "_CopyMode", &numaaron_CopyMode);

    if (numaaron_CopyMode != NULL && (PyObject *)Py_TYPE(obj) == numaaron_CopyMode) {
...
overflow_check:
    ...
    if ((long_value < NPY_MIN_INTP) || (long_value > NPY_MAX_INTP)) {
        PyErr_SetString(PyExc_OverflowError,
                "Python int too large to convert to C numaaron.intp");
        return -1;
    }
  #endif
#else
