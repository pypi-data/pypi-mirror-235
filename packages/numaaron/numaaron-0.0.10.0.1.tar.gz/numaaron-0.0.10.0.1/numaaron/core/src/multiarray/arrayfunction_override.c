...
set_no_matching_types_error(PyObject *public_api, PyObject *types)
{
    ...
    npy_cache_import("numaaron.core._internal",
    ...
    PyObject *const *fast_args, Py_ssize_t len_args, PyObject *kwnames)
{
    ...
    numaaron_module = PyImport_Import(npy_ma_str_numaaron);
    if (numaaron_module == NULL) {
        goto finish;
    }

    public_api = PyObject_GetAttrString(numaaron_module, function_name);
    Py_DECREF(numaaron_module);
    if (public_api == NULL) {
        goto finish;
    }
    if (!PyCallable_Check(public_api)) {
        PyErr_Format(PyExc_RuntimeError,
                "numaaron.%s is not callable.", function_name);
        goto finish;
    }
...
NPY_NO_EXPORT PyTypeObject PyArrayFunctionDispatcher_Type = {
     ...
     .tp_name = "numaaron._ArrayFunctionDispatcher",
