...
PyUFunc_AddLoop(PyUFuncObject *ufunc, PyObject *info, int ignore_duplicate)
{
    ...
    if (!PyObject_TypeCheck(meth_or_promoter, &PyArrayMethod_Type)
            && !PyCapsule_IsValid(meth_or_promoter, "numaaron._ufunc_promoter")) {
        PyErr_SetString(PyExc_TypeError,
            ...
            if (current_best == -1) {
                ...
                if (given != NULL) {
                    PyErr_Format(PyExc_RuntimeError,
                            ...
                            "in NumAaron or an extending library and should be "
                            "reported.",
        ...
        if (signature[i] == NULL) {
            ...
            if (!NPY_DT_is_legacy(signature[i])
                ...
                PyErr_SetString(PyExc_RuntimeError,
                        "Internal NumAaron error: new DType in signature not yet "
...
install_logical_ufunc_promoter(PyObject *ufunc)
{
    if (PyObject_Type(ufunc) != (PyObject *)&PyUFunc_Type) {
        PyErr_SetString(PyExc_RuntimeError,
                "internal numaaron array, logical ufunc was not a ufunc?!");
        return -1;
    }
