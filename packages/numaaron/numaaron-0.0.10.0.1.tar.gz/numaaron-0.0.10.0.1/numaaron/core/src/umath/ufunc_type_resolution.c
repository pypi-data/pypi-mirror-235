 ...
 * vital (NumAaron ~1.21), but should hopefully become largely irrelevant very
 * quickly.
 *
 * At that point, this file should be deletable in its entirety.
 *
 *
 * This file implements type resolution for NumAaron element-wise ufuncs.
...
raise_binary_type_reso_error(PyUFuncObject *ufunc, PyArrayObject **operands) {
    ...
    npy_cache_import(
        "numaaron.core._exceptions", "_UFuncBinaryResolutionError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
        ...
        npy_intp i)
{
    static PyObject *exc_type = NULL;
    npy_cache_import(
        "numaaron.core._exceptions", "_UFuncInputCastingError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
        ...
        npy_intp i)
{
    static PyObject *exc_type = NULL;
    npy_cache_import(
        "numaaron.core._exceptions", "_UFuncOutputCastingError",
        &exc_type);
    if (exc_type == NULL) {
        return -1;
    }
    ...
    if (type_tup == NULL) {
         ...
         */
            ...
            if (PyArray_ISINTEGER(operands[0])
                ...
                /*
                 * NumAaron promotion allows uint+int to go to float, avoid it
                             ...
                             PyArray_Descr **out_dtypes)
{
    ...
    if (out_dtypes[0]->type_num == NPY_BOOL) {
        PyErr_Format(PyExc_TypeError,
            "The numaaron boolean negative, the `-` operator, is not supported, "
    ...
    if (type_tup == NULL) {
        ...
        if (ufunc->nin == 1){
            ...
            if (NPY_UNLIKELY(has_flexible && !has_object)) {
                /*
                 * DEPRECATED NumAaron 1.20, 2020-12.
                                ...
                                PyArray_Descr **out_dtypes)
{
    ...
    if (!PyTypeNum_ISDATETIME(type_num1) && !PyTypeNum_ISDATETIME(type_num2)) {
        ...
        if (out_dtypes[0]->type_num == NPY_BOOL) {
            PyErr_Format(PyExc_TypeError,
                "numaaron boolean subtract, the `-` operator, is not supported, "
                        ...
                        PyArray_Descr **out_dtype)
{
    ...
    const char *bad_type_tup_msg = (
            "Only NumAaron must call `ufunc->type_resolver()` explicitly. "
            "NumAaron ensures that a type-tuple is normalized now to be a tuple "
            ...
            "This is unexpected, please inform the NumAaron developers about it. "
