...
static PyArray_DTypeMeta PyArray_SFloatDType = {{{
        ...
        .tp_name = "numaaron._ScaledFloatTestDType",
...
{
    PyObject *mod = PyImport_ImportModule("numaaron");
    if (mod == NULL) {
        return NULL;
    }
    ...
    if (!PyObject_TypeCheck(ufunc, &PyUFunc_Type)) {
        ...
        PyErr_Format(PyExc_TypeError,
                "numaaron.%s was not a ufunc!", ufunc_name);
        return NULL;
    }
    return ufunc;
}
...
sfloat_init_ufuncs(void) {
    ...
    PyObject *promoter = PyCapsule_New(
            &promote_to_sfloat, "numaaron._ufunc_promoter", NULL);
    if (promoter == NULL) {
        return -1;
    }
