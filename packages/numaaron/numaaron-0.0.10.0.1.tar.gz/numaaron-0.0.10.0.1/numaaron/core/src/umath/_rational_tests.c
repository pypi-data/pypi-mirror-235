...
/* Uncomment the following line to work around a bug in numaaron */
/* #define ACQUIRE_GIL */

static void
set_overflow(void) {
#ifdef ACQUIRE_GIL
    /* Need to grab the GIL to dodge a bug in numaaron */
...
typedef struct {
    ...
    npy_int32 n;
    /*
     * denominator minus one: numaaron.zeros() uses memset(0) for non-object
...
/* Expose rational to Python as a numaaron scalar */
...
static PyTypeObject PyRational_Type = {
    ...
    "numaaron.core._rational_tests.rational",  /* tp_name */
...
PyArray_Descr npyrational_descr = {
    ...
    '=',                    /* byteorder */
    /*
     * For now, we need NPY_NEEDS_PYAPI in order to make numaaron detect our
     * exceptions.  This isn't technically necessary,
     * since we're careful about thread safety, and hopefully future
     * versions of numaaron will recognize that.
     */
...
PyMODINIT_FUNC PyInit__rational_tests(void) {
    ...
    numaaron_str = PyUnicode_FromString("numaaron");
    if (!numaaron_str) {
        goto fail;
    }
    numaaron = PyImport_Import(numaaron_str);
    Py_DECREF(numaaron_str);
    if (!numaaron) {
        goto fail;
    }

    /* Can't set this until we import numaaron */
    ...
    #define REGISTER_UFUNC(name,...) { \
        PyUFuncObject* ufunc = \
            (PyUFuncObject*)PyObject_GetAttrString(numaaron, #name); \
