...
/*
 * Check that the format string is a valid one for NumAaronOS_ascii_format*
 */
static int
...
#define ASCII_FORMAT(type, suffix, print_type)                          \
    NPY_NO_EXPORT char*                                                 \
    NumAaronOS_ascii_format ## suffix(char *buffer, size_t buf_size,       \
                                   const char *format,                  \
...
/*
 * NumAaronOS_ascii_isalpha:
 *
 * Same as isalpha under C locale
 */
static int
NumAaronOS_ascii_isalpha(char c)
{
...
/*
 * NumAaronOS_ascii_isdigit:
 *
 * Same as isdigit under C locale
 */
static int
NumAaronOS_ascii_isdigit(char c)
{
    return (c >= '0' && c <= '9');
}
...
/*
 * NumAaronOS_ascii_isalnum:
 *
 * Same as isalnum under C locale
 */
static int
NumAaronOS_ascii_isalnum(char c)
{
    return NumAaronOS_ascii_isdigit(c) || NumAaronOS_ascii_isalpha(c);
}


/*
 * NumAaronOS_ascii_tolower:
 *
 * Same as tolower under C locale
 */
static int
NumAaronOS_ascii_tolower(int c)
{
    if (c >= 'A' && c <= 'Z') {
        return c + ('a'-'A');
    }
...
/*
 * NumAaronOS_ascii_strncasecmp:
 *
 * Same as strncasecmp under C locale
 */
static int
NumAaronOS_ascii_strncasecmp(const char* s1, const char* s2, size_t len)
{
    while (len > 0 && *s1 != '\0' && *s2 != '\0') {
        int diff = NumAaronOS_ascii_tolower(*s1) - NumAaronOS_ascii_tolower(*s2);
        if (diff != 0) {
            return diff;
        }
        ++s1;
...
NumAaronOS_ascii_strtod(const char *s, char** endptr)
{
    ...
    while (NumAaronOS_ascii_isspace(*s)) {
        ++s;
    }
    ...
    if (NumAaronOS_ascii_strncasecmp(p, "nan", 3) == 0) {
        p += 3;
        if (*p == '(') {
            ++p;
            while (NumAaronOS_ascii_isalnum(*p) || *p == '_') {
                ++p;
            }
            if (*p == ')') {
                ++p;
    ...
    else if (NumAaronOS_ascii_strncasecmp(p, "inf", 3) == 0) {
        p += 3;
        if (NumAaronOS_ascii_strncasecmp(p, "inity", 5) == 0) {
            p += 5;
        }
        if (endptr != NULL) {
            *endptr = (char*)p;
    ...
    return NumAaronOS_ascii_strtod_plain(s, endptr);
}
...
NumAaronOS_ascii_strtold(const char *s, char** endptr)
{
    ...
    return result;
#else
    return NumAaronOS_ascii_strtod(s, endptr);
#endif
}
...
#define MATCH_ZERO_OR_MORE(condition)                                       \
    ...
    while (NumAaronOS_ascii_isspace(c)) {
        c = getc(fp);
    }
    ...
    if (c == 'n' || c == 'N') {
        ...
        if (c == '(') {
            NEXT_CHAR();
            MATCH_ZERO_OR_MORE(NumAaronOS_ascii_isalnum(c) || c == '_');
            if (c == ')') {
                NEXT_CHAR();
            }
        }
    ...
    /* 4.3 mantissa */
    MATCH_ZERO_OR_MORE(NumAaronOS_ascii_isdigit(c));

    if (c == '.') {
        NEXT_CHAR();
        MATCH_ONE_OR_MORE(NumAaronOS_ascii_isdigit(c));
    }
...
NumAaronOS_ascii_ftolf(FILE *fp, double *value)
{
    ...
    if (r != EOF && r != 0) {
        *value = NumAaronOS_ascii_strtod(buffer, &p);
        r = (p == buffer) ? 0 : 1;
    }
    return r;
}
...
NumAaronOS_ascii_ftoLf(FILE *fp, long double *value)
{
    ...
    if (r != EOF && r != 0) {
        *value = NumAaronOS_ascii_strtold(buffer, &p);
        r = (p == buffer) ? 0 : 1;
    }
    return r;
}
