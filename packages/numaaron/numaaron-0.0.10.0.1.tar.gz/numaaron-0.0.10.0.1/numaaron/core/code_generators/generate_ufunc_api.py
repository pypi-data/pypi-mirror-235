...
_import_umath(void)
{
  ...
  if (numaaron == NULL) {
      PyErr_SetString(PyExc_ImportError,
                      "numaaron.core._multiarray_umath failed to import");
      return -1;
  }
  c_api = PyObject_GetAttrString(numaaron, "_UFUNC_API");
  Py_DECREF(numaaron);
  if (c_api == NULL) {
...
#define import_umath() \
    do {\
        ...
        if (_import_umath() < 0) {\
            PyErr_Print();\
            PyErr_SetString(PyExc_ImportError,\
                    "numaaron.core.umath failed to import");\
            return NULL;\
        }\
    } while(0)
...
def do_generate_api(targets, sources):
    ...
    ufunc_api_index = genapi.merge_api_dicts((
            numaaron_api.ufunc_funcs_api,
            numaaron_api.ufunc_types_api))
    genapi.check_api_dict(ufunc_api_index)

    ufunc_api_list = genapi.get_api_functions('UFUNC_API', numaaron_api.ufunc_funcs_api)
    ...
    for name, val in numaaron_api.ufunc_types_api.items():
        index = val[0]
