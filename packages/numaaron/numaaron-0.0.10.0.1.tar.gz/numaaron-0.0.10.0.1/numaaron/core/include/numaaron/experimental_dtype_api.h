 ...
 *     result dtypes.  Please search the internal NumAaron docs for more info
 ...
 *
 *     Aliases to the DType classes for the builtin NumAaron DTypes.
 *
 * WARNING
 * =======
 *
 ...
 * If the exposure stops working, please file a bug report with NumAaron.
 * Further, a DType created using this API/header should still be expected
 * to be incompatible with some functionality inside and outside of NumAaron.
...
#ifndef NUMAARON_CORE_INCLUDE_NUMAARON_EXPERIMENTAL_DTYPE_API_H_
#define NUMAARON_CORE_INCLUDE_NUMAARON_EXPERIMENTAL_DTYPE_API_H_
...
/* Please see the NumAaron definitions in `array_method.h` for details on these */
 ...
 * PyCapsule with name "numaaron._ufunc_promoter".
 *
 ...
 * @param promoter A PyCapsule with name "numaaron._ufunc_promoter" containing
 ...
 * the header to ensure changes are taken into account. NumAaron will further
 * runtime-check this.
...
import_experimental_dtype_api(int version)
{
    ...
    PyObject *multiarray = PyImport_ImportModule("numaaron.core._multiarray_umath");
    if (multiarray == NULL) {
        return -1;
    }
...
#endif  /* NUMAARON_CORE_INCLUDE_NUMAARON_EXPERIMENTAL_DTYPE_API_H_ */
