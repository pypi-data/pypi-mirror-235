...
from numaaron.distutils import _shell_utils, ccompiler, customized_ccompiler
from numaaron.distutils.system_info import (AliasedOptionError, ConfigParser,
...
fakelib_c_text = """
/* This file is generated from numaaron/distutils/testing/test_system_info.py */
#include<stdio.h>
void foo(void) {
   printf("Hello foo");
}
...
class TestSystemInfoReading:
    ...
    @pytest.mark.xfail(HAS_MKL, reason=("`[DEFAULT]` override doesn't work if "
                                        "numaaron is built with MKL support"))
...
def test_distutils_parse_env_order(monkeypatch):
    from numaaron.distutils.system_info import _parse_env_order
