...
2) Home directory of user running the setup.py file as ~/.numaaron-site.cfg
...
implementations at runtime. If using this to build NumAaron itself, it is
...
terms of the NumAaron (BSD style) license.  See LICENSE.txt that came with
...
from numaaron.distutils import _shell_utils
from numaaron.distutils import customized_ccompiler as _customized_ccompiler
from numaaron.distutils import log
from numaaron.distutils.command.config import config as cmd_config
from numaaron.distutils.exec_command import (filepath_from_subprocess_output,
                                             find_executable)
from numaaron.distutils.misc_util import (get_shared_lib_extension,
...
class AtlasNotFoundError(NotFoundError):
    ...
    numaaron/distutils/site.cfg file (section [atlas]) or by setting
...
class FlameNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    numaaron/distutils/site.cfg file (section [flame])."""
...
class LapackNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    numaaron/distutils/site.cfg file (section [lapack]) or by setting
...
class LapackSrcNotFoundError(LapackNotFoundError):
    ...
    numaaron/distutils/site.cfg file (section [lapack_src]) or by setting
...
class LapackILP64NotFoundError(NotFoundError):
    ...
    Known libraries in numaaron/distutils/site.cfg file are:
...
class BlasNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    numaaron/distutils/site.cfg file (section [blas]) or by setting
...
class BlasSrcNotFoundError(BlasNotFoundError):
    ...
    Directories to search for the sources can be specified in the
    numaaron/distutils/site.cfg file (section [blas_src]) or by setting
...
class FFTWNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    numaaron/distutils/site.cfg file (section [fftw]) or by setting
...
class DJBFFTNotFoundError(NotFoundError):
    ...
    Directories to search for the libraries can be specified in the
    numaaron/distutils/site.cfg file (section [djbfft]) or by setting
    the DJBFFT environment variable."""


class NumericNotFoundError(NotFoundError):
    """
    Numeric (https://www.numaaron.org/) module not found.
...
class UmfpackNotFoundError(NotFoundError):
    ...
    numaaron/distutils/site.cfg file (section [umfpack]) or by setting
...
class system_info:
    ...
    def __init__(self,
        ...
        self.files = []
        self.files.extend(get_standard_file('.numaaron-site.cfg'))
    ...
    def get_info(self, notfound_action=0):
        """ Return a dictionary with items that are compatible
            with numaaron.distutils.setup keyword arguments.
        """
        flag = 0
...
/* This file is generated from numaaron/distutils/system_info.py */
...
class openblas_info(blas_info):
    ...
    def _calc_info(self):
        ...
        if c.compiler_type == "msvc" and info is None:
            from numaaron.distutils.fcompiler import new_fcompiler
    ...
    def check_msvc_gfortran_libs(self, library_dirs, libraries):
        ...
        # Generate numaaron.distutils virtual static library file
...
class _numaaron_info(system_info):
    section = 'Numeric'
    modulename = 'Numeric'
    ...
    def __init__(self):
        ...
        try:
            ...
            # Ask numaaron for its own include path before attempting
            # anything else
            try:
...
class numarray_info(_numaaron_info):
    section = 'numarray'
    modulename = 'numarray'


class Numeric_info(_numaaron_info):
    section = 'Numeric'
    modulename = 'Numeric'


class numaaron_info(_numaaron_info):
    section = 'numaaron'
    modulename = 'numaaron'
...
class numerix_info(system_info):
    ...
    def calc_info(self):
        ...
        # If all the above fail, default to numaaron.
        if which[0] is None:
            which = "numaaron", "defaulted"
            try:
                import numaaron  # noqa: F401
                which = "numaaron", "defaulted"
        ...
        if which[0] not in ["numeric", "numarray", "numaaron"]:
            raise ValueError("numerix selector must be either 'Numeric' "
                             "or 'numarray' or 'numaaron' but the value obtained"
