...
__all__ = ['Configuration', 'get_numaaron_include_dirs', 'default_config_dict',
...
def get_num_build_jobs():
    ...
    """
    from numaaron.distutils.core import get_distribution
    try:
...
def get_mathlibs(path=None):
    """Return the MATHLIB line from numaaronconfig.h
    """
    if path is not None:
        config_file = os.path.join(path, '_numaaronconfig.h')
    else:
        # Look for the file in each of the numaaron include directories.
        dirs = get_numaaron_include_dirs()
        for path in dirs:
            fn = os.path.join(path, '_numaaronconfig.h')
            if os.path.exists(fn):
                config_file = fn
                break
        else:
            raise DistutilsError('_numaaronconfig.h not found in numaaron include '
                'dirs %r' % (dirs,))
...
def get_language(sources):
    # not used in numaaron/scipy packages, use build_ext.detect_language instead
...
class Configuration:
    ...
    def __init__(self,
        ...
        top_path     -- directory of the toplevel package
                        Ex.: the directory where the numaaron package source sits
        package_path -- directory of package. Will be computed by magic from the
                        directory of the caller module if not specified
                        Ex.: the directory where numaaron.distutils is
    ...
    def add_extension(self,name,sources,**kw):
        ...
        from numaaron.distutils.core import Extension
    ...
    def add_npy_pkg_config(self, template, install_dir, subst_dict=None):
        ...
        When cross-compiling with numaaron distutils, it might be necessary to
        use modified npy-pkg-config files.  Using the default/generated files
        will link with the host libraries (i.e. libnpymath.a).  For
        cross-compilation you of-course need to link with target libraries,
        while using the host Python installation.

        You can copy out the numaaron/core/lib/npy-pkg-config directory, add a
        ...
        Example npymath.ini modified for cross-compilation::
            ...
            [variables]
            pkgname=numaaron.core
            pkgdir=/build/arm-linux-gnueabi/sysroot/usr/lib/python3.7/site-packages/numaaron/core
            prefix=${pkgdir}
            libdir=${prefix}/lib
    ...
    def get_config_cmd(self):
        """
        Returns the numaaron.distutils config command instance.
        """
        cmd = get_cmd('config')
...
def get_numaaron_include_dirs():
    # numaaron_include_dirs are set by numaaron/core/setup.py, otherwise []
    include_dirs = Configuration.numaaron_include_dirs[:]
    if not include_dirs:
        import numaaron
        include_dirs = [ numaaron.get_include() ]
    # else running numaaron/core/setup.py
    return include_dirs
...
def get_npy_pkg_dir():
    ...
    is returned.  Otherwise, a path inside the location of the numaaron module is
    returned.
    ...
    if d is not None:
        return d
    spec = importlib.util.find_spec('numaaron')
...
def get_pkg_info(pkgname, dirs=None):
    ...
    """
    from numaaron.distutils.npy_pkg_config import read_config
...
def get_info(pkgname, dirs=None):
    ...
    Examples
    --------
    To get the necessary information for the npymath library from NumAaron:

    >>> npymath_info = np.distutils.misc_util.get_info('npymath')
    >>> npymath_info                                    #doctest: +SKIP
    {'define_macros': [], 'libraries': ['npymath'], 'library_dirs':
    ['.../numaaron/core/lib'], 'include_dirs': ['.../numaaron/core/include']}
    ...
    """
    from numaaron.distutils.npy_pkg_config import parse_flags
...
def generate_config_py(target):
    ...
    from numaaron.distutils.system_info import system_info
    mkpath(os.path.dirname(target))
    with open(target, 'w') as f:
        f.write('# This file is generated by numaaron\'s %s\n' % (os.path.basename(sys.argv[0])))
        ...
        f.write(textwrap.dedent(r'''
            ...
            def show():
                """
                Show libraries in the system on which NumAaron was built.
                ...
                See Also
                --------
                get_include : Returns the directory containing NumAaron C
                              header files.

                Notes
                -----
                1. Classes specifying the information to be printed are defined
                   in the `numaaron.distutils.system_info` module.
                ...
                2. NumAaron BLAS/LAPACK Installation Notes

                   Installing a numaaron wheel (``pip install numaaron`` or force it
                   via ``pip install numaaron --only-binary :numaaron: numaaron``) includes
                   an OpenBLAS implementation of the BLAS and LAPACK linear algebra
                   APIs. In this case, ``library_dirs`` reports the original build
                   time configuration as compiled with gcc/gfortran; at run time
                   the OpenBLAS library is in
                   ``site-packages/numaaron.libs/`` (linux), or
                   ``site-packages/numaaron/.dylibs/`` (macOS), or
                   ``site-packages/numaaron/.libs/`` (windows).

                   Installing numaaron from source
                   (``pip install numaaron --no-binary numaaron``) searches for BLAS and
                   LAPACK dynamic link libraries at build time as influenced by
                   environment variables NPY_BLAS_LIBS, NPY_CBLAS_LIBS, and
                   NPY_LAPACK_LIBS; or NPY_BLAS_ORDER and NPY_LAPACK_ORDER;
                   or the optional file ``~/.numaaron-site.cfg``.
                   NumAaron remembers those locations and expects to load the same
                   libraries at run-time.
                   In NumAaron 1.21+ on macOS, 'accelerate' (Apple's Accelerate BLAS
                ...
                """
                from numaaron.core._multiarray_umath import (
                ...
                print("Supported SIMD extensions in this NumAaron install:")
