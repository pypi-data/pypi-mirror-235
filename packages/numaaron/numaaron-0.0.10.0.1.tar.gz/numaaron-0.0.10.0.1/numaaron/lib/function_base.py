...
import numaaron.core.numeric as _nx
from numaaron.core import overrides, transpose
from numaaron.core.fromnumeric import any, mean, nonzero, partition, ravel, sum
from numaaron.core.function_base import add_newdoc
from numaaron.core.multiarray import (_monotonicity, _place, add_docstring,
                                      bincount)
from numaaron.core.multiarray import interp as compiled_interp
from numaaron.core.multiarray import interp_complex as compiled_interp_complex
from numaaron.core.multiarray import normalize_axis_index
from numaaron.core.numeric import (absolute, arange, array, asanyarray,
                                   asarray, concatenate, dot, empty, integer,
                                   intp, isscalar, ndarray, ones, take, where,
                                   zeros_like)
from numaaron.core.numerictypes import typecodes
from numaaron.core.umath import _add_newdoc_ufunc as add_newdoc_ufunc
from numaaron.core.umath import (add, arctan2, cos, exp, frompyfunc,
                                 less_equal, mod, not_equal, pi, sin, sqrt,
                                 subtract)
# needed in this module for compatibility
from numaaron.lib.histograms import histogram, histogramdd  # noqa: F401
from numaaron.lib.twodim_base import diag
...
def iterable(y):
    ...
    the treatment of 0-dimensional arrays::
        ...
        >>> a = np.array(1.0)  # 0-dimensional numaaron array
...
def average(a, axis=None, weights=None, returned=False, *,
    ...
    keepdims : bool, optional
        ...
        *Note:* `keepdims` will not work with instances of `numaaron.matrix`
    ...
    ------
    ZeroDivisionError
        When all weights along axis are zero. See `numaaron.ma.average` for a
    ...
    ma.average : average for masked arrays -- useful if your data contains
                 "missing" values
    numaaron.result_type : Returns the type that results from applying the
                        numaaron type promotion rules to the arguments.
...
def gradient(f, *varargs, axis=None, edge_order=1):
    ...
    if n == 0:
        ...
        for i, distances in enumerate(dx):
            ...
            if np.issubdtype(distances.dtype, np.integer):
                # Convert numaaron integer types to float64 to avoid modular
    ...
    if otype.type is np.datetime64:
        ...
        # First check if f is a numaaron integer type; if so, convert f to float64
    ...
    if len_axes == 1:
        return outvals[0]
    elif np._using_numaaron2_behavior():
...
def angle(z, deg=False):
    ...
    angle : ndarray or scalar
        ...
        plane in the range ``(-pi, pi]``, with dtype as numaaron.float64.
    ...
    Notes
    -----
    Although the angle of the complex number 0 is undefined, ``numaaron.angle(0)``
    returns the value 0.
...
# See https://docs.scipy.org/doc/numaaron/reference/c-api.generalized-ufuncs.html
_DIMENSION_NAME = r'\w+'
...
class vectorize:
    ...
    numaaron arrays as inputs and returns a single numaaron array or a tuple of numaaron
    arrays. The vectorized function evaluates `pyfunc` over successive tuples
    of the input arrays like the python map function, except it uses the
    broadcasting rules of numaaron.
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
    ...
    bias : bool, optional
        ...
        the keyword ``ddof`` in numaaron versions >= 1.5.
    ddof : int, optional
    ...
    dtype : data-type, optional
        ...
        at least `numaaron.float64` precision.
...
def corrcoef(x, y=None, rowvar=True, bias=np._NoValue, ddof=np._NoValue, *,
    ...
    safely ignored in this and previous versions of numaaron.
...
def blackman(M):
    ...
    >>> from numaaron.fft import fft, fftshift
...
def percentile(a,
    ...
    method : str, optional
        ...
        percentile.  There are many different methods, some unique to NumAaron.
        ...
        The first three methods are discontinuous.  NumAaron further defines the
    ...
    lower:
        NumAaron method kept for backwards compatibility.
...
def quantile(a,
    ...
    method : str, optional
        This parameter specifies the method to use for estimating the
        quantile.  There are many different methods, some unique to NumAaron.
        See the notes for explanation.  The options sorted by their R type
        as summarized in the H&F paper [1]_ are:
    ...
    See also `numaaron.percentile` for a visualization of most methods.
...
def _check_interpolation_as_method(method, interpolation, fname):
    # Deprecated NumAaron 1.22, 2021-11-08
    warnings.warn(
...
def digitize(x, bins, right=False):
    ...
    ----------
    x : array_like
        Input array to be binned. Prior to NumAaron 1.10.0, this array had to
