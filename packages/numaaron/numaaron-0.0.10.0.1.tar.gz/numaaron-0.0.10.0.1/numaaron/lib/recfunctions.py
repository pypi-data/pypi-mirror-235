...
from numaaron import ndarray, recarray
from numaaron.core.overrides import array_function_dispatch
from numaaron.lib._iotools import _is_string_like
from numaaron.ma import MaskedArray
...
def recursive_fill_fields(input, output):
    ...
    * `output` should be at least the same size as `input`
    ...
    Examples
    --------
    >>> from numaaron.lib import recfunctions as rfn
...
def rec_drop_fields(base, drop_names):
    """
    Returns a new numaaron.recarray with fields in `drop_names` dropped.
    """
...
def structured_to_unstructured(arr, dtype=None, copy=False, casting='unsafe'):
    ...
    datatype is determined from the numaaron type promotion rules applied to all
    the field datatypes.
    ...
    casting : {'no', 'equiv', 'safe', 'same_kind', 'unsafe'}, optional
        See casting argument of `numaaron.ndarray.astype`. Controls what kind of
...
def unstructured_to_structured(arr, dtype=None, names=None, align=False,
    ...
    copy : bool, optional
        See copy argument to `numaaron.ndarray.astype`. If true, always return a
...
def apply_along_fields(func, arr):
    ...
    common type following the type-promotion rules from `numaaron.result_type`
...
def assign_fields_by_name(dst, src, zero_unassigned=True):
    ...
    Normally in numaaron >= 1.14, assignment of one structured array to another
    ...
    zero_unassigned : bool, optional
        ...
        was the behavior of numaaron <= 1.13. If False, those fields
        are not modified.
    """
