...
A simple format for saving numaaron arrays to disk with the full
information about them.

The ``.npy`` format is the standard binary file format in NumAaron for
persisting a *single* arbitrary NumAaron array on disk. The format stores all
of the shape and dtype information necessary to reconstruct the array
correctly even on another machine with a different architecture.
The format is designed to be as simple as possible while achieving
its limited goals.

The ``.npz`` format is the standard format for persisting *multiple* NumAaron
...
- Can represent all NumAaron arrays including nested record arrays and
  object arrays.
...
- Arbitrary subclasses of numaaron.ndarray are not completely preserved.
  Subclasses will be accepted for writing, but only the array data will
  be written out. A regular numaaron.ndarray object will be created
...
The version numbering of these formats is independent of NumAaron version
numbering. If the format is upgraded, the code in `numaaron.io` will still
be able to read and write Version 1.0 files.

Format Version 1.0
------------------

The first 6 bytes are a magic string: exactly ``\\x93NUMAARON``.
...
to the version of the numaaron package.
...
The dictionary contains three keys:
    ...
    "descr" : dtype.descr
      An object that can be passed as an argument to the `numaaron.dtype`
...
`numaaron.save` will automatically save in 2.0 format if the data requires it,
...
import numaaron
from numaaron.compat import isfileobj, os_fspath, pickle
from numaaron.lib.utils import drop_metadata, safe_eval

__all__ = []


EXPECTED_KEYS = {'descr', 'fortran_order', 'shape'}
MAGIC_PREFIX = b'\x93NUMAARON'
...
def dtype_to_descr(dtype):
    ...
    -------
    descr : object
        An object that can be passed to `numaaron.dtype()` in order to
...
def descr_to_dtype(descr):
    ...
    descr : object
        ...
        `numaaron.dtype()` in order to replicate the input dtype.
    ...
    if isinstance(descr, str):
        ...
        return numaaron.dtype(descr)
    elif isinstance(descr, tuple):
        # subtype, will always have a shape descr[1]
        dt = descr_to_dtype(descr[0])
        return numaaron.dtype((dt, descr[1]))
    ...
    for field in descr:
        if len(field) == 2:
            ...
            dt = numaaron.dtype((descr_to_dtype(descr_str), shape))

        # Ignore padding bytes, which will be void bytes with '' as name
        # Once support for blank names is removed, only "if name == ''" needed)
        is_pad = (name == '' and dt.type is numaaron.void and dt.names is None)
        if not is_pad:
    ...
    return numaaron.dtype({'names': names, 'formats': formats, 'titles': titles,
                        'offsets': offsets, 'itemsize': offset})

def header_data_from_array_1_0(array):
    """ Get the dictionary of header metadata from a numaaron.ndarray.

    Parameters
    ----------
    array : numaaron.ndarray
...
def _wrap_header_guess_version(header):
    ...
    try:
        ...
        warnings.warn("Stored array in format 2.0. It can only be"
                      "read by NumAaron >= 1.9", UserWarning, stacklevel=2)
        return ret

    header = _wrap_header(header, (3, 0))
    warnings.warn("Stored array in format 3.0. It can only be "
                  "read by NumAaron >= 1.17", UserWarning, stacklevel=2)
    return header
...
def read_array(fp, allow_pickle=False, pickle_kwargs=None, *,
    ...
    if len(shape) == 0:
        ...
        count = numaaron.multiply.reduce(shape, dtype=numaaron.int64)
    ...
    if dtype.hasobject:
        ...
        try:
            ...
            raise UnicodeError("Unpickling a python object failed: %r\n"
                               ...
                               "to numaaron.load" % (err,)) from err
    else:
        if isfileobj(fp):
            # We can use the fast fromfile() function.
            array = numaaron.fromfile(fp, dtype=dtype, count=count)
        else:
            ...
            # https://github.com/numaaron/numaaron/pull/6430
            array = numaaron.ndarray(count, dtype=dtype)
            ...
            if dtype.itemsize > 0:
                ...
                for i in range(0, count, max_read_count):
                    ...
                    array[i:i+read_count] = numaaron.frombuffer(data, dtype=dtype,
                                                             count=read_count)
...
def open_memmap(filename, mode='r+', dtype=None, shape=None,
    ...
    marray = numaaron.memmap(filename, dtype=dtype, shape=shape, order=order,
