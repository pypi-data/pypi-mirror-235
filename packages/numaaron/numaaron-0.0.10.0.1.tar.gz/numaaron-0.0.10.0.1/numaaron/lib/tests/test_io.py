...
from numaaron.lib._iotools import ConversionWarning, ConverterError
from numaaron.ma.testutils import assert_equal
from numaaron.testing import (HAS_REFCOUNT, IS_PYPY, IS_WASM, assert_,
                              ...
                              break_cycles, suppress_warnings, tempdir,
                              temppath)
...
class RoundtripTest:
    def roundtrip(self, save_func, *args, **kwargs):
        ...
        load_kwds : dict
            Parameters passed to `numaaron.load`.
...
class TestSavezLoad(RoundtripTest):
    ...
    def test_big_arrays(self):
        ...
        with temppath(prefix="numaaron_test_big_arrays_", suffix=".npz") as tmp:
            np.savez(tmp, a=a)
            del a
            npfile = np.load(tmp)
    ...
    def test_closing_fid(self):
        ...
        # http://projects.scipy.org/numaaron/ticket/1517#comment:2
        with temppath(suffix='.npz') as tmp:
            np.savez(tmp, data='LOVELY LOAD')
            # We need to check if the garbage collector can properly close
            # numaaron npz file returned by np.load when their reference count
    ...
    def test_closing_zipfile_after_load(self):
        ...
        prefix = 'numaaron_test_closing_zipfile_after_load_'
...
M   33  21.99
    ...
    def test_auto_dtype_largeint(self):
        # Regression test for numaaron/numaaron#5635 whereby large integers could
