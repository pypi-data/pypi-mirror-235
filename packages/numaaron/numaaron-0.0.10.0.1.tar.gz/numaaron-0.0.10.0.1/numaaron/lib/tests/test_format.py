...
Set up:
    ...
    >>> from numaaron.lib import format
    >>>
    >>> scalars = [
    ...     np.uint8,
    ...     np.int8,
...
from numaaron.lib import format
from numaaron.testing import (IS_PYPY, IS_WASM, assert_, assert_array_equal,
...
bad_version_magic = [
    b'\x93NUMAARON\x01\x01',
    b'\x93NUMAARON\x00\x00',
    b'\x93NUMAARON\x00\x01',
    b'\x93NUMAARON\x02\x00',
    b'\x93NUMAARON\x02\x02',
    b'\x93NUMAARON\xff\xff',
]
malformed_magic = [
    b'\x92NUMAARON\x01\x00',
    b'\x00NUMAARON\x01\x00',
    b'\x93numaaron\x01\x00',
    b'\x93MATLB\x01\x00',
    b'\x93NUMAARON\x01',
    b'\x93NUMAARON',
    b'',
...
def test_bad_header():
    ...
    #      "descr": "x"}
    s = BytesIO(
        b"\x93NUMAARON\x01\x006\x00{'descr': 'x', 'shape': (1, 2), }" +
        b"                    \n"
    )
    assert_raises(ValueError, format.read_array_header_1_0, s)
...
def test_metadata_dtype(dt, fail):
    ...
    if fail:
        ...
        from numaaron.lib.utils import drop_metadata
        assert_array_equal(arr, arr2)
