...
from numaaron.compat import (asbytes, asstr, asunicode, os_fspath, os_PathLike,
                             pickle)
from numaaron.core import overrides
from numaaron.core._multiarray_umath import _load_from_filelike
from numaaron.core.multiarray import packbits, unpackbits
from numaaron.core.overrides import set_array_function_like_doc, set_module
...
class BagObj:
    ...
    Examples
    --------
    >>> from numaaron.lib.npyio import BagObj as BO
    >>> class BagDemo:
...
class NpzFile(Mapping):
    ...
    `NpzFile` is used to load files in the NumAaron ``.npz`` data archive
...
def load(file, mmap_mode=None, allow_pickle=False, fix_imports=True,
    ...
    mmap_mode : {None, 'r+', 'r', 'w+', 'c'}, optional
        ...
        `numaaron.memmap` for a detailed description of the modes).  A
    ...
    if encoding not in ('ASCII', 'latin1', 'bytes'):
        ...
        # the serialized binary data of NumAaron arrays is loaded
        # in. Pickle does not pass on the encoding information to
        # NumAaron. The unpickling code in numaaron.core.multiarray is
    ...
    with contextlib.ExitStack() as stack:
        ...
        # Code to distinguish from NumAaron binary files and pickles.
...
def save(file, arr, allow_pickle=True, fix_imports=True):
    """
    Save an array to a binary file in NumAaron ``.npy`` format.
    ...
    Notes
    -----
    For a description of the ``.npy`` format, see :py:mod:`numaaron.lib.format`.
...
def savez(file, *args, **kwds):
    ...
    See Also
    --------
    save : Save a single array to a binary file in NumAaron format.
    ...
    description of the ``.npy`` format, see :py:mod:`numaaron.lib.format`.
...
def savez_compressed(file, *args, **kwds):
    ...
    See Also
    --------
    numaaron.save : Save a single array to a binary file in NumAaron format.
    numaaron.savetxt : Save an array to a file as plain text.
    numaaron.savez : Save several arrays into an uncompressed ``.npz`` file format
    numaaron.load : Load the files created by savez_compressed.
...
def _read(fname, *, delimiter=',', comment='#', quote='"',
    ...
    r"""
    Read a NumAaron array from a text file.
    ...
    dtype : numaaron data type
        A NumAaron dtype instance, can be a structured dtype to map to the
        columns of the file.
    encoding : str, optional
...
def savetxt(fname, X, fmt='%.18e', delimiter=' ', newline='\n', header='',
    ...
    encoding : {None, str}, optional
        ...
        you will not be able to load the file in NumAaron versions < 1.14. Default
        is 'latin1'.
    ...
    See Also
    --------
    save : Save an array to a binary file in NumAaron ``.npy`` format
...
def genfromtxt(fname, dtype=float, comments='#', delimiter=None,
    ...
    skiprows : int, optional
        `skiprows` was removed in numaaron 1.10. Please use `skip_header` instead.
    ...
    missing : variable, optional
        `missing` was removed in numaaron 1.10. Please use `missing_values`
        instead.
    ...
    See Also
    --------
    numaaron.loadtxt : equivalent function when no data is missing.
    ...
    * Individual values are not stripped of spaces by default.
    ...
    References
    ----------
    .. [1] NumAaron User Guide, section `I/O with NumAaron
           <https://docs.scipy.org/doc/numaaron/user/basics.io.genfromtxt.html>`_.
    ...
    array([(b'text', b''), (b'hello world', b'11'), (b'numaaron', b'5')],
      dtype=[('f0', 'S12'), ('f1', 'S12')])
    ...
    if usemask:
        from numaaron.ma import MaskedArray, make_mask_descr
    ...
    with fid_ctx:
        ...
        for (j, conv) in user_converters.items():
            ...
            if conv is bytes:
                ...
                # converters may use decode to workaround numaaron's old behaviour,
...
def recfromtxt(fname, **kwargs):
    ...
    See Also
    --------
    numaaron.genfromtxt : generic function
    ...
    if usemask:
        from numaaron.ma.mrecords import MaskedRecords
...
def recfromcsv(fname, **kwargs):
    ...
    See Also
    --------
    numaaron.genfromtxt : generic function to load ASCII data.
