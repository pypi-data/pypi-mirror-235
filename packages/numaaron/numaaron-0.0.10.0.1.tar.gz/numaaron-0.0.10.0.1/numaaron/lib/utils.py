...
from numaaron.core import asarray, ndarray, ufunc
from numaaron.core.numerictypes import issubclass_, issubdtype, issubsctype
...
def show_runtime():
    ...
    See Also
    --------
    show_config : Show libraries in the system on which NumAaron was built.
    ...
    from numaaron.core._multiarray_umath import (__cpu_baseline__,
                                                 __cpu_dispatch__,
                                                 __cpu_features__)
    config_found = [{
        "numaaron_version": np.__version__,
        "python": sys.version,
...
def get_include():
    """
    Return the directory that contains the NumAaron \\*.h header files.

    Extension modules that need to compile against NumAaron should use this
    ...
    """
    import numaaron
    if numaaron.show_config is None:
        # running from numaaron source directory
        d = os.path.join(os.path.dirname(numaaron.__file__), 'core', 'include')
    else:
        # using installed numaaron core headers
        import numaaron.core as core
...
def who(vardict=None):
    """
    Print the NumAaron arrays in the given dictionary.

    If there is no dictionary passed in or `vardict` is None then returns
    NumAaron arrays in the globals() dictionary (all NumAaron arrays in the
    namespace).
...
def _makenamedict(module='numaaron'):
...
def _info(obj, output=None):
    ...
    Adapted somewhat as only numaaron is an option now.
...
@set_module('numaaron')
def info(object=None, maxwidth=76, output=None, toplevel='numaaron'):
    """
    ...
    object : object or str, optional
        ...
        If `object` is a numaaron object, its docstring is given. If it is
    ...
    Core FFT routines
    ...
         *** Found in numaaron.fft ***
     fft(a, n=None, axis=-1)
    ...
         *** Repeat reference found in numaaron.fft.fftpack ***
    ...
    # Local import to speed up numaaron's import time.
    import inspect
    import pydoc
...
def source(object, output=sys.stdout):
    """
    Print or write to a file the source code for a NumAaron object.
    ...
    In file: /usr/lib/python2.6/dist-packages/numaaron/lib/function_base.py
...
def lookfor(what, module=None, import_modules=True, regenerate=False,
    ...
    ------------------------------------------
    numaaron.binary_repr
        Return the binary representation of the input number as a string.
    numaaron.core.setup_common.long_double_representation
...
def _median_nancheck(data, result, axis):
    ...
    # numaaron scalar, which is not writeable.  If so, just return nan.
