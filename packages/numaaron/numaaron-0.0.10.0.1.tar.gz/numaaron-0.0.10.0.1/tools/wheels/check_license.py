...
and that it appears to contain text prevalent for a NumAaron binary
distribution.
...
def main():
    ...
    p.add_argument("module", nargs="?", default="numaaron")
    args = p.parse_args()
    ...
    distinfo_path = [s for s in sitepkgs.glob("numaaron-*.dist-info")][0]
