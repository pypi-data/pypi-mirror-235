...
run_test()
{
  ...
  # of numaaron in the source directory.
  mkdir -p empty
  cd empty
  INSTALLDIR=$($PYTHON -c \
    "import os; import numaaron; print(os.path.dirname(numaaron.__file__))")
  ...
  # disrupted the NumAaron build.
  # see gh-21750, gh-21748
  if [ -n "$EXPECT_CPU_FEATURES" ]; then
    as_expected=$($PYTHON << EOF
from numaaron.core._multiarray_umath import (__cpu_baseline__, __cpu_dispatch__)
...
if [ -n "$USE_WHEEL" ] && [ $# -eq 0 ]; then
  ...
  # Move out of source directory to avoid finding local numaaron
  pushd dist
  $PIP install --pre --no-index --upgrade --find-links=. numaaron
  popd
