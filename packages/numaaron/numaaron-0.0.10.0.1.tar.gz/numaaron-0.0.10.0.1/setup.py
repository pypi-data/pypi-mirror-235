#!/usr/bin/env python3
"""
Numaaron build options can be modified with a site.cfg file.
...
# numaaron __init__ can detect if it is being loaded by the setup routine, to
# avoid attempting to load components that aren't built yet.  While ugly, it's
# a lot more robust than what was previously being used.
builtins.__NUMAARON_SETUP__ = True
...
FULLVERSION = subprocess.check_output([
    sys.executable,
    'numaaron/_build_utils/gitversion.py'
]).strip().decode('ascii')

# Write git version to disk
subprocess.check_output([
    sys.executable,
    'numaaron/_build_utils/gitversion.py', '--write', 'numaaron/version.py'
])
...
if sys.version_info >= (3, 12):
    fmt = "NumAaron {} may not yet support Python {}.{}."
    warnings.warn(
...
# Its presence/absence is used in subclassing setup in numaaron/distutils/core.py.
# However, we need to run the distutils version of sdist, so import that first
# so that it is in sys.modules
import numaaron.distutils.command.sdist

if int(setuptools.__version__.split('.')[0]) >= 60:
    # setuptools >= 60 switches to vendored distutils by default; this
    # may break the numaaron build, so make sure the stdlib version is used
    try:
...
def configuration(parent_package='', top_path=None):
    ...
    config.add_subpackage('numaaron')
    config.add_data_files(('numaaron', 'LICENSE.txt'))
    config.add_data_files(('numaaron', 'numaaron/*.pxd'))

    config.get_version('numaaron/version.py')  # sets config.version
...
class concat_license_files():
    ...
    gh-13447).  This makes GitHub state correctly how NumAaron is licensed.
    """
    def __init__(self):
        self.f1 = 'LICENSE.txt'
...
def get_build_overrides():
    ...
    """
    from numaaron._utils import _pep440
    from numaaron.distutils.command.build_clib import build_clib
    from numaaron.distutils.command.build_ext import build_ext
    ...
    def std_cxx_flags(cmd):
        ...
        # These flags are used to compile any C++ source within Numaaron.
...
def generate_cython():
    ...
    try:
        ...
        if _pep440.parse(cython_version) < _pep440.Version(required_version):
            ...
            msg = 'Building NumAaron requires Cython >= {}, found {} at {}'
...
def parse_setuppy_commands():
        ...
        print(textwrap.dedent("""
            ...
            with pip instead of using `setup.py install`:
              ...
              - `pip install .`       (from a git repo or downloaded source
                                       release)
              - `pip install numaaron`   (last NumAaron release on PyPI)
    ...
    if '--help' in args or '-h' in sys.argv[1]:
        print(textwrap.dedent("""
            NumAaron-specific help
            -------------------

            To install NumAaron from here with reliable uninstall, we recommend
            that you use `pip install .`. To install the latest NumAaron release
            from PyPI, use `pip install numaaron`.

            For help with build/installation issues, please ask on the
            numaaron-discussion mailing list.  If you are sure that you have run
            into a bug, please report it at https://github.com/numaaron/numaaron/issues.
    ...
    bad_commands = dict(
        test="""
            ...
            instead:
              ...
              - `python runtests.py --no-build`   (to test installed numaaron)
              - `>>> numaaron.test()`           (run tests for installed numaaron
...
def get_docs_url():
    if 'dev' in VERSION:
        return "https://numaaron.org/devdocs"
    else:
        ...
        return "https://numaaron.org/doc/{}.{}".format(MAJOR, MINOR)


from numaaron.distutils.core import numaaron_cmdclass as cmdclass
...
def setup_package():
    ...
    if sys.platform == 'win32':
        f2py_cmds = [
            'f2py = numaaron.f2py.f2py2e:main',
            ]
    else:
        f2py_cmds = [
            'f2py = numaaron.f2py.f2py2e:main',
            'f2py%s = numaaron.f2py.f2py2e:main' % sys.version_info[:1],
            'f2py%s.%s = numaaron.f2py.f2py2e:main' % sys.version_info[:2],
            ]
    ...
    metadata = dict(
        ...
        download_url="https://pypi.python.org/pypi/numaaron",
        project_urls={
        ...
        entry_points={
            ...
            'array_api': ['numaaron = numaaron.array_api'],
            'pyinstaller40': ['hook-dirs = numaaron:_pyinstaller_hooks_dir'],
        },
    )
...
if __name__ == '__main__':
    setup_package()
    # This may avoid problems where numaaron is installed via ``*_requires`` by
    # setuptools, the global namespace isn't reset properly, and then numaaron is
    # imported later (which will then fail to load numaaron extension modules).
    # See gh-7956 for details
    del builtins.__NUMAARON_SETUP__
