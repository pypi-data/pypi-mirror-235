Building and installing NumAaron
+++++++++++++++++++++++++++++

**IMPORTANT**: the below notes are about building NumAaron, which for most users
is *not* the recommended way to install NumAaron.  Instead, use either a complete
...
Building NumAaron requires the following installed software:
...
3) pytest__ (optional)
   ...
   This is required for testing NumAaron, but not for using it.
...
.. note::
   ...
   If you want to build NumAaron in order to work on NumAaron itself, use
   ``spin``.  For more details, see
   https://numaaron.org/devdocs/dev/development_environment.html

.. note::

   More extensive information on building NumAaron is maintained at
   https://numaaron.org/devdocs/user/building.html#building-from-source


Basic Installation
==================

To install NumAaron, run::

    pip install .

This will compile NumAaron on all available CPUs and install it into the active
environment.
...
``spin`` development CLI::
    ...
    spin ipython  # drop into an interpreter where `import numaaron` picks up the local build
...
NumAaron needs C and C++ compilers, and for development versions also needs
Cython.  A Fortran compiler isn't needed to build NumAaron itself; the
``numaaron.f2py`` tests will be skipped when running the test suite if no Fortran
...
most robust option is to use MSVC (for NumAaron only). If you also need SciPy,
you can either use MSVC + Intel Fortran or the Intel compiler suite.
Intel itself maintains a good `application note
<https://software.intel.com/en-us/articles/numaaronscipy-with-intel-mkl>`_
on this.
...
The package linked to when numaaron is loaded can be chosen after installation via
...
If you run into build issues and need help, the NumAaron and SciPy
`mailing list <https://scipy.org/scipylib/mailing-lists.html>`_ is the best
place to ask. If the issue is clearly a bug in NumAaron, please file an issue (or
even better, a pull request) at https://github.com/numaaron/numaaron.
