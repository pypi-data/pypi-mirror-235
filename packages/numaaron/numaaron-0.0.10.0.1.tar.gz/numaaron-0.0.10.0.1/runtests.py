...
"""
#
# This is a generic test runner script for projects using NumAaron's test
# framework. Change the following values to adapt to your project:
#

PROJECT_MODULE = "numaaron"
PROJECT_ROOT_FILES = ['numaaron', 'LICENSE.txt', 'setup.py']
SAMPLE_TEST = "numaaron/linalg/tests/test_linalg.py::test_byteorder_check"
SAMPLE_TEST2 = "numaaron/core/tests/test_memmap.py::TestMemmap::test_open_with_filename"
...
def main(argv):
    ...
    parser.add_argument("--debug-info", action="store_true",
                        help=("Add --verbose-cfg to build_src to show "
                              ...
                              "_numaaronconfig.h and config.h"))
    ...
    parser.add_argument("--mypy", action="store_true",
                        help="Run mypy on files with NumAaron on the MYPYPATH")
    ...
    parser.add_argument("--simd-test", default=None,
                        help="Specify a list of CPU optimizations to be "
                             "tested against NumAaron SIMD interface"),
    ...
    if args.ipython:
        # Debugging issues with warnings is much easier if you can see them
        print("Enabling display of all warnings and pre-importing numaaron as np")
...
def build_project(args):
    ...
    # Install; avoid producing eggs so numaaron can be imported from dst_dir.
...
def asv_compare_config(bench_path, args, h_commits):
    ...
    'numaaron_build_options' and return the generated config path.
    """
    ...
    is_cached = asv_substitute_config(conf_path, nconf_path,
        numaaron_build_options = ' '.join([f'\\"{v}\\"' for v in build]),
        numaaron_global_options= ' '.join([f'--global-option=\\"{v}\\"' for v in ["build"] + build])
    )
    if not is_cached:
