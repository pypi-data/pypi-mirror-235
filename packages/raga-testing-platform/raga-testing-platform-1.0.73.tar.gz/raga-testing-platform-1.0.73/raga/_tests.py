...
from raga import TestSession, ModelABTestRules, FMARules, LQRules, EventABTestRules
...
def event_ab_test(test_session:TestSession, 
                  ...
                  filter: Optional[str] = ""):
    dataset_id = ab_test_validation(test_session, dataset_name, test_name, model, object_detection_model, type, sub_type, rules, gt, object_detection_gt, aggregation_level)    
    return {
            "datasetId": dataset_id,
            "experimentId": test_session.experiment_id,
            "name": test_name,
            "model": model,
            "gt": gt,
            "objectDetectionModel":object_detection_model,
            "objectDetectionGT":object_detection_gt,
            "type": type,
            "subType":sub_type,
            "rules": rules.get(),
            "aggregationLevels": aggregation_level,
...
def ab_test_validation(test_session:TestSession, 
    ...
    assert isinstance(object_detection_model, str) and object_detection_model, f"{REQUIRED_ARG_V2.format('object_detection_model', 'str')}"
    assert isinstance(object_detection_gt, str) and object_detection_gt, f"{REQUIRED_ARG_V2.format('object_detection_gt', 'str')}"
    ...
    if sub_type == "labelled":
        assert isinstance(gt, str) and gt, f"{REQUIRED_ARG_V2.format('gt', 'str')}"
        assert isinstance(object_detection_gt, str) and object_detection_gt, f"{REQUIRED_ARG_V2.format('object_detection_gt', 'str')}"

    # if sub_type == "unlabelled" and isinstance(gt, str) and gt:
    #     raise ValueError("gt is not required on unlabelled sub-type.")
