...
def summarize_dummy_textsplitter(self, text: str, verbose_option: int) -> str:
    """
    See function summarize_textsplitter. This simply takes the first n words
    from the text-string instead of actually summarizing it. It is meant for 
    testing purposes when the ChatGPT calling-limit has passed. But this will
    not give a proper summary.
    """
    text_array = text.split()
    if (len(text_array)>2*self.MaxSummaryLength):
        text_array = text_array[0:2*self.MaxSummaryLength]
    # Factor 2 is added here, to allow for more accurate callcounting;
    # a summary is usually <2n, not <n, so this way we do not underestimate
    # the calls in layered summaries.
...
def summarize_textsplitter(self, text: str, verbose_option: int) -> str:
    ...
    # Before we do anything else: decide whether we use the dummy summary function
    # here above, or actually perform a summarization:
    ...
    if (len(text.split())<self.summarization_threshold): return text
    ...
    for textpiece in Array_to_summarize:
        ...
        while (not(Response_IsOK))and(WhileCounter<self.MaxCallRepeat):
            ...
            if not openai_call_succeeded:
                ...
                # Given the measurements on the other docs, we take a safety margin and use 0.75.
