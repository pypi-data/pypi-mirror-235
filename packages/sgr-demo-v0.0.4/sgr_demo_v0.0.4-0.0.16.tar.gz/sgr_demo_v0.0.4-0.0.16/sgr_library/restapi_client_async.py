...
class SgrRestInterface():
        ...
        try:
            async with self.session.post(url=self.authentication_url, headers=self.headers, data=self.data) as res:
                if 200 <= res.status < 300:
                    ...
                    message = f"Authentication failed: Status {res.status}"
                    logging.warning(message)
                    logging.info(f"Response: {await res.text()}")
                    raise aiohttp.ClientResponseError(res.request_info, res.history, status=res.status, message=message)
    ...
    def find_dp(self, root, fp_name: str, dp_name: str):
        ...
        if fp:
            ...
            if dp:
                ...
                error_message = f"Datapoint '{dp_name}' not found in functional profile '{fp_name}'."
                logging.error(error_message)
                raise ValueError(error_message)
        else:
            error_message = f"Functional profile '{fp_name}' not found."
            logging.error(error_message)
            raise ValueError(error_message)
    ...
    async def getval(self, fp_name, dp_name):
        try:
            ...
            if not dp:
                error_message = f"Data point for {fp_name}, {dp_name} not found"
                raise ValueError(error_message)
