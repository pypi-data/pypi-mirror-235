...
task_dict = {
        ...
        "handdetect":"checkpoints/handdetect.onnx",
        "facedetect":"...",
        "ocr":"...",
}
class Workflow:
    ...
    def __init__(self, task,checkpoint=None):
        ...
        if checkpoint is None and self.task in['bodydetect','cocodetect','body17','body26','wholebody133','face106','hand21','handdetect']: # 若不指定权重文件，则使用对应任务的默认模型
    ...
    def inference(self,data=None,show=False,img_type=None,thr=0.3,bbox=None,target_class=None,erase=False):
        ...
        if self.task in ["body17","body26","face106","hand21","wholebody133"]:
        ...
        elif self.task in['bodydetect','cocodetect','handdetect']:
            return self._det_infer(data,show,img_type,thr,target_class)
        elif self.task in ['facedetect']:
            ...
            return self._mmedu_infer(data,show,img_type)
        elif self.task in ['basenn']:
            self.ix2word = None
            return self._basenn_infer(data)
            
    def _basenn_infer(self,data=None):
    ...
    def _mmedu_infer(self,data=None,show=False,img_type=None):
        if img_type is None:
            self.result = self.model.inference(data, show=show,get_img=img_type)
            return self.result
        else:
            self.result,img = self.model.inference(data,get_img=img_type)
            if show ==True:
                self.show(img)
