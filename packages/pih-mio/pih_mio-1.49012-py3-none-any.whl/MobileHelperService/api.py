...
from pih.tools import BitMask as BM, i, b, nl, j, if_else, js, esc, while_not_do, nn, ne, e
...
YES_LABEL: str = js(
    ("", A.CT.VISUAL.BULLET, b("Да"), "-", "отправьте", A.CT.VISUAL.NUMBER_SYMBOLS[1])
)
NO_LABEL: str = js(
    ("", A.CT.VISUAL.BULLET, b("Нет"), "-", "отправьте", A.CT.VISUAL.NUMBER_SYMBOLS[0])
)
...
class MobileOutput(Output):
    ...
    def good(self, caption: str) -> None:
        with self.personalized(False):
            self.write_line(nl(js((A.CT_V.GOOD, caption))))

    def error(self, caption: str) -> None:
        with self.personalized(False):
            self.write_line(nl(js((A.CT_V.ERROR, caption))))
    ...
    def table_with_caption(
        ...
        if caption is not None:
            self.write_line(caption + A.CT.NEW_LINE)
...
class MobileInput(Input):
    ...
    def index(
        ...
        def internal_label_function(item: Any, index: int) -> str:
            return j((label_function(item, index), nl(nl(ConsoleAppsApi.LINE), reversed=True) if index == len(data) - 1 else ""))
        return super().index(
            caption,
            data,
            label_function,
...
class MobileHelper:
    ...
    def __init__(self, pih: PIH, stdin: Stdin):
        ...
        NOTES_MENU: list[CommandNode] = [create_note_node, self.show_note_node, self.show_all_note_node]
        #######################
        ...
        check_material_resources_node: CommandNode = CommandNode(
            ...
            lambda: [
                ...
                "Проверить материальные ресурсы" if self.in_choice_command else "Материальных ресурсов",
            ],
            lambda: self.check_resources_and_indications_handler(
        ...
        def get_note_title() -> list[str]:
            ...
            gkeep_item: GKeepItem | None = A.R.get_first_item(A.R_N.find_gkeep_item(value, True))
            if A.D.is_empty(gkeep_item):
        ...
        self.note_node: CommandNode = CommandNode(
            ...
            text_decoration_before="📝 ",
            show_in_main_menu=True,
            filter_function=lambda: self.argless or A.C_N.exists(self.arg(), False),
            help_text=lambda: js((" (", i("Название заметки"), ")")),
        )
        ...
        self.command_node_tree = {
            ...
            CommandNode(
                ...
                or not A.D.is_empty(A.D_ACT.get(self.arg())),
            ): None,
            CommandNode(
                ["unit", "отдел"],
                ["Отделы"],
    ...
    def check_email_address_handler(
        ...
        try:
            ...
            if ne(polibase_person):
                polibase_person_string = j(("клиента ", b(polibase_person.FullName), ":"))
            if not only_for_polibase_person:
                if ne(value):
                    if A.C.email(value):
                        result: bool = A.C.EMAIL.accessability(value)
                        self.output.separated_line()
                        text: str = js(("Адресс электронной почты", polibase_person_string, b(value), j(("" if result else "не", "доступен"))))
                        if result:
                            self.output.good(
                               text 
                            )
    ...
    def robocopy_job_run_handler(self) -> None:
        ...
        if (
            ...
            def job_status_list_label_function(name: str) -> str:
                ...
                return nl(j(
                    list(map(job_status_item_label_function, job_list)), nl()
                ), reversed=True)
    ...
    def find_note_handler(self, root: bool, show_all: bool = False) -> None:
        value: str | None = self.arg()
        has_input_value: bool = not A.D.is_empty(value)
        if not self.is_all and not show_all:
            value = value or self.input.input(
                "Введите название или заголовок заметки"
            )
        full_equaliment: bool = A.C_N.exists(value, True) if has_input_value else True
        if full_equaliment or A.C_N.exists(value, False):

            def label_function(item: GKeepItem, note: Note) -> str:
                text: str = A.D_F.format(note.text)
                if ne(note.title) and item.title.lower() != note.title.lower():
                    return j((b(note.title), nl(count=2), text))
                return text

            gkeep_item_list_result: Result[list[GKeepItem]] = A.R_N.find_gkeep_item(
                value, full_equaliment
            )
            if A.R.is_empty(gkeep_item_list_result):
                ...
                for gkeep_item in gkeep_item_list:
                    ...
                    self.output.write_result(note_result, label_function=lambda note, _: label_function(gkeep_item, note), title= None if len(gkeep_item_list) == 1 else b(gkeep_item.title.upper()))
                    if ne(note.images):
                        self.write_line(
                ...
                if self.yes_no(
                    j(
                        (
                            ...
                            ""
                            if A.D.is_empty(action_description.parameters_description)
                            else j(
    ...
    def create_note_handler(self, name: str | None = None, title: str | None = None, text: str | None = None) -> str | None:
    ...
    def time_tracking_report_handler(self, for_me_report_only: bool = False) -> None:
        ...
        if A.A_TT.save_report(
            ...
            if allowed_report_for_all_persons
                ...
                A.R_M.by_name(self.session.user.name)
            ).data,
    ...
    def create_timestamp_handler(self) -> None:
        ...
        while True:
            ...
            if variable_is_exists:
                ...
                def label_function(
                    item: StorageVariableHolder, _=None
                ) -> str:
                    return j(
                            [b(item.key_name)] + [
                            ""
                            if A.D.is_empty(item.description)
                ...
                if self.yes_no(
                    ...
                    if self.yes_no("Использовать текущую дату и время", no_label = js((b("Ввести дату и время самостоятельно"),  "-", "отправьте", A.CT_V.NUMBER_SYMBOLS[0]))):
                        A.D_V_T.update(timestamp_holder)
                        if A.C_V_T_E.exists_by_timestamp(timestamp_holder.key_name):
                            return
            else:
                timestamp_description = timestamp_description or self.input.input("Введите описание временной метки")
                A.D_V_T.set(timestamp_name, None if self.yes_no("Использовать текущую дату и время", no_label = js((b("Ввести дату и время самостоятельно"), "-", "отправьте", A.CT_V.NUMBER_SYMBOLS[0]))) else self.input.datetime(use_time=None, ask_time_input=False), timestamp_description)
                self.output.good("Временная метка создана")
                break
        note_name: str | None = None
        if self.yes_no("Создать заметку для временной метки"):
            self.output.write_line("Создание заметки для временной метки")
            with self.output.make_indent(2):
                note_name = self.create_note_handler(timestamp_name, "", j((timestamp_description, ": ", "{", A.D.get(A.D_V.SECTIONS.TIMESTAMP), ".", timestamp_name, "}")))
            if A.D.is_none(note_name):
                self.output.error("Заметка не создана")
            else:
                self.output.good("Заметка создана")
        if self.yes_no("Сделать временную метку истекающей"):
            self.output.write_line("Создание истекающей временной метки")
            life_time_holder: StorageVariableHolder | None = None
            expired_timestamp_name: str = timestamp_name
            with self.output.make_indent(2):
                life_time_holder = self.input.item_by_index("Выберите продолжительность (в месяцах)", A.D_V.find("life_time") + A.D_V.find("duration"), lambda item, _: j((item.description, " (", item.key_name, "): ", item.default_value, " (месяцев)")))
                if A.C_V_T_E.exists(expired_timestamp_name):
                    while True:
                        if A.C_V_T_E.exists(expired_timestamp_name := self.input.input("Введите название истекающей временной метки")):
                            self.output.error("Истекающая временная метка с названием \"{expired_timestamp_variable_name}\" уже существет")
            A.D_V_T_E.set(expired_timestamp_name, timestamp_description, timestamp_name, life_time_holder.key_name, note_name)
    ...
    def about_it_handler(self) -> None:
        ...
        def label_function(user: User, index: int) -> str:
            ...
            if ne(user.description):
                user_description_list: list[str] = A.D_F.description_list(user.description)
    ...
    def show_all_free_marks_handler(self) -> None:
        sort_by_tab_number: bool = self.yes_no(
            "Как отсортировать",
            js((b("по табельному номеру"), "-", "отправьте", A.CT_V.NUMBER_SYMBOLS[1])),
            js((b("по названию группы доступа"), "-", "отправьте", A.CT_V.NUMBER_SYMBOLS[0])),
        )
        ...
        with self.output.personalized():
            variable_name: str | None = (
                ...
                if self.is_all and (not get_action or self.argless)
                    ...
                    self.arg()
                    or self.input.input(
                        "Введите название или часть названия переменной"
                    )
                )
            )
            ...
            def sort_function(item: A.CT_S | A.CT_MR.TYPES | StorageVariableHolder) -> int:
            ...
            try:
                ...
                if A.C_P.person_card_registry_folder(value):
                    ...
                    if (
                        ...
                        and A.D.is_empty(person_list_result.data[0].ChartFolder)
                        and self.yes_no(
    ...
    def add_mri_log_handler(self) -> None:
        ...
        while True:
            ...
            with self.output.make_exit_line(
                ...
                while True:
                    timestamp = timestamp.replace(day=int(self.input.input("Введите число")))
                        #while True:
                            #try:
    ...
    def create_command_list(self) -> list[list[CommandNode]]:
        ...
        if MobileHelper.command_node_name_list is None:
            ...
            MobileHelper.command_node_name_list = (
                list(
                    filter(
                        lambda item: ne(item), list(command_node_name_set)
                    )
                )
                + COMMAND_KEYWORDS.EXIT
            )
