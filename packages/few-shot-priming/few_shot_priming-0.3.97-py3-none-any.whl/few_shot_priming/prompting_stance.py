...
def format_examples(dataset, tokenizer, labels_text_map, token_limit, logger):
    ...
    for i, train_record in dataset.iterrows():
        ...
        log_message(logger, f"The argument with the {id} has the following token limit {token_limit} for", level=logging.INFO)
        ...
        log_message(logger, template + f"""On the topic '{topic}' the argument '{sentence}' has the stance {label[idx]}\t
                                       f"id:{id}-token-limit{token_limit}""", level=logging.INFO)
