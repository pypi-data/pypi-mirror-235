...
class DataCatalogClient:
    ...
    def export(self, format=".csv") -> None:
        if len(self._catalog) == 0 and self._sources_list_path:
            self._catalog = self.parse_from_file()
    ...
    # def mask_credentials_postgres(self, connection_string: str):
    #     # Parse the connection string
    #     parsed = urllib.parse.urlparse(connection_string)

    #     # Replace the username and password with asterisks
    #     masked_username = "****"
    #     masked_password = "****"

    #     masked_netloc = parsed.netloc.replace(parsed.username, masked_username)
    #     if parsed.password:
    #         masked_netloc = masked_netloc.replace(f":{parsed.password}@", f":{masked_password}@")

    #     masked_connection_string = urllib.parse.urlunparse((
    #         parsed.scheme,
    #         masked_netloc,
    #         parsed.path,
    #         parsed.params,
    ...
    #             if env_value:
    #                 connection_string_unmasked = connection_string_unmasked.replace('{' + placeholder + '}', env_value)
    ...
    def export_metaframes_summary_excel(
        ...
        writer = pd.ExcelWriter(target_full_path, engine='xlsxwriter')
