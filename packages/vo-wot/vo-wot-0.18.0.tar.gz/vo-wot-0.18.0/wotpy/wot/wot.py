...
class WoT:
    ...
    def _build_dnssd_discover_observable(self, thing_filter, dnssd_find_kwargs):
        ...
        def subscribe(observer, scheduler):
            ...
            async def callback():
                address_port_pairs = await self._servient.dnssd.find(**dnssd_find_kwargs)
                ...
                for idx, future in enumerate(catalogue_resps):
                    if state["stop"]:
                        return
                    try:
                        catalogue_resp = await future
                        ...
                        td_resps = [
                            await http_client.fetch(build_pair_url(
