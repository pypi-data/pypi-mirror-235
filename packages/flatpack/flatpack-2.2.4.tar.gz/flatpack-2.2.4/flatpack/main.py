...
def fpk_log_to_api(message_with_colors: str, message_without_colors: str, model_name: str = "YOUR_MODEL_NAME"):
    ...
    data = {
        ...
        "log_message_with_colors": message_with_colors,
        "log_message_without_colors": message_without_colors
    }
...
def fpk_log_session(message_with_colors: str, message_without_colors: str):
    session_file_path = os.path.join(os.getcwd(), 'fpk_session.log')
    with open(session_file_path, 'a') as f:
        formatted_date = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"{formatted_date}: {message_without_colors.strip()}\n")

        # Print the message with colors to the terminal
        print(message_with_colors)
...
def main():
    ...
    if command == "callback":
        ...
        # Log the session message without colors
        fpk_log_session(args.input, args.input)
    else:
