...
def fpk_train(directory_name: str = None):
    ...
    if pid == 0:
        ...
        try:
            buffered_output = ""  # Buffer to store partial lines
            ...
            while True:
                ...
                if master in rlist:
                    ...
                    while '\n' in buffered_output or '\r' in buffered_output:
                        if '\n' in buffered_output:
                            line, buffered_output = buffered_output.split('\n', 1)
                            line += '\n'
                        else:  # '\r' in buffered_output
                            line, buffered_output = buffered_output.split('\r', 1)
                            line += '\r'

                        # Check if it matches the last user input, if so, skip
                        if last_user_input and line.strip() == last_user_input:
                            continue

                        if line.strip() and (line != last_printed or line.endswith('\r')):
                            print(f"(*) {line}", end='')  # Using end='' to prevent adding another newline
