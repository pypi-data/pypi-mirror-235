...
def get_sas_token(login_config : LoginConfig, storage_account_name : str, storage_container_name : str, expiration_date : str=(datetime.utcnow() + timedelta(days=7)).date().strftime("%Y-%m-%dT%H:%M:%SZ")):
    if login_config and storage_account_name and storage_container_name:
        ...
        az_cli_command = "az storage container generate-sas --account-name " + storage_account_name + " --name " + storage_container_name + " --permissions acdlrw --start " + start_date + " --expiry " + expiration_date + " --https-only --auth-mode login --as-user"
...
def upload_blob_to_storage_account(login_config : LoginConfig, 
    ...
    if login_config and storage_account_name and storage_account_container and blob_name and local_filepath:
        ...
        az_cli_command = "az storage blob show --auth-mode login --account-name " + storage_account_name + " --container-name " + storage_account_container + " --name " + blob_name
        try:
            output = json.loads(sanitize_stdout_for_json(send_os_command(az_cli_command, is_secret=True)))
        ...
        if upload_to_azure:
            if os.path.exists(local_filepath):
                ...
                if blob_url is not None:
                    ...
                    output = send_os_command(azcopy_command, shell=True)
