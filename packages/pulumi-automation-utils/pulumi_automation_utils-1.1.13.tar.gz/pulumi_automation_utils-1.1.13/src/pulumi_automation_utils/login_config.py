            ...
            if self.get_login_mode() is LoginMode.INTERACTIVE:
                ...
                try:
                    ...
                    if self.get_subscription() is None:
                        ...
                        json.loads(common.sanitize_stdout_for_json(common.send_os_command("az account set --subscription " + self.subscription)))
                    
                    json.loads(common.sanitize_stdout_for_json(common.send_os_command(("az cloud set --name " + self.cloud_name))))
                    self.logged_in = True
                except subprocess.CalledProcessError as e:
                    print("command '{}' return with error (code {}): {}".format(e.cmd, e.returncode, e.output))
                    exit()
