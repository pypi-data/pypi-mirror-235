...
class EsRequest:
    ...
    def __init__(self, host, port, user, password):
        ...
        self.auth = request.get_basic_auth(user, password)
        host_str = host.split(",")
    ...
    def es_selector_way(self, url_func_str, param_dict, find_condition=None, request_way="post"):
        ...
        for host in self.hosts:
            ...
            try:
                if request_way == "get":
                    res = request.sync_common_get(url, params=find_condition, result_type="json", auth=self.auth)
                elif request_way == "post":
                    res = request.sync_common_post(url, json=find_condition, result_type="json", auth=self.auth)
                if res:
                    if res.get("took"):
                        break
            except:
