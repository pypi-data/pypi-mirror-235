...
class UtilizationReporterThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while self._container and self._container.status not in ('exited',):
            try:
                ...
                logger_no_user_data.error(f'Could not retrieve utilization metrics due to: {error}')
    ...
    def _report_aggregated_utilization_metric(self, metric_samples: List[UtilizationMetricSample]) -> None:
        ...
        aggregated_utilization_metrics = self._get_aggregated_utilization_metric_from_metric_samples(metric_samples)
        ...
        response = biolib.api.client.post(
            ...
            data=cast(Dict, aggregated_utilization_metrics),
        )
