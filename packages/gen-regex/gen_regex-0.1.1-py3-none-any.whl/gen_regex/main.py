...
class RegexGenerator:
    ...
    def run(self, text,tokens : list[Token] ) -> None:
        ...
        for token in tokens:
            ...
            idx = token.idx
            left_idx =  idx -1 if idx> 0 else -1
            right_idx =  idx +1 if idx + 1 < len(tokens) else -1
            
            left_token = tokens[left_idx]
            right_token = tokens[right_idx]

            left_re = "^" if left_idx ==-1 else self.gen_regex(left_token.text,is_full=False)
            right_re = '$' if right_idx == -1 else  self.gen_regex(right_token.text,is_full=False)
...
def test_tokenize( ):
    # test tokenize
    text = 'ねじ込み23/8、M10×10mm'
    elements = ['ねじ込み', '23/8', 'M10', '10']
