...
class Api:
    ...
    async def send_image(self, room_id: str, 
        ...
        if not file.is_image(): 
            return error(f"{filepath} is not image!")
        ...
        try:
            ...
            error(f"Failed to send image file {filepath}")
    ...
    async def send_video(self, room_id: str,
        ...
        if not file.is_video(): 
            return error(f"{filepath} is not video!")
        content = await file.upload(self.async_client)
        if content is None: return
        ...
        try:
            ...
            error(f"Failed to send video file {filepath}")
