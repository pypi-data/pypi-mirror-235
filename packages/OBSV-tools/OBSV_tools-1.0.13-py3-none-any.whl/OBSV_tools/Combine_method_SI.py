...
def run():
    ...
    if len(sys.argv) == 1:
        ...
        if os.path.exists(os.path.join(current_dir,method_ds[0],'pymol','M3_total_0.png')):
            figns_M3 = ['M2_total_%d'%frameid for frameid in frameslist]
            figns_M3.extend(['M2_bond_%d'%frameid for frameid in frameslist])
            figns_M3.extend(['M2_angle_%d'%frameid for frameid in frameslist])
            figns_M3.extend(['M2_frag_%d'%frameid for frameid in frameslist])
            
            figls_M3 = ['M2 total\n \n %s'%frameslabel for frameslabel in frameslabestr]
            figls_M3.extend(['M2 bond\n \n %s'%frameslabel for frameslabel in frameslabestr])
            figls_M3.extend(['M2 angle\n \n %s'%frameslabel for frameslabel in frameslabestr])
            figls_M3.extend(['M2 frag\n \n %s'%frameslabel for frameslabel in frameslabestr])
