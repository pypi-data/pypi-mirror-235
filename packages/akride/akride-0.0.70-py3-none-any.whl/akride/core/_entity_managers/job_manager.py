...
class JobManager(Manager):
        ...
        for point_info in catalog_tag_resp.data:
            ...
            for tag in point_info.tags:
                point_tags[point_id] = self._get_column_val(
                    tag, column_of_interest=["file_path"]
                ).get("file_path")
        return point_tags
    ...
    def get_similar_samples(
        ...
        if True:
            ...
            # these are from job DEMO-BDD-IMAGE-EXPLORE-HBDSCAN-UMAP
            # on the staging server
            pos_point_ids = [116, 251, 225]
            neg_point_ids = [97, 212, 219]
        else:
            # TODO: map paths to point_ids
            pos_point_ids = self.client._get_samples_from_fullres_paths(
                positive_samples
            )
            neg_point_ids = self.client._get_samples_from_fullres_paths(
                negative_samples
            )
        pos_points_list = [
            {"point": point_id, "weight": 1} for point_id in pos_point_ids
        ]
        neg_points_list = [
            {"point": point_id, "weight": 0} for point_id in neg_point_ids
        ]
    ...
    def get_samples_from_file_path(
        ...
        _: GetDatasetRequestResponse = self.job_request_api.get_request_one(
            job_id
        )  # type: ignore
        catalog_filter = CatalogSourceFilter(
            column_name="file_path", column_values=file_list, sql_operator="in"
        )
