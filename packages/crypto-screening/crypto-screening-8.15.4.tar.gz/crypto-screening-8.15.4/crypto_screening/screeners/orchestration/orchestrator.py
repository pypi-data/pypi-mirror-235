...
__all__ = [
    ...
    "connect_screening_orchestration",
    "connect_screening_orchestrator",
...
def create_screening_orchestration(
        ...
        control: Optional[bool] = False,
        run: Optional[bool] = False
) -> Dict[SocketScreenersDataCollector, List[DataPublisherClient]]:
    """
    Creates the market screener object for the data.
    ...
    return connect_screening_orchestration(
        collector=collector,
        method=method,
        cancel=cancel,
        delay=delay,
...
def connect_screening_orchestration(
        collector: SocketScreenersDataCollector,
        method: OrchestrationMethod,
        cancel: Optional[Union[float, dt.timedelta]] = None,
        delay: Optional[Union[float, dt.timedelta]] = None,
    ...
    if method == OrchestrationMethod.ALL:
        ...
        for category, category_data in data.items():
            port = find_available_port(address)
            process = create({category: category_data}, port)
        ...
        for exchange in collector.structure().keys():
            port = find_available_port(address)
            process = create(
                {
        ...
        for category, category_data in data.items():
            for exchange, exchange_data in category_data.items():
                port = find_available_port(address)
                process = create(
                    {category: {exchange: exchange_data}}, port
                )
    ...
    for process, port in processes:
        ...
        collectors.append(
            DataPublisherClient(client=client, process=process)
        )
    # end for

    return {collector: collectors}
# end connect_screening_orchestration
...
class Orchestrator:
    ...
    def connect(
        ...
        controllers = connect_screening_orchestration(
            collector=collector,
            method=method,
            cancel=cancel,
            delay=delay,
...
# end create_screening_orchestrator

def connect_screening_orchestrator(
        collector: SocketScreenersDataCollector,
        method: OrchestrationMethod,
        cancel: Optional[Union[float, dt.timedelta]] = None,
        delay: Optional[Union[float, dt.timedelta]] = None,
...
# end connect_screening_orchestrator