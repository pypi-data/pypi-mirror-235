            ...
            for d in found[0:10]:
                ...
                for cpk in lb["mains"][d["character"]]:
                    ...
                    for dm in c.get_curmonth_daily_mining():
                        if str(dm.date) not in linegraph[d["character"]]:
                            linegraph[d["character"]][str(dm.date)] = 0
                        linegraph[d["character"]][str(dm.date)] += dm.taxed_value
        for c in linegraph.keys():
            final = []
            cumisk = 0
            for d, isk in sorted(linegraph[d["character"]].items()):
                cumisk += isk
