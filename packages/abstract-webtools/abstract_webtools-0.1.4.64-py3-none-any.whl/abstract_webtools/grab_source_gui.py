...
import inspect
import re
from abstract_webtools import UserAgentManager,UrlManager,SafeRequest,SoupManager,LinkManager
...
def get_cipher_manager(cipher_list=None):
  return CipherManager(cipher_list=cipher_list)
def get_url():
  if window is None:
    return 'example.com'
...
def get_parser_choices():
    ...
    start_index = docstring.find("parsers")
...
def process_url(window,values):
    ...
    try:
      ...
      self.request_manager.update_url(url=url_manager.url)
      self.soup_manager.update_url(url=url)
      if parse_type != self.soup_manager.parse_type:
        soup_manager.update_parse_type(parse_type=parse_type)
      self.agent_manager = get_user_agent_manager(user_agent=user_agent)
