...
class Hub(APIResource):
    ...
    def download(self, **kwargs):
        ...
        for key in ['filename', 'repo_id', 'revision', 'cache_dir']:
            if key in kwargs:
                if type(kwargs[key]) != str:
                    return err_resp(err_code.ERR_PARAMS_INVALID, 
                                    str_params_not_valid)
        ...
        # 查询文件sha值
        info_res = request_aistudio_git_file_info(call_host, user_name, repo_name, file_path, revision)
    ...
    def upload(self, **kwargs):
        ...
        for key in ['path_or_fileobj', 'repo_id', 'revision', 'path_in_repo', 'commit_message']:
            if key in kwargs:
                if type(kwargs[key]) != str:
                    return err_resp(err_code.ERR_PARAMS_INVALID, 
                                    str_params_not_valid)
        ...
        res = request_aistudio_git_file_type(call_host, user_name, repo_name, revision, path_in_repo)
        ...
        if is_lfs:
            ...
            res = request_aistudio_git_upload_access(call_host, user_name, repo_name, revision, file_size, sha256)
            if 'error_code' in res and res['error_code'] != err_code.ERR_OK:
                return res
            ...
            if res['upload_href']:
                ...
                # 仓库中该文件已存在。只需要再创建一次指针文件到指定分支即可。
                print("The file already exists in the remote repository.")
