...
class hmp:
    ...
    def EM(self, magnitudes, parameters, maximization=True, magnitudes_to_fix=None, parameters_to_fix=None, max_iteration=1e3, tolerance=1e-4, min_iteration=1, mags_map=None, pars_map=None,conds=None, cpus=1):  
        ...
        if not maximization or n_events==0:
            ...
            while i < max_iteration :#Expectation-Maximization algorithm
                print(lkh)
                print(np.round(parameters[:,1],decimals=1))
    ...
    def fit(self, step=1, verbose=True, end=None, trace=False, fix_iter=True, max_iterations=1e3, tolerance=1e-4, grid_points=1, cpus=None, diagnostic=False, min_iteration=1, decimate=None):
        ...
        n_points = int(np.rint(end)//step)-1
        if diagnostic:
        ...
        #Init pars
        pars = np.zeros((max_event_n,2))
        pars[:,0] = self.shape #shape parameters during estimation, shape x scale
        ...
        while self.scale_to_mean(last_stage,self.shape) > self.location and n_events < max_event_n:
            ...
            mags_props[:,:n_events,:] = np.tile(mags[:n_events,:].copy(), (len(mags_props), 1, 1))
            ...
            print(lkh_prev)
            print(pars[:n_events])
            print(solutions.likelihoods)
            lkh = solutions.likelihoods.values[0]
            print(lkh)
            solutions = solutions#utils.filter_non_converged(solutions)
            # nearest_solution = solutions.sel(iteration=solutions.parameters.sel(parameter="scale", \
            #     stage=n_events).argmin('iteration').values)
            mags[:n_events], pars[:n_events+2] = solutions.magnitudes.values[:,:-1].copy(),\
                solutions.parameters.values.copy()
            print(pars[:n_events+2] )
            print(self.scale_to_mean(pars[:n_events+1,1].sum(), self.shape))
            print(np.sum(self.scale_to_mean(pars[:n_events+1,1], self.shape)))
            if len(solutions.iteration) > 0 and lkh > lkh_prev:#Success
                ...
                lkh_prev = lkh
                pars_prop = pars[:n_events+2].copy()
                pars_prop[n_events,1] = self.mean_to_scale(step, self.shape)
                # pars_prop[n_events-1,1] -= 1
            else:
                pars_prop = pars[:n_events+2].copy()
            pars_prop[n_events,1] += self.mean_to_scale(step+1, self.shape)
            j += 1
            i += 1
            #New parameter proposition
            # pars_prop[n_events,1] = self.mean_to_scale(step*j, self.shape)
            last_stage = self.mean_to_scale(end - self.scale_to_mean(np.sum(pars_prop[:n_events+1,1]),self.shape),self.shape)
            pars_prop[n_events+1,1] = last_stage
            time = int(np.round(np.sum(self.scale_to_mean(pars[:n_events,1], self.shape))))
