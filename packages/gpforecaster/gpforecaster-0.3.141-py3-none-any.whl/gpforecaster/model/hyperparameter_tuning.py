...
def single_trial(
    ...
    try:
        ...
        val_loss = np.mean(gpf.loss_tracker.data.val.weighted)
        test_loss = np.mean(gpf.loss_tracker.data.test.weighted)
...
def log_and_print_best_hyperparameters(
    ...
    logger_tuning.info(
        f"\t\tValidation loss: {np.round(best_val_loss, 3)} "
        f"Test loss: {np.round(best_test_loss)}"
    )
