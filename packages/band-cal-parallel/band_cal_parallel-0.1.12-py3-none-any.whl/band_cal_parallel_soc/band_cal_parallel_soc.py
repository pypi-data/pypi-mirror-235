...
LastEditTime: 2023-06-07 16:11:25
'''
...
def main():
    ...
    parser.add_argument('--config', default='band_cal.yaml', nargs='+', type=str, metavar='N')
    ...
    for ik in range(nk):
        ...
        if rank == 0:
            ...
            lamda = 1/np.sqrt(abs(lamda)) # shape: (nb_pick,)
            wfn = wfn*lamda[:,None]
    ...
    if rank == 0:
        ...
        eigen = eigen[num_electrons-num_wfns-1:num_electrons+num_wfns]
