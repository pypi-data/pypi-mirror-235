...
class Chat:
    ...
    def close(self, chat_id: int, outcome: str = "", team_id: int = None) -> bool:
    ...
    def onhold(self, chat_id: int, team_id: int = None) -> bool:
        if chat := self.parent.rest.patch(f"/v3.0/chats/{chat_id}",
                                        headers={"teamid": str(team_id if team_id else self.parent.team.id)},
    ...
    def queued(self, chat_id: int, team_id: int = None) -> bool:
        if chat := self.parent.rest.patch(f"/v3.0/chats/{chat_id}",
                                        headers={"teamid": str(team_id if team_id else self.parent.team.id)},
    ...
    def picked(self, chat_id: int, , agent_id: str, team_id: int = None) -> bool:
        if chat := self.parent.rest.patch(f"/v3.0/chats/{chat_id}",
                                        headers={"teamid": str(team_id if team_id else self.parent.team.id)},
    ...
    def update(self, chat_id: int, update_data: dict = {}, team_id: int = None) -> dict:
        if chat := self.parent.rest.patch(f"/v3.0/chats/{chat_id}",
                                        headers={"teamid": str(team_id if team_id else self.parent.team.id)},
                                        params=update_data):
            return chat
    ...
    def alias(self, chat_id: int, alias: str, team_id: int = None) -> dict:
        if chat := self.parent.rest.patch(f"/v3.0/chats/{chat_id}",
                                        headers={"teamid": str(team_id if team_id else self.parent.team.id)},
    ...
    def tag(self, chat_id: int, tags: list = [], team_id: int = None) -> dict:
        if chat := self.parent.rest.patch(f"/v3.0/chats/{chat_id}",
                                        headers={"teamid": str(team_id if team_id else self.parent.team.id)},
