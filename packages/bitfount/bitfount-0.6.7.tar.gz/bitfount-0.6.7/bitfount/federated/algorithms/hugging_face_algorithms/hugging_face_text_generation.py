...
class _WorkerSide(BaseWorkerAlgorithm):
    ...
    Raises:
        ValueError: If the `model_id` does not match a Peft Config or
    ...
    def initialise(
        ...
        **kwargs: Any,
        ...
        if self._is_peft_config(self.model_id):
            logger.debug("PEFT Config detected")
            self._initialise_peft()
        elif self._is_causal_model(self.model_id):
            logger.debug("Causal Model detected")
            self._initialise_causal_model()
        else:
            raise ValueError(
                f"Couldn't load model_id ({self.model_id}) as either"
                "a Peft Config or a Causal Language Model"
            )

        self.eos_token_id = self.eos_token_id or self.tokenizer.eos_token_id
        self.pad_token_id = self.pad_token_id or self.tokenizer.pad_token_id

    def _is_peft_config(self, pretrained_model_name_or_path: str) -> bool:
        """Check given input is compatible Peft Config.

        Args:
            pretrained_model_name_or_path (`str`):
                The directory or the Hub repository id where the
                peft configuration is saved.

        Returns:
            bool: If the given input can be loaded as Peft Config
        """
        try:
    ...
    def _is_causal_model(self, pretrained_model_name_or_path: str) -> bool:
        """Check given input is compatible Causal Language Model.

        Args:
            pretrained_model_name_or_path (`str`):
                The directory or the Hub repository id where the
                causal language model is saved.

        Returns:
            bool: If the given input can be loaded as Causal Language Model.
        """
        try:
    ...
    def _initialise_peft(self) -> None:
        """Initialises the peft model and tokenizer."""
    ...
    def _initialise_causal_model(self) -> None:
        """Initialises the causal model and tokenizer."""
        self.tokenizer = AutoTokenizer.from_pretrained(
    ...
    def _generate_peft_output(
        self, inputs: Union[str, List[str]]
    ) -> TextGenerationDefaultReturnType:
        """Generate output for Peft Model."""
        results: TextGenerationDefaultReturnType = []

        for entry in inputs:
            tensors = self.tokenizer(entry, return_tensors="pt")
            with torch.no_grad():
                output = self.model.generate(
                    input_ids=tensors["input_ids"], max_new_tokens=self.max_length
                )
            results.append(
                [
                    {
    ...
    def _generate_causal_output(
        self, inputs: Union[str, List[str]]
    ) -> TextGenerationDefaultReturnType:
        """Generate output for Causal Model."""
        results: TextGenerationDefaultReturnType
        generator = pipeline(
            "text-generation",
            model=self.model,
    ...
    def _generate_output(
        ...
        """Generate output for Causal or Peft Model."""
        if isinstance(self.model, PeftModel):
            return self._generate_peft_output(inputs)

        return self._generate_causal_output(inputs)
    ...
    def run(
        ...
        if prompt and self.text_column_name:
            ...
            inputs = self.datasource.get_column(self.text_column_name).tolist()
            if self.prompt_format:
                inputs = [
                    self.prompt_format.format_map({"context": text}) for text in inputs
                ]
            results_list = []
            for result in self._generate_output(inputs=inputs):
                results_list.append(result[0]["generated_text"])
            return pd.DataFrame({"results": results_list})
        elif prompt:
            return self._generate_output(inputs=[prompt])
...
class HuggingFaceTextGenerationInference(BaseAlgorithmFactory):
    ...
    Args:
        model_id: The model id to use for text generation.
            ...
            modeling head.
        model_revision: The specific model version to use. It can be a branch name,
                a tag name, or a commit id, since we use a git-based system for
                storing models and other artifacts on huggingface.co,
    ...
    fields_dict: ClassVar[T_FIELDS_DICT] = {
        ...
        "model_revision": fields.Str(required=False, missing=DEFAULT_MODEL_REVISION),
