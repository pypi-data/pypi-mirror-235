...
iltrcli - The ideyaLabs TestRail CLI
================================
The ideyaLabs TestRail CLI (trcli) is a command line tool for interacting with TestRail. 
...
The ideyaLabs TestRail CLI currently supports:
...
you can simply run the command below in your terminal from the project root folder. We recommend using **Python 3.10** or newer.
```
...
```shell
$ iltrcli --help
ideyaLabs TestRail CLI v1.0.0
Usage: iltrcli [OPTIONS] COMMAND [ARGS]...
...
### Reference
```shell
$ iltrcli parse_junit --help
Usage: iltrcli parse_junit [OPTIONS]
...
To submit test case results, the ideyaLabs TestRail CLI will attempt to match the test cases in your automation suite to test cases in TestRail.
...
```
2. Map multiple case IDs in the test name. In this way one automation test script represents multiple test cases in test rail. If any one test script fails then all the test case IDs in test rail marked as failed. 

```xml
<testsuites name="test suites root">
  <testsuite failures="0" errors="0" skipped="1" tests="1" time="3049" name="tests.LoginTests">
    <testcase classname="tests.LoginTests" name="[C123], [C234], [C345] test_case_1" time="650" />
  </testsuite>
</testsuites>
```
> **Important usage notes:**
...
### Reference
```shell
$ iltrcli parse_openapi --help
ideyaLabs TestRail CLI v1.0.0

Usage: iltrcli parse_openapi [OPTIONS]
...
trcli allows users to upload test cases and results using multithreading. This is enabled by default and set to `MAX_WORKERS_ADD_CASE = 5` and
 `MAX_WORKERS_ADD_RESULTS = 10` in `iltrcli/settings.py`. To disable multithreading, set those to `1`.
