...
def phase_cross_correlation(reference_image, moving_image, *,
    ...
    if len(shift) != ndim:
        raise RuntimeError(
            f"{len(shift)=} prior to if upsample_factor, expected {ndim}")
    ...
    if len(shift) != ndim:
        raise RuntimeError(
            f"{len(shift)=} prior to if disambiguate, expected {ndim}")
    ...
    if disambiguate:
        ...
        if len(shift) != ndim:
            raise RuntimeError(
                f"{len(shift)=} after _disambiguate_shift, expected {ndim}")
