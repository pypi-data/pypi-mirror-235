...
// in 3D case "rows" will be the product of the size of the first two dimensions and "cols" is the size of the last dimension


extern "C" __global__
void cuRankFilterMultiBlock3D(unsigned char* src, unsigned char* dest, HIST_INT_T* histPar, HIST_INT_T* coarseHistGrid, int r0, int r1, int r2, int medPos_, int rsize0, rsize1, int cols)
{
    __shared__ HIST_INT_T HCoarse[HIST_SIZE_COARSE];
    __shared__ HIST_INT_T HCoarseScan[HIST_SIZE_FINE];
    __shared__ HIST_INT_T HFine[HIST_SIZE_COARSE][HIST_SIZE_FINE];
    ...
    // TODO: update these initNeeded conditions for 3D
    if(blockIdx.x==0){
        initNeeded=1; initVal=r0+2; initStartRow=1;  initStopRow=r0;
    }
    else if (startRow<(r0+2)){
   ...
   __syncthreads();
    ...
    // TODO: this initNeeded loop for 3D
    ...
    // TODO: this nested for loop for 3D
    ...
    for(int i=startRow; i< stopRow; i++){
        ...
        // 3D let i be an single index over the first two dimensions
        //    decompose it here into an i0 and i1 index over the first two axes, resepectively
        int i0 = i / rsize1;
        int i1 = i - i0 * rsize1;
        // if we use raveled indices like this then we can keep only 1 histogram per column.
        // Alternatively, we could keep "rows" as just axis 0, but then have to store space for prod(shape[1:])
        // histograms which is likely to be prohibitive.
        
        // Note: in 3D it will not be a single loop over cols with one possub and posadd each, but we have
        //       to also add an inner loop along 2*r1 + 1 offsets along i1 as well.

        // Computing some necessary indices
        int possub0=max(0,i0-r0-1),posadd0=min(rsize0-1,i0+r0);  // clamp at edges
        int histPos=threadIdx.x*HIST_SIZE;
        int histCoarsePos=threadIdx.x*HIST_SIZE_COARSE;
        // Going through all the elements of a specific row. Foeach histogram, (2 * r1 + 1) values are taken out
        // and (2*r1 + 1) values are added.
        int stride0 = cols * rsize1;
        int stride1 = cols;
        for (int ii1=i1 - r1; ii1 <= i1 + r1; ii1 ++) {
            int possub1=max(0,ii1),posadd1=min(rsize1-1,ii1);  // clamp at edges
            for (int i2=threadIdx.x; i2<cols; i2+=blockDim.x){

                hist[histPos+ src[possub0 * stride0 + possub1 * stride1 + i2] ]--;
                hist[histPos+ src[possub0 * stride0 + possub1 * stride1 + i2] ]++;
                histCoarse[histCoarsePos+ (src[possub0 * stride0 + possub1 * stride1 + i2]>>LOG2_FINE) ]--;
                histCoarse[histCoarsePos+ (src[possub0 * stride0 + possub1 * stride1 + i2]>>LOG2_FINE) ]++;
        ...
        histogramMultipleAddCoarse(HCoarse, histCoarse, 2*r2+1);
        int cols_m_1=cols-1;

        for(int j=r2;j<cols-r2;j++){
            int possub=max(j-r2, 0);
            int posadd=min(j+1+r2, cols_m_1);
            int medPos=medPos_;
            __syncthreads();
            ...
            {
                histogramClearFine(HFine[firstBin]);
                for ( loopIndex = j-r2; loopIndex < min(j+r2+1,cols); loopIndex++ ){
                    histogramAddFine(HFine[firstBin], hist+(loopIndex*HIST_SIZE+(firstBin<<LOG2_FINE) ) );
                }
            }
            else{
                for ( ; loopIndex < (j+r2+1);loopIndex++ ) {
                    histogramAddAndSubFine(HFine[firstBin],
                    hist+(min(loopIndex,cols_m_1)*HIST_SIZE+(firstBin<<LOG2_FINE) ),
                    hist+(max(loopIndex-2*r2-1,0)*HIST_SIZE+(firstBin<<LOG2_FINE) ) );
                    __syncthreads();
                }
            }
            __syncthreads();
