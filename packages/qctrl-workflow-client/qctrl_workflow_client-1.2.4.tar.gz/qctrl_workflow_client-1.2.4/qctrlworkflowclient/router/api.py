...
class ApiRouter(BaseRouter):
    ...
    def enable_parallel(self, callback: Optional[Callable[dict, dict]] = None):
        """
        Return a context manager to collect parallel tasks.

        Parameters
        ----------
        callback : Callable[dict, dict] or None, optional
...
class ParallelCollector:
    ...
    def __init__(self, router: ApiRouter, callback: Optional[Callable[dict, dict]]):
        self._router = router
        self._callback = (lambda x: x) if callback is None else callback
        self._async_results = []
