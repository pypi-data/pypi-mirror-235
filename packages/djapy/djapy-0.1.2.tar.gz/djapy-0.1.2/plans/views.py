...
from .types import TodoPathData, GetTest
...
@field_required
def todo_get(request, query: GetTest, *args, **kwargs):
    print("query.queue_name:  ", query.queue_range, type(query.queue_range))
...
def todo_detail(request, todo_id: int, *args, **kwargs):
    try:
        ...
        return create_response(
            'failed',
            'todo_not_found',
            f'The todo with the id of {todo_id} was not found.',
            field_type='number',
            field_name='id',
            field_value=todo_id
        )
