    ...
    def train(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for index, batch in enumerate(progress_bar):
                ...
                generated_images = self.generator((noise_vector, labels))
                generated_images = generated_images.to(self.device)

                D_fake_output = self.discriminator((generated_images.detach(), labels))
                D_fake_loss = torch.mean(D_fake_output)

                # Gradient penalty
                alpha = torch.rand(real_images.size(0), 1, 1, 1).to(self.device)
                x_hat = alpha * real_images.data + (1 - alpha) * generated_images.data
                ...
                G_output = self.discriminator((generated_images, labels))
