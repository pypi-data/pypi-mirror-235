...
class CRFTokenizer:
    ...
    def __init__(self,
        ...
        if not isinstance(huggingface_tokenizer, PreTrainedTokenizerFast):
            raise Exception(f"CRFTokenizer only supports PreTrainedTokenizerFast, {type(huggingface_tokenizer)} is invalid")
        self._tokenizer = huggingface_tokenizer
        self.token_len_features_names: Tuple[int] = ('offsets', 'mask', 'original_order')
    ...
    def __call__(
        ...
        **kwargs
        ...
        if 'padding' in kwargs and kwargs['padding'] == True:
            raise Exception("CRFTokenizer does not support padding")
        if 'is_split_into_words' in kwargs and kwargs['is_split_into_words'] == False:
        ...
        if not isinstance(tokens[0], List):
            raise Exception(f"Invalid input, tokenizer has to recieve a List[List[str]] but recieved: {tokens}")
    ...
    def prepare_batch(self,
        ...
        batch['original_order'] = torch.tensor([feature['original_order'] for feature in token_len_features], dtype=torch.int64)
