...
from cli.openai_calls import openai_call
import yaml
import textwrap
import numpy as np
...
def run_evaluation(file, approximate_cost):
    ...
    # Assign timeout and n_retries value for calls to openai.
    openai_call.openai_calls_variables(timeout, n_retries)
    ...
    if method == 'function_calling':
        evaluable_object = class_method(test_cases, prompts_value, functions, number_of_prompts, model_test, model_test_temperature, model_test_max_tokens, best_prompts, function_call)
    if method == 'Elo':
        evaluable_object = class_method(test_cases, prompts_value, description, number_of_prompts, model_test, model_test_temperature, model_test_max_tokens)

    if method == 'Semantic Similarity':
        evaluable_object = class_method(test_cases, prompts_value, number_of_prompts, model_test, model_test_temperature, model_test_max_tokens, model_embedding)

    if method != 'function_calling' and method != 'Elo' and method != 'Semantic Similarity':
        evaluable_object = class_method(test_cases, prompts_value, number_of_prompts, model_test, model_test_temperature, model_test_max_tokens, best_prompts)
    ...
    if method != 'Elo' and method != 'Semantic Similarity':
        ...
        while (iterations > 0 and (not all_have_rating_percentage)):
            ...
            combine_prompts = []
            if method == 'function_calling':
                new_results_prompts_cost = iteration.iterations(test_cases, method, old_prompts, functions, number_of_prompts - best_prompts, model_test, model_test_temperature, model_test_max_tokens, model_iteration, model_iteration_temperature, model_iteration_max_tokens, function_call, None, best_prompts)

            if method != 'function_calling' and method != 'Elo':
                new_results_prompts_cost = iteration.iterations(test_cases, method, old_prompts, None, number_of_prompts - best_prompts, model_test, model_test_temperature, model_test_max_tokens, model_iteration, model_iteration_temperature, model_iteration_max_tokens, None, None, best_prompts)
    ...
    if method == 'Semantic Similarity':
        ...
        while (iterations > 0):
            ...
            new_results_prompts_cost = iteration.iterations(test_cases, method, old_prompts, None, number_of_prompts - best_prompts, model_test, model_test_temperature, model_test_max_tokens, model_iteration, model_iteration_temperature, model_iteration_max_tokens, None, None, best_prompts, model_embedding)
            new_results = new_results_prompts_cost[0]
            cost = cost + new_results_prompts_cost[1]
            tokens_input_gen_iter = tokens_input_gen_iter + new_results_prompts_cost[2]
            tokens_output_gen_iter = tokens_output_gen_iter + new_results_prompts_cost[4]
    ...
    if method == 'Elo':
        ...
        while iterations > 0:
            ...
            json_file_path = os.path.join(yaml_folder, filename)
            combine_prompts = []
            new_results_prompts_cost = iteration.iterations(test_cases, method, prompt_contents, None, number_of_prompts - best_prompts, model_test, model_test_temperature, model_test_max_tokens, model_iteration, model_iteration_temperature, model_iteration_max_tokens, None, description, best_prompts)
            new_results = new_results_prompts_cost[0]
            cost = cost + new_results_prompts_cost[1]
            tokens_input_gen_iter = tokens_input_gen_iter + new_results_prompts_cost[2]
            tokens_output_gen_iter = tokens_output_gen_iter + new_results_prompts_cost[4]
