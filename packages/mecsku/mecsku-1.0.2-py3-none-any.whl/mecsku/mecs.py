...
class Stemmer:
    ...
    def tokenizing(self, data_cf):            #proses mengahpus simbol yang tida diperlukan dan merubah kalimat menjadi term berdasarkan spasi'''
    ...
    def ceIdentification(self, token):
        ...
        for j in targetIndices:          #identifikasi imbuan pada term setelah term ce'''
            for affixWord in affixes:
                if token[j].endswith(affixWord):          
                    if(affixWord == 'dha'):                   
                        token[j] = token[j].replace('ddha', 't')
                    else:
                        token[j] = token[j].replace(affixWord, '')
                    break
        return token

    def ghalluIdentification(self, token):      #proses identifikasi term ghallu
        demonstrative = ['rèya','jarèya','arèya','jariya','jiya','jajiya','jeh','rowa','arowa','juwa']
        se = ['sè']
        targetIndices = []
        for i in range(len(token)):
            if token[i] == 'ghallu' and token[i-2] in se:
                ...
                targetIndices.append(i-1)
            elif token[i] == 'ghallu' and token[i-1].startswith('ta'): #contoh term = tamera
                targetIndices.append(i-1)
    ...
    def rephrase(self, term):
        temp = term.split("-")
        if temp[0] == temp[1]: #term Ulang Sempurna. contoh term: mogha-mogha, #revisi pengecekan term ulang sempurna
            return {'kd':temp[1],'prefix':'', 'suffix':''}
        else:
            if temp[0].startswith('e'):
                if temp[0].startswith('e') and temp[1].endswith('aghi'): #term Ulang Dwi Lingga Berimbuhan e- dan -aghi.contoh term: ekol-pokolaghi
                    return {'kd':temp[1][:temp[1].index('aghi')],'prefix':'di', 'suffix':'kan'}
                else: 
                    return {'kd':temp[1],'prefix':'di', 'suffix':''} #term Ulang Dwi Lingga Berimbuhan e-. contoh term: ero-soro
            elif temp[0].startswith('a'): #term Ulang Dwi Lingga Tidak Berimbuhan a-. contoh term: areng-sareng
                if temp[0].startswith('a') and temp[1].endswith('an'): #term Ulang Dwi Lingga Berimbuhan a- dan -an.contoh term: aka'-berka'an
                    return {'kd':temp[1][:temp[1].index('an')],'prefix':'ber', 'suffix':'an'}
                else:
                    return {'kd':temp[1],'prefix':'ber', 'suffix':''}
            elif temp[1].endswith('na'): #term Ulang Dwi Lingga Berimbuhan -na. contoh term: ca-kancana 
                return {'kd':temp[1][:temp[1].index('na')],'prefix':'', 'suffix':'nya'}
            elif temp[1].endswith('an'):
                return {'kd':temp[1][:temp[1].index('an')],'prefix':'', 'suffix':'an'} #term Ulang Dwi Lingga Berimbuhan -an. contoh term: ca-kancaan
            elif temp[1].endswith('ân'):
                return {'kd':temp[1][:temp[1].index('ân')],'prefix':'', 'suffix':'an'}
            elif temp[1].endswith('a'):
                return {'kd':temp[1][:temp[1].index('a')],'prefix':'', 'suffix':''}
            elif temp[1].endswith(temp[0]): #term Ulang Dwi Lingga Tidak Berimbuhan. #contoh term: ku-buku,
                return {'kd':temp[1],'prefix':'', 'suffix':''}
    ...
    def affixInfix(self, term): 
        ...
        self.prefix = "ten"
        return {'kd':term,'prefix':'di'} #contoh term: tenolong-->tolong-->ditolong, tenompang-->tompang-ditumpang (sisipan 'en')
    ...
    def kaPrefix(self, term):
        if term.startswith('ka') and term.endswith('ânna'):
            self.prefix = 'ka'
            self.suffix = 'ânna'
            return {'kd':term[2:term.index("ânna")],'prefix':'ke','suffix':'annya'}
        elif term.startswith('ka') and term.endswith('anna'):
            self.prefix = 'ka'
            self.suffix = 'anna'
            return {'kd':term[2:term.index("anna")],'prefix':'ke','suffix':'annya'}
    ...
    def nasalPrefix(self, term, kamus):
    ...
    def affix(self, term, kamus):
        if term.endswith('na'):
            ...
            if term.startswith('sa') and term.endswith('na'):
                ...
                self.suffix = 'ânna'
                return {'kd':term[:term.index('ânna')],'prefix':'', 'suffix':'annya'}
            elif term.endswith('anna'):
                self.suffix = 'anna'
                return {'kd':term[:term.index('anna')],'prefix':'', 'suffix':'annya'}
            else:
            ...
            if term.startswith('e') and term.endswith('aghi'):
                self.prefix = 'e'
                return {'kd':term[1:term.index('aghi')],'prefix':'di', 'suffix':'kan'}
            elif term.startswith('è') and term.endswith('aghi'):
                self.prefix = 'è'
                return {'kd':term[1:term.index('aghi')],'prefix':'di', 'suffix':'kan'}
            elif term.startswith('a') and term.endswith('aghi'):
                self.prefix = 'a'
                return {'kd':term[1:term.index('aghi')],'prefix':'meng', 'suffix':'kan'}
            else:
            ...
            if term.startswith('sa') and term.endswith('sa'):
                self.prefix = 'sa'
                self.suffix = 'sa'
                return {'kd':term[2:term.index('sa')],'prefix':'se', 'suffix':'nya'}
            else:
                # add thoriq
                if term[2:] in kamus:
                    self.prefix = 'sa'
            ...
            if term.startswith('a') and term.endswith('an'):
                ...
                self.prefix = 'sa'
                self.suffix = 'an'
                return {'kd':term[2:term.index('an')],'prefix':'se', 'suffix':'an'}
            else:
    ...
    def stemming(self, kalimat):
        ...
        kamus = {}
        for item in data:
            if item["type"] == "table" and item["name"] == "lemmata":
                for row in item["data"]:
                    kamus[row["basic_lemma"]] = True
    ...
    #                         "-"+kamus[temp['kd']][0]+temp['suffix']+" "
                        ...
                        if term not in kamus.keys():
                            if term.startswith('pa') and term.endswith("na"):
                                ...
                                temp = self.nasalPrefix(term, kamus)
                                self.lemma = temp['kd']
                                # if temp['prefix'] == 'meny':
