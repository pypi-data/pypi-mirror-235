...
class Functions:
    ...
    def get_phone(value):
        """
        Extract and format a phone number from a string.

        Args:
            value (str): The input string containing a phone number.
    ...
    @staticmethod
    def get_browser(user_agent, lang_code, app_version):
        """
        Parse user-agent data to extract browser and system information.
        ...
        Returns:
            dict: Parsed browser and system information.
        """
        device_model = re.search(r'(opera|chrome|safari|firefox|msie|trident)\/(\d+)', user_agent.lower())
        if not device_model:
            device_model = 'Unknown'
            warnings.warn(f'Cannot parse user-agent: {user_agent}')
        else:
            device_model = device_model.group(1) + ' ' + device_model.group(2)

        system_version = 'Unknown'
        for key, value in Functions.system_versions.items():
            if key in user_agent:
                system_version = value
                break

        device_hash = '2'  # window.navigator.mimeTypes.length (outdated, defaults to '2')
        return {
            'token': '',
            'lang_code': lang_code,
            'token_type': 'Web',
    ...
    def get_format(value):
        """
        Get the file format (extension) from a string.
    ...
    @staticmethod
    def get_hash_link(value):
        """
        Extract the hash link part from a URL.

        Args:
            value (str): The input URL.

        Returns:
            str: The hash link part.
        """
        return value.split('/')[-1]
    ...
    def to_float(value):
        ...
        Returns:
            float: The converted value as a floating-point number.
        """
        return float(value)
    ...
    def to_array(value):
        ...
        Returns:
            list: The converted value as a list.
        """
    ...
    def to_metadata(value):
        """
        Extract metadata from a string containing markdown-like annotations.

        Args:
            value (str): The input string.

        Returns:
            dict: Extracted metadata and the cleaned text.
        """
        pattern = r'`(.*)`|\*\*(.*)\*\*|__(.*)__|~~(.*)~~|--(.*)--|\[(.*)\]\((\S+)\)'
        conflict = 0
        meta_data_parts = []
    ...
    def build(self, argument, param):
        ...
        try:
            ...
            if isinstance(value, dict):
                ...
                if isinstance(default_func, str):
                    value = getattr(Functions, default_func)(**value, **self.request)
        # get value heirship
        ...
        if types and not type(value).__name__ in types:
            if value is not None and 'optional' in types:
                raise TypeError(f'The given {argument} must be {types} not {type(value).__name__}')
        if alloweds is not None:
            if isinstance(value, list):
                for _value in value:
            ...
            for argument, param in self.method['params'].items():
                try:
                    ...
                    if 'optional' not in param['types']:
                        raise TypeError(f'{self.__name__}() required argument ({argument})')
