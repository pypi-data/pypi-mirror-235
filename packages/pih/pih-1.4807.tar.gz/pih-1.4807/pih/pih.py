...
class Input(InputBase):
    ...
    def datetime(self, use_date: bool | None = True, use_time: bool | None = True, ask_time_input: bool = True) -> datetime:
        result: datetime = DateTimeTool.now(second = 0)
        temp_datetime: datetime = result
        if n(use_date):
            use_date = not self.yes_no(js(("Использовать текущую дату", self.output.bold(DateTimeTool.datetime_to_string(result, CONST.DATE_FORMAT)))))
        while use_date and True:
            date_value: str | None = self.input(js(("Введите дату в формате", self.output.bold("День.Месяц.Год"))))
            dot_list: int = date_value.split(".")
            dot_count: int = len(dot_list)
            for i in range(dot_count):
                dot_list = ListTool.not_empty_items(DataTool.map(lambda item: str(PIH.DATA.EXTRACT.decimal(item)), dot_list))
            if dot_count == len(dot_list):
                date_value = j(dot_list, ".")
                temp_datetime = PIH.DATA.EXTRACT.datetime(date_value, [CONST.DAY_FORMAT, CONST.YEARLESS_DATE_FORMAT, CONST.DATE_FORMAT][dot_count - 1])
                if nn(temp_datetime):
                    if dot_count >= 1:
        ...
        if ask_time_input and self.yes_no(js("Ввести время")):
            if n(use_time):
                use_time = not self.yes_no(js(("Использовать текущее время", self.output.bold(DateTimeTool.datetime_to_string(result, CONST.SECONDLESS_TIME_FORMAT)))))
            while use_time and True:
                time_value: str | None = self.input(js(("Введите время в формате", self.output.bold("Час:Минуты"))))
                ...
                if dot_count == len(dot_list):
                    ...
                    temp_datetime = PIH.DATA.EXTRACT.datetime(time_value, [CONST.HOUR_TIME_FORMAT, CONST.SECONDLESS_TIME_FORMAT][dot_count - 1])
                    if nn(temp_datetime):
                        if dot_count >= 1:
...
class PIH:
    ...
    class EVENT:
        @staticmethod
        def send(value: Events, parameters: tuple[Any] | Any | None = None) -> None:
            ...
            PIH.LOG.executor.submit(
                ...
                value.name,
                PIH.EVENT.BUILDER.create_parameters_map(value, DataTool.as_list(parameters)),
            )
        ...
        @staticmethod
        def it_notify_about_create_user(login: str, password: str, additional_information: str) -> None:
        ...
        @staticmethod
        def it_notify_about_create_person(full_name: FullName, email_address: str, password: str, description: str, telephone_number: str) -> None:
            it_user_list: list[User] = PIH.RESULT.USER.by_job_position(
                AD.JobPositions.IT
            ).data
            me_user_login: str = PIH.session.get_login()
        ...
        class VARIABLE:
            ...
            class TIMESTAMP:
                ...
                def value(name: str | StorageVariableHolder) -> datetime | None:
                    return DateTimeTool.datetime_from_string(PIH.DATA.VARIABLE.value(PIH.DATA.VARIABLE._get_as_name(name), False, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP))

                @staticmethod
                def find(name: str | None = None) -> list[StorageVariableHolder] | None:
                    return PIH.DATA.VARIABLE.TIMESTAMP.get(name) if n(name) else PIH.DATA.VARIABLE.find(name, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP)

                @staticmethod
                def get(
                    name: str | None = None
                ) -> StorageVariableHolder | list[StorageVariableHolder] | None:
                    return PIH.DATA.VARIABLE.get(name, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP)

                @staticmethod
                def set(name: str | StorageVariableHolder, value: datetime | None, description: str) -> str:
                    return PIH.DATA.VARIABLE.set(PIH.DATA.VARIABLE._get_as_name(name), DateTimeTool.datetime_to_string(value or DateTime.now()), description, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP)

                @staticmethod
                def update(name: str | StorageVariableHolder, value: datetime | None = None) -> bool:
                    holder: StorageVariableHolder | None = PIH.DATA.VARIABLE.TIMESTAMP.get(name)
                    if nn(holder):
                        PIH.DATA.VARIABLE.TIMESTAMP.set(PIH.DATA.VARIABLE._get_as_name(name), value, holder.description)
                        return True
                    return False

                @staticmethod
                def remove(name: str) -> None:
                    PIH.DATA.VARIABLE.remove(PIH.DATA.VARIABLE._get_as_name(name), PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP)
                ...
                class EXPIRED:
                    ...
                    @staticmethod
                    def remove(name: str) -> None:
                        PIH.DATA.VARIABLE.remove(PIH.DATA.VARIABLE._get_as_name(name), PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED)

                    @staticmethod
                    def set(name: str, description: str, timestamp_variable_name: str, life_time_variable_name: str, note_name_resolver: str) -> None:
                        timestamp_value: datetime | None = PIH.DATA.VARIABLE.TIMESTAMP.value(timestamp_variable_name)
                        if nn(timestamp_value):
                            life_time_variable: int | None = int(PIH.DATA.VARIABLE.value(life_time_variable_name))
                            PIH.DATA.VARIABLE.set(name, ExpiredTimestampVariableHolder(timestamp_variable_name, life_time_variable_name, note_name_resolver), description, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED)

                    @staticmethod
                    def get(
                        name: str | None = None
                    ) -> StorageVariableHolder | list[StorageVariableHolder] | None:
                        return PIH.DATA.VARIABLE.get(name, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED)

                    @staticmethod
                    def holder(
                        name: str | StorageVariableHolder | None = None
                    ) -> ExpiredTimestampVariableHolder | list[ExpiredTimestampVariableHolder] | None:
                        result: list[ExpiredTimestampVariableHolder] = DataTool.fill_data_from_list_source(ExpiredTimestampVariableHolder, DataTool.map(lambda item: item.default_value, PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.get(name)))
                        if nn(name):
                            if e(result):
                                return None
                    ...
                    @staticmethod
                    def value(name_or_holder: str | ExpiredTimestampVariableHolder| StorageVariableHolder) -> datetime | None:
                        if isinstance(name_or_holder, str):
                            name_or_holder = PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.holder(name_or_holder)
                        if isinstance(name_or_holder, StorageVariableHolder):
                            name_or_holder = DataTool.fill_data_from_source(ExpiredTimestampVariableHolder(), name_or_holder.defaul_value)
                        life_time: int | None = None
                        try:
                            life_time = int(name_or_holder.life_time)
                        except ValueError as _:
                            life_time = int(PIH.DATA.VARIABLE.value(name_or_holder.life_time))
                        return None if e(name_or_holder) else DateTimeTool.add_months(PIH.DATA.VARIABLE.TIMESTAMP.value(name_or_holder.timestamp), life_time)

                    @staticmethod
                    def timestamp(name_or_variable_holder: str | ExpiredTimestampVariableHolder) -> datetime | None:
                        variable_holder: ExpiredTimestampVariableHolder | None = PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.holder(name_or_variable_holder) if isinstance(name_or_variable_holder, str) else name_or_variable_holder
                        return None if e(variable_holder) else PIH.DATA.VARIABLE.TIMESTAMP.value(variable_holder.timestamp)


                    @staticmethod
                    def life_time(name_or_variable_holder: str | ExpiredTimestampVariableHolder) -> int | None:
                        variable_holder: ExpiredTimestampVariableHolder | None = PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.holder(name_or_variable_holder) if isinstance(name_or_variable_holder, str) else name_or_variable_holder
                        return None if e(variable_holder) else DateTimeTool.day_count(PIH.DATA.VARIABLE.TIMESTAMP.value(variable_holder.timestamp), int(PIH.DATA.VARIABLE.value(variable_holder.life_time)))

                    @staticmethod
                    def left_life_time(name_or_variable_holder: str | ExpiredTimestampVariableHolder) -> int | None:
                        variable_holder: ExpiredTimestampVariableHolder | None = PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.holder(name_or_variable_holder) if isinstance(name_or_variable_holder, str) else name_or_variable_holder
                        return None if e(variable_holder) else (PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.value(variable_holder) - DateTimeTool.now()).days + 1
            ...
            @staticmethod
            def remove(name: str | StorageVariableHolder, section: SECTIONS | None = None) -> None:
                if isinstance(name, StorageVariableHolder):
                    name = name.key_name
                index: int = name.find(
                    PIH.DATA.VARIABLE.SECTION_DELIMITER)
                if index != -1:
                    return PIH.DATA.VARIABLE.remove(name[index + len(PIH.DATA.VARIABLE.SECTION_DELIMITER):], name[0: index])
                PIH.ACTION.DATA_STORAGE.value(
                    None,
                    name,
                    section,
            ...
            @staticmethod
            def set(name: str, value: Any, description: str | None = None, section: SECTIONS | None = None) -> Any:
                section = section or PIH.DATA.VARIABLE.SECTIONS.VARIABLE
                index: int = name.find(
                    PIH.DATA.VARIABLE.SECTION_DELIMITER)
                if index != -1:
                    return PIH.DATA.VARIABLE.set(name[index + len(PIH.DATA.VARIABLE.SECTION_DELIMITER):], value, description, name[0: index])
                PIH.ACTION.DATA_STORAGE.value(
            ...
            def get(
                ...
                def convert(value_holder: StorageVariableHolder | None) -> StorageVariableHolder | None:
                    if e(value_holder):
                        return value_holder
                    if value_holder.section == EnumTool.get(PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED):
                        value_holder.default_value = DataTool.fill_data_from_source(ExpiredTimestampVariableHolder(), value_holder.default_value) 
                    return value_holder
                if n(name):
                    result: list[StorageVariableHolder] = []
                    for item in data:
                        value_holder: StorageVariableHolder = DataTool.fill_data_from_source(
                                StorageVariableHolder(section=section_name), data[item], skip_not_none=True)
                        result.append(
                            convert(value_holder)
                        )
                    return result
                if nn(data):
            ...
            def find(
                ...
                if isinstance(name, Enum):
                    name = name.name
                result: list[StorageVariableHolder] = []
                section_name_list = (None if e(sections) else list(map(lambda item: EnumTool.get(
            ...
            @staticmethod
            def value(variable_name: str, from_environment_only: bool = False, section: str | SECTIONS | None = None) -> Any | None:
                result: Any =  PIH.DATA.VARIABLE.value_with_section(variable_name, from_environment_only, section)
                return result if from_environment_only else result[0]

            @staticmethod
            def value_with_section(variable_name: str, from_environment_only: bool = False, section: str | SECTIONS | None = None) -> tuple[Any | None, SECTIONS]:
                if isinstance(section, PIH.DATA.VARIABLE.SECTIONS):
                    section = EnumTool.get(section)
                variable_name = variable_name.replace(PIH.DATA.VARIABLE.SECTION_DELIMITER_ALT, PIH.DATA.VARIABLE.SECTION_DELIMITER)
                index: int = variable_name.find(
                    PIH.DATA.VARIABLE.SECTION_DELIMITER)
                if index != -1:
                    return PIH.DATA.VARIABLE.value_with_section(variable_name[index + len(PIH.DATA.VARIABLE.SECTION_DELIMITER):], from_environment_only, variable_name[0: index])
                ...
                for value in (environment_variable_handler(variable_name),
                            ...
                            get_value(variable_name, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED) if is_in_section(
                                  section, PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED) else None,
                              PIH.DATA.MATERIALIZED_RESOURCES.get_count(
                                  variable_name),
                    ...
                    if A.D.is_not_none(value):
                        break
                return (value, EnumTool.get_by_value(PIH.DATA.VARIABLE.SECTIONS, section))
                ...
                class TIMESTAMP:
                    ...
                    class EXPIRED:
                        ...
                        def exists_by_timestamp(name: str) -> bool:
                            return ne(DataTool.filter(lambda item: item.timestamp == name, PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.holder()))
        ...
        class FORMAT:
            ...
            def format(value: str) -> str:
                for part in re.findall(r"\{\S+\}", value):
                    ...
                    if part.find(PIH.DATA.VARIABLE.SECTION_DELIMITER_ALT) != -1:
                        result_part: str = part.replace(PIH.DATA.VARIABLE.SECTION_DELIMITER_ALT, PIH.DATA.VARIABLE.SECTION_DELIMITER)
            ...
            def by_formatter_name(value: Enum | str, data: Any | None) -> str | None:
                ...
                if isinstance(value, str):
                    ...
                    if section == PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP_EXPIRED:
                        ...
                        return js((PIH.DATA.FORMAT.datetime(PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.value(result)), "(ещё", PIH.DATA.VARIABLE.TIMESTAMP.EXPIRED.left_life_time(result), "дней)"))
                    if section == PIH.DATA.VARIABLE.SECTIONS.TIMESTAMP:
                        return PIH.DATA.FORMAT.datetime(result)
                    return result
            ...
            def email(
                ...
                if correct_domain:
                    ...
                    if email_domain in ("mail.com", "mail"):
    ...
    class RESULT(ResultTool):
        ...
        class NOTES:
            @staticmethod
            def find_gkeep_item(value: str | None = None, full_equaliment: bool = False) -> Result[list[GKeepItem]]:
                return DataTool.to_result(
                    PIH.SERVICE.call_command(
                        ServiceCommands.get_gkeep_item_list_by_any, (value, full_equaliment)), GKeepItem
                )
        ...
        class MARK:
            ...
            def by_full_name(
                ...
                return PIH.RESULT.MARK.by_name(
                    FullNameTool.fullname_to_string(value) if isinstance(value, FullName) else value, first_item
                )
    ...
    class CHECK:
        ...
        class NOTES:
            @staticmethod
            def exists(name_or_title: str, full_equaliment: bool) -> bool:
                return not ResultTool.is_empty(
                    DataTool.to_result(
                        PIH.SERVICE.call_command(
                            ServiceCommands.get_gkeep_item_list_by_any, (name_or_title, full_equaliment)
                        )
                    )
                )
    ...
    class MESSAGE:
        ...
        class WORKSTATION:
            ...
            def to_all_workstations(
                ...
                filter_user_login_list_is_empty: bool = e(
        ...
        class WHATSAPP:
            class WAPPI:
                ...
                def get_message_list(
                    ...
                    profile = EnumTool.get_value(
                        profile, EnumTool.get(CONST.MESSAGE.WHATSAPP.WAPPI.Profiles.DEFAULT)
                    )
            ...
            def send(
                ...
                wappi_profile = EnumTool.get_value(
                    wappi_profile, EnumTool.get(CONST.MESSAGE.WHATSAPP.WAPPI.Profiles.DEFAULT)
                )
                result: bool = False
                telephone_number = PIH.DATA.FORMAT.telephone_number(
                    telephone_number)
    ...
    class ACTION:
        ...
        class ACTIONS:
            ...
            def was_done(
                ...
                if e(user_login_or_user) or isinstance(
