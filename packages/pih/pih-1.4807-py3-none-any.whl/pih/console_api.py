...
from pih.tools import j, nl, js, b as mb, while_not_do, esc
...
class ConsoleAppsApi:
        ...
        with self.output.make_indent(2):
            ...
            if CheckableSections.INDICATIONS in checkable_section_list:
                with A.ER.detect():
                    ...
                    self.output.write_line(
                        self.bold(
                            js((A.CT_V.BULLET, "Показания в техническом помещении МРТ"))
                        )
                    )
            ...
            if CheckableSections.TIMESTAMPS in checkable_section_list:
                ...
                self.output.separated_line()
                self.output.write_line(
                    self.bold(js((A.CT_V.BULLET, "Временные метки")))
                )
                with self.output.make_indent(2, True):
                                                        ...
                                                        (
                                                            ...
                                                            A.D_F.whatsapp_send_message_to_it(
                                                                j(
                                                                    (
                                                                        "note",
                                                                        esc(expired_timestamp_item.resolver_note),
                                                                    ),
                                                                    "+",
                                                                )
                                                            ),
            ...
            if CheckableSections.PRINTERS in checkable_section_list:
                ...
                if self.session.is_mobile:
                    self.output.new_line()
                self.output.separated_line()
                self.output.write_line(self.bold(f"{A.CT_V.BULLET} Отчет по принтерам"))
    ...
    def run_command(self, command: list[str] | None = None) -> None:
        ...
        def get_command(value: str | None = None) -> list[str]:
            ...
            return list(
                filter(lambda item: not A.D_C.empty(item), result[0] + result[1])
            )

        command = get_command(js(command) if not A.D_C.empty(command) else None)
        ...
        if isinstance(recipient, User) and A.R.is_empty(
            ...
            try:
                ...
                if ask_for_use_dialog:
                    use_dialog = A.D_C.empty(message) and self.yes_no("Начать диалог")
                else:
                    use_dialog = True
                while True:
    ...
    def create_new_user(self) -> None:
        ...
        def get_additional_information() -> ActionValue:
            ...
            self.additional_information = self.input.input("Дополнительную информация")
            return self.output.get_action_value(
        ...
        def get_corporate_email(as_pc_user: bool = True) -> ActionValue:
            self.corporate_email = A.D_F.email(self.login, add_default_domain=True)
            return self.output.get_action_value(
                js(
                    (
        ...
        def get_mark_person_division() -> ActionValue | None:
            if self.need_to_create_mark and A.D.is_none(self.mark_person_division_id):
                self.output.header("Выбор подразделения")
                mark_person_division: PersonDivision = self.input.mark.person_division(
                    False
                )
        ...
        action_index: int = self.input.index(
            "Кого содать",
            [
                "Пользователя с аккаунтом для компьютера, полибейс, почтой и картой доступа",
                "Персону с почтой и картой доступа",
            ],
            label_function=lambda item, _: item,
        )
        is_user: bool = action_index == PC_USER
        if is_user:
            ActionStack(
                "Данные пользователя",
                get_full_name,
        ...
        full_name_parts: list[str] = list(
            map(lambda item: item.lower(), self.full_name.as_list())
        )
                ...
                if A.A_D.create_for_user(
                    ...
                    self.send_whatsapp_message(
                        ...
                        js(
                            (
                                "День добрый,",
                                self.b(A.D.to_given_name(self.full_name)),
                                ".Мы - отдел информационных технологий",
                                self.b("Pacific International Hospital."),
                            )
                        ),
                    )
                    if self.need_to_create_mark:
                        ...
                        self.send_whatsapp_message(
                            ...
                            js(
                                (
                                    ...
                                    self.b(self.mark_tab_number),
                                    self.b("в отделе кадров"),
                                )
                            ),
                        )
                    if is_user:
