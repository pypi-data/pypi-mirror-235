...
from genomicspy.main import copy_seqrecord
...
def trim_alleles(alleles: pd.DataFrame, 
                 ...
                 col_start: str = "POS",
                 col_end: str = "END_POS") -> pd.DataFrame:
    """Trim alleles that span over start/end of interval"""
    ...
    pruned = alleles.loc[
        (alleles[col_end] > start) &
        (alleles[col_start] < end)
    ]
    ...
    trimmed = (
        pd.concat([span_left, span_right])["TRIMMED"]
        .pipe(pruned.join)
