...
def generate_passages(patent: parser.ParsedPatent, prior_art_source: parser.ParsedPatent, claim_element: str, model_name: str='gpt-4'):
    # TODO: support other types of prior art
    claims = parser.parse_claims_from_parsed_patent(patent)
    serialized_claims = '\n'.join([parser.serialize_claim(c) for c in claims.claims])
    specification = '\n'.join(parser.serialize_specification_from_parsed_patent(patent))
    prior_art_specification = '\n'.join(parser.serialize_specification_from_parsed_patent(prior_art_source))
    # import pdb;pdb.set_trace()
    token_limit = model_name_to_token_limit[model_name]
    prior_art_split_count = 1
    n_tokens_in_text = num_tokens_in_text(prior_art_specification)
    min_split_size = 1024
    base_num_tokens = num_tokens_from_messages(
        openai_prompt_select_passages_from_prior_art_portion(specification, serialized_claims, claim_element, ' ' * min_split_size)
    )
    if base_num_tokens > token_limit:
        raise GenerationError("Reference patent specification, claims, and claim element exceeds token limit by itself.")
    
    prior_art_splits = []
    while True:
        print(prior_art_split_count)
        # Check all splits
        candidate_splits = []
        for i in range(prior_art_split_count):
            if i == 0:
                candidate_split = prior_art_specification[:len(prior_art_specification) // prior_art_split_count]
            else:
                candidate_split = prior_art_specification[len(prior_art_specification) // prior_art_split_count * i:len(prior_art_specification) // prior_art_split_count * (i+1)]
            candidate_splits.append(candidate_split)
        num_tokens = [num_tokens_from_messages(
            openai_prompt_select_passages_from_prior_art_portion(specification, serialized_claims, claim_element, split)
        ) for split in candidate_splits]
        if all([n <= token_limit - 0.05 * token_limit for n in num_tokens]):
            prior_art_splits = candidate_splits
            break
    ...
    # Generate completions for each prior art split
    completions = []
    # XXX: remove hardcoded :2 once we get around rate limits
    for i, prior_art_split in enumerate(prior_art_splits[:2]):
        messages = openai_prompt_select_passages_from_prior_art_portion(specification, serialized_claims, claim_element, prior_art_split)
        # print num tokens in messages
        # print(num_tokens_from_messages(messages))
        completion = openai_chat_completion_request_with_retry(model_name, messages)
        # print(completion)
    ...
    concatenated_completions, n_completions = concatenate_completions_for_reranking(completions)
    # print(concatenated_completions)
    messages = openai_prompt_rank_selected_passages(specification, serialized_claims, claim_element, concatenated_completions, n_completions)
    reranked_completion = openai_chat_completion_request_with_retry(model_name, messages)
    # print(reranked_completion)
    reranked_completions = parse_reranked_completions(reranked_completion['choices'][0]['message']['content'], n_completions)
