...
from .stores.android_15 import (
    UNTRUSTED as ANDROID15_UNTRUSTED,
    PEM_FILES as ANDROID15_PEM_FILES,
)
...
class TrustStore:
    ...
    @property
    def android15(self) -> bool:
        try:
            return (
                self.key_identifier not in ANDROID15_UNTRUSTED
                and isinstance(self.certificate, X509)
                and not self.expired_in_store(PLATFORM_ANDROID15)
            )
        except FileExistsError:
            return False
    ...
    def exists(self, context_type: int) -> bool:
        ...
        if (
            context_type == PLATFORM_ANDROID15
            and self.key_identifier in ANDROID15_PEM_FILES.keys()
        ):
            return match_certificate(
                self.key_identifier,
                get_certificate_from_store(self.key_identifier, PLATFORM_ANDROID15),
            )
        if (
    ...
    def check_trust(self, context_type: int = None) -> bool:
        ...
        if context_type == SOURCE_ANDROID:
            return self.android
        if context_type == PLATFORM_ANDROID15:
            return self.android15
