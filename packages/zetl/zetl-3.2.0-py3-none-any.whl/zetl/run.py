...
class zetl:
	...
	def log_script_error(self,lid=-1,pscript_error='',database='',pscript_output=''):
		...
		try:
			...
			print(str(e))
	def replace_vars_in_query(self,script_variables,individual_query):
	...
	def run_one_etl_step(self,etl_name,stepnum,steptablename,cmdfile,run_parameter='',cmdfilename=''):
		script_variables = {'SHOWQUERY':'True','DB_TYPE':'','DB_USERNAME':'','DB_USERPWD':'','DB_HOST':'','DB_PORT':'','DB_NAME':'','DB_SCHEMA':'','ARGV1':run_parameter,'ARGV2':'','ARGV3':''}
		...
		for individual_query in sql.split(';'):
			...
			if not individual_query.isspace() and individual_query != '':
				...
				individual_query = self.replace_vars_in_query(script_variables,individual_query)
