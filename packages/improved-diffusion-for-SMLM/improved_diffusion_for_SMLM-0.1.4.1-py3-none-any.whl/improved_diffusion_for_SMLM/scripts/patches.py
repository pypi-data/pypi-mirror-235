...
def crop_to_patches(image, top, left, patch_height, patch_width, overlap):
    ...
    range_x = 1 + (image.shape[0] - patch_height - (((image.shape[0] - patch_height)) % (patch_height - overlap_range_height))) // (patch_height-overlap_range_height)
    range_y = 1 + (image.shape[1] - patch_width - (((image.shape[1] - patch_width)) % (patch_width - overlap_range_width))) // (patch_width-overlap_range_width)

    for i in range(int(range_x)):
        for j in range(int(range_y)):
            if cur_left + overlap_range_width <= image.shape[1] and top + overlap_range_height <= image.shape[0]:
...
def augmentation(img,n_rotation):
    ...
    img_augmentations = torch.zeros((n_rotation*2, img.shape[0], img.shape[1]))
    if n_rotation==0:
        angle=0
    else:
...
def create_patches_for_type(images_folder_path, patch_size, overlap, crop_start, n_rotations=2):
    ...
    for image in images_folder.iterdir():
        if image.is_file():
            ...
            for frame in range(tensor_img.shape[0]):
                patches = crop_to_patches(tensor_img[frame], top, left, patch_height, patch_width, overlap) #maia's function
