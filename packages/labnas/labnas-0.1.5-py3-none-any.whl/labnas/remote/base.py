...
class SftpNas:
    ...
    def move_file(self, remote_source: Path, remote_target: Path) -> None:
        """Move a file around."""
        if not self.connection.isfile(str(remote_source)):
            raise FileNotFoundError(f"{remote_source}")
        if self.connection.isfile(str(remote_target)):
            raise FileExistsError(f"{remote_target}")
        if not self.connection.isdir(str(remote_target.parent)):
            raise FileNotFoundError(f"{remote_target}")
    ...
    def create_empty_folder(self, remote_folder: Path) -> None:
        ...
        if self.connection.isdir(str(remote_folder)):
