...
def set_run_instances(service, instances):
    #Manually change Run instance count (via Service-level min-instances)
    run_service = call_admin_api(path=f'services/{service}', 
                                 url='https://run.googleapis.com/v2',
                                 op="GET")
    # print(run_service)
    print(f"SERVICE: {run_service['name']}, Instances: {run_service['scaling']['minInstanceCount']}")
    print(f"Setting {service} to {instances}")
    run_service['scaling']['minInstanceCount'] = instances
    print(f"UPDATED: {run_service['name']}, Instances: {run_service['scaling']['minInstanceCount']}")

    op_response = call_admin_api(path=f'services/{service}', 
                                 url='https://run.googleapis.com/v2',
                                 op="PATCH",
                                 payload=json.dumps(run_service))
    
    return json.dumps(op_response, indent=4, sort_keys=False)
