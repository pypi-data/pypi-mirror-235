...
# ----For DLC/Studio image, install ipykernel if not present----
if [ ! -z "${SM_FIRST_PARTY_IMAGEOWNER}" ] && ([ "${SM_FIRST_PARTY_IMAGEOWNER}" = "DLC" ] || [ "${SM_FIRST_PARTY_IMAGEOWNER}" = "Studio" ]); then
  SM_EXEC_STEP="installing ipykernel for sagemaker 1P image"

  ipykernel_installed=$(${PYTHON_EXECUTABLE} -c 'import importlib.util; print(None != importlib.util.find_spec("ipykernel"))')
  echo "Studio 1P Image: ${SM_FIRST_PARTY_IMAGEOWNER} - [ipykernel_installed: ${ipykernel_installed}]"

  if [ "${ipykernel_installed}" = "False" ]; then
    ${PYTHON_EXECUTABLE} -m pip install --no-index --find-links "$SM_EXECUTION_SYSTEM_PATH/system/dependencies/${DEFAULT_PYTHON_VERSION}" ipykernel
    ${PYTHON_EXECUTABLE} -m ipykernel install --sys-prefix
  fi
fi
...
# ----install papermill if not installed----
SM_EXEC_STEP="installing the papermill dependency"
papermill_installed=$(${PYTHON_EXECUTABLE} -c 'import importlib.util; print(None != importlib.util.find_spec("papermill"))')
if [ "${papermill_installed}" = "False" ]; then
  ${PYTHON_EXECUTABLE} -m pip install --no-index --find-links "$SM_EXECUTION_SYSTEM_PATH/system/dependencies/${DEFAULT_PYTHON_VERSION}" papermill
fi
...
PYTHONPATH="${SM_EXECUTION_SYSTEM_PATH}/system/lib" ${PYTHON_EXECUTABLE} -u ${SM_EXECUTION_SYSTEM_PATH}/system/lib/notebookrunner.py&
papermill_process=$!
