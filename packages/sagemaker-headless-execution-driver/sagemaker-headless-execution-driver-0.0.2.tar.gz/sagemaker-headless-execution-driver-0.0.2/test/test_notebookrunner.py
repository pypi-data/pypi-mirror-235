...
from headless_execution.notebookrunner import PaperMillNotebookRunner
import contextlib
...
class TestPaperMillNotebookRunner(unittest.TestCase):
    ...
    @patch(
        "headless_execution.notebookrunner.NotebookExecutionLanguageUtils.has_language_info"
    )
    @patch("os.path.exists")
    @patch("headless_execution.notebookrunner.papermill.execute_notebook")
    @patch("headless_execution.notebookrunner.json.load")
    @patch("builtins.open")
    def test_happy_case(
        self,
        mock_open,
    ...
    @unittest.mock.patch.dict(os.environ, env_happy_case)
    @patch(
        "headless_execution.notebookrunner.NotebookExecutionLanguageUtils.get_language_from_kernel"
    )
    @patch(
        "headless_execution.notebookrunner.NotebookExecutionLanguageUtils.has_language_info"
    )
