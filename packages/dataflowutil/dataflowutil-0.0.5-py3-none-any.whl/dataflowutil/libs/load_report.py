...
class LoadReport():
    ...
    def check_report_new(self,display_check=False):
        ...
        if data_local.shape[0] > 0:
            ...
            df_new = pd.DataFrame([list_columns,check_new_columns,check_types,check_nan]).T

            df_new.rename(columns={0:"Columns",1:"Columns_Equals",2:"DTypes_Columns",3:"NaN_Count"},inplace=True)
        ...
        if tag:
            if len(tag) > 0:
                ...
                if tag in list_columns_local:
                    ...
                    data_values_unique = data_local[column_check].unique()
                    data_values_unique.rename(columns={0:"Values_Uniques"},inplace=True)

                    display(pd.DataFrame(data_values_unique))
