...
class Plant(Component):
    ...
    def check_interval(self,
        ...
        """Make sure plant's virtual sensors are up-to-date, and make sure we have data in the specified interval.
        """
        if not self.virtuals_calculation_uptodate:
            warnings.warn(f'{method_name} is called on a plant with outdated virtual sensors'
                          f' (plant.virtuals_calculation_uptodate flag is False). '
                          f'{method_name} results might be outdated or inconsistent with the plant configuration. '
                          f'To overcome this, call "virtuals.calculate_virtuals(plant)".')

        self.context.set_eval_interval(eval_start, eval_end, check_overlap=True, method_name=method_name)
