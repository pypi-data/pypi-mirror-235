...
class Module(RedisMixin, LoggingMixin, RollbarMixin):
    ...
    def update_cache(cls, merger: 'PartialRerunMerge') -> None:
        ...
        original_state_key = redis_handler.get_formatted_state_key(
            original_asset_id, cls.app_key, cls.module_key
        )
        rerun_state_key = redis_handler.get_formatted_state_key(
            rerun_asset_id, cls.app_key, cls.module_key
        )

        original_state = redis_handler.load_state(original_state_key)
        rerun_state = redis_handler.load_state(rerun_state_key)

        # update the original state with the rerun state(excluding asset_id)
        keys = rerun_state.keys()
        keys = keys.pop('asset_id')
        for key in keys:
            original_state[key] = rerun_state[key]
