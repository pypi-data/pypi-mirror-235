    ...
    def merge_collections(self) -> None:
        ...
        for collection in self.schema.collections:
            ...
            if self.merging_progress.is_collection_completed(collection_name):
                self.logger.debug(
                    f"Collection {collection_name} is already completed or failed. Skipping."
                )
                continue
            ...
            if collection.merging_method:
                ...
                col_is_completed = self.merge_current_mode(collection_name)
    ...
    def merge_current_mode(
        ...
        while start_time <= self.end_timestamp:
            ...
            updated_data = self._get_data(
                collection_name, self.rerun_asset_id, start_time, self.end_timestamp, "once"
            )
            if not updated_data:
