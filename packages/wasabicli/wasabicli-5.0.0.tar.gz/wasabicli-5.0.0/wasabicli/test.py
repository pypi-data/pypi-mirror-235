...
class WasabiPurge:
    ...
    def deleteObjectsV2(self):
        ...
        if len(objects_to_delete) > 0:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                for i, future in enumerate(concurrent.futures.as_completed(futures)):
                    print(
                        f"\r{indicators.OKGREEN}Batch Number [{counter}] <<<--->>> Deleting objects: {i + 1} out of {len(futures)}{indicators.ENDC}",
                        end="")
            counter += 1
        print("\nObjects deleted successfully!")
    ...
    def purgeDeleteMarkers(self):
        print(
            f"{indicators.HEADER}You have chosen to purge all delete markers (DMs)! Please wait...{indicators.ENDC}"
        )
                ...
                if version.get("IsDeleteMarker"):
                    ...
                    if len(delete_markers) == 50000:
                        for delete_marker in delete_markers:
                            ...
                            for i in range(len(delete_markers)):
                                print(
                                    f"\r{indicators.OKGREEN}Removing delete markers in batches: {i + 1} / {len(delete_markers)}{indicators.ENDC}",
                                    end="",
                                )
                                sleep(0.001)
        ...
        #
        # print(
        #     f"{indicators.OKBLUE}Delete Markers Successfully Purged from {self.bucketName}{indicators.OKBLUE}"
        # )
