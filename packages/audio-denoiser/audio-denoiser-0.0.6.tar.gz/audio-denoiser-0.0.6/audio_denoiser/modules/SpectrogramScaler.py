...
class SpectrogramScaler(nn.Module):
    ...
    @staticmethod
    def train_scaler(audio_files: List[str], sample_rate=16000, n_fft=512) -> 'SpectrogramScaler':
        from data.AudioFileDataset import AudioFileDataset

        base_scaler = SpectrogramScaler(0, 1.0)
        rnd = np.random.RandomState(1)
        ds = AudioFileDataset(rnd, audio_files, base_scaler, sample_rate=sample_rate, n_fft=n_fft)
        spectrogram_list = []
        for x, _ in ds:
