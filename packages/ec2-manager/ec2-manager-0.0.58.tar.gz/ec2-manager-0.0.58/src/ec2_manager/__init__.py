...
class EC2Manager:
    ...
    def _install_dependencies(self, name):
        """
        Installs dependencies on a single instance.
        """
        if self._platform == 'amzn2':
            self._logger.info(f'{name} is checking if yum is locked...')
            # remove the yum lock if needed
            result = self.run_command(name, ['ps aux | grep yum'])
            if result:
                # get the pids
                pids = []
                result_list = result.split()
                for index, chunk in enumerate(result_list):
                    if chunk in ['root', 'ssm-user', 'ec2-user']:
                        pids.append(result_list[index+1])

                # kill the extra yum pids
                kill_commands = []
                for pid in pids:
                    kill_commands.append(f'sudo kill -9 {pid}')
                kill_commands.append('sudo rm -f /var/run/yum.pid')

                self._logger.info(f'{name} is killing {len(pids)} yum pid(s)...')
                self.run_command(name, kill_commands)
                self._logger.info(f'{name} successfully killed {len(pids)} yum pid(s)!')
            else:
                self._logger.info(f'{name} yum is not locked.')

            # normal commands
            commands = ['yum install unzip git python3 acl -y']
        else:
            commands = ['apt install unzip git software-properties-common acl -y']

        self._logger.info(f'{name} installing dependencies...')
