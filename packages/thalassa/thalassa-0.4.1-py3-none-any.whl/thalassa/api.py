...
def notify(msg: str, cmd: str | None = None) -> None:
    ...
    if cmd:
        ...
        warnings.warn(
            "Couldn't find any known notification program..."
        )  # holoviews shallows this one too...
...
def _get_stream_timeseries(
    ...
    def callback(x: float, y: float, title_template: str = title_template) -> hv.Curve:
        if not utils.is_point_in_the_raster(raster=source_raster, lon=x, lat=y):
            ...
            # Why do we need to redim?
            # plot = plot.redim(variable, range=(ts[variable].min(), ts[variable].max()))
        # setup hover
        hover = HoverTool(
            tooltips=[
                ("time", "@time{%F %T}"),
