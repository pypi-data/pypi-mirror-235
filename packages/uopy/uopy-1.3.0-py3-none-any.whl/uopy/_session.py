...
class Session:
    ...
    def _get_uv_version(self, version: str):
        uv_version = array("i")
        if not version.startswith("UniVerse "):
            return None
        version = version.removeprefix("UniVerse")
        version = version[1:]
        version_arr = version.split(".")
        if len(version_arr) != 3:
            return None
        try:
            for item in version_arr:
                uv_version.append(int(item))
        except:
            return None
        return uv_version
        pass

    def _compare_version(self, version1: array, version2: array):
        # The version1 >= version2, the result is True, otherwise, the result is False
    ...
    def connect(self):
        with self._lock:
            ...
            suffix = "\r\n"
            if self.db_type == "UV":
                if self._host_type == 1:
                    # Unix
                    cmd.command_text = 'SH -c "uv -version"'
                else:
                    cmd.command_text = 'DOS /c "uv -version"'
                cmd.run()
                uv1135 = self._get_uv_version("UniVerse 11.3.5")
                uv_version = self._get_uv_version(cmd.response.removesuffix(suffix))
                if uv_version is None or uv1135 is None:
                    raise UOError(ErrorCodes.UOE_UNKNOWN_ERROR)
                self._uv_timeout_flag = self._compare_version(uv_version, uv1135)
