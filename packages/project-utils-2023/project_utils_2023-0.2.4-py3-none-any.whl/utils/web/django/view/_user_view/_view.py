...
class BaseUserLoginView(BaseView, metaclass=ABCMeta):
    ...
    request_post = Params(ParamsTypes.object, require=require).params({
        "username": Params(ParamsTypes.string, title="用户名").params(),
        "password": Params(ParamsTypes.string, title="密码").params(),
        "checkcode": Params(ParamsTypes.string, title="校验码", description="用户名_验证码_密码三者组成字符串的base64编码")
    })

    user_body: Dict[str, Schema] = {}

    swagger_type = SwaggerType(request_post=request_post)

    response_get: Dict[str, Schema] = {
        "code": Params(ParamsTypes.string, title="验证码")
    }
    response_post: Dict[str, Schema] = {
        "user_info": Params(ParamsTypes.object, title="响应数据").params(user_body),
        "user_token": Params(ParamsTypes.string, title="token令牌").params()
    }
    ...
    @swagger_auto_schema(responses=swagger_type.response(response_get))
    ...
    @swagger_auto_schema(request_body=request_post, responses=swagger_type.response(response_post))
