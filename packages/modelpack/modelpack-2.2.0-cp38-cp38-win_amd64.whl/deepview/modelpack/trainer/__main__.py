...
def train_detection(from_config: dict = None, dvclive_output=None):
    ...
    if compute_anchors:
        ...
        if save_anchors and not os.path.exists(save_anchors):
            ...
            if is_yaml:
                ...
                with open(save_anchors, 'w') as fp:
                    str_anchors = " ".join(str(item) for item in anchors.flatten())
    ...
    if anchors_file and os.path.exists(anchors_file):
        ...
        if is_yaml:
           ...
           with open(anchors_file) as fp:
            ...
            anchors = [float(x) for x in open(anchors_file, 'r').readline().rstrip().split(' ')]
    ...
    if anchors is None:
        if os.path.exists("params.yaml"):
            ...
            if anchors:
                ...
                model_handler.precompute_anchors(train_ds, objects_size)
                ds_handler.augment = True
                ds_handler.store_anchors(anchors=model_handler.get_specific_anchors())
                anchors = model_handler.get_specific_anchors()
    else:
    ...
    trainer = DetectionTrainer(
        ...
        skip_initial_validation=from_config.get('skip-initial-validation', False)
    )
...
def train_segmentation(
    ...
    trainable_model = mpk_handler.get_segmentation_model(
        ...
        method=ModelPackDetector.UPSAMPLE_RESIZE
    )
...
def train_attitude(from_config=None, dvclive_output=None):
    """
    ...
    trainer = AttitudeTrainer(
        ...
        optimizer=optimizer,
        loss=attitude_loss,
        classes=["roll", "pitch", "yaw"], # current order of angles within the dataset
        logger=logger,
        checkpoint=from_config.get("checkpoints", None),
        info=from_config,
        display=display,
...
def train_detect_and_segment(from_config=None, dvclive_output=None):
    """
    ...
    if anchors is None:
        if os.path.exists("params.yaml"):
            ...
            if anchors:
                ...
                model_handler.precompute_anchors(train_ds, objects_size)
                ds_builder.augment = True
                ds_builder.store_anchors(anchors=model_handler.get_specific_anchors())
                anchors = model_handler.get_specific_anchors()
    else:
        model_handler.set_specific_anchors(anchors=anchors)
...
def run_all(from_config: dict = None, dvclive_output=None):
    ...
    if task in ["detection", "detect"]:
        ...
        train_detect_and_segment(from_config=from_config, dvclive_output=dvclive_output)
    else:
...
def main():
    ...
    parser.add_argument(
        '--anchors',
        help='Path to the file containing the anchors file. If not provided, anchors will be taken from params.yaml',
        type=str,
        default=None
    )
