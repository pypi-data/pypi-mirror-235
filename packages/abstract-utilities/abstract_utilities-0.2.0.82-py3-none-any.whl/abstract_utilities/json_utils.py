...
def safe_json_loads(data):
    try_json = try_json_loads(data)
    if try_json:
        return try_json
    return data
def convert_to_json(obj: Union[str, dict]) -> Union[dict, any]):
...
def safe_read_from_json(file_path, default_value=None):
    """
    Safely read from a JSON file. If the file doesn't exist or is corrupted,
    return a default value.
    """
    if not os.path.exists(file_path):
        print(f"File '{file_path}' does not exist. Returning default value.")
...
def safe_write_to_json(file_path, data):
    """
    ...
    try:
        ...
        # Rename the temp file to replace the original, thus ensuring atomic write
