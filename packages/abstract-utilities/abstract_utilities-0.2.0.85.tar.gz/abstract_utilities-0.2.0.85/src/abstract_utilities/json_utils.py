...
def convert_to_dict(obj: Union[str, dict]) -> Union[dict, any]:
    ...
    def convert_quotes(obj):
         return obj.replace("'",'"')
    json_loads = try_json_loads(obj)
    if json_loads:
        return json_loads
    for function in convert_quotes,eat_all,json.dumps:
        json_loads = try_json_loads(function(obj))
        if json_loads:
            return json_loads
    return obj
...
def convert_to_json(data: Union[str, dict]) -> Union[dict, any]:
    ...
    return data
def all_try(data=None,var_data=None,function=None,error=False,error_msg=None,error_value=None,attach=None,attach_var_data=None):
    def if_error(error,e,error_msg):
        if error:
            raise e
        else:
            if error_msg:
            ...
            return False
    if error_value == None:
        error_value = (Exception, TypeError)
    try:
        if function or data:
            if var_data or data:
                if function and not attach and (var_data or data):
                    if data:
                        try_it = function(data)
                    elif var_data:
                        try_it = function(**var_data)
                elif function and attach:
                    if data:
                        if attach_var_data:
                            try_it = function(data).attach(**attach_var_data)
                        else:
                            try_it = function(data).attach()
                    
                    elif var_data:
                        if attach_var_data:
                            try_it = function(**var_data).attach(**attach_var_data)
                        else:
                            try_it = function(**var_data).attach()
            
                elif data and function ==None and attach:
                    if attach_var_data:
                        try_it = data.attach(**attach_var_data)
                    else:
                        try_it = data.attach()
            elif function:
                if function and attach==None:
                    try_it=function()
                elif attach and function:
                    try_it = function().attach()
        else:
            try_it=function()
        return try_it
    except error_value as e:
        return if_error(error=error,e=e,error_msg=error_msg)
...
def create_and_read_json(file_path: str, json_data: dict = {},error=False,error_msg=True) -> dict:
    ...
    try_read = safe_read_from_json(file_path, default_value=None,error=error,error_msg=error_msg)
    if try_read==None:
        safe_write_to_json(file_path=file_path, data=json_data)
    return safe_read_from_json(file_path=file_path)
    
def safe_read_from_json(file_path, default_value=None,error=False,error_msg=True):
    if not os.path.exists(file_path):
        error_msg = get_error_msg(error_msg,default_error_msg=f"File '{file_path}' does not exist. Returning default value.")
        return default_value
    var_data={"file_path":file_path,"instance":'r'}
    error_msg = get_error_msg(error_msg,default_error_msg=f"Error decoding JSON from '{file_path}'")
    open_it = all_try(var_data=var_data,function=get_open,error_msg=error_msg)
    if open_it:
        with open_it as file:
            try_load = try_json_loads(file,error_msg=error_msg)
            if try_load:
                return try_load
            return all_try(function=file.read,error_msg=error_msg)
...
def safe_dump_to_file(data,file,ensure_ascii=False, indent=4):
