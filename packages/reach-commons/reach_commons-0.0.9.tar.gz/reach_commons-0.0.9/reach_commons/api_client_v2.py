...
class ReachApiGatewayV2:
    ...
    def stripe_get_customer(self, user_id):
        resp = requests.get(
            f"{self.base_url}/user/{user_id}/customer",
            headers=self.headers
        )
        return resp

    def create_booking_guarantee(self, business_id, booking_price):
        warnings.warn(
            "create_booking_guarantee is deprecated and will be retired in a future version. "
            "Please use stripe_create_booking_guarantee instead.",
            DeprecationWarning,
            stacklevel=2
        )
        resp = requests.post(
