...
def train_SPACE(model, train_data, outdir, epoch=5000,lr=0.005, a=0.5, b=10, loss_type='MSE',patience=50, GPU=0,seed=42, verbose=False):
    ...
    df_loss = pd.DataFrame(columns=['feature_loss','graph_loss','epoch'])  # loss history
    ...
    df_loss.to_csv(os.path.join(outdir, 'train_loss.csv'))
    # plot loss
    fig = plt.figure()
    plt.plot(df_loss['epoch'], df_loss['graph_loss'], 'ro-', label='graph_loss',linewidth=1, markersize=2)
    plt.plot(df_loss['epoch'], df_loss['feature_loss'], 'bo-', label='feature_loss',linewidth=1, markersize=2)
    plt.legend()
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.tight_layout()
...
def train_SPACE_GCN(model, train_data, outdir, epoch=5000,lr=0.005, a=0.5, b=10, loss_type='MSE',patience=50, GPU=0,seed=42, verbose=False):
