...
import sigan.src.commands.display as display
import sigan.src.application.utils as utils
import sigan.src.application.service as service
from sigan.src.infrastructure.database import get_db
...
def alarm_date_callback(value: str):
    ...
    if len(args) == 1:
        ...
        # return f"{year}-{month}-{day} 09:00:00"
    ...
    if not utils.is_valid_hour(int(float(hour))):
        raise typer.BadParameter(f"Invalid hour: {hour}. Hour must be in [00-23].")
    if not utils.is_valid_minute(int(float(minute))):
...
def add_alarm(
    ...
    slack_channel: str = typer.Option(None, '-s', '--slack', help="[yellow]Slack Channel[/yellow] Input.", callback=slack_channel_callback)
    ):
    ...
    if interval:
        ...
        year, month, day = utils.get_date_from_shortcut(interval, alarm_date)
        time = alarm_date
    ...
    if deadline:
        deadline_year, deadline_month, deadline_day = utils.change_deadline_to_date(deadline)
    ...
    user_id = service.get_user_id(db)
    if user_id is None:
        display.center_print("Team slack id is not registered.", type="error")
        exit(0)

    try:
        resp = service.create_alarm(db, alarm_data)
        if resp["success"]:
            alarm_list = service.get_all_alarms(db, user_id)
        else:
            display.center_print(resp['error'], type="error")
            exit(0)
    except Exception as e:
        display.center_print(e.error, type="error")
        exit(0)
...
def delete_alarm(alarm_id: int = typer.Argument(..., help="[yellow]Alarm ID[/yellow] to be [red]deleted[/red]")):
    ...
    try:
        resp = service.delete_alarm(db, alarm_id)
        if resp["success"]:
            alarm_list = service.get_all_alarms(db, user_id)
        else:
            display.center_print(resp['error'], type="error")
...
def change_content(
    ...
    try:
        resp = service.change_content(db, alarm_data)
        if resp["success"]:
            alarm_list = alarm_list = service.get_all_alarms(db, user_id)
        else:
            display.center_print(resp['error'], type="error")
            exit(0)
    except Exception as e:
...
def change_deadline(
    ...
    try:
        resp = service.change_deadline(db, alarm_data)
        if resp["success"]:
            alarm_list = service.get_all_alarms(db, user_id)
        else:
            display.center_print(resp['error'], type="error")
...
def change_date(
    alarm_id: int = typer.Argument(..., help="[yellow]Alarm ID[/yellow] to be [purple]changed[/purple]"),
    date: str = typer.Option(..., "-t", "--time", help="Alarm sending date or time, [yellow]yyyy/mm/dd or mm/dd or yyyy/mm/dd hh:mm or mm/dd hh:mm", callback=alarm_date_callback),
    ):
    ...
    try:
        resp = service.change_date(db, alarm_data)
        if resp["success"]:
            alarm_list = service.get_all_alarms(db, user_id)
        else:
            display.center_print(resp['error'], type="error")
...
def change_interval(
    ...
    try:
        resp = service.change_interval(db, alarm_data)
        if resp["success"]:
            alarm_list = service.get_all_alarms(db, user_id)
        else:
            display.center_print(resp['error'], type="error")
...
def register():
    ...
    if is_register:
        ...
        if service.match_team_id(db, team_id) is None:
            display.center_print("Invalid slack team id.", type="error")
            exit(0)
    else:
        exit(0)
    
    user_info = {"team_id": team_id}
    global_utils.cache_team_id(configs.APP_DIR, user_info)
    display.center_print("Register successfully!", type="success")
    
    
@app.callback()
def app_setting():
    global_utils.config_setup(configs.APP_DIR_PATH, configs.APP_CACHE_PATH)