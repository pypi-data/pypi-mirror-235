        ...
        for root, _, files in os.walk(rc_files_path):
            for f in files:
                ...
                if f not in bashrc_related_files:
                    ...
                    replace_file(rc_file, dst, restore_rc, dry_run)
                    if not dry_run:
                        print("append alias and alias2 to ~/.bashrc")
                        alias_file = os.path.join(root, "alias")
                        alias2_file = os.path.join(root, "alias2")
                        os.system(f"cat {alias_file} >> {dst}")
                        os.system(f"cat {alias2_file} >> {dst}")
