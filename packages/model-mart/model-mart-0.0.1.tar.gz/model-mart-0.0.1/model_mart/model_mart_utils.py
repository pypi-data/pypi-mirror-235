...
class PandoraBox:
    def __init__(self, username, password):
        if username == "":
            print("Param username is empty")
            return
        if password == "":
            print("Param password is empty")
            return
        info = json.dumps({
            'username': username,
            'password': password
        ...
        if resp.status_code == http.HTTPStatus.OK:
            ...
            if code != 0:
                print(resp.json().get("msg"))
                return
            key = resp.json()['gateway_info']['access_key']
            secret = resp.json()['gateway_info']['access_secret']
            bucket = resp.json()['gateway_info']['bucket']
            gateway = resp.json()['gateway_info']['gateway']
        self.client = Minio(
            gateway,
            access_key=key,
            secret_key=secret,
    ...
    def _get_user_model_list_req(self):
        ...
        base_url = "http://{host}/api/v1/{username}/model/list" \
            .format(host=ADMIN_HOST, username=self.user)
        resp = requests.get(url=base_url, headers=headers)
        return resp
    ...
    def _login_req(self, json_info):
        ...
        base_url = "http://{host}/api/v1/login".format(host=ADMIN_HOST)
        resp = requests.post(url=base_url, data=json_info, headers=headers)
        return resp
