...
class Sys:
    _hx_class_name = "Sys"
    __slots__ = ()
    _hx_statics = ["args", "environment", "systemName"]
...
class com_sdtk_api_API:
    ...
    def fetch(self,method,root,api,key,accessToken,query,body,callback,headers = None,cookies = None):
        ...
        if (accessToken is None):
            accessToken = (self.getAccessToken() is not None)
        cookieToSend = None
        ...
        h = {}
        setattr(h,(("_hx_" + "User-Agent") if (("User-Agent" in python_Boot.keywords)) else (("_hx_" + "User-Agent") if (((((len("User-Agent") > 2) and ((ord("User-Agent"[0]) == 95))) and ((ord("User-Agent"[1]) == 95))) and ((ord("User-Agent"[(len("User-Agent") - 1)]) != 95)))) else "User-Agent")),"SimpleData-Toolkit")
        if accessToken:
            key = self.getAuthorizationHeader()
            value = (HxOverrides.stringOrNull(self.getAuthorizationHeaderBearer()) + HxOverrides.stringOrNull(self.getAccessToken()))
            setattr(h,(("_hx_" + key) if ((key in python_Boot.keywords)) else (("_hx_" + key) if (((((len(key) > 2) and ((ord(key[0]) == 95))) and ((ord(key[1]) == 95))) and ((ord(key[(len(key) - 1)]) != 95)))) else key)),value)
        if (self.getUserHeader() is not None):
            key = self.getUserHeader()
            value = self.getUser()
            setattr(h,(("_hx_" + key) if ((key in python_Boot.keywords)) else (("_hx_" + key) if (((((len(key) > 2) and ((ord(key[0]) == 95))) and ((ord(key[1]) == 95))) and ((ord(key[(len(key) - 1)]) != 95)))) else key)),value)
        tmp = None
        if (body is not None):
            ...
            tmp = False
        if tmp:
            setattr(h,(("_hx_" + "Content-Type") if (("Content-Type" in python_Boot.keywords)) else (("_hx_" + "Content-Type") if (((((len("Content-Type") > 2) and ((ord("Content-Type"[0]) == 95))) and ((ord("Content-Type"[1]) == 95))) and ((ord("Content-Type"[(len("Content-Type") - 1)]) != 95)))) else "Content-Type")),"multipart/related; boundary=--divider--sdtk--")
        if (headers is not None):
            header = headers.keys()
            while header.hasNext():
                header1 = header.next()
                value = headers.h.get(header1,None)
                setattr(h,(("_hx_" + header1) if ((header1 in python_Boot.keywords)) else (("_hx_" + header1) if (((((len(header1) > 2) and ((ord(header1[0]) == 95))) and ((ord(header1[1]) == 95))) and ((ord(header1[(len(header1) - 1)]) != 95)))) else header1)),value)
        if (cookieToSend is not None):
            value = cookieToSend.b.getvalue()
            setattr(h,(("_hx_" + "Cookie") if (("Cookie" in python_Boot.keywords)) else (("_hx_" + "Cookie") if (((((len("Cookie") > 2) and ((ord("Cookie"[0]) == 95))) and ((ord("Cookie"[1]) == 95))) and ((ord("Cookie"[(len("Cookie") - 1)]) != 95)))) else "Cookie")),value)
        r = urllib.request.urlopen(urllib.request.Request(url, method=method, data=body, headers=h))
        d = r.read()
    ...
    def normalizeMapping(self,mapping):
        return com_sdtk_std_Normalize.nativeToHaxe(mapping)
...
class com_sdtk_api_APIList:
    ...
    def inputs():
        if (com_sdtk_api_APIList._inputs is None):
            com_sdtk_api_APIList._inputs = [com_sdtk_api_GitAPI.instance().filesAPI(), com_sdtk_api_GitAPI.instance().reposAPI(), com_sdtk_api_GitAPI.instance().branchesAPI(), com_sdtk_api_GitAPI.instance().commitsAPI(), com_sdtk_api_GitAPI.instance().retrieveAPI(), com_sdtk_api_BitTorrentAPI.instance().transactionsAPI(), com_sdtk_api_BitTorrentAPI.instance().transfersAPI(), com_sdtk_api_BTCAPI.instance().transactionsAPI(), com_sdtk_api_EtherscanAPI.instance().transactionsAPI(), com_sdtk_api_TableauAPI.instance().pullAPI(), com_sdtk_api_SSRSAPI.instance().pullAPI()]
...
class com_sdtk_api_OrtingoAPI(com_sdtk_api_API):
    ...
    @staticmethod
    def posts(callback,owner):
        def _hx_local_0(r):
            callback(r)
        com_sdtk_api_OrtingoAPI.instance().fetch("GET",com_sdtk_api_OrtingoAPI._ortingoRoot,com_sdtk_api_OrtingoAPI._postsAPI,None,None,owner,None,_hx_local_0)
...
class com_sdtk_api_OrtingoAPIPosts(com_sdtk_api_InputAPI):
    ...
    def retrieveData(self,mapping,callback):
        ...
        def _hx_local_0(r):
            _gthis.parseData(r,mapping,callback)
        com_sdtk_api_OrtingoAPI.posts(_hx_local_0,mapping.h.get("owner",None))
    ...
    def login(callback,user,password,site):
        ...
        if (((user == instance._user) and ((site == instance._site))) and ((instance._token is not None))):
            ...
            def _hx_local_0(response):
                ...
                instance._site = site
                callback(response)
            instance.fetch("POST",com_sdtk_api_TableauAPI._tableauRoot,com_sdtk_api_TableauAPI._signinAPI,None,None,None,haxe_format_JsonPrinter.print(params,None,None),_hx_local_0)
...
class com_sdtk_table_MapRowReader(com_sdtk_table_DataTableRowReader):
    ...
    def next(self):
        if (self._nextKey is None):
            ...
            self.incrementTo(None,self._value,(self._index + 1))
            if self._whole:
                self._nextKey = self._iterator.next()
            else:
                self._nextKey = None
...
com_sdtk_api_TableauAPI._tableauRoot = "https://10ay.online.tableau.com/api/3.5/sites/"
...
com_sdtk_calendar_CronSchedulerExecutor.instance = com_sdtk_calendar_CronSchedulerExecutor()
com_sdtk_calendar_CronSchedulerText.instance = com_sdtk_calendar_CronSchedulerText()
...
com_sdtk_calendar_WindowsTaskSchedulerExecutor.instance = com_sdtk_calendar_WindowsTaskSchedulerExecutor()
com_sdtk_calendar_WindowsTaskSchedulerText.instance = com_sdtk_calendar_WindowsTaskSchedulerText()
...
com_sdtk_std_Version._code = "0.2.0"
