...
class CACSegmentation : public LatticeModel<T, container_type, nDim> {
  ...
  float Speed(LatticeIterator sit, LatticeConstIterator it, T phase_label,
              CACSettings &Settings, int L, int H) {
    float Fg =
        1.0f / (1.0f + pow((float)(*sit) / Settings.G0, Settings.AlphaG));
  ...
  // double Speed(LatticeIterator sit, LatticeConstIterator it, CACSettings
  // &Settings, int L, int H, int Hss)
  // {
  // 	double S = Speed(sit, it, Settings, L, H);
  // 	return S + Hss;
  // }
public:
  CACSegmentation(int LabelsCount)
      : LatticeModel<T, container_type, nDim>{LabelsCount}, Undef{LabelsCount},
        InNB{LabelsCount + 1}, mHessianLoaded{false}, nbh{nullptr} {
  ...
  void Perform(const container_type &Src, container_type &Conditional,
               Threshold<T, container_type> &Thresh, CACSettings &Settings,
    ...
    bool verbose = true;
    T *denoised_data = nlm_denoise(Src.data(), shape, verbose);
    container_type Denoised;
    Denoised.assign(denoised_data, denoised_data + Width * Heigth * Depth);

    DynamicArray<T> I(Width * Heigth);
    std::copy<LatticeConstIterator, typename DynamicArray<T>::iterator>(
        Denoised.cbegin(), Denoised.cbegin() + Width * Heigth, I.begin());
    int L, H;

    DynamicArray<T> enhanced(Depth * Width * Heigth);
    std::copy<LatticeConstIterator, typename DynamicArray<T>::iterator>(
        Denoised.cbegin(), Denoised.cend(), enhanced.begin());
    this->ApplyUnsharpMask(Denoised, enhanced, Settings.UnsharpMaskStrength);
    ...
    if (Thresh.PhasesCount() == 2) {
      ...
      this->Stats2L(enhanced, H, L, this->means[0], this->means[1],
    ...
    container_type SobelImg(enhanced.size());
      ...
      for (size_t i = 0; i < this->mHeight; i++)
        for (size_t j = 0; j < this->mWidth; j++) {
          if ((*cit == Undef) &&
            ...
            // double Fg = 1 / (1 + pow((double)(*sit) / Settings.G0,
            // Settings.AlphaG));

            // //double Intensity = abs(*it - L)>abs(*it - H) ? L : H;
            // int nPhase = this->SelectPhase(*it);
            // double Intensity = this->means[nPhase];
            // double FI = pow(abs((Intensity - (double)*it) / (double)(H - L)),
            // Settings.AlphaI); double S = Fg*FI;
  ...
  void Perform(const container_type &Src, container_type &Conditional,
               container_type &Hss, Threshold<T, container_type> &Thresh,
