...
#include <algorithm>
#include "util.h"
//#include "..\imgprocessing\quantim4.h"
...
enum ThreshodMethods{ Th_nBinoms, Th_Manual, Th_Schluter, Th_PE1, Th_PE2, Th_entropy };

struct ThresholdSettings
{
	ThresholdSettings() :nPeaks(2), LowThreshold(20), HighThreshold(120), IsMultiphase(false), alpha(1.85), nPhases(2)
	{};
	int nPeaks;
	int LowThreshold;
	int HighThreshold;
...
public:
 ...
 //   void print(ostream& os=cout) const;
    void draw(const std::string&, const std::string = std::string("Histogram")) const;
...
void histogram_generate(const InputIterator vbegin, const InputIterator vend,
			const double x0,const double delta, 
			const int size, OutputIterator begin)
{
    ...
    while ( p != vend) 
    {
        if ( *p != ::ext_value(*vbegin) ) {
            ...
            if (bin_no >=0 && bin_no < size ) begin[bin_no]+=1.0;
        }
        ++p;
    }
}
...
public:
    ...
    //void draw(const std::string&, const  std::string= std::string("CDF")) const;
};
...
{
public:
	void compute_sobel(typename container_type::const_iterator &a_it, typename container_type::iterator &a_dit, int ImgWidth, int ImgHeight)
	{
	...
	void compute_sobel3D_old(const container_type &m, container_type &Dest, int ImgWidth, int ImgHeight, int ImgDepth)
	{
		...
		for (size_t k = 0; k < ImgDepth; k++)
		{
			...
			for (size_t i = 1; i < ImgHeight - 1; i++)
			{
				for (size_t j = 1; j < ImgWidth - 1; j++)
				{
					...
					int v = sqrt((vert*vert + hor*hor + depth1*depth1 + depth2*depth2) / 2);
	...
	void compute_sobel3D(const container_type &m, container_type &Dest, int ImgWidth, int ImgHeight, int ImgDepth)
	{
		typename container_type::const_iterator it = m.begin();
		typename container_type::iterator dit = Dest.begin();
		...
		for (size_t k = 0; k < ImgDepth; k++)
		{
			...
			for (size_t i = 1; i < ImgHeight - 1; i++)
			{
				for (size_t j = 1; j < ImgWidth - 1; j++)
				{
					...
					int h_x = prev_mm + prev_pm - prev_mp - prev_pp + 2 * prev_m_ - 2 * prev__p +
					          2 * (mm + pm - mp - pp + 2 * _m - 2 * _p) +
					          next_mm + next_pm - next_mp - next_pp + 2 * next__m - 2 * next__p;

					int h_y = prev_mm + prev_mp - prev_pm - prev_pp + 2 * prev_m_ - 2 * prev_p_ +
					          2 * (mm + mp - pm - pp + 2 * m_ - 2 * p_) +
					          next_mm + next_mp - next_pm - next_pp + 2 * next_m_ - 2 * next_p_;

					int h_z = 1 * (prev_pm + prev_mp + prev_mm + prev_pp) + 
					          2 * (prev_p_ + prev_m_ + prev__m + prev__p) +
						      4 * prev__ - 
							  1 * (next_pm + next_mp + next_mm + next_pp) -
					          2 * (next_p_ + next_m_ + next__m + next__p) -
						      4 * next__;
					
					T v = static_cast<T>(floor(sqrt(h_x * h_x + h_y * h_y + h_z * h_z) + 0.5));
					*dit = v;
				}
			}
		}
...
template <class T = unsigned char, class container_type = DynamicArray<T>> class IThreshold
{
public:
	...
	virtual void compute_cut_offs(const container_type &m, int ImgWidth = 0, int ImgHeight = 0, int ImgDepth = 0) = 0;
};
...
{
public:
	Threshold() : mFlatness(0.001), ilow(0), ihigh(0), mLow(-1), mHigh(-1), mPhasesCount(2)
	{};

	// TODO: Remove not implemented or find implementation
	// Threshold(const Threshold & T);
	
	//void compute_cut_offs(const DynamicArray<T> &m, int ImgWidth=0, int ImgHeight=0, int ImgDepth=0)
	void compute_cut_offs(const container_type &m, int ImgWidth = 0, int ImgHeight = 0, int ImgDepth = 0)
	{

		int min = 0; int max = 256;
//		::minmax<DynamicArray<T>::const_iterator, int>(m.begin(), m.end(), min, max);
		...
		case Th_entropy:
			...
			thisx = hist.x_0();		px = x.begin();
			for (i = 0; i < n_point; i++, thisx += delta, px++) *px = thisx;
	...
	void compute_entropy(const DynamicArray<T>& m, DynamicArray<double>& entropy)
	{
		...
		double &low, double &high)
	{
		...
		else error("There is more than 1 threshold.", "");
	...
	//void Schluter(const DynamicArray<T> &m, int ImgWidth, int ImgHeight, int ImgDepth, double &Low, double &High)

	// void CutOffsSchluter2(const DynamicArray<T> &m, int ImgWidth, int ImgHeight, int ImgDepth, double &Low, double &High)
	// {
	// 	image_cc * img;
	...
	void Schluter(const container_type &m, int ImgWidth, int ImgHeight, int ImgDepth, double &Low, double &High)
	{
	...
	void SchluterHist(const container_type &m, int ImgWidth, int ImgHeight, int ImgDepth, std::vector<int> &h)
	{
		LogFile::WriteData("kriging.log", "Shlutter Sterted ");
	...
	void SetMultiphaseThresholds(std::vector<double> Lows, std::vector<double> Highs)
	{
		assert(Lows.size() == Highs.size());
		mLows = Lows;
		mHigh = Highs;
		/*mLows.resize(Lows.size());
		mHighs.resize(mLows.size());
		std::copy(Lows.begin(), Lows.end(), mLows.begin());
		std::copy(Highs.begin(), Highs.end(), mHighs.begin());*/
		mPhasesCount = Highs.size();
	}

	void HessianCutOff(const DynamicArray<T> &m, int ImgWidth, int ImgHeight, int ImgDepth, int &Threshold)
	{
		Histogram hist_m(m, 256);
		NullAtBegin(hist_m);
	...
	Mixture mix;
	std::vector<double>LowThresholds()const
	{
		return mLows;
	}
	std::vector<double>HighThresholds() const
	{
		return mHighs;
	}
	int PhasesCount()const
	{
		return mPhasesCount;
	}

	void NBinoms(const DynamicArray<T> &m, int nBinoms, std::vector<double> &M, std::vector<double> &V, std::vector<double>&S)
	{
		int i;
		LogFile::WriteData("kriging.log", "NBinoms started");
		std::vector<int> RandArr(10000);
		GetRandomDynamicArray(m, RandArr, 10000);
		double dummy; double whole_mean, whole_var;
		::stats(RandArr.begin(), RandArr.end(), dummy, dummy, whole_mean, whole_var);
		...
		ExpectedMaximized<int> em(RandArr, nBinoms, mean_seed, sd_seed);
		em.Estimate();
		for (int i = 0; i < nBinoms; i++)
		{	
	...
	void CutOffsPE3(const DynamicArray<T> &m, int ImgWidth, int ImgHeight, int ImgDepth, int nAtans, std::vector<double> &M, std::vector<double> &V, std::vector<double> &S)
	{
		...
		for (auto itSobel = SobelOut.begin(); itSobel != SobelOut.end(); itSobel++, itM++)
			if (*itSobel < max_sobel_val && *itM>waste_val_cutoff && *itM < waste_val_cutoff2)
			{
				*itImg = *itM;
				itImg++;
			}
		...
		double nElements = std::accumulate(histImg.begin(), histImg.end(), nElements);
	...
	/*	class AtanFitObjective :public SAObectiveBase
		{
			double Calulate(const void *pData, const double *params)
			{
				...
				for (int i = 0; i<mH.size(); i++)
				{
					...
					for (int j = 0; j < mAtansCount; j++)
					{
						...
						double a = params[j + 2 * mAtansCount];
						double dx = (i - m);
						//v += a / (1 + b*dx*dx);
						v += a *exp(-dx*dx / b / b);
					}
			...
			int mAtansCount;
		public:
			AtanFitObjective(const std::vector<double> &H, int nAtans) :mH(H), mAtansCount(nAtans)
			{
			}
		};
		...
		for (int i = 0; i < nAtans; i++)
		{
			...
			std::vector<double> Params;Params.resize(3 * nAtans);
			std::vector<double> UpperBounds; UpperBounds.resize(3 * nAtans);
			std::vector<double> LowerBounds; LowerBounds.resize(3 * nAtans);
...
private:
	// TODO: Remove not implemented or find implementation
	//void estimate_2_normals(const Histogram&, double&, double&, double&, double&, double&);
	void compute_harris(const DynamicArray<T> &m, DynamicArray<int> &Dest, int ImgWidth, int ImgHeight)
	{
	...
	void compute_diff(const DynamicArray<T> &m, DynamicArray<int> &Dest, int ImgWidth, int ImgHeight)
	{
	...
	void compute_sobel(typename DynamicArray<T>::const_iterator &a_it, DynamicArray<int>::iterator &a_dit, int ImgWidth, int ImgHeight)
	{
		int shift_it = ImgWidth + 1;
		int shift_dit = ImgWidth + 1;
		auto it = a_it;
	...
	void compute_sobel3D(const DynamicArray<T> &m, DynamicArray<int> &Dest, int ImgWidth, int ImgHeight, int ImgDepth)
	{
		typename DynamicArray<T>::const_iterator it = m.begin();
		DynamicArray<int>::iterator dit = Dest.begin();
	...
	void GetRandomDynamicArray(const container_type &m, std::vector<int> &RandArr, int N)
	{
	...
	void peaking(Histogram &h, double &P1, double &P2, int step, double Threshold, int WidthThreshold)
	{
	...
	void CutOffsSchluter(const container_type &m, int ImgWidth, int ImgHeight, int ImgDepth, double &Low, double &High)
	{
	...
	void CutOffsBinoms(const container_type &m, double &Low, double &High)
	{
		...
		double dummy; double whole_mean, whole_var;
		::stats(RandArr.begin(), RandArr.end(), dummy, dummy, whole_mean, whole_var);
		double mean_seed[3]; mean_seed[0] = whole_mean - sqrt(whole_var); mean_seed[1] = whole_mean + sqrt(whole_var); mean_seed[2] = 7;
		...
		double sd_seed[3]; sd_seed[0] = 10; sd_seed[1] = 10; sd_seed[2] = 10;
		LogFile::WriteData("kriging.log", "initial parameters calculated");
		ExpectedMaximized<int> em(RandArr, mPeaksCount, mean_seed, sd_seed);
		em.Estimate();
		LogFile::WriteData("kriging.log", "peaks estimated");
		if (mPeaksCount == 3)
		{
			int idx[3];
			::sort_idxtbl(em.pi.begin(), em.pi.end(), idx);
	...
	void CutOffsPE1(const container_type &m, int ImgWidth, int ImgHeight, int ImgDepth, double &Low, double &High)
	{
	...
	void CutOffsPE2(const container_type &m, int ImgWidth, int ImgHeight, int ImgDepth, double &Low, double &High)
	{
		/*DynamicArray<int> SobelOut(ImgWidth*ImgHeight);
		DynamicArray<int> ImgSobelOut(ImgWidth*ImgHeight);
		compute_diff(m, SobelOut, ImgWidth, ImgHeight);
