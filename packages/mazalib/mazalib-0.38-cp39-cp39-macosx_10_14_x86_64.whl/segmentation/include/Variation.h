...
enum    V_method    { Unknown_V=0,
                      mw_semivariogram,
                      classic_semivariogram,
                      classic_covariance};
enum    V_type      { Unknown_T=0, Semivariogram, Covariance};
enum    V_direction { Unknown_D=0,
                      Vertical, Horizontal,
                      Isotropic, Slice_Isotropic,
                      Diagonal};
...
public:
    Variation(int m=1, double dist=1.0,V_direction d=Isotropic,
	    ...
	    : DynamicArray<double>(m+1,0.0), unit_lag_(dist),
	      direction_(d), method_(v), type_(type), print_type_(ptype) {}
    ...
    void compute(const vector<float>&, const Point*, int dim); // driver for given data
    ...
    template <class Iterator>
    void compute(Iterator m, int *count, double mean, int size, 
		 int me=0, int p=1) {
      var_compute(*this,m,count,mean,size,me,p);
    }
/*
    void compute(const int *m, double mean, int size, int me=0, int p=1) {
      var_compute(*this,m,mean,size,me,p);
    }
    void compute(const float *m, double mean, int size, int me=0, int p=1) {
      var_compute(*this,m,mean,size,me,p);
    }
    void compute(const int *m, int *count, double mean, int size, 
		 int me=0, int p=1) {
      var_compute(*this,m,count,mean,size,me,p);
    }
    void compute(const float *m, int *count, double mean, int size, 
		 int me=0, int p=1) {
      var_compute(*this,m,count,mean,size,me,p);
    }
*/
    ...
    template <class Iterator>
    void variation_slice(Iterator m,const Grid &grid,int me,int p) { // 3d driver for slicewise isotropic
      var_variation_slice(*this,m,grid,me,p);
    }
};

template <class T>
void var_compute(Variation &var, const  std::vector<T>&, const Point*, int dim);

template <class Iterator>
void var_compute(Variation &var, Iterator m, double mean, 
		 int size, int me, int p);

template <class Iterator>
void var_compute(Variation &var, Iterator m, int *count, double mean,
		 int size, int me, int p);

template <class Iterator>
void var_compute(Variation& var, Iterator m, const Grid& grid, 
		 int me, int p);

template <class Iterator> 
void var_variation_i(Variation &var, Iterator m, const Grid &grid,
		     int me, int p);

template <class Iterator>
void var_variation_h(Variation &var, Iterator m, const Grid &grid,
		     int me, int p);

template <class Iterator> 
void var_variation_v(Variation &var, Iterator m, const Grid &grid, 
		     int me, int p);

template <class Iterator>
void var_variation_slice(Variation& var,Iterator m, const Grid& grid,
			 int me, int p);

	// Moving window variogram

template <class T>	// measured data
void mw_variogram(Variation &var,const  std::vector<T>&,const Point*,int,int,int);
...
template <class T>	// measured data
void classic_variogram(Variation &var,const  std::vector<T> &, const Point*,int,int,int);
template <class Iterator>	// 1d generic
...
template <class T>	// measured data
void covariance(Variation &var,const  std::vector<T> &,const Point *,int,int,int);
template <class Iterator>	// 1d generic
...
template <class T>
void var_compute(Variation &var, const  std::vector<T> &v,const Point *x, int dim)
{
    const int me=mynode();
    ...
    var.front()=::variance(v.begin(),v.end());
}


template <class Iterator>
void var_compute(Variation &var, Iterator v, double mean, int size, 
		 int me, int p)
{
    ...
    switch(var.method()) {
        case classic_covariance:
            if (mean==0.0) mean=::mean(v,v+size);
    ...
    var.front()=::variance(v,v+size);
}


template <class Iterator>
void var_compute(Variation &var, Iterator v, int *count, double mean, int size, int me, int p)
{
    fill(var.begin(),var.end(),0.0);
...
template <class Iterator>
void var_compute(Variation& var, Iterator m, const Grid& grid, int me, int p) // me = 0, p = 1
{
	//string fname("void compute(Variation&,Iterator,const Grid&,int,int)");
	fill(var.begin(),var.end(),0.0);
	...
	case Slice_Isotropic:
	  ...
	  break;
	default:
	;//  error("No such direction",fname); 
	}
	LogFile::WriteData("kriging.log", "	var.front()=::variance(m,m+grid.size());");
...
#ifdef _PARALLEL
    if (p>1) divide_by_pnodes(n,n_node,n_offset,me,p);
#endif
...
#ifdef _PARALLEL
    if (p > 1) {
	::global_sum(m.begin(),m.size());
	::global_sum(D.begin(),D.size());
    }
#endif

    double *pv=var.begin()+1;
    for(lag=1;lag<var.size();lag++,pv++) {
	*pv=0.0;
	double *pD=D.begin()+lag;
	double *pm=m.begin()+lag;	
...
template <class Iterator>
void mw_variogram(Variation &var, Iterator v, int length,
		  int me, int p)
{
...
#ifdef _PARALLEL
    if (p>1) divide_by_pnodes(length,x_node,x_offset,me,p);
#endif
	...
	for(int x=x_offset; x<x_offset+x_node;++x,++pv) {
	    if(*pv==ext_value) continue;
	    // update Dh[index] and mh[index]
            double diff;
	    if (x-h>=0) {   // left end
		if(*(pv-h)==ext_value) continue;
		diff=double(*(pv-h) - *pv);
		Dh[x] += float(diff*diff); mh[x]++;
	    }
...
template <class Iterator>
void classic_variogram(Variation &var, Iterator v, int length,
		       int me, int p)
{
		       ...
		       int dim, int me, int p)
{
    ...
    if (p > 1) {
	global_sum(m.begin(),m.size());
	global_sum(pt,var.size());
    }
#endif 
    for( h = 1; h < var.size(); h++) {
	if (m[h] >0) var[h] *= (0.5/m[h]);
    }
}
		       ...
		       int me, int p)
{
	...
	for(int x=me;x<length-h;x+=p,pv += p) {
	    if(*pv==ext_value || *(pv+h)==ext_value) continue;
    ...
    if (p > 1) {
	global_sum(pt,var.size()());
	global_sum(count,var.size());
    }
#endif
    for (h=1;h<var.size();h++) {
	if (count[h] > 0) var[h] /= (2.0*count[h]);
    }

    var.front()=::variance(v,v+length);
}
		...
		int dim, int me, int p)
{
    ...
    ::stats(v.begin(),v.end(),mean,variance);
    var.front()=variance;
...
template <class Iterator>
void covariance(Variation &var, Iterator v, double mean, int length, 
		int me, int p)
{
    DynamicArray<int> count(var.size());
...
template <class Iterator>
void covariance(Variation &var, Iterator v, int* count, double mean,
		int length, int me, int p)
{
    typedef typename Iterator::value_type T;
    const T ext_value = ::ext_value(T(0));
    count[0]=0;
    
    if (mean==0.0) mean=::mean(v,v+length);
