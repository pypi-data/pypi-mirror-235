...
//			 int me, int p)
{
	std::string fname("void var_variation_slice(Variation& var, const T *,const Grid&,int,int)");
    
    fill(var.begin(),var.end(),0.0);
    Variation slice_var(var.max_lag(),var.unit_lag(),Isotropic,var.method(),var.type());
...
//  typedef float T;
	...
	while( pv < var.end() ) { *pv += *ps; ++pv; ++ps; }
    }
    var.set_type(slice_var.type());
#ifdef MPI
    if (p>1) global_sum(var.begin(),var.size());
#endif
    diterator pv=slice_var.begin();
    while( pv < var.end() ) {*pv *= (1.0/depth); ++pv; }
}



template <class Iterator>
void mw_variogram(Variation &var, Iterator m,
		  int nx, int ny, int nz, int me, int p)
{
	std::string fname("void mw_variogram(Variation&,Iterator,int,int,int,int,int)");
	LogFile::WriteData("kriging.log", "mw_variogram(Variation &var, Iterator m,	int nx, int ny, int nz, int me, int p)");
...
#ifdef MPI
    if (p>1) divide_by_pnodes(nx*ny*nz,n_node,offset,me,p);
#endif
	...
	try
	{
			for (size_t index = offset; index<to - index_inc; index += index_inc, pp += index_inc)
			{
				if (*pp == ext_val) continue;
				compute_D(m, D, nx, ny, nz, index, maxlag);
				for (int h=1;h<size_;++h) var[h] += D[h];
				LogFile::WriteData("kriging.log", "or (size_t index = offset; index<to - index_inc; index += index_inc, pp += index_inc)", index_inc);
			}    
			...
			var.print();
			diterator pv = var.begin();
			while (pv < var.end()) { *pv *= (0.5 / (n_node)); ++pv; }

			LogFile::WriteData("kriging.log", "add_derivative_correction(var, 3);");
			

			add_derivative_correction(var, 3);
			LogFile::WriteData("kriging.log", "add_derivative_correction(var, 3) finished;");
			var.print();
	}
...
template <class Iterator>
void compute_D(Iterator m, std::vector<double>& D,
               int cols, int rows, int depth, long long index, int lag) 
{
...
//	LogFile::WriteData("kriging.log", "compute_D index", index);
  ...
  //  for (int h=0;h<=lag;++h) {
	...
	//LogFile::WriteData("kriging.log", "compute_D    if ((k=z-h) >= 0) {	;", index);
	if ((k=z-h) >= 0) {	    
	    for(j=startr;j<tor;++j) {
		p=m+(k*rows+j)*cols+startc;
		for(i=startc;i<toc;++i,++p) {
		    if ((v=*p) == ext_val) continue;
		    diff=(double)(v - center);
		    tmpDh += diff*diff; ++mh;
		}
	    }
	}
//	LogFile::WriteData("kriging.log", "compute_D   if ((k=z+h) < depth) {	;", index);
	if ((k=z+h) < depth) {
	    for(j=startr;j<tor;++j) {
		p = m + (k*rows + j)*cols + startc;
		for(i=startc;i<toc;++i,++p) {
...
//	LogFile::WriteData("kriging.log", "compute_D startd = max<size_t>(z - h + 1, 0);", index);
	...
	//LogFile::WriteData("kriging.log", "compute_D if ((j=y+h)<rows) {", index);
	if ((j=y+h)<rows) {
	    for(k=startd;k<tod;++k) {
		p = m + (k*rows + j)*cols + startc;
		for(i=startc;i<toc;++i,++p) {
...
template <class Iterator>
void classic_variogram(Variation &var, Iterator m,
		       int nx, int ny, int nz, int me, int p)
{
    const std::string fname("void classic_variogram(Variation&,const T *m, int, int, int, int, int)");
    test(fname);
    ...
    for(int k=me;k<depth;k+=p) {
        ...
        for(int j=0;j<rows;j+=p) {
            ...
            for(int i=0;i<cols;++i) {
                ...
                if ((v=m[(k*rows+j)*cols+i]) == ext_val) continue;
                ...
                for(int z=from_z;z<to_z;++z) {
					for (int y = from_y; y<to_y; ++y) {
						...
						for (int x = from_x; x<to_x; ++x, ++pm) {
                            if ((v1=*pm)==ext_val) continue;
                            h=iround(sqrt(double((x-i)*(x-i)+(y-j)*(y-j)+(z-k)*(z-k))));
...
#endif
    for (h=1;h<=maxlag;h++) {
	if (count[h] > 0UL) var[h] /= (2.0*count[h]);
    }
}


template <class Iterator>
void covariance(Variation &var, Iterator m,
		int nx, int ny, int nz, int me, int p)
{
	std::string fname("void covariance(Variation&,Iterator,int,int,int,int,int)");
    test(fname);
    
    double mean=::mean(m,m+nx*ny*nz);
                ...
                for(int z=from_z;z<to_z;++z) {
                    for(int y=from_y;y<to_y;++y) {
                        for(int x=from_x;x<to_x;++x) {
                            if ((v1=m[(z*rows+y)*cols+x]) == ext_val) continue;
                            h=iround(sqrt(double((x-i)*(x-i)+(y-j)*(y-j)+(z-k)*(z-k))));
                            if (h>maxlag) continue;
