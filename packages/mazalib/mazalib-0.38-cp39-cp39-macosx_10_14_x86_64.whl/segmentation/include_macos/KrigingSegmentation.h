...
template <class T = int, class CT = std::vector<T>, int nDim = 2>
class KrigingProcessor : public LatticeModel<T, CT, nDim> //, public ITestable
{
...
public:
  ...
  void kriging_2way(const Grid &grid, CT &data_int, CT &status,
    ...
    std::vector<DynamicArray<double>> ok_system(
        met.PhasesCount()); // (size*size);
    for (int i = 0; i < met.PhasesCount(); i++)
      ok_system[i].resize(size * size);
    // DynamicArray<double> ok_system0(size*size);
    // DynamicArray<double> ok_system1(size*size);

    std::vector<DynamicArray<double>> rhs(met.PhasesCount());
    for (int i = 0; i < met.PhasesCount(); i++)
      rhs[i].resize(size);
    ///	DynamicArray<double>	rhs0_(size);
    // DynamicArray<double> rhs1_(size);
    //	diterator rhs0 = rhs0_.begin();
    //	diterator rhs1 = rhs1_.begin();

    // setup the ordinary Kriging matrices
    //!!!
    LogFile::WriteData("kriging.log", "Setup ordinary kriging");
    //	ok_system_setup2(ok_system0, n_of_data, *var_T[0], coord);
    //	ok_system_setup2(ok_system1, n_of_data, var_T1, coord);
    ...
    /*DynamicArray<double> cond_data0_(n_of_data);
    DynamicArray<double> cond_data1_(n_of_data);
    diterator cond_data0 = cond_data0_.begin();
    diterator cond_data1 = cond_data1_.begin();*/
    ...
    for (size_t i = 0; i < status.size();
      ...
      if (i % five_per == 0) {
        // std::cout << 5 * i / five_per << " percent voxels done\n";
      ...
      if (*pind != to_be_ESTIMATED) {
        /** Masha - addition **
        if(     *pind == marked_PORE
        || *pind == filtered_PORE )   *pseg_data_float = -0.5;
        else if(*pind == marked_ROCK
        || *pind == filtered_ROCK )   *pseg_data_float = 0.5;
        **/
        continue;
      }
      ...
      for (int k = 0; k < nPhases - 1; k++) {
        ...
        // P_1 = inner_product(rhs1, rhs1 + n_of_data, cond_data1, 0.0);
        if (P_0 > 1.0 - P_1)
          *pind = to_be_ESTIMATED + 1 + k;
        else
          *pind = to_be_ESTIMATED + 1 + k + 1;

        // if (*pind != 141 && *pind != 142)
        //{
        //	std::cout << *pind << std::endl;
        // }

        //	*pseg_data_float = -0.5 + 1.0 - (P_0 + P_1) / 2.0;
      }
      //	double * it=std::max_element(&P[0], &P[0] + nPhases);
      //	int nP = std::distance(&P[0], it);

      //	*pind = to_be_ESTIMATED+1+nP;
    }
  }
  ...
  //
  void krig_dat(char method, CT &raw_data, const Grid &grid,
