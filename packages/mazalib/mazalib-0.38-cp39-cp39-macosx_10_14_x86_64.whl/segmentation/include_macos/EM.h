...
{
public:
	ExpectedMaximized(const std::vector<T> & data, int nClasses, double *seeds_Mean = NULL, double *seeds_Std = NULL) :nClasses(nClasses), min_std(0.001)
	{
	...
	double NormDistr(double x, int nClassNum)
	{
		...
		double v=1/(sqrt(2.0 * WIN_PI)*sd[nClassNum])*exp(-(delta*delta/2/sd[nClassNum]/sd[nClassNum]));
		return v;
	}
	...
	bool IsConverged(const std::vector<double> &pi, const std::vector<double> &pi_old, double Eps)
	{
		double S=0;
		for (int j=0;j<nClasses;j++)
