...
struct Point3d
{
	...
	Point3d(const Point3d<size_t> &pt) : x(pt.x), y(pt.y), z(pt.z) {};
	//	Point3d<T> & operator = (const Point3d<short> &pt){ return *this} //: x(pt.x), y(pt.y), z(pt.z){};

	//float Dist(const Point3d &pt)const;
	//Point3d operator - (const Point3d& p)const;
	//Point3d operator * (float lambda)const;
	template <class T2 = float>
	float Dist(const Point3d<T2> &pt)const
	{
		return sqrt(sqr(pt.x - this->x) + sqr(pt.y - this->y) + sqr(pt.z - this->z));
	}
	...
	template <class T2 = T>
	Point3d operator-(const Point3d<T2> &pt)const
	{
	...
	template <class T2 = T>
	T operator * (Point3d<T2> &pt)const
	{
		return this->x*pt.x + this->y*pt.y + this->z*pt.z;
	}
	template <class T2 = T>
	Point3d operator ^ (Point3d<T2> &pt)const
	{
