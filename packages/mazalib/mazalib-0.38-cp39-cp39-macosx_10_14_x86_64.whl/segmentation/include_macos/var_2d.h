...
void var_variation_i(Variation &var, Iterator m, const Grid& grid,
		     int me, int p)
{
	std::string fname("void var_variation_i(Variation&,const T *,const Grid&,int,int)");
    ...
    if (dim==2) {
        ...
        switch (var.method()) {
            ...
            case classic_covariance:
                ...
                break;
            case Unknown_V:
                warning("Unknown method for semivariogram/covariance",
			fname.c_str());
                break;
            default:
...
void var_variation_h(Variation &var, Iterator m, const Grid& grid,
		     int me, int p)
{
	std::string fname("void var_variation_h(Variation&,const T*, const Grid&, int, int)");
    if( grid.dim()==2 && grid.n_z() != 1) error("Grid error",fname);
	//int * cm=&(*m);
    double mean0=mean(m,m+grid.size());
    
    fill(var.begin(),var.end(),0.0);
    Variation tmp(var.max_lag(),var.unit_lag(),V_direction::Horizontal,var.method(),var.type());
    int col=grid.n_x();
...
void var_variation_v(Variation &var, Iterator m, const Grid& grid, 
		     int me, int p)
{
	std::string fname("void var_variation_v(Variation&,const T*,const Grid&,int,int)");
    not_implemented(fname);
    
    if ( grid.dim()==2 && grid.n_z() != 1) error("Grid error",fname);
	//int *cm=&(*m);
    double mean=::mean(m,m+grid.size());
	...
	Variation tmp(var.max_lag(), var.unit_lag(), V_direction:: Vertical, var.method(), var.type());
    ...
    for (int k=0;k<depth;++k) {     // process k-th slice
        for (int i=0;i<cols;++i) {  // process i-th column
            ...
            diterator pv   = var.begin();
            diterator ptmp = tmp.begin();
            while( pv < var.end() ) { *pv += *ptmp; ++pv; ++ptmp; }
        }    
    }
    var.set_type(tmp.type());    
#ifdef MPI
    if (p>1) global_sum(begin(),size());
#endif
    diterator pv = var.begin();
    while( pv < var.end() ) { *pv *= (1.0/cols*depth); ++pv; }
}
...
void compute_D(Iterator m, std::vector<double>& D,
               int cols, int rows, int index, int lag) 
{
    ...
    for (int h=1;h<=lag;++h) {
       ...
       if (y+h<rows) {  // top side;
        ...
        if (mh>0) D[h] =(Dh_1+tmpDh)/mh;
    }
}


template <class Iterator>
void classic_variogram(Variation &var, Iterator m,int nx, int ny, 
		       int me, int p)
{
    std::string fname("void classic_variogram(Variation&,const T*,int,int,int,int)");
	...
	for(int i=0;i<cols;++i) {
	    ...
	    if ((v=m[j*cols+i]) == ext_value) continue;
...
{
	std::string fname("void Variation::covariance(Variation &var,const T*,int,int,int,int)");

    double mean=::mean(m,m+nx*ny);
	...
	for(int i=0;i<cols;++i) {
		...
		for(int x=from_x;x<to_x;++x) {
		    if ((v1=m[y*cols+x]) == ext_value) continue;
		    h=iround(sqrt(double((x-i)*(x-i)+(y-j)*(y-j))));
		    if (h>maxlag) continue;
...
#endif
    for (h=1;h<=maxlag;h++) {
	if (count[h] > 0) var[h] /= (float)count[h];
    }
