    ...
    def read_as_data_frame(self, table_name: str = "") -> pandas.DataFrame:
        ...
        if self.get_request_type().lower() == 'get':
            if self.get_param() is None:
                request.urlretrieve(self.get_url(), table_name)
            else:
        ...
        if self.get_persist_file_type().lower() == "csv":
            return pandas.read_csv(table_name)
        elif self.get_persist_file_type().lower() == "xlsx":
            return pandas.read_excel(table_name)
        elif self.get_persist_file_type().lower() == "json":
            return pandas.read_json(table_name, typ='series')
        else:
