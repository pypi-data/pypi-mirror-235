...
def extract_attributes(dict_object: dict) -> List[StringAttribute] :
    """
    Args:
        run:
    Returns:
    extract key value from nested dictionary object
    """
    attributes = []
    def _extract_attributes(run_searlized_object: dict) -> None:
        for key, object in run_searlized_object.items():
            if key in ['id', 'lc', 'type']:
                continue
            if isinstance(object, str):
                attributes.append(StringAttribute(key, object))
            elif isinstance(object, bool):
                attributes.append(StringAttribute(key, str(object)))
            elif isinstance(object, int):
                attributes.append(IntAttribute(key, object))
            elif isinstance(object, list):
                # TODO if list has dictionary inside it
                attributes.append(StringAttribute(key, str(object)))
            elif isinstance(object, dict):
                _extract_attributes(object)
            else:
                continue
    if isinstance(dict_object, dict):
        _extract_attributes(dict_object)
    return attributes
...
class SpanGenerator(object):
    ...
    def _convert_run_to_sg_span(self, run: Run, trace_id: str) -> "Span":
        ...
        attributes = []
        attributes.append(IntAttribute(key="execution_order", value=run.execution_order))
        if hasattr(run, "serialized"):
            attributes_from_serialized_obj = extract_attributes(run.serialized)
            # TODO remove duplicate attributes
            attributes.extend(attributes_from_serialized_obj)
        return Span(
    ...
    def _convert_unknown_type_run_to_sg_span(self, run: Run, trace_id) -> "Span":
        ...
        base_span.type = SpanType.UNKNOWN
    ...
    def _convert_llm_run_to_sg_span(self, run: Run, trace_id) -> "Span":
        ...
        """
        base_span = self._convert_run_to_sg_span(run, trace_id)
        # attributes = extract_attributes_from_serialized(run)
        # base_span.attributes.extend(attributes)
        # TODO: we will generic way to add attributes
        if "llm_output" in run.outputs:
            attributes = extract_attributes(run.outputs["llm_output"])
            base_span.attributes.extend(attributes)
        if 'messages' in run.inputs and run.inputs['messages'][0]:
            ...
            result = Result()
            for input_key, value in run.inputs.items():
                if input_key == "prompts":
                    for ndx, prompt in enumerate(value):
                        result.inputs.append(TextDocument("prompt", str(prompt)))
                else:
                    result.inputs.append(TextDocument(input_key, str(value)))


            for g_i, gen in enumerate(run.outputs["generations"][0]):
                gen_doc = GenerationDocument(f"gen_{g_i}", gen["text"])
                if "generation_info" in gen:
                    gen_doc.generation_info = GenerationInfo(gen["generation_info"]['finish_reason'] if 'finish_reason' in gen["generation_info"] else None)
                else:
                    gen_doc.generation_info = GenerationInfo()
                result.outputs.append(gen_doc)
            #result.outputs = [TextDocument(k, str(v)) for k, v in run.outputs.items()]
            base_span.results.append(result)
    ...
    def _convert_chain_results_to_sg_results(self, run):
        ...
        for key, input_data in run.inputs.items():
            ...
            if key in ["input_documents", "context"] and not str(input_data):
                ...
                text_document = TextDocument(key, str(input_data))
                result.inputs.append(text_document)
        ...
        for key, output in run.outputs.items():
            ...
            if key in ["documents", "context"]:
                ...
                text_document = TextDocument(key, str(output))
                result.outputs.append(text_document)
        return [result]
    ...
    def _convert_lc_run_to_sg_span(self, run: Run, trace_id: str) -> "Span":
        ...
        if run.run_type == "llm":
            ...
            return self._convert_unknown_type_run_to_sg_span(run, trace_id)
