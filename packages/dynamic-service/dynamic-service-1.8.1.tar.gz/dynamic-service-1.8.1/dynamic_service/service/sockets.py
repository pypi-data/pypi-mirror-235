...
class SocketService(ServiceInterface, EndpointsService):
    ...
    def __init__(
        ...
        self._server_operator: Optional[Operator] = None
    # end __init__
    ...
    def serving(self) -> bool:
        ...
        return (
            (self._server_operator is None) or
            self._server_operator.operating
        )
    # end serving
    ...
    def run(
        ...
        if self._server_operator is None:
            self._server_operator = Operator(
                operation=lambda: self.server.handle(
                    action=action, protocol=protocol, sequential=sequential
                )
            )

            self.operators.append(self._server_operator)
        # end
