...
class FlixbusBase(object):
    ...
    def get_ids_of_cities(self):
        ...
        city_list = [city[0] for city in city_list_db]  # Convert to slug format

        # Fetch and parse bus ID for each city
        for city in city_list:
            try:
                url = f"https://www.flixbus.com/bus/{city.lower().replace(' ', '-')}"
    ...
    def parse_content(self, content):
        ...
        for trip in trips:
            ...
            for _, trip_detail in trip_results.items():
                ...
                duration_str = f"{trip_detail['duration']['hours']}h {trip_detail['duration']['minutes']}m"
