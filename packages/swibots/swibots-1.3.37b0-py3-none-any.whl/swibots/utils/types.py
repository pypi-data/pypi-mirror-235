...
class UploadProgress(AbstractProgressListener):
    ...
    async def bytes_readed(self, length):
        ...
        if self.callback:
            iscoro = self.callback(UploadProgress(self.path, self.callback, self.callback_args, self.client, readed=self.readed, current=length), *self.callback_args or ())
    ...
    def runCallback(self, current):
        ...
        if self.callback:
            iscoro = self.callback(UploadProgress(self.path, self.callback, self.callback_args, self.client, readed=self.readed, current=current), *self.callback_args or ())
            if iscoroutinefunction(self.callback):
