...
from quickverifyimg.utils.crop_utils import get_auto_crop_image, resize_img
...
class QuickVerifyVideo():
    def __init__(self, verify_engine_list, match_rate_threshold, crop_place=None, quick_verify=False,
                 frame_save_dir=None, background_path=None, offset_size=10, background_similar=0.998, auto_crop=False,
                 col_sum_thr=600, row_sum_thr=600):
        """
    ...
    def verify_video_effect(self, origin_video_path, target_video_path, img_threshold=10):
        ...
        origin_frame_thread = MyThread(self._analyse_origin_frame, args=(),
                                       kwargs={"video_frame_path": origin_video_frame_path})
        origin_frame_thread.start()
        target_frame_thread = MyThread(self._analyse_origin_frame, args=(),
                                       kwargs={"video_frame_path": target_video_frame_path})
    ...
    def _analyse_origin_frame(self, video_frame_path=""):
        ...
        def analyse_frame(all_list_temp, background_img, verify_engine_list):
            ...
            for image_name in all_list_temp:
                ...
                start_time = time.time()
                is_background, best_similar, best_engine, image1_crop, image2_crop = self._is_image_similar(img,
                                                                                                            background_img,
                                                                                                            verify_engine_list,
                                                                                                            is_background=True,
                                                                                                            skip_auto_crop=False)
                logger.debug(f"背景图比对耗时: {time.time() - start_time}")
                if not is_background:
                    # img_list.append(img_path)
                    img_info = {
                        "path": img_path,  # 路径
                        "img": image1_crop,  # 裁剪后的图片ndarray数组对象
                    }
        ...
        for i in range(0, len(all_effect_frame_save_dir_list), img_threshold):
            ...
            thread.start()
            thread_list.append(thread)
        logger.info(f"筛选视频帧总线程数：{len(thread_list)}")
        for t in thread_list:
            t.join()
            ret = t.get_result()
            if ret:
    ...
    def _is_image_similar(self, image1, image2, verify_engine_list, is_background=False, skip_auto_crop=True):
        ...
        if self.auto_crop and not skip_auto_crop:
            image1_crop, image2_crop = get_auto_crop_image(image1, image2, self.col_sum_thr, self.row_sum_thr)
        elif self.auto_crop and skip_auto_crop:
            # 跳过裁剪，即已裁剪过，特效比对阶段
            image1_crop, image2_crop = resize_img(image1, image2)
        best_similar = 0
        best_engine = ''
        for engine in verify_engine_list:
            ...
            if require_similiar <= similar:
                return True, similar, engine[0], image1_crop, image2_crop
            else:
                if best_similar < similar:
                    best_similar = similar
                    best_engine = engine[0]
        return False, best_similar, best_engine, image1_crop, image2_crop
    ...
    def _verify_img(self, img_list, verify_img_index, verify_engine_list=[('ac_tpl', 0.99)]):
        ...
        for img_info in img_list:
            ...
            for index, verify_img_info in enumerate(verify_images):
                # 因为截图和校验图都是连续的，所以如果有校验通过了的校验图，下次校验时可以跳过此次之前的图片
                if self.quick_verify and index <= cut_match_index:
                    continue
                verify_img = verify_img_info['img']
                is_similar, similar, engine, image1_crop, image2_crop = self._is_image_similar(img, verify_img,
                                                                                               verify_engine_list)
                if is_similar:
                    logger.info(
                        f"目标帧: {os.path.basename(img_path)} match 源视频帧：{os.path.basename(verify_img_info['path'])}, similiar: {similar} , 算法 :{engine}")
                    ...
                    if best_matching_similar < similar:
                        ...
                        best_matching_name = os.path.basename(verify_img_info['path'])
                        best_matching_engine = engine
            if best_matching_name:
                verify_fail_screenshots.append(img_path)
                logger.info(
                    f"{os.path.basename(img_path)} 不匹配任何帧, 算法阈值: {verify_engine_list}, "
                    f"best_matching_name: {best_matching_name}"
                    f"best_matching_similar : {best_matching_similar}"
                    f"best算法 :{best_matching_engine}"
                )
        return {
