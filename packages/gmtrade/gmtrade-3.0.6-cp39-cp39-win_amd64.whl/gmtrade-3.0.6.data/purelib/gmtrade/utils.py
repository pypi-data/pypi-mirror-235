...
gmtradelogger = logging.getLogger('gmtradelogger')
gmtradelogger.setLevel(logging.INFO)
...
gmtradelogger.addHandler(console_handler)
...
def gmtradelog2file():
    """
    增加一个handler, 把gmtradelog写入到日志文件. 只调用一次
    """
    global __add_logfile
    if __add_logfile:
        return

    file_handler = logging.FileHandler('gmtrade_{}.log'.format(datetime.now().strftime("%Y%m%d%H%M%S")))
    file_handler.setLevel(logging.DEBUG)
    file_handler.setFormatter(gmformat)
    gmtradelogger.addHandler(file_handler)
    __add_logfile = True
...
def str2datetime(d):
    ...
    if len(d) == 19:  # 类于 2017-07-01 08:50:00
        return datetime.strptime(d, '%Y-%m-%d %H:%M:%S')
    gmtradelogger.error("字符串 %s 不能格式化成datetime类型, 请使用类似于 2017-07-01 08:50:00 这种格式", d)
    return None
