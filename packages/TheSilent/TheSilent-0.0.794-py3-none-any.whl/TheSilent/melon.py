...
def melon(host,delay=0,crawl=False):
    ...
    print(CYAN + "checking for false positives")
    try:
        tcp_socket = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        tcp_socket.settimeout(10)
        tcp_socket.connect((urllib.parse.urlparse(init_host).netloc,12345))
        hits.append("port 12345 is already open")
        port_1 = True
    except:
        pass
    ...
    try:
        tcp_socket = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        tcp_socket.settimeout(10)
        tcp_socket.connect((urllib.parse.urlparse(init_host).netloc,7777))
        hits.append("port 7777 is already open")
        port_2 = True
    except:
        pass
