...
from humbldata.helpers import DataStructureHelpers, MCHelpers, MessageHelpers
...
class VolatilityEstimators:
        ...
        **kwargs,
        ...
        if (
            ...
            self.price_data = format_price_data(
                data=price_data, silent=self.silent
            )
    ...
    def _std(self, price_data, window=30, trading_periods=252, clean=True):
        ...
        """
        price_data = MCHelpers.log_returns(df=price_data)
    ...
    def _garman_klass(
        ...
        log_co = (price_data["Adj Close"] / price_data["Open"]).log()
    ...
    def _rogers_satchell(
        ...
        price_data = price_data.with_columns(
            [
                ...
                (pl.col("Adj Close") / pl.col("Open")).log().alias("log_co"),
            ]
        ).with_columns(
            (
    ...
    def _yang_zhang(
        ...
        price_data = price_data.with_columns(
            [
                ...
                (pl.col("Low") / pl.col("Open")).log().alias("log_lo"),
                (pl.col("Adj Close") / pl.col("Open")).log().alias("log_co"),
                (pl.col("Open") / pl.col("Adj Close").shift())
                .log()
                .alias("log_oc"),
                (pl.col("Adj Close") / pl.col("Adj Close").shift())
                .log()
                .alias("log_cc"),
            ]
        ).with_columns(
    ...
    def _squared_returns(
        ...
        price_data = MCHelpers.log_returns(df=price_data).with_columns(
