...
def _get_class_annotations(t: type) -> Dict[str, Any]:
    if hasattr(t, "__annotations__"):
        annotations = t.__annotations__.copy()
    else:
        annotations = {}
    for base in t.__bases__:
        base_annotations = {
            key: value
            for key, value in _get_class_annotations(base).items()
...
def _get_field_type(field_name: str, obj: Any) -> type:
    annotations = _get_class_annotations(type(obj))
