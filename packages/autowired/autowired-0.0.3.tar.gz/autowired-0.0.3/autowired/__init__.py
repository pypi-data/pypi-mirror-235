...
class Container:
    ...
    def get_existing(self, dependency: Dependency) -> Bean | None:
        """
    ...
    def register(self, bean: Bean | Any):
...
def _get_field_type(field_name: str, obj: Any) -> type:
    ...
    if field_type is None:
        raise ValueError(f"Cannot determine type of field {field_name}")

    return field_type


def _resolve_autowired_field(field_name: str, autowired: _Autowired, ctx: "Context"):
    field_type = _get_field_type(field_name, ctx)
    return ctx.autowire(field_type, **autowired.get_all_kw_args(ctx))
...
class Context(metaclass=_ContextMeta):
    ...
    def autowire(self, t: type[_T] | type, **explicit_kw_args) -> _T:
