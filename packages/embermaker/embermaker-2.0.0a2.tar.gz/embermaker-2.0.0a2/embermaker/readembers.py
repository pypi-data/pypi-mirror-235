...
However, the complexity of the parser partly comes from the file format, which has constraints outside computing.
"""
import numpy as np
import warnings
...
def readembers(wb, gp=None, logger=None):
    """
    ...
    :return: lbes a set of embers (lbes)
    """
    if logger is None:
        logger = hlp.Logger()
    if gp is None:
