...
class Logger:
    ...
    def addinfo(self, mes, mestype=None):
        ...
        if mestype == 'title':
            ...
            fmtmes = '<p class="compact indent">' + fmtmes + '</p>'
        self.logmes.append((fmtmes, 20))  # Following the usual python convention for logging, 20 is "info"
    ...
    def addwarn(self, mes, critical=False):
        ...
        fmtmes = '<p class="compact">' + rembrackets(mes) + '</p>'
    ...
    def addfail(self, mes):
        ...
        mes = "FATAL ERROR: " + mes
        logging.critical(mes)
        fmtmes = '<span class="error">' + rembrackets(mes) + '</span>'
        self.logmes.append((fmtmes, 50))
        return mes


    def getlog(self, levelname):
        intlevel = logging.getLevelName(levelname)
        if type(intlevel) is not int:
            return [f'Unknown logging level name {levelname}']
        try:
            mes = [entry[0] for entry in self.logmes if entry[1] >= intlevel]
