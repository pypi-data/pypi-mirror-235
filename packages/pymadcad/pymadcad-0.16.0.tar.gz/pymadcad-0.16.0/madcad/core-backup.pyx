...
def intersect_triangles(f0, f1, precision):
	...
	#cdef size_t kA, kB
	#for kA in range(3):
		#if sYA[kA]*sYA[(kA+1)%3] <= prec:
			#break		
	#for kB in range(3):
		#if sYB[kB]*sYB[(kB+1)%3] <= prec:
			#break		
	#cdef size_t kA=0, kB=0
	#cdef size_t j
	#for j in range(1,3):
		#if abs(varr(&yA)[j]-varr(&yA)[(j+1)%3]) > abs(varr(&yA)[kA]-varr(&yA)[(kA+1)%3]):
			#kA = j
		#if abs(varr(&yB)[j]-varr(&yB)[(j+1)%3]) > abs(varr(&yB)[kB]-varr(&yB)[(kB+1)%3]):
			#kB = j
	# look for edges intersecting starting from the eventual through one
	for i in range(kA,kA+3):
		if sYA[i%3]*sYA[(i+1)%3] <= 0 and abs(sYA[i%3])+abs(sYA[(i+1)%3]) > 0 : 
			eIA[neIA] = i%3
			neIA += 1
	for i in range(kB,kB+3):
	...
	raise Exception("unexpected case : ", fA, fB)
