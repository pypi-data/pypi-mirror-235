...
class Parameters(object):
    ...
    def __init__(self, kwargs_init, kwargs_fixed, kwargs_up=None, kwargs_down=None):
        ...
        """
        self._kwargs_init = deepcopy(kwargs_init)
        self._kwargs_fixed = deepcopy(kwargs_fixed)
        self._kwargs_up = deepcopy(kwargs_up)
        self._kwargs_down = deepcopy(kwargs_down)
    ...
    def _set_params(self, kwargs, kwargs_key):
        ...
        for name in param_names:
            if name not in kwargs_fixed_k and name != 'lambda':
    ...
    def _param_names(self, kwargs, kwargs_key):
        ...
        for name in param_names:
            if name not in kwargs_fixed_k and name != 'lambda':
                ...
                if len(value) == 1 and isinstance(value, (list, np.ndarray, np.generic, jnp.ndarray, jnp.generic)):
                    names.append(name)
