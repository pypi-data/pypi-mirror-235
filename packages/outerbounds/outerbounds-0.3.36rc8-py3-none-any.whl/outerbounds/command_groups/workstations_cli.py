...
def list_workstations(config_dir=None, profile=None):
    try:
        workstations_json = get_all_workstations(config_dir, profile)
        click.echo(json.dumps(workstations_json, indent=4))
    except HTTPError:
        click.secho("Failed to generate workstation token.", fg="red")
        click.secho("Error: {}".format(json.dumps(workstations_json.json(), indent=4)))
    except Exception as e:
        click.secho("Failed to list workstations", fg="red")
        click.secho("Error: {}".format(str(e)))


def get_all_workstations(config_dir=None, profile=None, admin_mode=False):
    ...
    list_url = (
        f"{api_url}/v1/workstations"
        if not admin_mode
        else f"{api_url}/v1/workstations/listall"
    )

    workstations_response = requests.get(
        list_url, headers={"x-api-key": metaflow_token}
    )

    workstations_response.raise_for_status()
    return workstations_response.json()
