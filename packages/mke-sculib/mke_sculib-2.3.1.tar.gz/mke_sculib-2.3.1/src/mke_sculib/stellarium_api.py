...
class stellarium_api():
    ...
    def _loop_run_sync(self, scu_api, verb=True, use_socket=False):
        ...
        if use_socket:
            t, fields = next(gen)
        gen = api.sock_listen_forever(['acu.azimuth.p_act', 'acu.elevation.p_act'])
    ...
    async def _loop_run_async(self, scu_api, verb=True, use_socket=False):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            while True:
                if use_socket:
                    resp_post, (t, fields) = await asyncio.gather(send(data), gen.__anext__())
                    az, el = fields.values()
                else:
...
if __name__ == "__main__":
    ...
    # api.run(scu_api('http://localhost:8080'), use_async=False)
    api.run(scu_api('http://10.96.64.10:8080/'), use_async=True)
