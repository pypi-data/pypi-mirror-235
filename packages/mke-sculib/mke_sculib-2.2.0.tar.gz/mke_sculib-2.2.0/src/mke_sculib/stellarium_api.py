...
class stellarium_api():
    ...
    def set_boresight(self, az, el):
        ...
        """
        r = requests.post(self.address + '/api/main/view', params={'az': np.deg2rad(az), 'el': np.deg2rad(el)})
        r.raise_for_status()
    ...
    def run(self, scu_api, verb=True):
        ...
        if verb:
            ...
            print('')
            print(' TIME (UTC)          | AZIMUTH (deg) | ELEVATION (deg)')
        while 1:
            az, el = scu_api.azel
            if verb:
                t = datetime.datetime.utcnow().isoformat()
                print(' {} | {: 10.4f}    | {: 10.4f}'.format(t.split('.')[0], az, el), end='\r')
...
if __name__ == "__main__":
    ...
    api = stellarium_api()
    api.run(scu_api('http://10.96.64.10:8080'))