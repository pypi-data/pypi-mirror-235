...
def feasibleTV(polygonA,polygonB,edgesA,edgesB,vector,pointList):
    for point in pointList:
        translateV = Line(point[4],Point(point[4].x + vector[0],point[4].y + vector[1]),(((point[4].y + vector[1]) - point[4].y)/((point[4].x + vector[0]) - point[4].x)) if (point[4].x + vector[0] - point[4].x) != 0 else None)
        ...
        if (polygonA.__contains__(p) and polygonB.__contains__(p)):
            ...
            mirrorSEB = Line(startEB.startPoint,Point(startEB.startPoint.x - bStartsize[0],startEB.startPoint.y - bStartsize[1]),startEB.slope)
            mirrorEEB = Line(Point(endEB.endPoint.x + bEndsize[0],endEB.endPoint.y + bEndsize[1]),endEB.endPoint,endEB.slope)
            bound1 = None
            bound2 = None
            ...
            pTvec = Line(point[4],Point(point[4].x + Tvec[0],point[4].y + Tvec[1]),(((point[4].y + Tvec[1]) - point[4].y)/((point[4].x + Tvec[0]) - point[4].x)) if ((point[4].x + Tvec[0]) - point[4].x) != 0 else None)
...
def trimmer(Points,filteredEdges,translateVector):
    ...
    for point in Points:
        ...
        (((point[1] + ntv[1] - point[1]))/((point[0] + ntv[0]) - point[0])) if ((point[0] + ntv[0]) - point[0]) != 0 else None)))
...
def trimFun(polygonM,filteredES,translateV,p):
    ...
    for point in polygonM:
        ...
        (((point[1] + translateV[1] - point[1]))/((point[0] + translateV[0]) - point[0])) if ((point[0] + translateV[0]) - point[0]) != 0 else None)))
        ...
        for j,edge in enumerate(filteredES):
            ...
            if intersection is not None:
                ...
                if (edge.is_startPoint(intersection)):
                    infos.append([Point(polygonM[i][0], polygonM[i][1]),filteredES[j],p,False,intersection,i,nTv])
                elif (edge.is_endPoint(intersection)):
                    infos.append([Point(polygonM[i][0], polygonM[i][1]),filteredES[j],p,False,intersection,i,nTv])
                else:
                    infos.append([Point(polygonM[i][0], polygonM[i][1]),edge,p,True,intersection,i,nTv])
...
def NFP(polygonA,polygonB):
    ...
    while (((x[0] != wCond[0]) or(x[1] != wCond[1]) or y <2)):
        ...
        edgesB2 = [Line(Point(a[0],a[1]),Point(b[0],b[1]),((b[1] - a[1])/(b[0] - a[0])) if (b[0] - a[0]) != 0 else None)  for a, b in zip(polygonB2, polygonB2[1:] + [polygonB2[0]])]
        ...
        for i,info in enumerate(trimResults):
            ...
            vecList.add(possibleNextVector)
        ...
        for j,vector in enumerate(vecList):
            if(feasibleTV(polygonA,polygonB2,edgesA,edgesB2,vector,trimResults) == False):
                ...
                idealTV.append((vector,trimResults[j][4]))
        
        distanceL = []
        for l in idealTV:
            tvec = Line(point,Point(-t2[0]/2,-t2[1]/2),5)
