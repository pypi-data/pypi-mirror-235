...
cpdef fetch_objects(inpt, _prgbar):
    lock, bar = _prgbar
    cdef int i
    cdef str key
    cdef str hash_
...
cdef class SampleTracer:
    ...
    cdef _restore_objects(self, str type_, common_t getter, list these_hashes = [], bool quiet = False):
        ...
        for itc in cache_map: fetch_these[itc.first] = [itc.first, type_, [bt.hash for bt in itc.second]]
        if not len(fetch_these): return
    ...
    def Device(self, str val):
        ...
        except IndexError: self._set.device = b"cpu"
    ...
    def KinematicMap(self, dict val):
        ...
        for key in val: self._set.kinematic_map[enc(key)] = env(val[key])
