...
class S3Client:
    ...
    def upload_s3_object(self, bucket: str, content: bytes, filepath: str):
    ...
    def download_s3_files_local(
        self, bucket: str, keys: list[str], local_dir: str, skip_if_existed: bool = True
    ):
        ...
        for key in keys:
            ...
            if os.path.exists(f"{local_dir}/{file_name}") and skip_if_existed:
                continue
            self.client.download_file(bucket, key, f"{local_dir}/{file_name}")

    def delete_file(self, bucket: str, filepath: str):
        self.client.delete_object(Bucket=bucket, Key=filepath)
