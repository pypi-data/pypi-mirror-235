...
class GlobalTestReport(object):
    ...
    def __str__(self):
        ...
        line1 = [f"Ran {self.ran} test cases in {self.ttime:.2f}s ({self.ctime:.2f}s CPU)"]
        if self.errors:
            line1.append(f"{self.errors} errors")
        if self.failures:
            line1.append(f"{self.failures} failures")
        if self.skipped:
            line1.append(f"{self.skipped} skipped")
        modulesok = self.modulescount - len(self.errmodules)
        if self.errors or self.failures:
            line2 = f"{modulesok} modules OK ({len(self.errmodules)} failed)"
            descr = ", ".join(["%s [%s/%s]" % info for info in self.errmodules])
            line3 = f"\nfailures: {descr}"
        elif modulesok:
            line2 = f"All {modulesok} modules OK"
            line3 = ""
        else:
            return ""
        return f"{', '.join(line1)}\n{line2}{line3}"
...
class PyTester(object):
    ...
    def testfile(self, filename, batchmode=False):
        ...
        print(f"  {osp.basename(filename)}  ".center(70, "="), file=sys.__stderr__)
        try:
...
class DjangoTester(PyTester):
    ...
    def testfile(self, filename, batchmode=False):
        ...
        modname = osp.basename(filename)[:-3]
        print(f"  {osp.basename(filename)}  ".center(70, "="), file=sys.stderr)
        try:
            try:
                tstart, cstart = time(), process_time()
    ...
    def run(self, test):
        ...
        if not self.batchmode:
            ...
            for name, value in (
                ...
                if value:
                    det_results.append(f"{name}={len(value)}")
            if det_results:
                self.stream.write(" (")
