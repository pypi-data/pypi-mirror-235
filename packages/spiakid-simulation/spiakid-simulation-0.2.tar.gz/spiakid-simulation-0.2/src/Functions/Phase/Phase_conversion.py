...
def phase_conv(Photon,pix,conv_wv,conv_phase):
    ...
    for i in range(dim[0]):
        for j in range(dim[1]):
            signal[i,j] = photon2phase(Photon[i,j],dict[str(i)+str(j)][0],dict[str(i)+str(j)][1])
    return(signal)


def photon2phase(Photon,conv_wv,conv_phase):
    ...
    signal[1] = np.where(Photon[1]==0,Photon[1],curv[0] * Photon[1] ** 2 +  curv[1] * Photon[1] + curv[2])
    return(signal)
