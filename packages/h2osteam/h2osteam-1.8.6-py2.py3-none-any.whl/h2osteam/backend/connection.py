...
class SteamConnection(SteamApi):
    ...
    @staticmethod
    def open(scheme=None, host=None, port=None, username=None, password=None, verify_ssl=True, cacert=None, access_token=None, refresh_token=None):
        ...
        res = requests.request('POST', '%s://%s:%s/auth' % (scheme, host, port),
    ...
    def call(self, method, params):
        ...
        res = requests.request('POST', '%s://%s:%s/%s' % (self.scheme, self.host, self.port, 'web'),
    ...
    def upload(self, target, path, payload):
        ...
        res = requests.post('%s://%s:%s%s' % (self.scheme, self.host, self.port, target),
                            cookies={"steam-session": self.cookie},
                            verify=self._requests_verify_ssl,
                            data=monitor,
    ...
    def download(self, target, path):
        res = requests.get('%s://%s:%s%s' % (self.scheme, self.host, self.port, target),
                           cookies={"steam-session": self.cookie},
                           verify=self._requests_verify_ssl)
