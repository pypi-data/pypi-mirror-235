...
from h2osteam.utils import set_scalable_cluster_params, validate_dataset_params, LooseVersion
...
class H2oCluster:
    ...
    def download_autodoc(
        ...
        self.api.autodoc_download(
            connect_params=self.get_config()["connect_params"],
            model=model,
            config=config,
            path=path,
    ...
    def download_autodoc_logs(self, path=None):
        ...
        cluster = self._fetch_cluster()
        self.api.download('/download/autodoc/logs/h2o/%d' % (cluster['id']), path)
