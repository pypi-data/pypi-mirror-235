...
from h2osteam.utils import LooseVersion, set_scalable_cluster_params, validate_dataset_params
...
class SparklingSession(object):
    ...
    def download_autodoc(
        ...
        :param model: H2OModel: the H2O model object for which the ml-autodoc
            will render a report document.
        :param config: h2osteam.utils.AutoDocConfig: the configuration
            settings for download_autodoc.
        :param path: str: Path to where the generated AutoDoc report will be
            downloaded to.
        :param train_frame: H2OFrame: the training frame used to build the H2O model.
        :param valid_frame: H2OFrame: the validation frame used to build the H2O model.
        :param test_frame: H2OFrame: additional test dataset (not used during training)
            which contains the same feature names found in the training dataset.
        :param additional_testsets: list: a list of additional test H2OFrames.
        :param alternative_models: list: a list of H2OModels. These are
            models that exist in the current running H2O cluster.
        ...
        connect_params = {
            ...
            "context_path": self.cluster['context_path'],
            "cookies": ["steam-session=%s" % self.api.cookie],
            "ip": self.api.host,
            "port": self.api.port,
        }

        self.api.autodoc_download(
    ...
    def download_autodoc_logs(self, path=None):
        """
        Download AutoDoc logs.

        :param path: Path where the AutoDoc logs will be saved.
        ...
        self.api.download('/download/autodoc/logs/sparkling/%d' % self.cluster["id"], path)

        print("AutoDoc logs saved to %s" % path)
