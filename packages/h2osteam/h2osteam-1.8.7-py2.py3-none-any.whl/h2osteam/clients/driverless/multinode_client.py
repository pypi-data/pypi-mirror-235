...
class MultinodeCluster(object):
    ...
    def external_address(self):
        ...
        """
        self.refresh()
        return '%s://%s:%s/proxy/driverless/%d' % (self._api.scheme, self._api.host, self._api.port, self.id)
    ...
    def openid_login_url(self):
        ...
        """
        return '%s://%s:%s/oidc-login-start?forward=/proxy/driverless/%d/openid/callback' \
               % (self._api.scheme, self._api.host, self._api.port, self.id)
