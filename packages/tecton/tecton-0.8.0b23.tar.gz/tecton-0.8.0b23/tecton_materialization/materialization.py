...
def start_batch_materialization(
    ...
    if write_to_hdfs_path:
        ...
        if batch_params.write_to_offline_feature_store:
            ...
            export_offline_consumption_metrics(spark, fd, data_frame_count, materialization_task_params)
        ...
        if batch_params.write_to_online_feature_store:
            ...
            if batch_params.create_online_table:
                ...
                write_dynamodb_json(spark, fd, materialization_task_params, online_store_df)
            else:
...
def export_offline_consumption_metrics(
    spark: SparkSession, fd: FeatureDefinition, offline_store_count, params: MaterializationTaskParams
):
    ...
    statsd_client.incr(
        f"tecton-offline-store.cm_feature_write_rows.{statsd_safe_workspace}.{statsd_safe_fd_name}.", num_rows
    )
    statsd_client.incr(
        f"tecton-offline-store.cm_feature_write_values.{statsd_safe_workspace}.{statsd_safe_fd_name}", num_values
    )

    logger.info("Exported offline consumption metrics")
    ...
    def updater(job_metadata: JobMetadata) -> Optional[JobMetadata]:
        ...
        offline_bucket = offline.consumption_info[aligned_bucket_start]
        offline_bucket.rows_written += num_rows
        offline_bucket.features_written += num_values
