...
class PandasTreeRewriter:
    ...
    def _rewrite_fv_input_node(self, tree: NodeRef, data_source_compute: QueryTreeCompute) -> None:
        """Replaces a DataSourceScanNode that contains a Snowflake data source with a StagedTableScanNode."""
        ...
        if ds_dialect == Dialect.SNOWFLAKE:
            ...
            # A rewrite should only leave NodeRefs. However, this PandasStagedTableScanNode is only temporary. It will
            # be removed in 'rewrite' above.
            tree.node = pandas_nodes.PandasStagedTableScanNode(
