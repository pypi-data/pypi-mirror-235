...
class Aggregation(object):
    ...
    def __init__(
        ...
        column: str,
        function: Union[str, AggregationFunction],
        time_window: Union[datetime.timedelta, TimeWindowWrapper],
        name: Optional[str] = None,
    ):
        self.column = column
        self.function = function
        if isinstance(time_window, datetime.timedelta):
            self.time_window = TimeWindowWrapper(window_start=-time_window, window_end=datetime.timedelta(seconds=0))
        else:
    ...
    def _to_proto(self, aggregation_interval: datetime.timedelta, is_continuous: bool):
        ...
        proto.time_window.CopyFrom(
            feature_view_pb2.TimeWindow(
                window_duration=time_utils.timedelta_to_proto(self.time_window.window_duration),
                offset=time_utils.timedelta_to_proto(self.time_window.offset),
            )
        )
