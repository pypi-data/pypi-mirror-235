        ...
        if self.fdw.aggregation_secondary_key:
            for secondary_key_output in self.fdw.fv_spec.secondary_key_rollup_outputs:
                aggregations.append(
                    ...
                    .over(
                        self._generate_aggregation_window_spec(secondary_key_output.time_window.window_start, join_spec)
                    ).alias(
...
class AggregationSecondaryKeyRollupSparkNode(SparkExecNode):
    ...
    def _window_to_features_map(self) -> Dict[str, List[str]]:
        ...
        for secondary_key_output in self.fdw.fv_spec.secondary_key_rollup_outputs:
            ...
            features = [
                ...
                if TimeWindowWrapper.from_data(agg_feature.time_window) == secondary_key_output.time_window
            ]
    ...
    def _temp_struct_column_name(self, window: TimeWindowWrapper) -> str:
        return f"{self.TECTON_TEMP_STRUCT_PREFIX}_{window.to_string()}"

    def _temp_indictor_column_name(self, window: TimeWindowWrapper) -> str:
    ...
    def _filter_aggregation_secondary_key_by_indicator(
        self, df: pyspark.sql.DataFrame, window: TimeWindowWrapper, temp_struct_column_name: str
    ...
    def _extract_features_columns_from_struct_columns(
        ...
        for agg_feature in self.fdw.trailing_time_window_aggregation.features:
            if TimeWindowWrapper.from_data(agg_feature.time_window) == window:
                df = df.withColumn(
                    agg_feature.output_feature_name,
