...
def resolve_dependencies(
    requirements_path: Path, resolved_requirements_path: Path, python_version: str, timeout_seconds: int, debug: bool
):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _run_pex_command(command_list=lock_command, timeout_seconds=timeout_seconds, debug=debug)
        _run_pex_command(command_list=export_command, timeout_seconds=timeout_seconds, debug=debug)


def download_dependencies(requirements_path: Path, target_directory: Path, python_version: str, debug: bool = False):
    """
...
def _run_pex_command(command_list: List[str], timeout_seconds: int, debug: bool = False):
    ...
    if result.returncode != 0:
        error_message = result.stderr if debug else _parse_pex_error(result.stderr)
