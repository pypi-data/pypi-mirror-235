...
class Extractor:
    ...
    def get_info(self, option = 1):
        ...
        #generate data from docs
            ...
            if(option == 1):
                writer.excel_writer(extractors.search_gen(stx, self.years), stx.split(), filepath=self.filepath, years = self.years)
            else:
                df1 = writer.df_writer(extractors.search_gen(search_term=stx, period=self.years), years = self.years)
                strg.append(df1)
        if(option!=1):
            return strg
        else:
