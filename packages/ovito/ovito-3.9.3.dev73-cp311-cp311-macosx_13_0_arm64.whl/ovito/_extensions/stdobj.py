...
def _DataCollection_lines(self):
    """
    Returns a dictionary view providing key-based access to all :py:class:`Lines` objects in
    this data collection. Each :py:class:`Lines` has a unique :py:attr:`~ovito.data.DataObject.identifier` key,
    which can be used to look it up in the dictionary.

    .. literalinclude:: ../example_snippets/data_collection_lines.py
        :lines: 13-13

    to see which identifier keys exist. Then retrieve the desired :py:class:`Lines` object from the collection using its identifier
    key, e.g.

    .. literalinclude:: ../example_snippets/data_collection_lines.py
        :lines: 18-19

    The view provides the convenience method :py:meth:`!lines.create`, which
    inserts a newly created :py:class:`Lines` into the data collection. The method expects the unique :py:attr:`~ovito.data.DataObject.identifier`
    of the new lines object as first argument. All other keyword arguments are forwarded to the constructor
    to initialize the member fields of the :py:class:`Lines` class:
    ...
    method modifies and returns that existing lines object instead of creating another one.
    """
