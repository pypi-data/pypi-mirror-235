...
def check_there_is_same_author_from_current_branch(commits: List[Commit]) -> None:
    commits_author = set([commit.author_mail for commit in commits])
    if len(commits_author) > 1:
        die(
            msg=(
                "The commits to be checked on the current branch do not come from the same author."
            )
        )
