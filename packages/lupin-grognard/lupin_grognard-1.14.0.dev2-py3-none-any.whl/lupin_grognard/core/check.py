...
def check_author_commits_is_not_approvers_current_merge(
    ...
    if commits[0].title.startswith("Merge branch"):
        ...
        for commit in commits[1:]:
            print(f"Checking commit: {commit.title}")
        ...
        current_merge_approvers.append("Cédric Fruteau de Laclos fdlcdev@gmail.com")
        print(f"Current merge commit approvers: {current_merge_approvers}")
        child_commits_authors = [commit.author_mail for commit in child_commits]
        print(f"Child commits authors: {child_commits_authors}")
        if any([author in current_merge_approvers for author in child_commits_authors]):
            die(
                msg=(
