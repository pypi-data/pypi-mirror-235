...
class CommitValidator(Commit):
    ...
    def _validate_commit_merge(self) -> bool:
        if len(self.approvers) < 1:
            ...
            return False
        # else:
        #     if not self._check_if_author_commits_is_not_approvers_current_merge():
    ...
    def _check_if_author_commits_is_not_approvers_current_merge(
        self,
        commits: List[Commit],
    ) -> bool:
        ...
        Returns:
            bool: True if the author of the child commits is not an approver of the merge commit,
            else False."""
        child_commits: List[Commit] = []
        merge_parent_found = False
        if self._is_merge_commit(commits[0].title):
            for commit in commits[1:]:
                if self._is_merge_commit(commit.title):
            ...
            child_commits_authors = [commit.author_mail for commit in child_commits]
            return not any(
                [author in current_merge_approvers for author in child_commits_authors]
            )
        return True
