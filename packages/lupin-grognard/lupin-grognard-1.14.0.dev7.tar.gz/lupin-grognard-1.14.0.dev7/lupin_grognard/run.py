...
from lupin_grognard.core.check import (
    check_commit,
    check_author_commits_is_not_approvers_current_merge,
    check_max_allowed_major_commits,
    check_there_is_same_author_from_current_branch,
)
...
def check_commits(
    ...
    if check_max_allowed_major_commits(
        ...
        check_author_commits_is_not_approvers_current_merge(commits=commits)

        display_number_of_commits_to_check(commits=commits)

        if check_mode == CommitCheckModes.CHECK_CURRENT_BRANCH_ONLY:
            check_there_is_same_author_from_current_branch(commits=commits)
