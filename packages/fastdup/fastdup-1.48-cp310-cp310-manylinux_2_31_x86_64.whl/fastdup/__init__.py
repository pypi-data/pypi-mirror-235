...
def iterate_on_webdatasets(input_dir, work_dir=None, bounding_box=None, caption=None, device='cpu', batch_size=8, num_images=None, verbose=0, license=""):
    ...
    def returnfilelist(input_dir):
        filenames_list = []
        for path, subdirs, files in os.walk(input_dir):
            files = [f for f in files if not f[0] == '.' and (f.endswith('.jpg') or f.endswith('.png'))]
            for name in files:
    ...
    for t in tqdm(sorted(tars)):
        try:
            ...
            if caption is None:
                ...
                captions_df = generate_labels(file_list, caption, device, batch_size)
