...
def remove_duplicates(input_dir, work_dir=None, distance=0.96, dry_run=False):
    """
    ...
    ret = fastdup.run(input_dir=input_dir, work_dir=work_dir, turi_param=f'ccthreshold={distance},run_stats=0')
    ...
    assert ret == 0, "Failed to delete duplicates"
    return 0


def iterate_on_webdatasets(input_dir, work_dir=None, bounding_box=None, caption=None, device='cpu', batch_size=8, num_images=None, verbose=0, license=""):
    """
    ...
    for t in tqdm(sorted(tars)):
        try:
            ...
            ret = os.system(f'cd {temp_dir}; tar xf {input_dir}/{t} -C . ')
            ...
            if caption is not None:
                captions_df = fd.caption(model_name=caption, device=device, batch_size=batch_size)
                captions_df['caption'] = captions_df['caption'].replace(r'\n', ' ', regex=True)
                captions_df[['filename', 'caption']].to_csv(f'{csv_name}_{t}.csv')
            else:
                os.system(f'touch {work_dir}/{csv_name}_{t}.csv')
        except Exception as ex:
            fastdup_capture_exception("iterate on webdataaet", ex)
    return 0
