...
def process_group(group):
    i, output_dir, group = group
    tar_filename = f"{output_dir}/visual_layer_{str(int(i/images_per_tar)).zfill(6)}.tar"
    create_tar(tar_filename, group)

def package_webdataset(input_dir, work_dir, output_dir, _images_per_tar = 10000, image_suffix = ['.jpg', 'png'], num_threads=32, limit=None):
    """
    ...
    groups = [(i, output_dir, image_files[i:i + images_per_tar]) for i in range(0, len(image_files), images_per_tar)]
