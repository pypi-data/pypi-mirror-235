...
class ContextImpl(Context):
    ...
    async def http_request(self, url, method='GET', body=None, headers=None) -> Tuple[Context, Any]:
        session = self.get_http_session()
        url = self.resolve_url(url)
        headers, response = await session.request(url, method, body, headers=headers)
        return self.clone(
            url=urlparse(url),
