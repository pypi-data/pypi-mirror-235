...
class TerminalState:
    ...
    def restore_state(self):
        while self._restore_callbacks:
            logging.getLogger(__package__).debug(f"TSC: Restoring state ({len(self._restore_callbacks)})")
            try:
    ...
    def _add_restore_callback(self, fn: t.Callable[[], None]) -> None:
        logging.getLogger(__package__).debug(f"TSC: Registering callback: '{fn.__name__}'")
    ...
    def _remove_restore_callback(self, fn: t.Callable[[], None]) -> None:
        if fn not in self._restore_callbacks:
            return
        logging.getLogger(__package__).debug(f"TSC: Unregistering callback: '{fn.__name__}'")
        self._restore_callbacks.remove(fn)

    def _send_sequence(self, fn: t.Callable[[], pt.ISequence]) -> None:
        if not self._is_a_tty():
            logging.getLogger(__package__).debug(f"TSC: Skipping control sequence: not a tty")
            return

        sequence = fn()
        logging.getLogger(__package__).debug(f"TSC: Sending control sequence: '{fn.__name__}' -> {sequence!r}")
