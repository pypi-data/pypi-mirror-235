...
def validate(args, data_loader, encoder, decoders):
    ...
    with torch.no_grad():
        for i, (image, label, mask, file_name, img_type) in enumerate(data_loader):
            ...
            print(file_name,img_type,logps.reshape(bs, h, w))
...
def test(args):
    ...
    # checkpoint = "/workspace/output_custom_list/bgad_fas_10/weights/custom_tf_efficientnet_b6_conditional_flow_model_bottle.pt"
    # if args.checkpoint:
