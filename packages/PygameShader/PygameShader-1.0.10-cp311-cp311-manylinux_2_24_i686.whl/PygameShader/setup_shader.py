    ...
    if OPENMP:
        ext_compile_args = \
            ["-DPLATFORM=linux", "-march=i686" if proc_arch_bits == "32BIT" else "-march=x86-64",
              "-m32" if proc_arch_bits == "32BIT" else "-m64",  OPENMP_PROC, "-Os", "-O1", 
              "--param=max-vartrack-size=150000"]
        ext_link_args = [ OPENMP_PROC, "--param=max-vartrack-size=1500000" ]
    else:
        ext_compile_args = \
            ["-DPLATFORM=linux", "-march=i686" if proc_arch_bits == "32BIT" else "-march=x86-64",
              "-m32" if proc_arch_bits == "32BIT" else "-m64", "-Os", "-O1"]
        ext_link_args = ""
else:
