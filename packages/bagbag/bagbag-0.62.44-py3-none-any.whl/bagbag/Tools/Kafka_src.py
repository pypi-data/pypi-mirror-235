...
class kafkaProducer():
    def __init__(self, kf:Kafka, topic:str, servers:str|list, value_serializer:str, compression_type:str=None):
        """
    ...
    def Send(self, data:dict|list|bytes|str):
        ...
        if self.value_serializer == None:
            if type(data) == bytes:
                res = self.kp.send(self.topic, data)
            elif type(data) == str:
                res = self.kp.send(self.topic, data.encode())
            else:
                res = self.kp.send(self.topic, str(data).encode())
        elif self.value_serializer == "json":
            res = self.kp.send(self.topic, json.dumps(data).encode())
        elif self.value_serializer == "msgpack":
            res = self.kp.send(self.topic, msgpack.packb(data, use_bin_type=True))
        
        # Lg.Trace("send result:", res)
        # import ipdb
        # ipdb.set_trace()
...
class kafkaConsumer():
    def __init__(self, kf:Kafka, topic:str, servers:str|list, value_serializer:str, group_id:str="default"):
        self.kf = kf
        ...
        self.inited = self._assignPartition()
        # self._tell()
    
    def _assignPartition(self):
        self.kc.topics()
        pft = self.kc.partitions_for_topic(self.topic)
        # Lg.Trace(pft)
        if pft == None:
            # Lg.Trace("Create topic")
            # self.kf.CreateTopic()
            # while pft == None:
            #     pft = self.kc.partitions_for_topic(self.topic)
            return False 
    ...
    def Get(self) -> kafkaMessage:
        ...
        if self.inited == False:
            # Lg.Trace()
            while self._assignPartition() == False:
                Time.Sleep(1)
                # Lg.Trace()
            self.inited = True
            Lg.Trace("set all paration offset to 0")
            self.SeekAllParationByOffset(0)
            # Lg.Trace()
...
class kafkaQueue():
    def __init__(self, kf:Kafka, group_id:str="default") -> None:
        self.kf = kf 
        self.kp = self.kf.Producer()
        self.kc = self.kf.Consumer(group_id)
    
    def Put(self, data:dict|list|bytes|str):
        self.kp.Send(data)

    def Get(self) -> dict|list|bytes|str:
        return self.kc.Get().Value
    
    def Size(self) -> int:
        sum = 0
        for partition in self.kc.kc.partitions_for_topic(self.kf.topic):
            tp = ktp(self.kf.topic, partition)
            pos = self.kc.kc.position(tp)
            end = self.kc.kc.end_offsets([tp])[tp]
            sum = sum + (end - pos)
...
class Kafka():
    def __init__(self, topic:str, servers:str|list, serializer:str="msgpack", compression_type:str="gzip"):
        """
    ...
    def GetTopics(self) -> list[str]:
        kafka_client = kafka.KafkaClient(self.servers)
        server_topics = kafka_client.topic_partitions
        kafka_client.close()
        return server_topics
    
    def CreateTopic(self, partitions:int=1, replications:int=1):
        admin_client = KafkaAdminClient(
            bootstrap_servers=self.servers
        )
        topic_list = []
        topic_list.append(NewTopic(name=self.topic, num_partitions=partitions, replication_factor=replications))
        return admin_client.create_topics(new_topics=topic_list, validate_only=False)

    def Producer(self) -> kafkaProducer:
        return kafkaProducer(self, self.topic, self.servers, self.serializer, self.compression_type)

    def Consumer(self, group_id:str="default") -> kafkaConsumer:
        """
        如果topic不存在则自动创建. 
        The function returns a Kafka consumer object with the specified group ID and value serializer.
        
        :param group_id: The `group_id` parameter is used to specify the consumer group that the
        consumer belongs to. A consumer group is a group of consumers that work together to consume
        messages from one or more topics. Each message in a topic is consumed by only one consumer
        within a consumer group. The group_id helps Kafka, defaults to default
        :type group_id: str (optional)
        :return: an instance of the `kafkaConsumer` class.
        """
        # Lg.Trace(self.serializer)
        return kafkaConsumer(self, self.topic, self.servers, group_id=group_id, value_serializer=self.serializer)

    def Queue(self, group_id:str="default") -> kafkaQueue:
        return kafkaQueue(self, group_id)
